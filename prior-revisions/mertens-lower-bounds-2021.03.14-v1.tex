...
\mathcal{D}_{\omega}(x) & =  
     ...
     \frac{(\log\log x)^{k-1}}{(k-1)!} + O(E_{\omega}(x)) \right\rvert \\ 
...
to bound the function $\mathcal{N}_{\omega}(x)$ as 
\begin{align*}
\left\lvert \mathcal{N}_{\omega}(x) \right\rvert & \leq 
     \int_{1}^{2} D(x, r) dr + \lim_{r \rightarrow 2^{-}} D(x, r) \\ 
     & \ll \int_1^2 (\log x)^{r-1-r\log r} dr + (\log x)^{1-2\log 2} \\ 
     & \ll (\log x)^{-1} + (\log x)^{1-2\log 2}. 
     %& \ll \int_0^{1} (\log x)^{r - (1+r)r} dr + (\log x)^{1-2\log 2} \\ 
     %& \ll \frac{\sqrt{pi}}{2 \sqrt{\log\log x}} \operatorname{erf}\left(\sqrt{\log\log x}\right) 
     %+ (\log x)^{1-2\log 2} \\ 
     %& = \frac{1}{2 \sqrt{\log\log x}} \left[\sqrt{\pi} - \frac{1}{(\log x) \sqrt{\log\log x}} + 
     %O\left(\frac{1}{(\log\log x)^{3/2}}\right)\right] + (\log x)^{1-2\log 2}. 
\end{align*} 

Then we see that 
\[
\left\lvert \frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \right\rvert \ll 
     \frac{\sqrt{\log\log x}}{(\log x)^{2\log 2 - 1}} = o(1), 
...
\[
\pi_k(x) \sim \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} (1 + o_k(1)), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
\sum_{n \leq x} C_{\Omega(n)}(n) = 
     \sum_{k \geq 1} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) \sim
...
\[
     \Gamma(a, z) \sim \frac{z^{a-1} e^{-z}}{1-\lambda^{-1}} + O_{\lambda}\left(z^{a-2} e^{-z}\right), 
\]
