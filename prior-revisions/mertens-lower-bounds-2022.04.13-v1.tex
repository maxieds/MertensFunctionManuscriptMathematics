...
$u := k - \log\log x$, then uniformly for $1 \leq k \leq \frac{3}{2}\log\log x$ we have 
\[
...
complete details of a proof verifying that the bound in 
\eqref{eqn_proof_tag_PartialSumsOver_HatCkx_EquivCond_v2} 
is attained for all sufficiently large $x$. 
\end{proof} 
...
For $x \geq 3$, the 
variance of $\log C_{\Omega}(x)$ is given by 
\[
...
\label{eqn_proof_tag_COmeganVariance_InitEqn_v2}
	& =  n^2 (\log\log n)^2 (\log\log\log n)^2 (1+o(1)) + I_A(n), 
\end{align} 
...
\]
For all real $y > e$, the principal branch of the Lambert $W$-function has the 
...
\[
\frac{1}{n} \times \sum_{k \leq n} \log |g(k)| = 
	\frac{1}{2} \cdot (\log\log n) (\log\log\log n) - 
...
\]
In effect, we may consider finding a $M_x \geq 1$ such that for large $x$ 
$\Phi(M_x) = 1 + o(1)$ and then apply the last bound to find 
...
That is, the differences where $|g(n)|$ diverges from the asymptotic bound for its average 
