...
lengthy at a few pages of typeset mathematics. 
We dedicate 
Appendix \hlocalref{Appendix_ProofOfCOmegan_LogarithmicAvgOrderFormula} 
to providing the full details needed to establish this result. 
...
We thank him very sincerely for the careful discussion of these results and 
for the helpful hints on applying the results from his articles. 
...
use the identity that $a^{an} = e^{n \cdot a \log a}$ in simplifying the main term obtained 
...
L_{E,\Omega}(x) & := 
	...
	\log C_{\Omega}(n).
\end{align*}
The choice to break the main and error term ranges of summation into the disjoint ranges of 
$\Omega(n) \in \left[1, \log\log x\right] \bigcup \left(\log\log x, \frac{\log x}{\log 2}\right]$ 
is somewhat arbitrary. The results in 
Appendix \hlocalref{subSection_TheKnownDistsOfThePrimeOmegaFunctions_IntroResults_v1} 
suggest that the values of $\Omega(n)$ are concentrated around $\log\log x$ for large $x$ 
so that we expect to obtain the main term summing over those $n \leq x$ such that 
$\Omega(n)$ is very close to the dominant term of the average order of this function. 
...
\left\lvert I_1(x) \right\rvert & \ll (\log\log x) \times \max_{1 \leq t \leq \log\log x} 
     \frac{(\log\log\log x) (\log\log x)^{t+2}}{\Gamma(2 + t)} \\ 
\notag 
...
The following observations are similarly not difficult to prove:
\begin{align}
...
\end{align}
\end{subequations}
Equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v3}, 
equation \eqref{eqn_proof_tag_LOmegax_MainTerm_v1}, and the last observations in 
...
L_{E,\Omega}(x) = o\left(x (\log\log x) (\log\log\log x)\right), 
     \mathtext{ as } x \rightarrow \infty. 
\end{equation}
We have that 
\begin{equation}
