...
\[
\mathbb{E}[C_k(n)] \gg (\log\log n)^{2k-2}, \mathrm{\ as\ }n \rightarrow \infty. 
\]
\end{theorem} 
...
\[
B + o(1) \leq \frac{1}{x} \cdot \#\left\{n \leq x: |G^{-1}(n)| - |G_E^{-1}(n)| \leq O(1)\right\} \leq 
     C + o(1), 
\]
...
\[
|G^{-1}(x_0)| \geq \left\lvert G_E^{-1}(x_0) \right\rvert + O(1). 
\]
\end{theorem} 
...
be that 
\[
\mathbb{E}[C_m(n)] \gg (\log\log n)^{2m-2}. 
\] 
Moreover, as we prove inductively below, for all $1 \leq m < k$ the ratio 
\[
\frac{\mathbb{E}[C_m(n)]}{(\log\log n)^{2m-1}} \gg \left(\frac{\log n}{\log\log n}\right)^{m-1} \times (\log\log n). 
\]
This infinitely tending ratio distinction is enough to justify the first of the next steps bounding 
$\mathbb{E}[C_k(n)]$ below for large enough $n \rightarrow \infty$ starting the induction steps below. 

Now in particular, we have by the recursive formula in \eqref{eqn_Ckn_recFormula_v1} that 
as $n \rightarrow \infty$ 
\begin{align} 
\notag 
\mathbb{E}[C_k(n)] & \gg \mathbb{E}\left[
     \sum_{p|n} \sum_{d\bigr\rvert\frac{n}{p^{\nu_p(n)}}} 
     \sum_{i=1}^{\nu_p(n)} \log\log(dp^i)^{2k-4} 
     \right] \\ 
\label{eqn_proof_tag_Ckn_AsymptoticExp_v1} 
     & \approx \mathbb{E}\left[\sum_{p|n} \sum_{d\bigr\rvert\frac{n}{p^{\nu_p(n)}}} \left[ 
     \int \log\log(dp^{\alpha})^{2k-4} d\alpha\right] \Biggr\rvert_{\alpha=\nu_p(n)}
     \right]. 
\end{align} 
...
In particular, for $p|n$ and $n \geq 2$ large, we let the parameters assume average order 
values (each with respect to $n$) of 
\[
\mathbb{E}[\nu_p(n)] = \log\log n, \mathbb{E}[p] = \frac{\pi(n)}{n} \sim \frac{1}{\log n}, 
     \mathbb{E}[d] = \frac{\log n}{n}. 
\]
Then we evaluate the integral from above where $\alpha \mapsto \nu_p(n)$ as 
\begin{align*} 
\int (\log\log(dp^{\alpha}))^{2k-4} d\alpha & \sim 
     \alpha \times \log\log(dp^{\alpha})^{2k-4} + 
     \frac{\log d}{\log p} \times \log\log(dp^{\alpha})^{2k-4} \\ 
     & \SuccSim 
     \alpha \times \log\log(dp^{\alpha})^{2k-4}. 
\end{align*} 
...
\begin{align*} 
\mathbb{E}[C_k(n)] & \gg (\log\log n)^2 \mathbb{E}\left[\frac{d(n)}{(\nu_p(n) + 1)}\right] \times 
     \log\left[\left\lvert \log\log n - \log n - (\log\log n)^2\right\rvert\right]^{2k-4} \\ 
     & \sim (\log n) \cdot (\log\log n) \times \left[\log\log n + \log(1 - o(1))\right]^{2k-4} \\ 
     & \sim (\log n) \cdot (\log\log n)^{2k-3} \\ 
     & \gg (\log\log n)^{2k-2}, \mathrm{\ as\ } n \rightarrow \infty. 
\end{align*} 
...
\[
\mathbb{E}[C_{\Omega(n)}(n)] \gg (\log n) (\log\log n)^{2\log\log n - 2} \gg \log n \cdot \log\log n.  
\]
...
|G^{-1}(x_0)| < |G_E^{-1}(x_0)| + O(1), \forall x_0 \in [(B-\varepsilon) x, (C+\varepsilon) x]. 
\end{equation} 
...
$$\mathcal{G}_0(x) := \frac{1}{x} \cdot \#\left\{n \leq x: |G^{-1}(x_0)| - |G_E^{-1}(x_0)| \leq O(1)\right\},$$ 
...
\begin{align*} 
\mathcal{G}_1(x) & := \frac{1}{x} \cdot \#\left\{n \in \mathcal{D}_1(x): |G^{-1}(x_0)| - |G_E^{-1}(x_0)| \leq O(1)\right\} 
     \in [(B-\varepsilon)^2, (B-\varepsilon) (C+\varepsilon)] + o(1) \\ 
\mathcal{G}_2(x) & := \frac{1}{x} \cdot \#\left\{n \in \mathcal{D}_2(x): |G^{-1}(x_0)| - |G_E^{-1}(x_0)| \leq O(1)\right\} 
     = C - B + 2\varepsilon + o(1) \\ 
\mathcal{G}_3(x) & := \frac{1}{x} \cdot \#\left\{n \in \mathcal{D}_3(x): |G^{-1}(x_0)| - |G_E^{-1}(x_0)| \leq O(1)\right\} 
...
such that $|G^{-1}(x_0)| \geq |G_E^{-1}(x_0)| + O(1)$ for all sufficiently large $x$. 
\end{proof} 
...
\begin{itemize} 
     \item[\textbf{(A)}] $\mathbb{E}[C_{\Omega(n)}(n)] \gg (\log n) \cdot (\log\log n)^{2\mathbb{E}[\Omega(n)]-2} \gg 
     ...
     \mathcal{C}_E := \left\{n \geq 1: 
          ...
          \sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2\log\log d - 2}  
          \right\}. 
     \]
...
\mathbb{E}[C_{\Omega(n)}(n)] & \gg \frac{1}{n} \sum_{d \leq n} (\log\log d)^{2\Omega(d)-1} \\ 
     & \sim (\log n) \cdot (\log\log n)^{2\log\log n - 2}, \mathrm{\ as\ } n \rightarrow \infty, 
\end{align*} 
and hence 
\begin{align*} 
...
\frac{1}{t} \times \int_{e^e}^{n} (\log\log t)^{2\log\log t - 2} dt & \approx 
     \frac{1}{t} \times \int_{e^e}^{n} \frac{(1 + \log\log t)^{2\log\log t}}{(\log\log t)^2} dt \\ 
     & = \frac{1}{t} \times \int_{e^e}^{n} 
     \sum_{s \geq 0} \sum_{k=0}^{s} \gkpSI{s}{k} (2\log\log t)^{k} (-1)^{s-k} \times 
     \frac{(\log\log t)^{s-2}}{s!} dt. 
     \qedhere 
\end{align*} 
\end{proof} 
...
\label{eqn_proof_tag_ExpectedUpperBound_v1} 
     & \gg (\log n) \cdot (\log\log n)^{2\log\log n - 2} + \frac{(\log n)^2}{2} \cdot (\log\log n)^{2\log\log n - 2}. 
\end{align} 
...
\sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2 \log\log d - 2} & \ll 
     \sum_{\substack{d|n \\ d > e^e}} (\log d) \cdot (\log\log d)^{2\log\log n - 2}. 
\end{align} 
...
\sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2 \log\log d - 1} & \leq 
     (\log n) \cdot (\log\log n)^{2\log\log n - 2} + 
     d(\floor{\sqrt{n}}) (\log n) (\log\log n)^{2\log\log n - 2}. 
\end{align} 
...
we have that 
\[
\sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2\log\log d - 2} \leq 
     (\log n) \cdot (\log\log n)^{2\log\log n - 2} + 
     \frac{(\log n)^2}{2} (\log\log n)^{2\log\log n - 2}. 
\]
...
\]
Moreover, re-iterating our argument from the proof of part (B) where we arrived at 
...
G^{-1}(x) \geq \sum_{n \leq x} \lambda(n) \sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d) + O(1). 
\end{equation} 
...
\mathcal{A}_E := \left\{x \geq 1: 
     G^{-1}(x) \geq G_E^{-1}(x) + O(1) \wedge G^{-1}(x) \leq 0 
     \right\}, 
\] 
we have that $|G^{-1}(x)| - |G_E^{-1}(x)| \leq O(1)$, $\forall x \in \mathcal{A}_E$. 
...
$|G^{-1}(x)| - |G_E^{-1}(x)| \leq O(1)$ holds: 
\[
B + o(1) \leq \frac{1}{x} \cdot \#\left\{n \leq x: |G^{-1}(n)| - |G_E^{-1}(n)| \leq O(1)\right\} \leq C + o(1), 
...
By Proposition \ref{prop_GInvGeqBehavior_v1} (see Remark \ref{remark_AsymptoticDensitiesOfExceptionalSets_v1}) 
and since 
\begin{align*} 
\int \frac{2}{t \cdot \log(x/t)} dt & = -2 \log\log(x/t) + C \\ 
\int \frac{(\log t)^3 (\log\log t)}{t \cdot \log x} dt & = \frac{1}{4} (\log t)^3 (\log\log t) - 
     \frac{(\log t)^3}{16} + C, 
\end{align*} 
no matter how we try to non-trivially minimize the expression in 
\eqref{eqn_proof_tag_MxAbsValueConsequence_v2}, we still find that 
\[
\frac{|M(x)|}{x} \xrightarrow{x \rightarrow \infty} \infty. 
\]
So we recover from the previous calculations that 
there is a tendency of $|M(x)| / x$ to grow without bound as $x \rightarrow \infty$ \footnote{ 
     Notice in fact bootstrapping and looking ahead to Theorem \ref{theorem_gInv_GeneralAsymptoticsForms}, 
     since we have that 
     \begin{align*} 
     \left\lvert \int_{e^e}^{x} \frac{\log t \cdot \log\log t}{t^2 \cdot \log x} dt \right\rvert & \gg 
          |\operatorname{Ei}(\log x)| 
          \geq \left\lvert \frac{1+\gamma+\log\log x - \frac{3}{4}\log x}{\log x} \right\rvert 
          \sim \frac{3}{4}, 
     \end{align*} 
     we still recover a contradiction to the known property that $M(x) = o(x)$ as 
     $x \rightarrow \infty$. 
}. 
