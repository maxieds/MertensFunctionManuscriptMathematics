...
\subsection{Fixing an exact expression for $M(x)$ tied to properties of strongly additive functions} 
...
\frac{|M(x_y)|}{\sqrt{x_y}} & \SuccSim 
     \frac{C_{\ell,1} (\log\log \sqrt{x_y})^{4} \sqrt{\log\log\log \sqrt{x_y}}}{ 
     (\log\log\log\log \sqrt{x_y})^{\frac{5}{2}}} + o(1), \mathrm{\ as\ } y \rightarrow \infty. 
\end{align*} 
...
\begin{align*} 
\mathbb{E}[C_{\Omega(n)}(n)] & = \frac{1}{n} \sum_{d \leq n} (\log\log d)^{2\Omega(d)-1} \\ 
...
\begin{align*} 
\sum_{d|n} \mathbb{E}[C_{\Omega(d)}(d)] - |g^{-1}(n)| & \geq 
     (\log n) \cdot \mathbb{E}[C_{\Omega(n)}(n)] - |\mathbb{E}[g^{-1}(n)]| \\ 
     & \SuccSim \left(\frac{\log n}{2} - 1\right) \mathbb{E}[C_{\Omega(n)}(n)] \\ 
...
\mathcal{A}_E := \left\{x \geq 1: 
     G^{-1}(x) \leq \sum_{n \leq x} \lambda(n) \sum_{\substack{d|n \\ d > e^e}} \mathbb{E}[C_{\Omega(d)}(d)] \wedge 
     G^{-1}(x) \geq 0 
     \right\}, 
\]
...
such that $f_k(n) > 0$ for all $n \geq 1$ and $1 \leq k \leq \Omega(n)$ where 
...
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     ...
     (\log\log\log\log x)^{\frac{5}{2}}} \left[ 
...
$|G_{\ell}^{-1}(x)|$ does not correspond the integral term calculated in 
\eqref{eqn_proof_thm_GInvFunc_v4_approx}, although is close up to an additional 
factor of $\log\log x$. 
...
- & \int_{\sqrt{x}}^{x/2} \frac{2 x}{t^2 \log(x)} \cdot |G_{\ell}^{-1}(t)| dt \SuccSim 
     ...
     & = C_{\ell,1} \cdot \sqrt{x} \times 
      \frac{(\log\log \sqrt{x})^{4} \sqrt{\log\log\log \sqrt{x}}}{ 
