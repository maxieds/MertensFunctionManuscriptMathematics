...
the average order of the unsigned sequence is bounded by by 
\[
\mathbb{E}|g^{-1}(n)| = \frac{3}{\pi^2} (\log n)^2 \sqrt{\log\log n}(1+o(1)),  
...
to bound the function $\mathcal{N}_{\omega}(x)$ as\footnote{
     ...
     \[
     \int_1^2 (\log x)^{r-1-r\log r} dr \ll \frac{1}{(\log x)^{\delta}}. 
     \]
}
\begin{align*}
...
\notag 
     & = 
     \frac{1}{x} \times \sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} 
     C_{\Omega(n)}(n) (1+o(1)), 
...
\frac{1}{x} \times 
     \sum\limits_{k > \log\log x} \sum\limits_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n)
...
by integration by parts with the Abel summation formula that whenever $1 < |z| < P(2)^{-1}$ 
...
\sum_{\substack{n \leq x \\ \Omega(n) \geq r\log\log x}} 
     C_{\Omega(n)}(n) r^{\Omega(n)} & 
     \ll x (\log x)^{-r \log r} \times \sum_{n \leq x} C_{\Omega(n)}(n) r^{\Omega(n)} \\ 
...
\sum_{\substack{n \leq x \\ \Omega(n) \geq r\log\log x}} 
     C_{\Omega(n)}(n) r^{\Omega(n)} 
     \ll x \sqrt{\log\log x} \times (\log x)^{r-r\log r}, 
     \text{ for all } 1 \leq r < P(2)^{-1}. 
\]
When $1 \leq r < P(2)^{-1}$ we have 
\[
x \sqrt{\log\log x} (\log x)^{r-r\log r} \gg \sum_{\substack{n \leq x \\ \Omega(n) \geq r\log\log x}} 
     C_{\Omega(n)}(n) r^{\Omega(n)} \gg 
     \sum_{\substack{n \leq x \\ \Omega(n) \geq r\log\log x}} 
     C_{\Omega(n)}(n) (r \log\log x)^{\log\log x}. 
\]
This implies that for $r \in [1, P(2)^{-1})$ we have 
\[
\widehat{B}(x, r) \ll \frac{x (\log x)^{-r\log r}}{(\log\log x)^{\log\log x-1/2}} = o(x). 
\]
...
S_2(x) & := \frac{1}{x \sqrt{\log\log x}} \times \sum_{k \geq \log\log x} \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     C_{\Omega(n)}(n) \\ 
     & \ll \frac{1}{x \sqrt{\log\log x}} \times \left[ 
     \int_{1}^{P(2)^{-1}} \widehat{B}(x, r) dr + 
     \frac{x}{(\log x)^{1.7546} (\log\log x)^{\log\log x-1/2}} \right] \\ 
     & \ll \int_1^{P(2)^{-1}} (\log x)^{r-r\log r} dr + o(1). 
\end{align*} 
We claim that the integral term in the previous equation is $\ll (\log x) / \log\log x$. 
...
\begin{align*}
\int_a^b (\log x)^{r-r\log r} dr & \ll \int_a^b \left[\sum_{k \geq 0} \sum_{s=0}^{k} 
     \binom{k}{s} \frac{(\log\log x)^{k}}{k!} \times (-n)^s (1+n)^{k-1} r^{k+\frac{s}{n}}\right] dr \\ 
     & \ll \frac{n}{r \cdot (\log\log x) (\log x)^{r(r^{1/n}-1-n)}} \Biggr\rvert_{r=a}^{r=b} \\ 
     & \ll \frac{\log x}{\log\log x}. 
\end{align*} 
The last step is obtained by evaluating $r$ at the lower limit of $r = a = 1$ and setting 
$n := \frac{1}{\log\log x}$ so that $(\log x)^{1/\log\log x} \rightarrow e$ as 
$x \rightarrow \infty$. 
In conclusion, we have proved that 
$S_2(x) \sqrt{\log\log x} = o\left((\log x) \sqrt{\log\log x}\right)$ 
as $x \rightarrow \infty$. 
\end{proof} 
...
\int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = 
     \frac{1}{2} (\log t)^2 \sqrt{\log\log t} (1+o(1)). 
\end{align*} 
...
By Taylor's theorem for the exponential function with remainder terms, 
...
S_3(x) := \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k+1/2}}{(2k+1) (k-1)!} = 
...
\]
By bounding the remaining hypergeometric series term in the expansion of 
$S_3(x)$, we see that 
\begin{align}
...
\end{align}
The rightmost sum corresponds to another error term. Indeed, we see that the inner sum in the 
second to last line over the bounds $2k+3 \geq \log\log x$ 
is convergent to some constant. The leading product factors remaining 
...
\sum_{\substack{k \geq 0 \\ 2k+3 < \log\log x}} 
     ...
     & = \sum_{\substack{k \geq 0 \\ 2k \leq \log\log x-3}} \left[ 
          1 + \frac{(2k+1)(2k+2)}{2 \log\log x} + O\left(\frac{k^2}{(\log\log x)^2}\right)\right] \\ 
     & \sim  
