...
The definitions of the primary subsequences we define, and the 
...
connect the distributions of $L(x)$, a precisely defined 
...
DGF used by Montgomery and Vaughan in the reference \cite[\S 7.4]{MV}.
...
\end{align*}
Then it follows that for $r := \frac{k-1}{\log\log x}$, the sums 
\[
...
\sum_{k=1}^{\mathbf{2}\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \sim 
...
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v1} 
     & = 
     \frac{1}{x} \times \sum_{k=1}^{\mathbf{2}\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} 
     C_{\Omega(n)}(n) (1+o(1)), 
...
\frac{1}{x} \times 
     ...
     = O\left((\log x)^{\mathbf{0.613706}} \times (\log\log x)\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{equation} 
...
\end{align}
The dropped error term in the last formula follows from the asymptotic series for 
...
applying \eqref{eqn_COmegannzPowOmeganLLRelation_v1} that for $1 \leq r < P(2)^{-1}$ 
\begin{align*}
...
\]
When $1 \leq r < P(2)^{-1}$ we also have 
\[
...
\]
This implies that for $\mathbf{r := 2}$ we have 
\begin{equation}
\label{eqn_BHatxrUpperBound_v1}
\widehat{B}(x, r) \ll x (\log x)^{r-2r\log r} \sqrt{\log\log x} = 
     O\left(x (\log x)^{\mathbf{0.613706}} \times \sqrt{\log\log x}\right)
\end{equation}
We wish to evaluate the limiting asymptotics of the sum 
\begin{align*}
S_2(x) & := \frac{1}{x \sqrt{\log\log x}} \times 
     \sum_{k \geq \mathbf{2}\log\log x} \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     C_{\Omega(n)}(n) \ll \widehat{B}(x, 2). 
\end{align*} 
We have proved that 
$S_2(x) \sqrt{\log\log x} = O\left((\log x)^{\mathbf{0.61306}} (\log\log x)\right)$ 
...
\begin{align*} 
\frac{1}{\sqrt{2\pi}} \times \int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = 
...
$$G^{-1}(x) = O\left(L(t_0) \times \mathbb{E}|g^{-1}(x)|\right).$$ 
If the RH is true, then 
for any $\varepsilon > 0$ and all large integers $x > e$ we have that 
\[
G^{-1}(x) = O\left(\frac{\sqrt{x} (\log x)^2}{\sqrt{\log\log x}} \times \exp\left( 
...
Much in the same way that variants of the famous Erd\H{o}s-Kac theorem are proved 
...
Suppose that $a,z > 0$ are such that $z = \lambda a$. If $\lambda > 1$, then as 
$z \rightarrow +\infty$ 
\[
...
\]
By the variant of Stirling's formula in \cite[\cf Eq.\ (5.11.8)]{NISTHB} 
\[
...
\begin{remark}
Gerg\H{o}, things have changed slightly where I need to use this lemma in my proof in the main body of the argument. 
I need that the upper bound of summation is $c \log\log x$ (for the same summands) where $2 \geq c > 1$ 
is an absolute constant (It really doesn't matter which constant, as long as it's in that range). 
With this, it seems that the argument using your reference at the transition point is no longer 
relevant. Can you please help me to adapt this proof using the references to your own work that you know so well? 
\end{remark}
...
For $x \rightarrow +\infty$, we have that 
\begin{align*}
S_3(x) := \sum_{1 \leq k \leq 2\log\log x} \frac{(\log\log x)^{k-1/2}}{(2k-1) (k-1)!} = 
     \frac{\log x}{4\sqrt{\log\log x}} + O\left(\frac{\log x}{(\log\log x)^{3/2}}\right).       
\end{align*}
\end{lemma}
\begin{proof}
...
(\textbf{TODO: Is this part of the introduction to the proof still relevant???})
...
\widetilde{S}_n(t) & = \int_0^1 \left(\sum_{k=1}^{n} \frac{(s^2t)^{k-1}}{(k-1)!}\right) ds \\ 
     ...
     & = \frac{1}{2(n-1)!} \times \int_0^1 \frac{e^{xt}}{\sqrt{x}} \Gamma(n, xt) dx. 
\end{align*}
Integration by parts performed one time with 
\[
     \left\{\begin{array}{ll} 
     u_x = \Gamma(n, xt) & v_x^{\prime} = \frac{e^{xt}}{\sqrt{x}} dx \\ 
     u_x^{\prime} = -t^n x^{n-1} e^{-xt} dx & v_x = 
     \sqrt{\frac{\pi}{t}} \operatorname{erfi}\left(\sqrt{tx}\right)
     \end{array} 
     \right\}, 
\]
implies that 
...
\notag
\widetilde{S}_n(t) & = 
     \frac{1}{2(n-1)!} \sqrt{\frac{\pi}{t}} \Gamma(n, xt) \operatorname{erfi}\left(
     \sqrt{xt}\right) \Bigr\rvert_0^1 + 
     \frac{\sqrt{\pi} \cdot t^{n-1/2}}{2(n-1)!} \times \int_0^1 x^{n-1} e^{-xt} 
     \operatorname{erfi}\left(\sqrt{tx}\right) dx \\ 
\label{eqn_ProofTag_AppendixLastSumLemma_IBP_resulting_exp_terms_v1}
     & = 
     \frac{1}{2(n-1)!} \sqrt{\frac{\pi}{t}} \Gamma(n, t) \operatorname{erfi}\left( 
     \sqrt{t}\right) + \frac{\sqrt{\pi}}{2t^{3/2} (n-1)!} \times \int_0^{t} 
     s^{n-1} e^{-s} \operatorname{erfi}\left(\sqrt{s}\right) ds. 
\end{align}
...
\]
(\textbf{TODO: Things change from here on ... }) 
