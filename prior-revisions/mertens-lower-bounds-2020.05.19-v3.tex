...
we have that 
\[
d(n) > \left\lvert \#\{d|n: d \mathrm{\ squarefree}, \mu(d)\lambda(n/d) = +1\} - 
     \#\{d|n: d \mathrm{\ squarefree}, \mu(d)\lambda(n/d) = -1\} \right\rvert. 
\]
This property shows that 
\[
\sum_{d|n} C_{\Omega(d)}(d) > |g^{-1}(n)|, 
\]
...
\begin{itemize} 
     \item[(A)] $\mathbb{E}[C_{\Omega(n)}(n)] \gg (\log\log n)^{2\mathbb{E}[\Omega(n)]-1}$; and 
     \item[(B)] $\mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] \geq \sum_{d|n} \mathbb{E}[C_{\Omega(d)}(d)]$ 
...
By Proposition \ref{prop_CondAvgOrderGInvxSummatoryFunc_KeyProp_v2} we have that 
\[
...
\]
Then we can see from the previous two 
equations that 
\[
0 < \alpha_0 \leq \lim_{x \rightarrow \infty} \frac{1}{x} \cdot 
...
$\alpha_0 \in \left\{\lambda_{-}\left(1-\frac{6}{\pi^2}\right), \frac{6}{\pi^2} \cdot \lambda_{+}\right\}$ (TODO) and 
$\alpha_1 \in \left\{\lambda_{-} + \lambda_{+} \cdot \frac{6}{\pi^2}, \frac{6}{\pi^2} \cdot \lambda_{-} + \lambda_{+}\right\}$ (TODO), 
each depending on which of the densities $\beta,1-\beta$ is positive. 
\end{proof} 
