...
average order is asymptotically large.
...
\left\lvert \sum_{k=1}^{a \log\log x} 
     ...
     \times \frac{(\log x)^{a-a\log a}}{\sqrt{\log\log x}} \times 
     \left(1 + O\left(\frac{1}{\log\log x}\right)\right). 
\end{align}
...
whenever $q \in \left(0, \frac{1}{3}\right)$. 
We perform the change of variable $v := \frac{t-\log\log x}{\sqrt{\log\log x}}$, noting that the 
...
(for example, set $q := \frac{3}{11} + \varepsilon$ for any 
