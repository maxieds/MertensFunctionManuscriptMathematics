...
\end{equation}
This inverse function is computed recursively 
...
The auxiliary unsigned function $C_{\Omega}(n)$ from 
\cite{FROBERG-1968} has the exact formula given by 
\begin{equation}
...
Theorem \hlocalref{cor_CLT_VII} is identically one 
(see Remark \hlocalref{remark_AbsConstB0IsIdenticallyOne_v2}). 
...
\begin{remark}
\label{remark_AbsConstB0IsIdenticallyOne_v2}
The multiplier function $\widetilde{G}(z)$ in 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz} 
satisfies $\widetilde{G}(1) = 1$ and that 
\[
\widetilde{G}\left(\frac{k-1}{\log\log x}\right) = \widetilde{G}(1) + O\left( 
	\frac{1 + \left\lvert k - \log\log x \right\rvert}{\log\log x}\right), 
	\text{ for } 1 \leq k \leq \frac{3}{2} \log\log x. 
\]
For fixed $r \in (0, 1) \cup \left(1, \frac{3}{2}\right]$ and 
$k \sim r\log\log x$ we also have 
\[
\frac{(\log\log x)^{k-1}}{(\log x) (k-1)!} \times k \log(1 + k) \ll_r 
	\frac{(\log x)^{r-1-r\log r}}{\sqrt{\log\log x}} \times k \log(1 + k) \ll 
	\sqrt{\log\log x} (\log\log\log x). 
\]
Thus, the absolute constant $B_0 > 0$ from 
Theorem \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1} 
is actually identically one. 
The existence of a limiting constant $B_0$ in 
equation \eqref{eqn_proof_tag_SumLogCOmegan_P0_exp_v2} 
follows from the mean value theorem applied to the expansion of 
equation \eqref{eqn_proof_tag_SumLogCOmegan_P0_exp_v1} by 
Theorem \hlocalref{theorem_HatPi_ExtInTermsOfGz} and 
Binet's formula for the log-factorial function, $\log(k!) = \log\Gamma(1 + k)$. 
\end{remark}

\begin{definition}
\label{def_AvgOrder_FirstAndSecondMomentsOfFuncs_v1}
For any integers $x \geq 1$, we define the expectated (mean) value of the 
