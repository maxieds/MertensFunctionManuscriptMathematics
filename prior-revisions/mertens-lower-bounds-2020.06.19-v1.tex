...
$x \geq 1$. It has a well-known disproof due to Odlyzko and t\'{e} Riele given in the early 1980's by computation of 
...
unbounded in the direction of both $\pm \infty$ along infinite subsequences of positive integers. 
We prove the unboundedness of $|M(x)| / \sqrt{x}$ by showing that 
$$\limsup_{x \rightarrow \infty} \frac{|M(x)| (\log\log x)^{\frac{11}{4}} (\log\log\log x)^{2}}{ 
...
For almost every large $x$ we have uniformly for $1 \leq k \leq \log\log x$ that 
\[
\widehat{\pi}_k(x) \SuccSim \frac{4}{3\sqrt{\pi}} \frac{x}{\log x} \left(\frac{\log 2}{\log x}\right)^{ 
     \frac{k-1}{\log\log x}+2} \frac{(\log\log x)^{k-1}}{(k-1)!} \left( 
...
is the next important summary corollary that 
...
generating function like identity\footnote{ 
     ...
     \prod_p \left(1 - \frac{z}{p^s}\right)^{-1} = \sum_{n \geq 1} \frac{z^{\Omega(n)}}{n^s}, 
          \Re(s) > 1.  
     \]
     It is also not a difficult exercise to prove that for any additive arithmetic function $a(n)$, 
     ...
     \sum_{n \geq 1} \frac{z^{a(n)}}{n^s} = \prod_p \left( 
          1 + \sum_{m \geq 1} \frac{z^{a(p^m)}}{p^{ms}}\right), \Re(s) > 1. 
     \]
}
\[
...
Theorem \ref{theorem_HatPi_ExtInTermsOfGz}. This parameter determines the 
precise range of $k$ up to which 
we obtain the valid uniform bounds in $x$ on the exact asymptotic formulas for 
$\widehat{\pi}_k(x)$. 
...
$\delta \rightarrow 0^{+}$ as $x \rightarrow \infty$. This provides a limiting constant-valued 
...
Thus, if we form an approximation to the function 
$\mathcal{G}(z)$ in Theorem \ref{theorem_HatPi_ExtInTermsOfGz} by truncating the 
product factor as $\widehat{P}(z, x)$ defined 
above by indexing only for primes $0 \leq p \leq x$, then what we 
...
\end{align*} 
We have the uniform bound parameter 
$1 < R < 2$ so that 
$$z \equiv z(k, x) = \frac{k-1}{\log\log x} \in [0, R),$$ 
in the notation of Theorem \ref{theorem_HatPi_ExtInTermsOfGz} 
for all $x \gg e + 1$ as $x \rightarrow \infty$.  
To perform analysis on the extremal values of the contributions from each 
weight term (in $z$) on the functions $(a_{\ell}, b_{\ell}, c_{\ell})$ given above, 
we have to rule out one slice of exceptional cases that can make the bound 
infinite at large $x$. Namely, we observe that $z \equiv z(k, x)$ satisfies $z = 1$ 
precisely if $x = \exp\left(e^{k-1}\right)$ for $x,k$ both integers. 
This case happens almost nowhere for the positive integers 
$x \in \mathbb{Z}^{+} \cap \{n > e: n \in \mathbb{Z}\}$. 
So for almost every sufficiently large $x \in \mathbb{Z}^{+}$, we have by 
elementary calculus that the extremal bounds of the coefficient weight functions in $z$ 
happen either for $z := 0$ or as $z \rightarrow 2^{-}$. 
...
\[
f(z) := 1 - \frac{2}{1-z} + \frac{1}{(1-z)^2}, z \in [0, 2), 
\] 
is also almost always bounded by $f(z) \in [0, 4)$. 
In the former case, we obtain that the limiting exponent is zero in order to obtain 
a lower bound on \eqref{eqn_proof_tag_PHatFiniteTruncProdFactorOfGz_v3}. 
We can similarly conclude that almost everywhere 
\[
1 + z - \frac{1}{1-z} \geq z, z \in [z, z+2). 
\]
Then we obtain the next effective lower bounds on the function from 
\eqref{eqn_proof_tag_PHatFiniteTruncProdFactorOfGz_v3} for almost every 
sufficiently large $x \in \mathbb{Z}^{+}$:  
\begin{align} 
\label{eqn_proof_tag_simpl_v1} 
\frac{e^{-Bz}}{(\log x)^{z}} \times \exp\left(\widehat{\mathcal{B}}(u, x; z)\right) & \gg 
     \left(\frac{\log 2}{\log x}\right)^{z + 2} =: \widehat{\mathcal{C}}(u, x; z) 
\end{align} 
Essentially, what we are arguing in excluding the case where $z = 1$ is that the possibility 
the function becomes infinite at $x$ when $z \equiv z(k, x)$ depends on this $x$ 
is negligible. In fact, we choose to exclude this 
possibility by just removing any possible cases where this can happen from our consideration 
in our later sums involving the new lower bounds on $\widehat{\pi}_k(x)$. 
...
More to this point, since $C_{\Omega(n)}(n) = |h^{-1}(n)|$ in the notation of the proof 
...
\subsubsection{The relation to the distribution of the primes moving forward} 

\begin{remark}[Combinatorial relations between the inverse functions and prime factorizations of $n$]
...
\]
Then we have essentially shown using \eqref{eqn_AbsValueOf_gInvn_FornSquareFree_v1} that we can write 
\[
...
Section \ref{Section_PrelimProofs_Config} then 
...
\end{align*} 
Moreover, the correctness of the transition from 
...
happen when $G^{-1}(x) = 0$. This singularity in the distribution of the signed summatory 
function occurs at most as frequently as on an 
asymptotically thin subset of the positive integers. 
...
we can establish that\footnote{ 
     This definition actually provides a lower bound on the famous special case of 
     the summatory function of the Liouville lambda function, 
     $L_0(x)$, for almost every sufficiently large $x$. 
} 
\begin{align} 
...
\notag 
     & \gg \frac{2\sqrt{2} e (\log 2)^3}{3\pi} \cdot 
...
T_E(x) := \frac{1}{\log\left[(\log x)^{\frac{3}{2}}\right]^{3} 
     \sqrt{\log\log\left[(\log x)^{\frac{3}{2}}\right]}} \gg 
     \frac{1}{(\log\log x)^{3} \sqrt{\log\log\log x}}. 
\end{equation} 
...
\[
C_{\ell,1} = \frac{8 e^2 \log^6(2)}{9 \pi^2} \approx 0.0738056.  
\]
...
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     ...
     (\log\log x)^{\frac{11}{4}} (\log\log\log x)^2}. 
\end{align*} 
\end{theorem} 
\NBRef{A10-2020.04-26} 
\begin{proof} 
...
\notag 
     & \SuccSim  
     \frac{2\sqrt{2} e (\log 2)^3}{3\pi} \cdot 
     \frac{t}{(\log t)^3 \sqrt{\log\log t}} \left( 
     1 + O\left(\frac{1}{\log\log t}\right) 
     \right) \\ 
\label{proof_thm_GInvFunc_v0} 
     & \gg \frac{2\sqrt{2} e (\log 2)^3}{3\pi} \cdot 
     \frac{t}{(\log t)^3 \sqrt{\log\log t}}, 
     \mathrm{\ as\ } t \rightarrow \infty. 
\end{align} 
...
$(-1)^{\floor{\log\log t}}$ in the form of\footnote{ 
     ...
     \left\{e^e \leq t \leq (\log x)^{\frac{3}{2}} (\log\log x): 
          ...
          \left(\bigcup_{k=1}^{\frac{1}{2} \log\log\left[(\log x)^{\frac{3}{2}} (\log\log x)\right]} 
...
$$I_{\ell}(t) := \frac{2\sqrt{2} e (\log 2)^3}{3\pi} \times 
...
I_{\ell}\left(e^{e^{2k}}\right) e^{e^{2k}}& \SuccSim 
     \frac{2 \sqrt{2} e^2 \log^6(2)}{9 \pi^2} \cdot \frac{e^{-\frac{11k}{2}}}{k^{\frac{3}{2}}} 
     =: \widehat{I}_{\ell}(k). 
\end{align} 
...
\begin{align} 
\notag 
\frac{2\sqrt{2} e (\log 2)^3}{3\pi} & \times (\log x)^{\frac{3}{2}} (\log\log x) \times T_E(x) \times 
     \left\lvert 
     ...
     e^{e^{2k}} dk \right\rvert \\ 
\notag 
     & \gg \frac{2\sqrt{2} e (\log 2)^3}{3\pi} \times (\log x)^{\frac{3}{2}} (\log\log x) \times T_E(x) \times 
     \Biggl\lvert 
...
\label{eqn_proof_thm_GInvFunc_v4v2_approx} 
     & \ll 
     \frac{e^{\frac{19}{4}} \cdot C_{\ell,1} \cdot (\log x)^{\frac{3}{2}}}{2 \cdot  
     (\log\log x)^{\frac{11}{4}} (\log\log\log x)^2}. 
\end{align} 
...
It is fortunate, and an indication of correct calculations, that up to distinct constant factors, the 
...
claim is accurate as the integral does not vanish for all large enough $x$. 
...
\]
Secondly, suppose that we have a smooth approximation for $|G^{-1}(t)|$ so that by the 
...
\sqrt{x} & \left\lvert \int_1^{x/2} \frac{|G^{-1}(t)|}{t^2 \cdot \log(x/t)} dt \right\rvert \\ 
     ...
     \right\rvert. 
\end{align*} 
Since $G^{-1}(x)$ changes only at $x \in \mathbb{Z}^{+}$, what we in fact exactly arrive at 
is a close variant of this mean value theorem type argument. The statements within the 
last few equations based on the smoothness approximation assumption for the function make it 
clear without complications what the tendencies are to bounding these growth rates. 

By Theorem \ref{theorem_GInvxLowerBoundByGEInvx_v1} proved in 
...
\frac{|M(x_{0,y})|}{\sqrt{x_{0,y}}} & \gg 
     ...
     (\log\log \sqrt{x_{0,y}})^{\frac{11}{4}} (\log\log\log \sqrt{x_{0,y}})^2}, 
