...
But is this the tightest possible $f$ provably (NO -- see below)? \\ 
...
\begin{remark}[A tighter bound on the terms of the DGF, $F(s, z)$, as a generating function in $z$] 
We have from TODO \ref{} that 
\[
TODO \leq TODO \leq TODO, 
\]

\end{remark} 

\begin{cor}[A tighter bound on $G_{\ell}^{-1}(x)$]
\end{cor} 
\begin{proof}
We omit the lengthy details which parallel the proof 
of Theorem \ref{} using the function $A_{\Omega}(t)$ 
noted in the previous remark. 
See the proof of the first theorem and make minor adaptations to obtain 
the result we have stated in this corollary. 
\end{proof} 

\begin{theorem}

\[
\limsup\limits_{x \rightarrow \infty} \left\{ 
     \frac{|M(x)|}{\sqrt{x} \cdot \left[\right]} 
     \right\} \geq TODO. 
\]
\end{theorem} 
\begin{proof} 
