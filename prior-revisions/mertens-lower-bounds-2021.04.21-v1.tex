...
Let $a,z,\lambda$ be positive real parameters such that $z=\lambda a$. If $0 < \lambda < 1$, then 
\[
...
\widetilde{S}_n(t) & = \int_0^1 \left(\sum_{k=1}^{n} \frac{(s^2t)^{k-1}}{(k-1)!}\right) ds \\ 
     ...
     & = \frac{1}{2t^{1/2}(n-1)!} \times \int_0^{t} \frac{e^{u}}{\sqrt{u}} \Gamma(n, u) du. 
\end{align*}
...
By \cite[Eq.\ (7.12.1)]{NISTHB} and the definition and asymptotic series for the 
function $\operatorname{erfi}(z)$ we gave in 
\eqref{eqn_Erfix_KnownAsymptoticSeries_v1}, 
...
$a := n$, $z := t$ and $\lambda := \frac{1}{2} + \frac{\xi}{n}$, we find that 
\[
