...
provide insight into why we arrived at the convenient approximations to 
$g^{-1}(n)$ proved in this section. The table data offers 
clear numerical data formed by examining the approximate behavior 
...
|G^{-1}(x_0)| < |G_E^{-1}(x_0)|. 
\end{equation} 
We have supposed that the constants $B,C \in (0, 1)$ are the tightest possible. That is, for all 
...
For $n \geq 1$, we have the disjoint interval decomposition of $\{n \leq x\}$ given by  
\[
\{n \leq x\} = \{1 \leq n < (B-\varepsilon) x\} \oplus \{(B-\varepsilon) x \leq n \leq (C + \varepsilon) x\} \oplus 
     \{(C+\varepsilon) x < n \leq x\}, 
\]
where the three disjoint sets denoted by $\mathcal{D}_i(x)$ for $i = 1,2,3$ yield that as 
$x \rightarrow \infty$, if \eqref{eqn_proof_tag_IneqSetG0x0_DNHold_v1} is true: 
\begin{align*} 
...
such that $|G^{-1}(x_0)| \geq |G_E^{-1}(x_0)|$. 
\end{proof} 
...
\begin{itemize} 
     \item[\textbf{(A)}] $\mathbb{E}[C_{\Omega(n)}(n)] = (\log n) \cdot (\log\log n)^{2\mathbb{E}[\Omega(n)]-1} \gg 
...
\eqref{eqn_proof_tag_OmeganGeqLeqAsymptoticDensityCalcs_v1} shows that we can expect the 
...
\end{align*} 
We claim that $\alpha_{+} = 0$. To prove this we first note the following classical 
...
\end{equation} 
Then we see that as $x \rightarrow \infty$, by Abel summation we have that 
\begin{align*} 
\sum_{\substack{n \leq x \\ n \in \mathcal{D}_{+}}} \log n & \geq \sum_{n \leq \alpha_{+}x} \log n \\ 
     & = (\alpha_{+} x) \cdot \log(\alpha_{+} x) - \int_0^{\alpha_{+}x} dt \\ 
     & \sim (\alpha_{+} x) \log x. 
\end{align*} 
...
\end{align*} 
which can only be true if $\alpha_{+} = 0$ through the formula given by \eqref{eqn_proof_tag_DnAsymptotics_v1}. 
By a similar argument reversing inequalities, we can see that indeed $\alpha_{-} = 0$ as well. 
...
approximation in the proof of fact (A). The result is calculated according to the 
...
Section \ref{subSection_OtherFactsAndResults}:  
\[
\int \frac{(\log t) (\log\log t)^{m}}{t} dt = \frac{(\log t)^2}{2} \times (\log\log t)^m. 
\]
Next, we bound the lower portion of the expected inequality from above to agree at a middleground 
where the $n \in \mathcal{C}_E$ converge: 
\begin{align} 
...
asymptotic density $\alpha_{+} = 0$. Now what this implies from \eqref{eqn_proof_tag_ExpectedUpperBound_v2} 
...
\]
Thus by \eqref{eqn_proof_tag_ExpectedUpperBound_v1}, we have that 
\[
...
What we have actually argued in less generality for the divisor function above holds for 
the expectation of positive arithmetic functions in general. That is, if $f(n) > 0$ for all $n \geq 1$, 
and if there exists a function $F$ such that 
\[
\sum_{n \leq x} f(n) \sim x \cdot F(x), 
\]
where $F(x) \not{\rightarrow} 0$ as $x \rightarrow \infty$, then the asymptotic density of the set of 
exeptional values of $f$ is zero: 
\[
...
\lambda_{-} & := \lim_{x \rightarrow \infty} \frac{1}{x} \cdot \#\{n \leq x: \lambda(n) = -1\}. 
\end{align*} 
Then we can prove that $\lambda_{+} = \lambda_{-} = \frac{1}{2}$. 
\end{facts} 
...
\[
2\sqrt{2} \leq \mathbb{E}[|g^{-1}(n)|] \leq \log(n) + o(1). 
\]
\end{lemma}
\begin{proof} 
Consider that using Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} we have 
\begin{align*} 
\frac{1}{x} \sum_{n \leq x} |g^{-1}(n)| & \geq \left\lvert 
     \frac{1}{x} \sum_{n \leq x} g^{-1}(n) \right\rvert \\ 
     ...
     & \geq \left\lvert \frac{1}{\sqrt{x}} \sum_{d \leq x} \frac{\lambda(d)}{\sqrt{d}} C_{\Omega(d)}(d) \right\rvert \\ 
     & \SuccSim \left\lvert \frac{1}{\sqrt{x}} \sum_{d \leq \lambda_{+} \cdot x} \frac{2}{\sqrt{d}} \right\rvert \\ 
     & \approx \left\lvert \frac{1}{\sqrt{x}} \int_0^{\lambda_{+} \cdot x} \frac{1}{t^{1/2}} dt \right\rvert \\ 
     & = 2\sqrt{2}. 
\end{align*} 
In the previous equations we used that $\lambda_{+} = \lambda_{-} = \frac{1}{2}$, that the function $1 / \sqrt{d}$ is 
monotone decreasing, and that $C_{\Omega(n)}(n) \geq 2$ for all $n \geq 1$. We also used that $|M(x)| \geq \sqrt{x}$. 

Notice that since $|M(x)| \leq x$ for all $x \geq 1$, we can follow similar steps to show instead that 
\[
\mathbb{E}[|g^{-1}(n)|] \leq \log(\lambda_{+} \cdot x) + \gamma + O\left(\frac{1}{x}\right), 
\]
which implies our upper bound. 
\end{proof} 
...
\mathbb{E}[C_{\Omega(d)}(d)] - |g^{-1}(n)| & \geq \mathbb{E}\left[ 
     ...
     & \sim \frac{(\log n)^2}{2} (\log\log n) - (\log n) \\ 
...
\mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] - |g^{-1}(n)| \geq 
     \sum_{d|n} \mathbb{E}[C_{\Omega(d)}(d)] - |g^{-1}(n)| \geq 0. 
\end{equation} 
\end{subequations} 
Notice that the intersection of the sets $\mathcal{C}_E \cap \mathcal{S}_E$ necessarily also 
...
on the set satisfying \eqref{eqn_proof_tag_IneqsHold_vb}, we have that both 
\begin{align*} 
...
$B, C \in (0, 1)$ such that there is a set $\mathcal{H}_T$ with 
...
\]
Hence, we have shown that the necessary condition in 
Theorem \ref{theorem_CondAvgOrderGInvxSummatoryFunc_v1} can in fact be attained. 
\end{proof} 
...
\]
From \eqref{eqn_proof_tag_MxAbsValueConsequence_v1}, we then obtain that 
\begin{align*} 
...
\]
This inequality clearly violates the known (obvious) bound that $|M(x)| \leq x$ for all $x \geq 1$. 
...
and in particular, that it cannot tend to zero as $x \rightarrow \infty$. 
\end{proof} 
...
\left\lvert G_E^{-1}(x) \right\rvert & \SuccSim 
     ...
     \lambda(n) \cdot \mathbb{E}[C_{\Omega(n)}(n)] \right\rvert, 
\end{align*} 
where the function 
\[
...
with $d \leq \log\log x$ and for all large $x \rightarrow \infty$. 
The limiting lower bound stated above for $\widehat{L}_0(x)$ is computed by symbolic summation 
...
G_{\ell}^{-1}(x) := \sum_{n \leq x} \lambda(n) \times \sum_{\substack{d|n \\ d > e^e}} 
     (\log d) \cdot (\log\log d)^{2\log\log d - 1}. 
\]
...
%\footnote{ 
%     The tendency of the stated lower bound not to grow arbitrarily small as 
%     $x \rightarrow \infty$ is surely an artifact of the average estimates we have applied in 
%     constructing the bound. It does not necessarily, and need not, imply that $M(x) = 0$ only 
%     finitely often. 
%}
\begin{align*} 
...
\end{align*} 
we notice that using the average case asymptotic bounds 
(rather than the complicated exact formulas) for the functions 
$C_{\Omega(n)}(n)$ from Theorem \ref{theorem_Ckn_GeneralAsymptoticsForms}, 
we recover an observation that have over-summed on the range over $n$ by quite a bit. 
...
many of the terms in the previous equation are actually zero 
(recall that $C_0(n) = \delta_{n,1}$). 
...
\[
\widehat{\pi}_k(x) \SuccSim \frac{A_0 \cdot x}{\log x \cdot (\log\log x)^4 \cdot (k-1)!} \cdot 
     \left(\frac{4}{3e\log 2}\right)^{k}, \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
\notag
     & \SuccSim  
     \sqrt{\frac{2}{\pi}} A_0 \cdot (\log\log t) 
     \frac{(\log\log\log t)^{2\log 2+ \frac{1}{3 \log 2} - 1}}{ 
     (\log\log\log\log t)^{\frac{5}{2} + \log\log\log\log t}}, 
\end{align} 
...
$\widehat{\tau}_0(t) = (\log t) \cdot (\log\log t)^{2\log\log t - 1}$, that 
\begin{align*} 
...
To make this observation precise, consider the following expansion for comparison with 
\eqref{eqn_proof_thm_GInvFunc_v4_approx}: 
\begin{align*} 
\widehat{L}_0(\log\log x) \widehat{\tau}_0(\log\log x) A_{\Omega}^{\ell}(\log\log x) & \SuccSim 
     \frac{4A^2}{\pi \log\log x} \cdot \frac{\log_{\ast}^{4}(x)^{2\log_{\ast}^{4}(x)+1}}{ 
     \log_{\ast}^{6}(x)^{2\log_{\ast}^{6}(x)+5}} \cdot 
     \log_{\ast}^{5}(x)^{4\log 2+\frac{2}{3\log 2} - 1} \\ 
     & \SuccSim \frac{4A^2}{\pi \log\log x} \cdot \frac{\log_{\ast}^{4}(x)^{3}}{ 
     \log_{\ast}^{4\log 2+\frac{2}{3\log 2}+4}(x)^{4} \cdot \log_{\ast}^{6}(x)^{5}}. 
\end{align*} 
The simplifications in arriving at the last equation follow from the bounds 
\begin{align*} 
\exp\left(2 \log_{\ast}^{4}(x) \cdot \log_{\ast}^{5}(x)\right) & \SuccSim 
     \exp\left(\log_{\ast}^{5}(x)^2\right) \SuccSim \log_{\ast}^{4}(x)^2 \\ 
\exp\left(-2 \log_{\ast}^{6}(x) \cdot \log_{\ast}^{7}(x)\right) & \SuccSim 
     \exp\left(-2 \log_{\ast}^{6}(x)^2\right) \SuccSim 
     \log_{\ast}^{5}(x)^4. 
\end{align*} 
So clearly, in absolute value the integral term dominates the asymptotics. 
\end{proof} 
