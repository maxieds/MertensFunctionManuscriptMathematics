...
\[
\frac{1}{n} \times \sum_{k \leq n} C_{\Omega(k)}(k) \sim \frac{2 A_0 \sqrt{2\pi} (\log n)}{\sqrt{\log\log n}}, 
...
\frac{1}{n} \times \sum_{k \leq n} |g^{-1}(k)| = 
     \frac{12A_0}{\pi} \cdot \frac{(\log n)^2}{\sqrt{\log\log n}} (1+o(1)),  
     \mathrm{\ as\ } n \rightarrow \infty. 
\]
...
M(x) & = G^{-1}(x) 
     ...
     \right], x \geq 1. 
\end{align} 
We also have that (\cf Section \ref{subSection_Relating_CknFuncs_to_gInvn}) 
\[
...
(\textbf{TODO:} Remarks and introduction to the last section ... ) 
...
\[
\frac{1}{n} \times \sum_{k \leq n} C_{\Omega(k)}(k) \sim 
...
We have that as $n \rightarrow \infty$ 
\begin{align*} 
\frac{1}{n} \times \sum_{k \leq n} |g^{-1}(k)| & \sim \frac{12A_0}{\pi} \cdot 
...
\begin{align*} 
\frac{1}{2 \sqrt{2\pi}} \times \int \frac{\sum_{n \leq t} C_{\Omega(n)}(n)}{t^2} dt & = 
...
\notag 
     ...
     \frac{\sum\limits_{k \leq d} C_{\Omega(k)}(k)}{d^2}\right] + O(1). 
     \qedhere 
\end{align} 
\end{proof} 
...
such that $1+P(s) = 0$. 
\end{theorem}
The ideas at the crux of the next proof are found in Davenport and 
Heilbronn \cite{DAVHEIL-1936A,DAVHEIL-1936B} and date back to the work of 
...
has a unique solution, which we denote by $\sigma = \sigma_1$, for $\sigma > 1$. 
...
\subsection{A proof of a famous conjecture}

The contributions of the local maxima for the absolute values of 
$|g^{-1}(n)|$ along the squarefree integers allow us to state the next key lemma 
...
\begin{align*}
G^{-1}((4m+1)\#) & \sim -(4m+1)!, \\ 
G^{-1}\left(\frac{(4m+1)\#}{p_k}\right) & \sim (e-1) (4m+1)!, 
     \text{ for all } 1 \leq k \leq 4m+1. 
\end{align*} 
\end{lemma}
\begin{proof} 
Let $m$ be a large positive integer. 
We have by part (B) of Proposition \ref{lemma_gInv_MxExample} that 
\begin{align*} 
G_{U}^{-1}((4m+1)\#) & := \sum_{\substack{n \leq (4m+1)\# \\ \omega(n)=\Omega(n)}} \lambda(n) |g^{-1}(n)| \\ 
     ...
     & \sim -(4m+1)!. 
\end{align*} 
We argue that the corresponding sums over the non-squarefree $n \leq (4m+1)\#$ are negligible 
in comparison with $G_{U}^{-1}((4m+1)\#)$. 
Suppose that $2 \leq n \leq (4m+1)\#$ is not squarefree. Then we have that 
$|g^{-1}(n)| \leq |g^{-1}(p_s^2 t)|$ for some $1 \leq s \leq 4m+1$ and $t$ squarefree. 
If $s = 1$ so that $p_2 = 2$ we have that the largest possible squarefree part $t$, or 
the radix $\operatorname{rad}(n)$, satisfies $t \leq p_3p_4 \cdots p_{4m+1}$. 
By Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1}, we have that 
\begin{align*} 
\left\lvert \sum_{k=2}^{\log_2\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4k-1}} 
     g^{-1}(p_1^k t) \right\rvert & \leq \left\lvert 
     \sum_{k=2}^{\log_2\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4k-1}} 
...
$n = p_s^2 t$ for $t \leq p_{r+1} p_{r+2} \cdots p_{4m+1}$ squarefree, 
we get that 
\begin{align*}
\left\lvert \sum_{k=2}^{\log_2\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4k+1-r}} 
...
G_L^{-1}((4m+1)\#) & := \left\lvert 
     ...
     \right\rvert \\ 
     & \leq \sum_{p} \frac{(2-\log 2 + o(1)) (4m+1-p)(4m)!}{p!} \\ 
     & \leq \sum_{r \geq 2} \frac{(2-\log 2+o(1)) (4m+1-r) (4m)!}{r!} \\ 
     & = (2-\log 2+o(1)) ((4e-8)m-1) (4m)! \\ 
     & < (4m+1)! (1+o(1)). 
\end{align*} 
Hence, we see that $G^{-1}((4m+1)\#) \sim -(4m+1)!$ as $m \rightarrow \infty$. 
Similarly, we can show that for any $1 \leq k \leq 4m+1$ 
\[
G^{-1}\left(\frac{(4m+1)\#}{p_k}\right) \sim \sum_{0 \leq k \leq 4m} \binom{4m+1}{k} (-1)^{k} k! 
     \left(e + O\left(\frac{1}{(k+1)!}\right)\right) \sim (e-1) (4m+1)!. 
     \qedhere 
\]
\end{proof}
...
\end{theorem}
\begin{proof}
By \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} and 
Lemma \ref{theorem_PrimorialSeqGInvCalcs_v1} we have that 
\begin{align}
\notag 
M((4m+1)\#) & \gg ((4m+1)(e-1)-1)(4m+1)! \\ 
\label{eqn_RmksOnFamousConj_MxLowerBound}
     & \gg (4m+1)(4m+1)!, \text{ as } m \rightarrow \infty. 
\end{align}
...
n\# \sim e^{\vartheta(p_n)} \asymp n^n (\log n)^n e^{-n(1+o(1))}, 
     \text{ as } n \rightarrow \infty. 
\]
It follows from \eqref{eqn_RmksOnFamousConj_MxLowerBound} and Stirling's formula that 
\begin{align*}
\limsup_{x \rightarrow \infty} \frac{M(x)}{\sqrt{x}} & \gg \limsup_{m \rightarrow \infty} 
     \frac{M((4m+1)\#)}{\sqrt{(4m+1)\#}} \\ 
     & \gg \lim_{m \rightarrow \infty} 
     \frac{(4m+1)^{2m+2}}{e^{3m} \log(4m+1)^{2m+\frac{1}{2}}} = + \infty. 
     \qedhere
\end{align*}
%We get that if $x = n\#$ where $n$ is large 
%\[
%\frac{\log x}{W_0(\log x) W_0\left(\frac{\log x}{W_0(\log x)}\right)} = 
%     \frac{\log x}{(\log\log x)^2}(1 + o(1)) \leq n \leq 
%     \frac{\log x}{W_0(\log x)} = \frac{\log x}{\log\log x} + o(1), 
%     \text{ as } n, x \rightarrow \infty. 
%\]
\end{proof}

A related argument taking the infinite sequence along $(4m+2)\#$ shows the analogous 
conjectured result given by 
