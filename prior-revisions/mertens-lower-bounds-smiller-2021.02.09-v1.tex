...
\newglossaryentry{MoebiusMuFunc}{
    symbol={\ensuremath{\mu(n)}},
    sort={MoebiusMuFunc},
    description={The M\"obius function defined such that $\mu^2(n)$ is the indicator function of the squarefree integers, and 
                 so that $\mu(n) = (-1)^{\omega(n)}$ whenever $n$ is squarefree. },
    type={symbols},
...
\newglossaryentry{Mx}{
    symbol={\ensuremath{M(x)}},
    sort={Mx},
    description={The Mertens function is the summatory function of $\mu(n)$ defined for all integers 
...
\newglossaryentry{fInvn}{
    ...
    is unique and satisfies the characteristic convolution relations 
    providing that $f^{-1} \ast f = f \ast f^{-1} = \varepsilon$.},
    type={symbols},
...
\newglossaryentry{PrimeZetaFunc}{
    symbol={$P(s)$},
    sort={PrimeZetaFunc},
    description={For complex $s$ with $\Re(s) > 1$, we define the prime zeta function to be the Dirichlet generating function  
                 $P(s) = \sum_{n \geq 1} \frac{\chi_{\mathbb{P}}(n)}{n^s} = \sum_{k \geq 1} \frac{\mu(k)}{k} \log\zeta(ks)$. 
                 },
    type={symbols},
...
\newglossaryentry{PikPiHatkx}{
    ...
    sort={PikPiHatkx},
    description={The prime counting function variant $\pi_k(x)$ denotes the number of integers 
                 $2 \leq n \leq x$ with 
...
\newglossaryentry{primeOmegaFunctions}{
    ...
    description={We define the strongly additive function 
                 ...
                 By convention, we require that $\omega(1) = \Omega(1) = 0$.},
    type={symbols},
...
\newglossaryentry{SimRelation}{
    symbol={$\sim$},
    sort={SimRelation},
    description={We say that two arithmetic functions $A(x), B(x)$ satisfy the relation $A \sim B$ if 
                 $\lim_{x \rightarrow \infty} \frac{A(x)}{B(x)} = 1.$ },
    type={symbols},
    name={Asymptotic relation symbol}
    }
\newglossaryentry{ApproxRelation}{
    symbol={$\approx$},
    sort={ApproxRelation},
...
\newglossaryentry{NormalCDFFunc}{
    symbol={$\Phi(z)$},
    sort={NormalCDFFunc},
    description={For $x \in \mathbb{R}$, we define the function giving the normal distribution CDF by 
...
\newglossaryentry{lambdaFuncAst}{
    ...
    sort={lambdaFuncAst},
    description={For positive integers $n \geq 2$, we define the next variant of the 
                 Liouville lambda function, $\lambda(n)$, as follows: 
                 $\lambda_{\ast}(n) := (-1)^{\omega(n)}$. 
...
\newglossaryentry{muSigmaTheoremConstants}{
    ...
    description={We define these analogs to the mean and variance of the function
                 $C_{\Omega(n)}(n)$ in the context of our new Erd{\"{o}}s-Kac like theorems as 
                 $\mu_x(C) := \log\log x + \log(2) - \log\left(\hat{C}_{\ast}(C)\right)$ and 
                 $\sigma_x(C) := \sqrt{\log\log x}$ where 
                 $\hat{C}_{\ast}(C) > 0$ is an absolute constant. 
                 },
    type={symbols},
    name={Erd{\"{o}}s-Kac theorem mean and variance analogs}
    }
...
defined as the summatory function of the M\"obius function for $x \geq 1$. 
...
We prove an Erd\"{o}s-Kac theorem analog for the distribution of the 
...
\]
A positive integer $n \geq 1$ is \emph{squarefree}, or contains no divisors (other than one) which are 
...
non-trivial simple zeta function zeros with comparitively small imaginary parts in the famous paper by 
...
subsequences, e.g., considering the asymptotics of the scaled 
function in the limit supremum and limit infimum senses. 

We cite that it is known by computation 
...
motivated an equivalent alternate characterization of $M(x)$ by constructing 
...
This new perspective offers equivalent exact characterizations of $M(x)$ for all $x \geq 1$ 
...
The definitions of the core subsequences we define and the 
parameterized bivariate DGF based proof method that is given 
in the spirit of Montgomery and Vaughan's 
work allow us to reconcile the property of strong additivity in a novel way with signed sums of 
multiplicative functions. 
The proof of characteristic properties of these new sequences 
...
The new results in this article concretely connect the distributions of $L(x)$, a well defined 
...
on an asymptotically dense infinite subset of integers and the seek 
...
for an absolute constant $\hat{G}_{\ast} > 0$, 
the approximate average order of the unsigned sequence satisfies 
\[
\mathbb{E}|g^{-1}(n)| \sim \hat{G}_{\ast} \cdot (\log n)^2 \sqrt{\log\log n}, 
     \mathrm{\ as\ } n \rightarrow \infty. 
\]
In Section \ref{Section_NewFormulasForgInvn}, 
we prove a variant of an Erd\"{o}s-Kac theorem 
for the sequence $C_{\Omega(n)}(n)$. 
This leads us to conclude the following statement holds for any fixed $Y > 0$, with 
$\mu_x(C) := \log\log x + \log 2 - \log(\hat{C}_{\ast}(C))$ and 
$\sigma_x(C) := \sqrt{\log\log x}$ for $\hat{C}_{\ast}(C) > 0$ an absolute constant, 
uniformly for all $-Y \leq y \leq Y$ (see Corollary \ref{cor_CLT_VII}): 
\[
...
quantifiable in so much as the distrubution of $|g^{-1}(n)|$ for $n \leq x$ 
tends to its average order with a non-central normal tendency 
depending on $x$ as $x \rightarrow \infty$. 
That is, if $x \geq 2$ is sufficiently large and 
...
by bounding the summatory function $G^{-1}(x)$. 
The take on the regularity of $|g^{-1}(n)|$ is then imperative to our arguments 
...
The results in 
Theorem \ref{cor_ExprForGInvxByLx_v1} prove that for almost every sufficiently large $x$\footnote{
...
we prove that as $x \rightarrow \infty$ 
\begin{align*}
...
the new structure behind $M(x)$ we identify in this article. 

\newpage 
\section{Initial proofs of new results} 
...
(g_{x,j}) = \hat{G} (I - U^{T}). 
\]
The square matrix $U$ with sufficiently large finite dimensions 
has $(i,j)^{th}$ entries are defined by $(U)_{i,j} = \delta_{i+1,j}$ such that 
\[
...
$\Re(s) > \sigma_f$ with $\sigma_f$ the abcissa of convergence of the series. 
...
\subsection{Statements of limiting asymptotic formulas} 
...
of these arithmetic functions towards their respective average orders. 
...
\end{cor} 
\begin{proof} 
First, we sum the function $\mathcal{D}_{\omega}(x)$ exactly by applying the limiting 
...
\mathcal{D}_{\omega}(x) & = \left\lvert \sum_{k \leq \log\log x} \frac{(-1)^k \cdot x}{\log x} \cdot 
     ...
     & \asymp \frac{x}{\sqrt{\log\log x}}, \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*} 
...
$\omega(n) \leq \log_2(n)$ for all $n \geq 2$. 
...
\frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \ll 
     ...
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
We have the following bounds for large $x$: 
\[
...
$\mathcal{D}_{\omega}(x)$ where we have the uniform bounds on the summands for 
...
$C_{\Omega(n)}(n)$ to prove an Erd\"os-Kac like analog that characterizes 
...
Proposition \ref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1}, 
Corollary \ref{lemma_AbsValueOf_gInvn_FornSquareFree_v1} shows that its summatory 
function satisfies 
\[
...
\]
It can infact be seen that 
\[
\mu(n) = g^{-1}(n) + \sum_{p|n} g^{-1}\left(\frac{n}{p}\right), n \geq 1. 
\]
This clear connection between the summatory function of $g^{-1}(n)$ and the primes is also 
shown in the form of the identity we prove for $M(x)$ in 
...
In particular, for any fixed $k \geq 1$ 
...
The combinatorial formula\footnote{ 
     This seqence is also considered using a different motivation based on the DGFs 
...
an Erd\"os-Kac theorem that shows that the density of a centrally 
shifted and scaled variant of each 
...
C_{\Omega(n)}(n) = \begin{cases} 
     ...
     \end{cases} 
\]
We can then generate scaled forms of these terms through a product identity of the following form: 
\begin{align*} 
\sum_{n \geq 1} \frac{C_{\Omega(n)}(n)}{(\Omega(n))!} \cdot 
     ...
     = \exp\left(-z \cdot P(s)\right), \Re(s) \geq 2, \Re(P(s)z) > -1. 
\end{align*} 
...
\sum_{n \geq 1} \frac{(-1)^{\omega(n)} C_{\Omega(n)}(n) z^{\Omega(n)}}{n^s} = 
     \zeta(s)^z \times \widehat{F}(s, z), \Re(s) > 1, |z| < |P(s)|^{-1}. 
\]
...
the DGF expansion of $\widehat{F}(s, z) \cdot \zeta(s)^{z}$ whenever $\Re(s) > 1$, then 
\[
...
\end{align} 
We can sum the coefficients for integers $u > e$ taken sufficiently large as follows: 
\begin{align*} 
...
\]
The total sum over the interval $m \leq x/2$ then corresponds to bounding the following 
...
uniformly for $1 \leq k < \log\log x$ 
\[
\widehat{C}_{k,\ast}(x) := 
...
\end{theorem} 
\begin{proof} 
We begin by bounding a contour integral over the error term of 
$\widehat{A}_z(x)$ from 
...
\end{align*} 
When $k = 1$ we have directly by the Cauchy integral formula that 
\begin{align*} 
...
to obtain the dominant order of the main term. 
...
so that $0 \leq r < 1$ when 
we parameterize $v = r e^{2\pi\imath t}$ by a real-line integral over $t \in [0, 1]$. 
We can numerically evaluate the bounded extremal values of this function as 
\begin{align*} 
...
\widetilde{A}_r(x) = \Theta\left(
     \int_{|v|=r} \frac{x \cdot (\log x)^{-v} \zeta(2)^{-v}}{(\log x) \cdot 
     v^{k} (1 + P(2) v)} dv\right), 
\end{align} 
where as $x \rightarrow \infty$ 
\[
1 \leq \frac{\widetilde{A}_r(x)}{\int_{|v|=r} \frac{x (\log x)^{-v} \zeta(2)^{-v}}{(\log x) \cdot 
     v^{k} (1 + P(2) v)} dv} \leq 1.92275. 
\] 
...
applying ordinary Abel summation in the form of the next equations. 
\begin{align} 
...
corresponding to \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1} be defined precisely 
...
define its summatory function by $L_{\ast}(x) := \sum_{n \leq x} \lambda_{\ast}(n)$ for 
$x \geq 1$. 
...
using the following uniform asymptotics for $\pi_k(x)$ that hold 
when $1 \leq k \leq \log\log x$: 
\[
...
\end{align*} 
The ordinary Abel summation formula and another application of 
integration by parts implies that we obtain the 
main term for our sums in the form of 
\begin{align*} 
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & \asymp 
...
S_{2,\varepsilon}(x) & := \sum_{\log\log x < k \leq 
     \frac{1}{\log 2} \cdot (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
...
\]
with equality as $\varepsilon \rightarrow 1$ so that the upper bound of summation tends to 
...
S_{2,\varepsilon}(x) & = 
     ...
     \frac{1}{\log 2} \cdot (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \\ 
     & \ll \sum_{k=\log\log x}^{\frac{1}{\log 2} \cdot (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     ...
     & \ll \frac{1}{(\log x)} \times \int_{\log\log x}^{\frac{1}{\log 2} \cdot  
...
\]
%We have a 
%reflection identity for the normal distribution CDF $\Phi(z) = 1 - \Phi(-z)$ where 
%\begin{align*} 
%\Phi(z) & = \frac{1}{2}\left(1 + \operatorname{erf}\left(\frac{z}{\sqrt{2}}\right)\right). 
%\end{align*}
%Since we have a MacLaurin series for the error function when $|z| < 1$ given by 
%\begin{align*}
%\Phi(-z) & = 1 - \Phi(z) = \frac{1}{2} - \frac{z}{\sqrt{2\pi}}\left(1 - \frac{z^2}{12} + O(|z|^5)\right), 
%\end{align*}
%the result in (A) holds as $x \rightarrow \infty$. 
...
$$G^{-1}(x) = O\left(\max_{1 \leq t \leq x} |L(t)| \cdot \mathbb{E}|g^{-1}(x)|\right).$$ 
If the RH is true, then 
for any $\varepsilon > 0$ and large integers $x > e$ 
\[
G^{-1}(x) = O\left((\log x)^2 \sqrt{\log\log x} \sqrt{x} \times \exp\left( 
...
We will need to bound the sums of the maximal values of $|g^{-1}(n)|$ over $n \leq x$
...
\]
Recall that the values of $|g^{-1}(n)|$ are maximized when $n$ is squarefree with 
\begin{align*}
...
\end{align*} 
For large $x$, let $\mathcal{R}_x := \{t \leq x: |g^{-1}(t)| \gg \mathbb{E}|g^{-1}(t)|\}$. 
The formula from \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1} then implies 
that for almost every large $x$ and any $m, r = o\left(\frac{(\log x)(\log\log\log x)}{\log\log x}\right)$ 
\begin{align*}
...
\end{align*} 
If the RH is true, by applying Humpries' result in 
...
\end{align*} 
The rightmost maximum term in the previous bound is 
...
is significant in providing a new window through which we can view bounding 
$M(x)$, in place of improving upon the best known bounds on this classical 
function. 
...
relation of the distinct (and repitition of) values of $|g^{-1}(n)|$ 
for $n \leq x$ are 
also suggestive towards bounding main terms for $G^{-1}(x)$ along 
infinite subsequences. 

One topic that we do not touch on in the article is to consider the 
correlation (if any) between $\lambda(n)$ and the unsigned sequence of $|g^{-1}(n)|$ 
whose limiting distribution is proved in 
Corollary \ref{cor_CLT_VII}. 
Much in the same way that variants of the Erd\"{o}s-Kac theorem are proved 
...
suggested for future work on these topics.
