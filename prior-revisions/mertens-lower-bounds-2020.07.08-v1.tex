...
M(x) & \approx G^{-1}(x) + \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
     x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{t^2 \cdot \log(x/t)} dt + o(1). 
\end{align} 
\end{prop} 
\begin{proof} 
...
\sum_{k=1}^{x/2} g^{-1}(x) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = o(1). 
\end{equation} 
...
\sum_{k=1}^{x/2} g^{-1}(x) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     O\left(\frac{x}{\log x} \times \sum_{k=1}^{x/2} \frac{g^{-1}(k)}{k}\right), 
\]
where by Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} and 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz} we have that 
\begin{align*} 
\left\lvert \frac{x}{\log x} \times \sum_{k=1}^{x/2} \frac{g^{-1}(k)}{k} \right\rvert & 
     \sim \frac{1}{\log x} \times \left\lvert \mathbb{E}\left[ 
     \sum_{d|\frac{x}{2}} g^{-1}(d)\right] \right\rvert 
     \ll \frac{1}{\log x} \times \mathbb{E}\left[C_{\Omega(x)}(x)\right] \\ 
     & = \frac{1}{x \cdot \log x} \times \sum_{k=1}^{x} C_{\Omega(k)}(k)  
     \ll \frac{1}{x^2 \cdot \log x} \times \sum_{k=1}^{\log x} \widehat{\pi}_k(x) \cdot k! \\ 
     & \ll \frac{1}{x \cdot (\log x)^2} \times \sum_{k=1}^{\log x} k (\log\log x)^{k-1} \\ 
     & \ll \frac{1}{x \cdot (\log x) (\log\log x)} \times (\log\log x)^{\log x} = o(1), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*} 
So by summation by parts, we have that 
\begin{subequations}
\begin{align} 
\notag 
\sum_{k=1}^{x/2} g^{-1}(k) \pi(x/k) & = \sum_{k=1}^{x/2} 
    \frac{x \cdot g^{-1}(k)}{k \cdot \log(x/k)} + o(1) \\ 
\notag 
    & = \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
    \sum_{k=1}^{x/2} G^{-1}(k) \left[ 
    \frac{x}{k \cdot \log\left(\frac{x}{k}\right)} - 
    \frac{x}{(k+1) \cdot \log\left(\frac{x}{k+1}\right)}\right] + o(1) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v3} 
     & \sim \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) 
     -\sum_{k=1}^{x/2} G^{-1}(k) \left[ 
     \frac{x}{k \cdot \log(x/k)} - \frac{x}{(k+1) \cdot \log(x/k)}\right] + o(1) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v4} 
     & \approx \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
     \sum_{k=1}^{x/2} G^{-1}(k) \frac{x}{k^2 \cdot \log(x/k)} + o(1). 
\end{align} 
\end{subequations} 
Indeed, step 
...
S_{\pm}(x) := 
     ...
     \frac{\varepsilon_{x,d} \cdot 
     \lambda(d) (\log d)^{\frac{1}{4}}}{d^{1/4} \cdot \log\log d} \times 
     \widehat{L}_0\left(\frac{(\log x)^{5} (\log\log x)}{d}\right) \right\rvert, 
\]
...
\frac{S_{\pm}(x)}{(\log x)^{5/4} (\log\log x)^{1/4}} & 
     ...
     \frac{\delta_{\max} \cdot dt}{t^{1/4} \cdot (\log t)^{3/4} (\log\log t)} \\ 
