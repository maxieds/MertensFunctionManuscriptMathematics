...
|G^{-1}(x)| & \gg \frac{6}{\pi^2} \times \left\lvert (\log x)(\log\log x) - 
     ...
     \right\rvert \\ 
     %& \sim \frac{6}{\pi^2} \times \left\lvert -(\log x)(\log\log x) + 
     %\sum_{n \leq \frac{x}{2}} \left[ 
...
defined as in \eqref{eqn_proof_thm_GInvFunc_v3_approx} as the following function of $k$: 
\begin{align} 
\label{eqn_proof_thm_GInvFunc_v3_approx} 
I_{\ell}\left(e^{e^{2k}}\right) e^{e^{2k}}& \SuccSim 
     ...
     =: \widehat{I}_{\ell}(k). 
\end{align} 
So using the upper bound on the integrand in 
