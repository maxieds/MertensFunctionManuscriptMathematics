...
$\Omega(n)$ for $n \leq x$ as\footnote{ 
     ...
     Now we are looking at $x \mapsto \log\log x$, so that our tail on $A_{\Omega}^{(\ell)}(t)$ is 
     \[
     A_{\Omega}^{(\ell)}(x) \sim 
...
\end{itemize} 
%Observe using the function $x \cdot \widehat{L}_0(x)$ from Corollary \ref{cor_ASemiForm_ForGInvx_v1} 
%that (inadvertently), we have shown the lower bound providing (TODO) 
%\[
%L_0(x) \succsim (-1)^{\floor{\frac{3}{2} \log\log x} + 1} \left( 
%     \sqrt{\frac{3}{\pi}} \frac{A (2e+3)}{4B \log^{\frac{3}{2}}(2)}\right) \cdot 
%     \frac{x \cdot (\log x)^{\frac{43}{14} + \frac{3}{2 \log 2} - \frac{3}{2 \log 3}}}{ 
%     \sqrt{\log\log x}} + O\left(x\right). 
%\] 
...
\limsup\limits_{x \rightarrow \infty} 
     ...
     G_{\ell}^{-1}\left(x^{\frac{\alpha-1}{2\alpha}}\right)\right]} \geq 
     \frac{3\sqrt{3} A^2(2e+3)^2}{16 \sqrt{2} \pi e^2 B^2 \log^2 2 (\log\log 2)^{3}}. 
\]
\end{cor} 

\begin{theorem} 
For real $-2 < \alpha \leq 1$ such that $\alpha$ is independent of $x$ in $M_{\alpha}(x)$, 
we have that 
\[
...
\limsup\limits_{x \rightarrow \infty} \frac{|M_{\alpha}(x)|}{\sqrt{x} 
     \cdot G_{\ell}^{-1}\left(\sqrt{x}\right)} \geq 
     \frac{3\sqrt{3} A^2(2e+3)^2}{16 \sqrt{2} \pi e^2 B^2 \log^2 2 (\log\log 2)^{3}}. 
\]
\end{cor}
...
\caption*{\textbf{\rm \bf Table \thesubsection:} 
          ...
          $\lambda(n) = (-1)^{\Omega(n)}$. Then the next two columns show the small-ish magnitude differences between the unsigned 
