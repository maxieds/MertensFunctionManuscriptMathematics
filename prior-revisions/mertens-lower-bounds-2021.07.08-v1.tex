...
\begin{align*} 
     ...
     \frac{2 C_{\Omega(t)}(t) \Iverson{\Omega(t) = k}}{A_0 \sqrt{2\pi \log\log t}} dt ds \\ 
     & \phantom{=-\ } + 
     \frac{(-1)^{\floor{\log\log x}+1}}{A_0 \sqrt{2\pi}} \times 
     \left(\int_3^x \frac{\widehat{C}_k(t)}{t(\log t) (\log\log t)^2} dt - 
     \frac{\widehat{C}_k(x)}{x(\log x)(\log\log x)^2}\right). 
\end{align*} 
...
Hence, we have that 
\[
-A_0 \sqrt{2\pi} x (\log x) \widehat{C}_{k,\ast}^{\prime}(x) \sim 
     \int_{x^{e^{-1}}}^{x} \frac{C_{\Omega(t)}(t) \Iverson{\Omega(t) = k}}{\sqrt{\log\log t}} dt + o(1). 
\]
Since $\widehat{C}_k(x) \gg \widehat{C}_k\left(x^{e^{-1}}\right)$ 
for all sufficienly large $x$, we find that 
\begin{equation}
...
much like in the first heuristics established for the Erd\H{o}s-Kac theorem 
...
\frac{A_0 \sqrt{2\pi}}{\log x} & \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \\ 
     & \sim \frac{1}{\sqrt{2\pi} \sigma_x(C)} \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \times \exp\left(-\frac{t^2}{2\sigma_x(C)^2}\right) \times \left( 
     1 + \widetilde{E}(x, t)
     \right). 
\end{align*} 
For $|t| \leq \mu_x(C)^{\frac{2}{3}}$, we have that 
\[
...
Corollary \ref{cor_CLT_VII}, we see that 
\[
\frac{1}{x} \times \#\left\{x^{\delta} \leq n \leq x: 
     |g^{-1}(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \leq y\right\} = 
