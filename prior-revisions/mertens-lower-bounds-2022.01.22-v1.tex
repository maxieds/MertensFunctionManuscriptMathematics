...
\title{
       ...
       signed partial sums of unsigned factorization symmetric functions  
       } 
}
...
auxiliary unsigned function $C_{\Omega}(n)$ whose DGF is given by 
...
The summatory functions of factorization symmetric functions are amenable to 
an application of the Selberg-Delange method. A special case of the method 
is applied in the article to formulate asymptotics for the restricted partial sums of 
$C_{\Omega}(n)$ over all $n \leq x$ such that $\Omega(n)=k$ 
which hold uniformly for all $1 \leq k \leq \frac{3}{2}\log\log x$. 
...
%Ordinary summation by parts implies that we can also express $G(x)$ in terms 
...
%\[
%G(x) 
%     = \sum_{n \leq x} \lambda(n) |g(n)| 
%     = L(x)|g(x)| - \sum_{n < x} 
%     L(n) \left(|g(n+1)| - |g(n)|\right), 
%     \text{ for } x \geq 1. 
%\]
