...
\frac{1}{\sqrt{\log\log x}} - \frac{1}{\sqrt{\log\log\left(x^{e^{-1}}\right)}} = 
     \frac{1}{\sqrt{\log\log x}} \left(1 + O\left(\frac{1}{\log\log x}\right)\right), 
\]
Hence, we have that 
\begin{equation} 
\label{eqn_ProofTag_HatCkx_Asymptotics_v1_v0}
     ...
     O\left(\frac{\widehat{C}_k^{\prime}(x)}{\sqrt{\log\log x}}\right). 
\end{equation} 
For all sufficiently large $x$, integration by parts implies that we have 
\[
\left\lvert \widehat{C}_k(x) - \frac{\widehat{C}_k^{\prime}(x)}{\sqrt{\log\log x}} \right\rvert \ll 
     \widehat{C}_k(x) \times \left\lvert 1 - \frac{1}{\sqrt{\log\log x}} - 
     \int \frac{dx}{2x(\log x)(\log\log x)^{\frac{3}{2}}} + o(1) \right\rvert = 
     \widehat{C}_k(x) (1 + o(1)). 
\]
This shows that $\frac{\widehat{C}_k^{\prime}(x)}{\sqrt{\log\log x}} = o\left(\widehat{C}_k(x)\right)$. 
The left-hand-side of \eqref{eqn_ProofTag_HatCkx_Asymptotics_v1_v0} then implies that 
\[
\widehat{C}_k(x) - \widehat{C}_k\left(x^{e^{-1}}\right) \asymp 
     \frac{x (\log\log x)^{k-\frac{1}{2}}}{(k-1)!}. 
\]
We can see by recursively unfolding the previous equation that 
\[
\widehat{C}_k(x) \asymp \frac{(\log\log x)^{k-\frac{1}{2}}}{(k-1)!} \times 
     \sum_{k=0}^{\log\log x} x^{e^{-k}} \gg 
     \frac{(\log\log x)^{k-\frac{1}{2}}}{(k-1)!} \times 
     \sum_{j \geq 0} \frac{e^j (\log x)^j}{e^j-1} \gg 
     \frac{x (\log\log x)^{k-\frac{1}{2}}}{(k-1)!}. 
\]
...
\label{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v0} 
     %& = \frac{\widehat{C}_k(x)}{(\log x) \sqrt{\log\log x}} + O\left((\log\log x)^2\right) \\ 
...
Furthermore, we know by inequalities that relate the $1$-norm and $2$-norm on 
$x$-dimensional vectors, we have that the variance $\sigma_k^2$ of 
$\frac{1}{n} \times \sum_{n \leq x} X_{n,k}$ is bounded by 
\[
\sigma_k^2 \leq \frac{1}{x^2} \times \left(\sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}}
     \right)^2. 
\]
If the ansatz holds, for $\Omega(n) = k$ with $\frac{1}{2}\log\log x \leq k \leq \log\log x$ and 
any $\varepsilon > 0$, by Chebyshev's inequality we then have that 
\begin{equation} 
\label{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v1} 
\mathbb{P}\left(\left\lvert \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     \frac{C_{\Omega(n)}(n)}{(\log n)\sqrt{\log\log n}} - 
     \frac{A_0 \sqrt{2\pi}}{\log x} \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \right\rvert \geq \varepsilon\right) \ll 
     \frac{1}{\varepsilon^2 (\log\log x)}. 
\end{equation}
...
Finally, by the result in \eqref{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v1} 
with $\varepsilon := \frac{\max\left(\mu_x(C), z\sigma_x(C)\right)}{\sqrt{\log\log x}}$ and 
Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2} we have that as 
$x \rightarrow \infty$ 
\begin{align*} 
\frac{1}{x} \times & \sum_{\mu_x(C)-\mu_x(C)^{\frac{2}{3}} \leq k \leq \mu_x(C) + z\sigma_x(C)} 
     \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}} = 
     \Phi(z) + O_z\left(\frac{1}{\sqrt{\log\log x}} + 
     \frac{\mu_x(C) + z\sigma_x(C)}{\varepsilon(\log\log x)}\right). 
\end{align*} 
The error term in the previous equation is given by $O_z\left((\log\log x)^{-\frac{1}{2}}\right)$. 
\end{proof} 
