...
\title{
       ...
       partial sums of factorization symmetric functions  
       } 
}
...
We prove exact formulas expressing $g(n)$ in terms of the 
auxiliary unsigned function $C_{\Omega}(n)$ whose Dirichlet generating function is given by 
$(1-P(s))^{-1}$ for $\Re(s) > 1$ where $P(s) := \sum_{p} p^{-s}$ is the 
prime zeta function. The unsigned functions 
$C_{\Omega}(n)$ and $|g(n)|$ belong to the class of arithmetic functions we term 
as factorization symmetric. 
Any arithmetic function that is factorization symmetric has distinct values that are 
repeated along identical partitions of the exponents of the 
distinct prime divisors of any $n \geq 2$. 
%The summatory functions of factorization symmetric arithmetic functions are amenable to 
%an application of the Selberg-Delange method. 
A special case of the Selberg-Delange method 
is applied in the article to prove uniform asymptotics for the restricted partial sums of 
$C_{\Omega}(n)$ over all $n \leq x$ such that $\Omega(n)=k$. 

We conjecture that there is a limiting probability measure on $\mathbb{R}$ with 
cumulative density function $\Phi_{\Omega}$ and an absolute constant $B_0 > 0$ 
such that the following holds for any real $y$ as $x \rightarrow \infty$: 
\[
\frac{1}{x} \times \left\{3 \leq n \leq x: 
     \frac{|g(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g(k)| - 
     B_0 \sqrt{\log\log\log x}}{B_0 \sqrt{(\log x) (\log\log\log x)}} \leq y\right\} = 
     \Phi_{\Omega}\left(\frac{\pi^2y}{6}\right) + o(1). 
\]
...
$$G(x) := \sum_{n \leq x} \lambda(n) |g(n)|,$$ with 
the prime counting function $\pi(x)$ determine new exact formulas for $M(x)$. 
In this way, we prove another concrete 
link between the Mertens function and the distribution of the partial sums  
$L(x) := \sum_{n \leq x} \lambda(n)$ and connect these two 
summatory functions with explicit probability distributions. 
...
$C_{\Omega}(n)$ and $|g(n)|$ and their partial sums (defined below). 
...
%\subsection{The auxiliary unsigned sequences} 
...
factorization symmetric arithmetic function with $f_{\omega}(0) = 0$, then 
...
%\subsection{Summary of new results proved in the article} 
...
%A central part of the argument used to prove 
%Corollary \hlocalref{cor_SummatoryFuncsOfUnsignedSeqs_v2} in 
%Section \hlocalref{subSection_Section4_AnalyticPrerequisiteProofsOfUniformBoundsOnCertainPartialSumTypes_v1} 
%is the asymptotic expansions of the incomplete gamma function and the 
%signed summatory function of $\lambda_{\ast}(n) := (-1)^{\omega(n)}$ given 
%by Lemma \hlocalref{cor_AsymptoticsForSignedSumsOfomegan_v1} in the appendix section. 
...
for any $y \in (-\infty, +\infty)$ as $x \rightarrow \infty$ 
\begin{align} 
...
%$C_{\Omega}(n)$, $g(n)$ and $|g(n)|$, and then finally the new exact formulas for $M(x)$ 
%that rely on the partial sums of these new functions. The supplementary sections given in 
%the appendices after the conclusion of the article provide a glossary of notation, 
...
For all sufficiently large $x \geq 2$ and $|z|< P(2)^{-1} \approx 2.21118$ 
\[
...
\]
For integers $x \geq 3$ and $k \geq 1$ let the unsigned sums 
\[
...
As $x \rightarrow \infty$, we have uniformly for $1 \leq k \leq 2\log\log x$ 
\[
...
The solution to \eqref{eqn_ProofTag_HatCkx_Asymptotics_v1_v0} is 
\[
...
We argue as in the proof of Corollary \hlocalref{cor_SummatoryFuncsOfUnsignedSeqs_v2} 
...
is given by by a counting argument. 
...
We can show that the second moment of $C_{\Omega}(n)$ satisfies 
\begin{align*}
...
Rigorous proofs of the conjectures below are outside of the scope of this manuscript. 
...
\caption{Numerically bounded envelopes for the local extremum of $M(x)$ expressed in terms of the partial sums of the unsigned 
         inverse function. The scaling factor is the 
...
The Riemann Hypothesis requires that the sums of the leading constants with opposing signs 
...
n\# \sim e^{\vartheta(p_n)} \asymp n^n (\log n)^n e^{-n(1+o(1))}, 
     \text{ as } n \rightarrow \infty. 
\]
Indeed, the observation follows from the fact that if we obtain a contrary result, 
equation \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} implies that  
\[
...
We expect that an outline of the method behind the proofs we 
...
We expect that such extensions exist in connection with the signed Dirichlet inverse 
...
these results imply a distinctive tendency 
...
\end{align*} 
The following upper bounds that hold as $x \rightarrow \infty$: 
\begin{align*} 
...
Remark \hlocalref{remark_MV_Pikx_FuncResultsAnnotated_v1}. We can argue 
...
is bounded by the following sum as $x \rightarrow \infty$ using Stirling's formula, 
...
\end{align*}
Finally, by an application of the second set of results in 
