...
for all $n$ even though the right-hand-side (RHS) terms involving $\Omega(n)$ 
oscillate in magnitude as $n$ ranges between $1$ and $x$. This is justified by 
maximizing (minimizing) the ratio of the RHS above to Binet's log-gamma formula cited above 
...
For $x > e^e$, the variance of $\log C_{\Omega}(x)$ is given by the formula 
\[
\sqrt{\frac{1}{x} \times \sum_{n \leq x} \log^2 C_{\Omega}(n)} = 
	\sqrt{x} (\log\log x) (\log\log\log x) \left( 
...
\end{align} 
where we define the integral term in the last equations by 
\[
I_A(x) := 2 \times \int_3^x t (\log\log t)^2 (\log\log\log t)^2 dt. 
\]
Since we can exactly integrate 
\[
\int_3^x \frac{(\log\log t)^2 (\log\log\log t)^2}{\log t} \cdot \frac{dt}{t} = 
	\frac{1}{3} (\log\log x)^3 (\log\log\log x)^3 (1+o(1)), 
	\text{ as } x \rightarrow \infty, 
\]
we see that for all sufficiently large $x$ there is a $c \equiv c(x) \in [3, x]$ 
such that 
\[
...
\[
c(x) \sim \frac{\sqrt{2} x \sqrt{\log x}}{\sqrt{\log\log\log x \cdot W\left( 
	\frac{2x^2 \log x}{\log\log\log x}\right)}}. 
\]
For all real $y > e^e$, the principal branch of the Lambert $W$-function has the 
...
We conclude from equation \eqref{eqn_proof_tag_COmeganVariance_InitEqn_v2} that 
\[
...
\frac{1}{n} \times \sum_{k \leq n} \log |g(k)| & = 
	\left(\frac{1}{2} \cdot (\log\log n)(\log\log\log n) - 
...
\notag 
     & \sim \sum_{d \leq n} C_{\Omega}(d) \left(\frac{6}{d \pi^2} + O\left(\frac{1}{\sqrt{dn}}\right) 
     \right) \\ 
...
\]
In effect, we may consider finding a $M_x \geq 1$ such that for large $x$ we have that
