...
\lhead{Maxie Dion Schmidt \hspace{1.75cm} -- 2022 --}
\rhead{\runtitle}
...
proportion of positive integers that are squarefree is asymptotically 
$\frac{6}{\pi^2}$, the limiting constant $P_0$ has the stated bound from below. 
The logarithm of the denominators in \eqref{eqn_proof_tag_hInvn_ExactNestedSumFormula_CombInterpetIdent_v3} 
that may reduce the order of the left-hand-side of 
\eqref{eqn_proof_tag_SumLogCOmegan_P0_exp_v1} 
for the $n$ that are not squarefree have a density when summed over all $n \leq x$ that 
is at most a bounded constant. These observations show that the formula in the last 
equation holds as $x \rightarrow \infty$. 
...
\sqrt{\frac{1}{n} \times \sum_{k \leq n} \log^2 C_{\Omega}(k)} = 
	B_0 \sqrt{n} (\log\log n) (\log\log\log n) \left(1 + o(1)\right). 
\]
\end{prop}
\begin{proof}
...
\begin{align}
\notag
V_{\Omega}(n) & \sim B_0^2 \left(3E_{\Omega}(n)^2 - 2 n^2 
	(\log\log n)^2 (\log\log\log n)^2 + I_A(n)\right) \\ 
\label{eqn_proof_tag_COmeganVariance_InitEqn_v2}
	& =  B_0^2 \left(n^2 (\log\log n)^2 (\log\log\log n)^2 + I_A(n)\right) (1+o(1)), 
...
\[
\sqrt{\frac{V_{\Omega}(n)}{n}} = B_0 \sqrt{n} (\log\log n) (\log\log\log n) (1 + o(1)), 
     \text{ as } n \rightarrow \infty. 
     \qedhere
\]
\end{proof}
...
\sqrt{\frac{1}{n} \times \sum_{k \leq n} \log^2 |g(k)|} = 
     \frac{B_0 \sqrt{2n}}{2} (\log\log n) (\log\log\log n) (1+o(1)), 
     \text{ as } n \rightarrow \infty. 
\]
...
\caption*{{\large{\rm \textbf{\rm \bf Table \thesection:} 
          Computations involving $g(n)$ 
	  and $G(x)$ for $1 \leq n \leq 500$.}} 
          \begin{itemize}[noitemsep,topsep=4pt,leftmargin=0.23in] 
