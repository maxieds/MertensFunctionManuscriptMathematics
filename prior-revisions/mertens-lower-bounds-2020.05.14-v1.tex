...
\pagestyle{empty}
\pagestyle{fancy}
\fancyhead[RO,RE]{M. D. Schmidt -- Prepared for Myself to Enjoy Eventually -- \today} 
\fancyhead[LO,LE]{}
...
notation from theoretical computer science and the analysis of algorithms. 
...
$C_k(n)$ is given by 
\[
C_k(n) \sim (\log\log n)^{2k-1}, \mathrm{\ as\ }n \rightarrow \infty. 
\]
\end{theorem} 
...
\eqref{eqn_AnExactFormulaFor_gInvByMobiusInv_v1} given directly by M\"obius inversion. Hence, 
...
\frac{|M(x_y)|}{\sqrt{x_y}} & \SuccSim 
     2C_{\ell,1} \cdot (\log\log \sqrt{x_y}) \frac{(\log\log\log \sqrt{x_y})^{2\log 2+\frac{1}{3\log 2} - 2}}{ 
     (\log\log\log\log \sqrt{x_y})^{\frac{5}{2}}} \cdot 
     \frac{\log_{\ast}^5(\sqrt{x_y})^{2\log 2 + \frac{1}{3\log 2} + 1}}{ 
...
\[
C_{\ell,1} := \frac{128 \cdot 2^{1/8}}{6561 e^6 \pi \log^6(2)} 
     \exp\left(-\frac{55}{2} \log^2(2)\right) \left(\frac{\pi^2}{3} - \frac{1}{2}\right)^2 
     \approx 2.15312 \times 10^{-9}. 
\]
\end{cor} 
...
\begin{remark}
Note that much of the effort taken in proving the complete set of results in this article is 
...
the core components that ate central to building the more combinatorially motivated approach to approximating 
$M(x)$ that we develop in this article. The full listing of ``routine'', or cut-and-dry, results we need as glue 
to make our full results precise are included in the key to the schematic of our proof logic given in the next 
subsection.
\end{remark} 
...
\item[\textbf{Step C:}] \textit{Re-writing the exact formula for $M(x)$}: Key interpretations used in 
     ...
     \end{itemize} 
\item[\textbf{Step D:}] \textit{The Holy Sub-Grail}: Proving that $\frac{|M(x)|}{\sqrt{x}}$ is 
...
$\chi_{\mathbb{P}}(n)$, evaluated at $-s$ is given by 
\begin{align*} 
...
\end{prop} 
\begin{proof} 
Let $D_f(s) := \sum_{n \geq 1} f(n) n^{-s}$ denote the Dirichlet generating function (DGF) of an 
aeithmetic function $f(n)$ convergent for $\Re(s) > \sigma_f$. 
For all $\Re(s) > 1$, 
...
(h^{-1} \ast 1)(n) & = \sum_{p_1|n} h^{-1}\left(\frac{n}{p_1}\right) 
     ...
     \end{cases} 
\end{align*} 
We need to compute the sign of the function $h^{-1} \ast \mu$. First, by M\"{o}bius inversion and the 
formula for $h^{-1} \ast 1$ we proved above, for each $n \geq 2$, there exist constants $C_{1,n},C_{2,n} > 0$ 
so that 
\[
C_{1,n} \cdot (\lambda \ast \mu)(n) \leq h^{-1}(n) \leq C_{2,n} \cdot (\lambda \ast \mu)(n). 
\]
Since both $\lambda,\mu$ are multiplicative, we can compute that for any prime $p$ and 
integers $\alpha \geq 1$, 
\[
(\lambda \ast \mu)(p^{\alpha}) = \lambda(p^{\alpha}) - \lambda(p^{\alpha-1}) = 2 \lambda(p^{\alpha}). 
\]
Thus the previous inequalities are re-stated in the form of 
\[
2 C_{1,n} \cdot \lambda(n) \leq h^{-1}(n) \leq 2 C_{2,n} \cdot \lambda(n). 
\] 
Now to bound $h^{-1} \ast \mu$, we similarly can see by multiplicativity that 
\[
4 C_{1,n} \cdot \lambda(n) \leq (h^{-1} \ast \mu)(n) \leq 4 C_{2,n} \cdot \lambda(n). 
\] 
Since the absolute constants (for each $n$) are positive, we recover the signedness of 
$g^{-1}(n)$ as $\lambda(n)$. 
\end{proof} 
...
$p \leq u$, e.g., $p \in \left\{2,3,5,\ldots,u\right\}$ for some minimal $u$ as $x \rightarrow \infty$. 
The results proved in Section \ref{subSection_PartialPrimeProducts_Proofs} identify a minimal parameter $u$. 

We state the following theorem reproduced from \cite[Thm.\ 7.20]{MV} that handles the relative 
...
Thus $(z-1)^{-m} \in [(-1)^m, 2^m]$ for integers $m \geq 1$, and so we can form the 
...
\end{align*} 
We also can find a recurrence relation between successive $C_k(n)$ values over $k$ of the form 
\begin{equation}
...
Theorem \ref{theorem_Ckn_GeneralAsymptoticsForms} is proved next. 
...
be that 
\[
C_m(n) \sim (\log\log n)^{2m-1}. 
\]
Now we have by the recursive formula that 
\begin{align} 
\notag 
...
more in-order sums of consecutive M\"obius function terms appear yet again. 
...
inexact, asymptotically accurate formula for these inverse functions we have in mind. 
...
easily given a short proof immediately after the next result is proved. 
...
\[
g^{-1}(n) \sim \left(\frac{\pi^2}{3} - \frac{1}{2}\right) 
...
\[
\left\lvert \frac{\lambda(n) \times 
     \sum\limits_{d|n} C_{\Omega(d)}(d)}{g^{-1}(n)} \right\rvert 
     \xrightarrow{n \rightarrow \infty} \frac{\pi^2}{3} - \frac{1}{2} \approx 2.78987. 
\]
\end{cor} 
\begin{proof} 
Let 
...
\begin{itemize} 
\item[(3)] The asymptotic density of the integers which are squarefree and satisfy (1) or (2) is $\frac{3}{\pi^2}$; 
...
$1 - \frac{6}{\pi^2} + \frac{3}{\pi^2} = 1 - \frac{3}{\pi^2}$. 

Now, the average order of the divisor function $d(n)$ is known to satisfy \cite[\S 27.11]{NISTHB} 
\[
\mathbb{E}[d(n)] = \log n + 2\gamma-1 + o(1). 
\]
To show that the limiting ratio tends to the prescribed constant, it suffices to bound 
\begin{align*} 
\lim_{n \rightarrow \infty} \left\lvert \frac{\lambda(n) \times 
     \sum\limits_{d|n} C_{\Omega(d)}(d)}{g^{-1}(n)} \right\rvert & = 
\lim_{n \rightarrow \infty} \left\lvert \frac{\mathbb{E}[d(n)] + \left(1-\frac{3}{\pi^2}\right) \mathbb{E}[d(n)]}{ 
    \frac{6}{\pi^2} \mathbb{E}[d(n)]} \right\rvert \xrightarrow{n \rightarrow \infty} \frac{\pi^2}{3} - \frac{1}{2}. 
\end{align*} 
Thus we adjust the approximation to the exact formula for $g^{-1}(n)$ from the lemma by weighting 
$S_R(n)$ by the constant on the right-hand-side of the previous equation. 
\end{proof} 
...
\begin{align*} 
G^{-1}(x) & \approx \sum_{n \leq x} \lambda(n) \sum_{d|n} C_{\Omega(d)}(d) \\ 
...
integral formula established below. 
...
the signedness of the summatory function term in the integral formula above is a moot point. 
\end{proof} 
...
The second formula for $F_{\lambda}(\log\log x)$ is valid by Abel summation provided that 
\[
...
$\lambda(n)$ are captured by the terms summed by $A_{\Omega}^{(\ell)}(t)$ for 
...
\left\lvert \frac{\sum\limits_{\log\log x < k \leq x} (-1)^k \cdot \widehat{\pi}_k(x)}{A_{\Omega}(x)} \right\rvert & 
     \PrecSim 
     \left\lvert \frac{B(x, r)}{\log\log x + B(x, 1)} \right\rvert 
     \ll 
     \left\lvert \frac{(\log x)^{r-1-r\log r}}{\log\log x + o(1)} \right\rvert = o(1), 
\end{align} 
...
\[
C_{\ell,1} = 4A_0^2  = 
     \frac{128 \cdot 2^{1/8}}{6561 e^6 \pi \log^6(2)} 
     \exp\left(-\frac{55}{2} \log^2(2)\right) \left(\frac{\pi^2}{3} - \frac{1}{2}\right)^2 
     \approx 2.15312 \times 10^{-9}.
\]
That is, we have 
...
$2\log 2 + \frac{1}{3\log 2} -2 \approx -0.402633$. 
\end{theorem} 
...
we recover from the main term approximation to $C_k(n)$, denoted by 
$\widehat{\tau}_0(t)$, that 
\begin{align*} 
\widehat{\tau}_0^{\prime}(t) & = \frac{d}{dx}\left[ 
     (\log\log t)^{2\log\log t-1} 
     \right] \SuccSim 
     \frac{2 (\log\log t)^{2\log\log t-1}}{t \cdot \log t}. 
\end{align*} 
...
I_{\ell}\left(e^{e^{2k}}\right) e^{e^{2k}}& \SuccSim 
     ...
     \frac{\log(2k)^{2\log 2 + \frac{1}{3\log 2} - 1}}{\log\log(2k)^{\frac{5}{2} + \log\log(2k)}}. 
\end{align} 
...
-\int_{\sqrt{x}}^{x/2} \frac{2 x}{t^2 \log(x)} \cdot G_{\ell}^{-1}(t) dt & \SuccSim 
     ...
     & = 2C_{\ell,1} \cdot \frac{(\log\log x)}{\log x} \frac{(\log\log\log x)^{2\log 2+\frac{1}{3\log 2} - 2}}{ 
     (\log\log\log\log x)^{\frac{5}{2}}} \cdot \frac{\log_{\ast}^5(x)^{2\log 2 + \frac{1}{3\log 2} + 1}}{ 
     \log_{\ast}^{6}(x)^{\frac{5}{2}}} + o(1). 
\end{align*} 
