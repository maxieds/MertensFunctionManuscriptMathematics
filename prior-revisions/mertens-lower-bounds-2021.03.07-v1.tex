...
\[
     \mathbb{E}|g^{-1}(n)| \sim \frac{3}{2\pi^2} (\log n)^2. 
...
$\sigma_x(C) := \sqrt{\log\log x}$, 
that holds uniformly for any $-Y \leq y \leq Y$ (see Corollary \ref{cor_CLT_VII}): 
\[
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: |g^{-1}(n)| - \frac{6}{\pi^2} 
     \mathbb{E}|g^{-1}(n)| \leq y\right\} = 
     \Phi\left(\frac{\pi^2 y + \left(12+\pi^2\right)\mu_x(C)}{2\pi^2}\right) + 
...
\tag{D}
     ...
     -\left(\frac{12+\pi^2}{\pi^2}\right) \mu_x(C) 
     \right) & = \frac{1}{2} + o(1) \\ 
\tag{E} 
     \mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     2\alpha -\left(\frac{12+\pi^2}{\pi^2}\right) \mu_x(C)\right) & = 
...
\[
G^{-1}(x) = O\left((\log x)^2 \sqrt{x} \times \exp\left(
...
\subsection{Statements of useful asymptotic formulas} 
...
\]
The following properties of $\Gamma(a, x)$ hold at positive real $a > 0$: 
\begin{align} 
\label{eqn_IncompleteGamma_PropA} 
\Gamma(a, x) & = (a-1)! \cdot e^{-x} \times \sum_{k=0}^{a-1} \frac{x^k}{k!}, a \in \mathbb{Z}^{+}, x > 0, \\ 
\label{eqn_IncompleteGamma_PropB} 
\Gamma(a, x) & \sim x^{a-1} \cdot e^{-x}, \mathrm{\ for\ fixed\ } a > 0, \mathrm{\ as\ } x \rightarrow \infty. 
\end{align}
...
\end{equation} 
and if $z,a \rightarrow \infty$ with $z = \lambda a$ for some $\lambda > 0$ such that 
...
Suppose that $z,a > 0$ are real parameters. 
...
\end{prop}
\begin{proof}
We can see that for $\lambda > 1$, $b_n(\lambda) \sim \lambda^n \cdot n!$. 
It follows from \eqref{eqn_IncompleteGamma_PropD} that (\cf \cite[\S A.1]{NEMES2015C})
\begin{align*}
\Gamma(a, z) & \sim z^{a-1} e^{-z} \times \sum_{0 \leq n < z} 
     \frac{(-\lambda)^n \cdot n!}{z^n \cdot (1-\lambda)^{2n+1}} \\ 
     & = z^{a-1} e^{-z} \times \sum_{0 \leq n < z} 
     \frac{n!}{\left(1-\lambda\right)(z-a)^n}
\end{align*}
Since $z-a \asymp z$, or $z-a = z(1-1/\lambda)$ is proportional to $z$, 
as $z \rightarrow \infty$ we get that for all indices 
of the previous sum $1 \leq n < z$ the asymptotic order of these terms is of lesser order 
than that of the summand corresponding to $n := 0$. 
Also, we see that the $n^{th}$ summand above is at most $c^n$ 
for some bounded constant $0 < c < 1$ whenever $0 \leq n < z$. 
\end{proof}
...
S_1(x) & := \frac{x}{\log x} \times \sum_{1 \leq k \leq \log\log x} \frac{(-1)^k (\log\log x)^{k-1}}{(k-1)!} 
     \sim x \left(1 - \frac{e^2}{\sqrt{2\pi} \sqrt{\log\log x}}\right), \\ 
\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnB}
S_2(x) & := \frac{x}{\log x} \times \sum_{1 \leq k \leq \log\log x} \frac{k (\log\log x)^{k-1}}{(k-1)!} 
     \sim \frac{x}{2}, \\ 
\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnC}
S_3(x) & := x \times \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} \sim 
     \frac{x(\log x)}{2}. 
\end{align}
\end{lemma}
...
convergent generalized hypergeometric functions such that for 
$b := \log\log x$ we have the following expansions: 
\begin{align*}
\frac{T_{1,2}(x) - T_{1,1}(x)}{\log x} & \sim 1 - \frac{2e^2}{\sqrt{2\pi} \cdot b^{1/2}} \times 
     \left[1 + \sum_{k \geq 1} 
     \frac{b^{2k}}{(2+b) \cdots (2k+2+b) \times (k+3+b) \cdots (2k+2+b)} 
     \right] \\ 
     & = 1 - \frac{2e^2}{\sqrt{2\pi} \cdot b^{1/2}} \times \sum_{k \geq 0} 
     \frac{1}{\left(1 + \frac{2}{b}\right) \cdots \left(1 + \frac{k+1}{b}\right)} \cdot 
     \frac{1}{\left(1 + \frac{k+3}{b}\right) \cdots \left(1 + \frac{2k+2}{b}\right)}.
\end{align*}
When $k < \Floor{\log\log x}{2}$, we see that the denominator terms 
coincide with a convergent geometric series, e.g., since 
$j / b < 1$ for all $2 \leq j \leq 2k+2$, and hence the product for the $k^{th}$ summand in these 
cases is approximated by $(1+o(1))^{2k} = 1+o(1)$. 
Then the main term contribution of the sum over $k$ within this range corresponds to 
$\Floor{\log\log x}{2} \sim \frac{\log\log x}{2}$. 
For $k \geq \Floor{\log\log x}{2}$, we have convergence of the series to an absolute constant 
$0 < C_0(x) < +\infty$ that varies (only slighly) with $b$. 
These cases of the previous displayed formula lead us to conclude that 
\[
T_{1,2}(x) - T_{1,1}(x) \sim 1 - \frac{e^2 \sqrt{\log\log x}}{\sqrt{2\pi}} + 
     O\left(\frac{1}{\sqrt{\log\log x}}\right). 
     \qedhere 
\]
\end{proof}
\begin{proof}[Proof of \eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnB}]
We can sum exactly to see that 
\[
S_2(x) = \frac{x \cdot \Gamma(\log\log x, \log\log x)}{\Gamma(\log\log x)}. 
\]
When we apply the form of the known incomplete gamma function asymptotics from 
\eqref{eqn_IncompleteGamma_PropC} as $x \rightarrow \infty$ and apply Stirling's formula 
to approximate $\Gamma(N+1) = N \cdot \Gamma(N)$ for large $N$, we can see that 
$S_2(x) \sim \frac{x}{2}$.
\end{proof}
\begin{proof}[Proof of \eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnC}]
We can sum $S_3(x)$ exactly to arrive at the following expression: 
\[
S_3(x) = \frac{x \cdot \Gamma(\log\log x, \log\log x)}{\Gamma(\log\log x)}. 
\]
...
We have written $\Gamma(\log\log x) = \frac{1}{\log\log x} \times \Gamma(\log\log x + 1)$ 
to apply Stirling's formula in the last step. 
\end{proof}
...
%For $x > e$, we have that 
%\begin{align}
%\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2__subeqnC} 
%S_3(x) := \sum_{1 \leq k \leq \log\log x} \frac{x (\log\log x)^{k+1/2}}{(2k+1) (k-1)!} \sim 
...
%     \sqrt{\log\log x}\right) - \frac{_2F_2\left(1, \frac{3}{2}+\log\log x; 1 + \log\log x, 
%     \frac{5}{2} + \log\log x; \log\log x\right)}{ 
%     (2\log\log x+3) \Gamma(\log\log x)}. 
%\]
...
%
%First, notice that as $|z| \rightarrow \infty$, the \emph{imaginary error function}, 
...
%\]
%The last term in the exact expansion of $S_3(x)$ stated above is 
%approached by applying Stirling's formula to approximate 
%$\Gamma(\log\log x + 1) = (\log\log x) \cdot \Gamma(\log\log x)$. We see that 
%when we write $b := \log\log x$ we get 
%\begin{align*}
%& \frac{_2F_2\left(1, \frac{3}{2}+\log\log x; 1 + \log\log x, 
%     \frac{5}{2} + \log\log x; \log\log x\right)}{ 
%     (2\log\log x+3) \Gamma(\log\log x)} \\ 
%     & \qquad = \frac{b^{1/2} e^{-b^2}}{\sqrt{2\pi}} \left[ 
%     \frac{1}{2b+3} + \sum_{k \geq 1} \frac{b^k}{(1+b) \cdots (k+b) \times (2k+3+2b)} \right] \\ 
%     & \qquad = \frac{b^{1/2} e^{-b^2}}{\sqrt{2\pi}} \left[ 
%     \frac{1}{2b+3} + \sum_{1 \leq k < \frac{b}{2}} \frac{(1+o(1))}{2b} + 
%     \sum_{k \geq \frac{b}{2}} \frac{1}{2b \cdot 
%     \left(1 + \frac{1}{b}\right) \cdots \left(1 + \frac{k}{b}\right) \times 
%     \left(1 + \frac{2k+3}{2b}\right)} 
%     \right] \\ 
%     & \qquad \sim \frac{\sqrt{\log\log x}}{2\sqrt{2\pi} (\log x)}\left(1 + 
...
\begin{cor}[Approximating signed sums by uniform asymptotics]
...
\mathcal{D}_{\omega}(x) & \asymp  
     ...
     & \sim x \left(1 - \frac{e^2}{\sqrt{2\pi} \sqrt{\log\log x}}\right) + O(E_{\omega}(x)), 
\end{align*}
where the error term in the above bound is defined according to 
...
Hence, we can drop the asymptotically lesser order error term to express the main 
term for $D_{\omega}(x)$ as follows: 
\[
\mathcal{D}_{\omega}(x) \asymp x \left(1 - \frac{e^2}{\sqrt{2\pi} \sqrt{\log\log x}}\right). 
\]
...
\frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \ll 
     ...
     \mathrm{\ as\ } x \rightarrow \infty, 
\]
or equivalently, we have shown that $\mathcal{N}_{\omega}(x) = o\left(\mathcal{D}_{\omega}(x)\right)$. 
The following bounds result for large $x$: 
\[
...
\begin{ansatz}[An example towards a counting argument based bound for $G^{-1}(x)$]
...
\begin{remark}[Necessary complexity in the next theorem statement]
...
by a constant factor $\lambda > 1$. More precisely, 
for large $x \rightarrow \infty$, we consider 
\begin{align*}
\mathcal{I}_{2,x} & := \{1 \leq k < \log\log x: \frac{\log\log x}{k} > 1\} \\ 
     \mathcal{I}_{1,x} & := \{1 \leq k < \log\log x: k \in \mathbb{Z}\} \setminus 
     \mathcal{I}_{2,x}. 
\end{align*}
...
\widehat{C}_{k,\ast}(x) = \widehat{G}\left(\frac{k-1}{\log\log x}\right) \frac{x}{\log x} 
     \frac{(\log\log x + \log\zeta(2))^{k-1} P(2)^{1-k}}{(k-1)!} \left[ 
...
\widehat{C}_{k,\ast}(x) = \widehat{G}\left(\frac{k-1}{\log\log x}\right) \frac{x}{\log x} 
     \frac{(\log\log x + \log\zeta(2))^{k-1} P(2)^{1-k}}{(k-1)!} \left[ 
     \frac{k}{k-\log\log x} + O\left(\frac{k}{(\log\log x)^2}\right)\right]. 
\]
\end{theorem} 
\begin{proof} 
...
For $2 \leq k \leq \log\log x$, we will apply the estimate from 
...
\widetilde{A}_r(x) := 
     ...
     v^{k} (1 + P(2) v)} dv. 
\end{align} 
%We will use the method established to prove 
%Theorem \ref{theorem_HatPi_ExtInTermsOfGz} in the reference 
%\cite[\S 7.4]{MV} to show that in fact 
%\begin{equation}
%\label{eqn_GHatCHatkAstxFunc_UniformAsymptotics_ExactFormula_proof_v1} 
%\widehat{C}_{k,\ast}(x) = \frac{\widehat{F}\left(2, \frac{k-1}{\log\log x}\right)}{\Gamma(z+1)} \cdot 
%     \frac{x}{\log x} \cdot \frac{(-1)^{k} (\log\log x + \log \zeta(2))^{k-1}}{(k-1)!} \times 
%     \left[1 + O_R\left(\frac{k}{(\log\log x)^2}\right)\right]. 
%\end{equation}
The main term for the sums $\widehat{C}_{k,\ast}(x)$ 
is given by $\frac{x}{\log x} \cdot I_k(r, x)$, where we take 
\begin{align*}
I_k(r, x) & = \frac{1}{2\pi\imath} \times \int_{|z|=r} 
     \frac{\widehat{G}(z) (\log x)^{-z} \zeta(2)^{-z}}{z^k \cdot (1 + P(2)z)} dz \\ 
     & =: I_{1,k}(r, x) + I_{2,k}(r, x). 
\end{align*}
The first of the component integrals in the last equation is defined to be 
\begin{align*}
I_{1,k}(r, x) & := \frac{\widehat{G}(r)}{2\pi\imath} \times \int_{|z|=r} 
     \frac{(\log x)^{-z} \zeta(2)^{-z}}{z^k \cdot (1+P(2)z)} dz. 
\end{align*}
...
\frac{1}{m!} \times \frac{\partial^{(m)}}{{\partial v}^{(m)}}\left[ 
     ...
     & \sim \begin{cases}
     \frac{(-1)^{m} (\log\log x + \log\zeta(2))^{m}}{m!}, & \text{if $m+1 \in \mathcal{I}_{1,x}$; } \\ 
          \frac{(-1)^{m+1} (\log\log x + \log\zeta(2))^{m}}{m! \left(1 - \frac{\log\log x}{m}\right)}, 
          & \text{otherwise.}
     \end{cases}
\end{align*} 
...
Thus we see by taking $z = -r = \frac{1-k}{\log\log x}$ that 
\[
I_{1,k}(r, x) \asymp \frac{\widehat{G}(r) (\log\log x + \log\zeta(2))^{k-1}}{(k-1)!}. 
\]
...
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|z|=r} (\widehat{G}(z) - \widehat{G}(r)) 
     \frac{(\log x)^{-z} \zeta(2)^{-z}}{z^k \cdot (1+P(2)z)} dz. 
\]
Now with $P(2) \approx 0.452247$ and integrating by parts, we can write that 
\[
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|z|=r} (\widehat{G}(z) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(z-r)) 
     (\log x)^{-z} \zeta(2)^{-z} \left[\sum_{i \geq 0} (-1)^i z^{i-k} P(2)^{i}\right] dz. 
\]
Notice that 
\[
...
T_{k,i}(r, x) & \ll r^{3-k+i} e^{k-i-1} \times \int_0^{\infty} \theta^2 e^{-8(k-i-1) \theta^2} d\theta \\ 
     ...
     \frac{(\log\log x + \log\zeta(2))^{k-3-i} e^{k-i-1}}{(k-1-i)^{3/2} (k-1)^{k-3-i}} \\ 
     & \ll 
     \frac{k \cdot (\log\log x + \log\zeta(2))^{k-3-i}}{(k-1)!}. 
\end{align*}
Then it follows that with $r := \frac{k-1}{\log\log x}$, the sums 
\[
\sum_{i \geq 0} |T_{k,i}(r, x)| P(2)^i \ll \frac{k \cdot (\log\log x + \log\zeta(2))^{k-3}}{(k-1)!} (1+o(1)). 
\]
...
\widehat{G}\left(\frac{k-1}{\log\log x}\right) = \frac{1}{\Gamma\left(1+\frac{k-1}{\log\log x}\right)} \cdot 
     \frac{\zeta(2)^{(1-k)/\log\log x}}{\left(1+\frac{P(2)(k-1)}{\log\log x}\right)} \gg 1, 
\]
so that we arrive at \eqref{eqn_GHatCHatkAstxFunc_UniformAsymptotics_ExactFormula_proof_v1}, as claimed. 
This implies the stated result in our theorem. 
\end{proof} 

\begin{cor} 
\label{cor_SummatoryFuncsOfUnsignedSeqs_v2} 
We have for large $x \geq 2$ 
%uniformly for $1 \leq k \leq \log\log x$ that 
\begin{align*} 
\widehat{C}_k(x) := 
     \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \sim 
     \begin{cases} 
     \frac{x (\log\log x)^{k-1} P(2)^{1-k}}{(k-1)!}, & k \in \mathcal{I}_{1,x}; \\ 
     \frac{x (\log\log x)^{k-1} P(2)^{1-k}}{\left(1-\frac{\log\log x}{k}\right) (k-1)!}, & 
          k \in \mathcal{I}_{2,x}. 
     \end{cases} 
\end{align*} 
\end{cor} 
\begin{proof} 
...
$1 \leq k \leq \log\log x$\footnote{
     We can easily show that for any $1 \leq k \leq x$, 
...
\pi_k(x) \asymp \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} (1 + o_k(1)), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
We have by the asymptotic approximation to the incomplete gamma function and 
...
L_{\ast}(t) & := \left\lvert \sum_{n \leq t} (-1)^{\omega(n)} \right\rvert \asymp 
     ...
     t\left(1-\frac{e^2}{\sqrt{2\pi} \sqrt{\log\log t}}\right), 
...
\[
\frac{1}{L_{\ast}^{\prime}(t)} \sim 1 + o(1). 
\]
...
\begin{align*} 
     ...
     C_{\Omega(x)}(x) \Iverson{\Omega(x) = k} & \sim \widehat{C}_{k,\ast}^{\prime}(x) (1+o(1)) 
     %\\ 
     %& \sim \frac{(\log\log x)^{k-1}}{(\log x) (k-1)!} 
     =: \widehat{C}_{k,\ast\ast}(x), \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*} 
The Abel summation formula and 
integration by parts imply that we obtain the 
main term for our sum in the form of 
\begin{align*} 
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & \sim
     \int \widehat{C}_{k,\ast\ast}(x)dx 
     \sim 
     \frac{x \cdot (\log\log x)^{k-1}}{(k-1)!}. 
     \qedhere 
\end{align*} 
\end{proof}
...
\[
\mathbb{E}\left[C_{\Omega(n)}(n)\right] \sim \frac{\log n}{2}. 
\] 
\end{lemma} 
\begin{proof} 
...
\begin{align*} 
\int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & \sim 
     \frac{(\log t)^2}{4}. 
\end{align*} 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: 
     ...
     \Phi\left(\frac{z+\mu_x(C)}{2\sigma_x(C)}\right) + O\left(\frac{1}{\sqrt{\log\log x}}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\] 
\end{theorem} 
\begin{proof} 
...
We will restrict ourselve to the cases where 
$\frac{\log\log x}{k} > 1$ as $x \rightarrow \infty$. 
We then use the formula proved in Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2} 
...
$t_x := \frac{t}{2\sqrt{3} e^{1/2} \mu_x(C)^{3/2} (\log x)^{1/12}}$. 
...
\frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & 
     \sim 
     \frac{(\log\log x)^{k-1}}{(k-1)! \cdot \left(1-\frac{\log\log x}{k}\right)} \\ 
     & \sim 
     \frac{(\log x) \mu_x(C)}{ 
     \left(1-(1+\delta_{t,x})^{-1}\right)} \cdot e^{t_x} \times (1+o(1))^{k-1} \times 
     \left(1+\delta_{t,x}\right)^{\frac{1}{2}-\mu_x(C)(1+\delta_{t,x})}. 
\end{align*} 
Notice that for any small $0 < \delta < 1$ we have 
\begin{align*}
     \frac{1}{1-(1+\delta)^{-1}} & \sim 1 + \frac{1}{\delta}. 
\end{align*}
We have the uniform estimate that 
$\log(1+\delta_{t,x}) = \delta_{t,x} - \frac{\delta_{t,x}^2}{2} + \frac{\delta_{t,x}^3}{3} + 
 O(|\delta_{t,x}|^4)$ whenever 
$|\delta_{t,x}| \leq \frac{1}{2}$. Then we can expand the factor involving $\delta_{t,x}$ 
from the previous equation as follows: 
\begin{align*} 
 & (1+\delta_{t,x})^{\frac{1}{2}-\mu_x(C) (1+\delta_{t,x})} \left(1 + \frac{1}{\delta}\right) \\ 
     & \qquad = 
     \exp\left(\frac{1}{2} - \frac{3t_x}{2} - \mu_x(C) + o(1)\right) \cdot 
     \exp\left(- \frac{t_x^2-2t_x^3}{12\mu_x(C)^2} + \frac{3t_x-4t_x^2}{12\mu_x(C)} + 
     \frac{t_x^3-2t_x^4}{6\mu_x(C)^3}+ 
...
\begin{align*} 
\frac{(\log\log x)^{k-1}}{(k-1)! \cdot \left(1-\frac{\log\log x}{k}\right)} & \sim 
     \frac{2\sqrt{3} e^{1/2} \mu_x(C)^{3/2} (\log x)^{1/12}}{2\sqrt{6\pi} \cdot \sigma_x(C)} 
     \cdot \exp\left(-\frac{(t_x+\mu_x(C))^2}{12\sigma_x(C)^2}\right) \times 
     \left[1 + \widetilde{E}(x, t_x)\right]. 
\end{align*} 
It follows that for $k \in \mathcal{I}_{2,x}$, or equivalently the 
$1 \leq k < \log\log x$ that satisfy $\frac{\log\log x}{k} > 1$, we get 
\begin{align*}
f(k, x) & = 
     \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \\ 
     & \sim 
     \frac{2\sqrt{3} e^{1/2} \mu_x(C)^{3/2} (\log x)^{1/12}}{\sqrt{2\pi} \cdot \sigma_x(C)} 
     \cdot \exp\left(-\frac{t^2}{24\sqrt{3} e^{1/2} \mu_x(C)^{7/2} (\log x)^{1/12}}\right) \times \\ 
     & \phantom{\sim\ \quad} \times 
     \left[1 + \widetilde{E}\left(x, \frac{|k - \mu_x(C)|}{ 
     24\sqrt{3} e^{1/2} \mu_x(C)^{5/2} (\log x)^{1/12}}\right)\right]. 
\end{align*}
Since our target probability density function approximating the PDF (in $t$) of the 
normal distribution is given by 
$$\frac{f(k, x)}{2\sqrt{3} e^{1/2} \mu_x(C)^{3/2} (\log x)^{1/12}} 
  \rightarrow \frac{1}{\sqrt{2\pi} \cdot \sigma_x(C)} \times \exp\left(-\frac{(t+\mu_x(C))^2}{12 
  \sigma_x(C)^2}\right),$$ 
we perform the change of variable $s \mapsto 2\sqrt{3} e^{1/2} \mu_x(C)^{3/2} (\log x)^{1/12} \cdot t$ 
to obtain the normalized form of our theorem stated above. 
...
\[
\mu_x(C) -\mu_x(C)^{2/3} \leq k < \mu_x(C) + z \cdot \sigma_x(C), 
\] 
...
we have uniformly that 
\begin{align*} 
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq z\right\} & = 
     \Phi\left(\frac{\pi^2 z + \left(12+\pi^2\right)\mu_x(C)}{2\pi^2}\right) + 
...
\tag{B} 
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     2\alpha -\left(\frac{12+\pi^2}{\pi^2}\right) \mu_x(C) 
     \right) & = 
     \Phi\left(\alpha\right) + o(1), \alpha \in \mathbb{R}. 
\end{align*} 
\end{lemma} 
...
\[
\gamma_z(x) := \frac{1}{x} \cdot \#\{2 \leq n \leq x: |g^{-1}(n)| - 
     \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq z\}. 
\]
To prove (A), observe that by Corollary \ref{cor_CLT_VII} for $z := 0$ we have that 
\[
\gamma_{-\left(\frac{12+\pi^2}{\pi^2}\right) \mu_x(C)}(x) = 
     \Phi\left(0\right) + o(1), \mathrm{\ as\ } x \rightarrow \infty. 
\]
The result in (B) follows easily by 
applying Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2}.
\end{proof} 
