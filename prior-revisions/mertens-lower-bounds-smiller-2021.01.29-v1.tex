...
\newglossaryentry{Nupn}{
    ...
    description={The valuation function that extracts the maximal exponent of $p$ in the prime factorization of $n$, e.g., 
                 ...
                 (or when $p^{\alpha}$ exactly divides $n$) for $p$ prime, $\alpha \geq 1$ and $n \geq 2$.},
    type={symbols},
...
\newglossaryentry{AverageOrderExpectation}{
    symbol={$\mathbb{E}[f(x)],\overset{\mathbb{E}}{\sim}$},
    sort={AverageOrderExpectation},
    description={We use the expectation notation of $\mathbb{E}[f(x)] = h(x)$, or sometimes write that 
                 $f(x) \overset{\mathbb{E}}{\sim} h(x)$, 
                 to denote that $f$ has an \emph{average order} 
                 of $h(x)$. This means that $\frac{1}{x} \sum_{n \leq x} f(n) \sim h(x)$, or equivalently that 
                 $$\lim_{x \rightarrow \infty} \frac{\frac{1}{x} \sum\limits_{n \leq x} f(n)}{h(x)} = 1.$$ },
    type={symbols},
...
exact formulas and new asymptotic bounds for $M(x)$. 
...
\textbf{Keywords and Phrases:} {\it M\"obius function; Mertens function; 
                                    ...
                                    prime counting function; Dirichlet generating function or DGF; 
...
multiplicative functions and their classical importance. 
...
as understanding the distribution of $M(x)$ well at large $x$. 
...
for an absolute constant $\hat{C}_{\ast}(C) > 0$, 
the approximate average order of the unsigned sequence satisfies 
\[
\mathbb{E}|g^{-1}(n)| \sim \hat{C}_{\ast}(C) \cdot (\log n)^2 \sqrt{\log\log n}, 
     \mathrm{\ as\ } n \rightarrow \infty. 
\]
In Section \ref{Section_NewFormulasForgInvn}, 
we prove the next variant of an Erd\"{o}s-Kac theorem like analog
...
\begin{align*} 
\tag{A}
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 0\right) & = \frac{1}{2} + o(1) \\ 
\tag{B} 
     \mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     \frac{\alpha \mathbb{E}|g^{-1}(x)|}{\log x} + \frac{6}{\pi^2} \sqrt{\log\log x}\right) & = 
\Phi\left(\frac{6\alpha \hat{G}_{\ast}}{\pi^2}\right) + o(1), \alpha \in \mathbb{R}. 
\end{align*} 
In the previous statement of (B), $\hat{G}_{\ast} > 0$ is an absolute constant. 
...
for $m,r = o\left(\frac{(\log x) (\log\log\log x)}{\log\log x}\right)$ and 
almost every large integer $x \geq 1$ 
\[
G^{-1}(x) = o\left(\frac{x}{(\log x)^m (\log\log x)^r}\right). 
\]
...
M(x) & = O\left(G^{-1}(x) + G^{-1}\left(\frac{x}{2}\right) + 
     \frac{x}{\log x} \times \max_{1 \leq k \leq \sqrt{x}} |G^{-1}(k)|  
...
\frac{D(x, r)}{x} \ll (\log x)^{r - 1 - r \log r} \ll 
     \frac{x^{1+\log\log\log x}}{(\log x)^{\log x}}. 
\]
Then we see that 
\[
\frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \ll 
     \frac{x^{1+\log\log\log x} \sqrt{\log\log x}}{(\log x)^{1 + \log x}} = o(1), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
\]
This clear connection between the summatory function of $g^{-1}(n)$ and the primes is also conveyed 
...
Let the function $\widehat{F}(s, z)$ be defined in terms of the prime zeta function 
...
$\widehat{F}(s, z)$ correspond to 
\[
\widehat{A}_z(x) := \sum_{n \leq x} (-1)^{\omega(n)} 
     C_{\Omega(n)}(n) z^{\Omega(n)}. 
\]
We have that for all sufficiently large $x$ and any $|z|< P(2)^{-1}$ 
\[
...
the DGF expansion of $\widehat{F}(s, z)$ whenever $\Re(s) > 1$, then 
\[
...
\[
\frac{1}{L_{\ast}^{\prime}(t)} \asymp (\log\log t)^{\frac{1}{2}}. 
\]
...
C_{\Omega(x)}(x) & \asymp \frac{(\log\log x)^{\frac{1}{2}}}{\log x} \cdot  
...
\hat{C}_{\ast}(C) := \lim_{n \rightarrow \infty} 
     \frac{\mathbb{E}[C_{\Omega(n)}(n)]}{(\log n) \sqrt{2\sqrt{2\pi} \cdot \log\log n}}. 
\]
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: 
     \frac{C_{\Omega(n)}(n) - \mu_x(C)}{(\log x) \sqrt{\log\log x} \cdot 
     \sigma_x(C)} \leq z\right\} = 
...
reference will see many parallels to their constructions. 
...
\frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & 
     ...
     \sim 
     \frac{(\log x)}{ 
     \left(1+\frac{1}{2k}\right)} \cdot e^{t} (1+o(1))^{k+\frac{1}{2}} \times 
...
\]
Similarly, forboth  $|t| \leq 1$ and $|t| > 1$, we have that 
\[
...
\[
f(k, x) := 
     \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \sim 
     \frac{(\log x) \sqrt{\log\log x}}{\sqrt{2\pi} \cdot \sigma_x(C)} 
     \cdot \exp\left(-\frac{(k-\mu_x(C))^2}{2\sigma_x(C)^2}\right) \times 
...
Since our target probability density function approximating the PDF (in $s$) of the 
normal distribution is given by 
$$f(k, x) \rightarrow \frac{1}{\sqrt{2\pi} \cdot \sigma_x(C)} \times e^{\frac{-s^2}{2\sigma_x^2}},$$ 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     ...
     \Phi\left(\frac{\frac{\pi^2}{6} z - \mu_x(C)}{(\log x)(\log\log x)}\right) + 
...
Let the absolute constant 
\[
\hat{G}_{\ast} := \lim_{n \rightarrow \infty} \frac{\mathbb{E}|g^{-1}(n)|}{(\log n)^2 \sqrt{\log\log n}}. 
\]
...
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 0\right) & = \frac{1}{2} + o(1) \\ 
\tag{B} 
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq \frac{\alpha \cdot 
     \mathbb{E}|g^{-1}(x)|}{\log x} + \frac{6}{\pi^2} \sqrt{\log\log x}\right) & = 
     \Phi\left(\frac{6 \alpha \hat{G}_{\ast}}{\pi^2}\right) + o(1), \alpha \in \mathbb{R}. 
\end{align*} 
\end{lemma} 
\begin{proof} 
...
\[
\gamma_0(x) = \Phi\left(-\frac{\mu_x(C)}{(\log x)(\log\log x)}\right) + o(1), \mathrm{\ as\ } x \rightarrow \infty. 
\]
We have a 
...
S_1(x) = O\left(\frac{x}{\log x} \times \sum_{k \leq \sqrt{x}} 
     \frac{|G^{-1}(k)|}{k^2}\right) = O\left(
     \frac{x}{\log x} \times \max_{k \leq \sqrt{x}} |G^{-1}(k)| \times 
     \int_1^{\sqrt{x}} \frac{dt}{t^2} 
     \right). 
\]
...
We thank and give much due appreciation to the following professors that offered 
