...
\newglossaryentry{muSigmaTheoremConstants}{
    ...
    description={We define $\mu_x(C) := \log\log x + \log(2) - \log\left(\hat{C}_{\ast}(C)\right)$ and 
                 $\sigma_x(C) := \sqrt{\log\log x}$ where 
                 $\hat{C}_{\ast}(C) > 0$ is an absolute constant. 
                 },
    type={symbols},
...
\[
\mathbb{E}|g^{-1}(n)| \sim \hat{G}_{\ast} \cdot (\log n)^2 \sqrt{\log\log n}, 
...
$\mu_x(C) := \log\log x + \log 2 - \log(\hat{C}_{\ast}(C))$ and 
$\sigma_x(C) := \sqrt{\log\log x}$ for $\hat{C}_{\ast}(C) > 0$ an absolute constant, 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: |g^{-1}(n)| - \frac{6}{\pi^2} 
     ...
     \Phi\left(\frac{6}{\pi^2} \left(y + \mu_x(C)\right) \sigma_x(C)\right) + 
...
\tag{D}
     ...
     \frac{6}{\pi^2}(\log\log x)^{3/2} 
     \right) & = \frac{1}{2} + o(1) \\ 
\tag{E} 
     \mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     \frac{6}{\pi^2} \left(\alpha + \log\log x\right) \sqrt{\log\log x}\right) & = 
...
M(x) & = O\left(G^{-1}(x) + G^{-1}\left(\frac{x}{2}\right) + 
     ...
     + (\log x)^2 (\log\log x)^{\frac{3}{2}}\right). 
\end{align*} 
...
Note our use of the asymptotic notation $A \asymp B$ to show the dominant terms 
(up to a constant factor) of the sums used in the proof below. 
...
\eqref{eqn_IncompleteGamma_PropB} and Stirling's formula with 
\eqref{eqn_Pikx_UniformAsymptoticsStmt_from_MV_v2} to obtain that 
\begin{align*} 
\mathcal{D}_{\omega}(x) & = \left\lvert \sum_{k \leq \log\log x} \frac{(-1)^k \cdot x}{\log x} \cdot 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \right\rvert \\ 
     & \sim \left\lvert \frac{x}{(\log x)^2} \cdot \frac{\Gamma(\log\log x, -\log\log x)}{ 
     \Gamma(\log\log x)} \right\rvert \\ 
     & \sim \frac{x}{\log x} \cdot \frac{(\log\log x)^{\log\log x - 1}}{\Gamma(\log\log x)} \\ 
     & \sim \frac{x}{\sqrt{2\pi} \sqrt{\log\log x}}, \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*} 
We see that the error terms given by \eqref{eqn_Pikx_UniformAsymptoticsStmt_from_MV_v2} 
are at most of the same order of the main term for these signed sums by computing that 
\begin{align*}
\left\lvert \sum_{k \leq \log\log x} \frac{x}{\log x} \cdot 
     \frac{k \cdot (\log\log x)^{k-1}}{(k-1)!} \right\rvert & \sim
     \frac{x}{\log x} \cdot \frac{(\log\log x)^{\log\log x-1}}{\Gamma(\log\log x)} \\ 
     & \sim \frac{x}{\sqrt{2\pi} \sqrt{\log\log x}}, \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*}
...
uniformly for $1 \leq k < \log\log x$ that 
\[
\widehat{C}_{k,\ast}(x) := 
     \sum_{\substack{n \leq x \\ \Omega(n) = k}} (-1)^{\omega(n)} 
     C_k(n) \asymp 
     \frac{x}{\log x} \cdot \frac{(-1)^{k-1} (\log\log x + \log\zeta(2))^{k-1}}{(k-1)!} \left[1 + 
     O\left(\frac{k}{(\log\log x)^2}\right)\right]. 
\]
\end{theorem} 
\begin{proof} 
We begin by bounding a parameterization of a 
contour integral over the error term of $\widehat{A}_z(x)$ from 
Proposition \ref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn} 
for fixed large $x$ when 
...
\end{align*} 
When $k = 1$ we have directly that 
\begin{align*} 
\left\lvert \int_{|v|=r} \frac{x \cdot (\log x)^{-(\Re(v) + 2)}}{v^{2}} dv \right\rvert & \ll 
     \left\lvert \frac{1}{1!} \times \frac{d}{dv}\left[x \cdot (\log x)^{-(\Re(v) + 2)}\right] 
     \Bigr\rvert_{v=0} \right\rvert \\ 
     & \ll \left\lvert \frac{d}{dm}\left[\frac{x}{(\log x)^2} \cdot \exp(-m \log\log x)\right] 
     \Bigr\rvert_{m=0} \right\rvert \\ 
...
to satisfy Proposition \ref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn}: 
\begin{align} 
...
\widehat{C}_{k,\ast}(x) = \frac{\widehat{F}\left(2, \frac{k-1}{\log\log x}\right)}{\Gamma(z+1)} \cdot 
     \frac{x}{\log x} \cdot \frac{(-1)^{k-1} (\log\log x + \log \zeta(2))^{k-1}}{(k-1)!} \times 
     \left[1 + O_R\left(\frac{k}{(\log\log x)^2}\right)\right]. 
\end{equation}
We set the function $\widehat{G}(z) := \widehat{F}(2, z) / \Gamma(z+1)$. 
The main term for the right-hand-side of 
\eqref{eqn_GHatCHatkAstxFunc_UniformAsymptotics_ExactFormula_proof_v1} 
defined by the sums $\widehat{C}_{k,\ast}(x)$ 
...
Thus we see that 
\[
I_{1,k}(r, x) \sim \frac{\widehat{G}(r) (-1)^{k-1} (\log\log x + \log\zeta(2))^{k-1}}{(k-1)!}. 
\]
...
\[
\widehat{G}\left(\frac{k-1}{\log\log x}\right) = \frac{1}{\Gamma(1+(k-1)/\log\log x)} \cdot 
     \frac{\zeta(2)^{(1-k)/\log\log x}}{(1+P(2)(k-1)/\log\log x)} \gg 1, 
\]
...
We have for large $x \geq 2$ uniformly for $1 \leq k \leq \log\log x$ that 
\begin{align*} 
\widehat{C}_k(x) := 
...
summatory function is asymptotic to
\[
\frac{1}{L_{\ast}^{\prime}(t)} \asymp \sqrt{\log\log t}. 
\]
...
\begin{align*} 
     ...
     \asymp \frac{\widehat{C}_{k,\ast}^{\prime}(x)}{L_{\ast}^{\prime}(x)} \qquad \implies \\ 
C_{\Omega(x)}(x) \Iverson{\Omega(x) = k} & \asymp \frac{(\log\log x)^{\frac{1}{2}}}{\log x} \cdot  
     \left[\frac{(\log\log x)^{k-1}}{(k-1)!} \left(1 - 
     \frac{1}{\log x}\right) + \frac{(\log\log x)^{k-2}}{(\log x) (k-2)!}\right] \\ 
     & \asymp \frac{(\log\log x)^{k-\frac{1}{2}}}{(\log x) (k-1)!} 
...
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & \asymp 
     ...
     \asymp 
     \frac{x \cdot (\log\log x)^{k+\frac{1}{2}}}{(2k+1)(k-1)!}. 
     \qedhere 
\end{align*} 
\end{proof}
...
\[
\mathbb{E}\left[C_{\Omega(n)}(n)\right] \asymp (\log n) \sqrt{\log\log n}. 
\] 
\end{lemma} 
\begin{proof} 
...
\notag 
     & \asymp \sum_{d \leq n} C_{\Omega(d)}(d) \left[\frac{6}{d \cdot \pi^2} + O\left(\frac{1}{\sqrt{dn}}\right) 
     \right] \\ 
\notag 
     & \asymp \mathbb{E}[C_{\Omega(n)}(n)] + \sum_{d<n} 
     \frac{\mathbb{E}[C_{\Omega(d)}(d)]}{d} \\ 
\label{eqn_proof_tag_ExpAbsgInvn_ExpandedByExpsOfCkn_v1} 
     & \asymp (\log n)^2 \sqrt{\log\log n}. 
     \qedhere 
\end{align} 
\end{proof} 
...
\[
\hat{C}_{\ast}(C) := \lim_{n \rightarrow \infty} 
     \frac{\mathbb{E}[C_{\Omega(n)}(n)]}{(\log n) \cdot \sqrt{\log\log n}}. 
\]
Set the mean and variance parameter analogs be defined by 
\[
\mu_x(C) := \log\log x - \log\left(\frac{1}{2} \hat{C}_{\ast}(C)\right), 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: 
     ...
     \Phi\left((\log x) \sqrt{\log\log x} \cdot z\right) + O\left(\frac{1}{\sqrt{\log\log x}}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\] 
\end{theorem} 
\begin{proof} 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     ...
     \Phi\left(\frac{6}{\pi^2} \left(z + \mu_x(C)\right) \sigma_x(C)\right) + 
...
\begin{align*} 
\tag{A}
     \mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq \frac{6}{\pi^2} (\log\log x)^{3/2} 
     \right) & = \frac{1}{2} + o(1) \\ 
\tag{B} 
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     \frac{6}{\pi^2}\left(\alpha + \log\log x\right) \sqrt{\log\log x}\right) & = 
...
\[
\gamma_{\frac{6}{\pi^2} (\log\log x)^{3/2}}(x) = \Phi\left(0\right) + o(1), \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
\bibitem{RENYI-TURAN}
A.~Renyi and P.~Turan.
\newblock On a theorem of {E}rd{\"{o}}s-{K}ac.
...
\bibitem{OEIS}
N.~J.~A. Sloane.
\newblock The {O}nline {E}ncyclopedia of {I}nteger {S}equences, 2020.
