...
$$G(x) := \sum_{n \leq x} g(n) = \sum_{n \leq x} \lambda(n) |g(n)|,$$ with 
the classical prime counting function $\pi(x)$ determine new exact formulas for $M(x)$. 
In particular, for all $x \geq 1$ we have that 
\[
M(x) = G(x) + \sum_{p \leq x} G\left(\left\lfloor \frac{x}{p} \right\rfloor\right), 
\]
The new exact 
formulas we prove for $M(x)$ in terms of the partial sums $G(x)$ motivates our 
study of the sequence of unsigned summands $|g(n)| = \lambda(n) g(n)$. 
We prove the following exact formula expressing $g(n)$ in terms of the 
function $C_{\Omega}(n)$ whose Dirichlet generating function is given by 
$(1-P(s))^{-1}$ for $\Re(s) > 1$ where $P(s) := \sum_{p} p^{-s}$ is the 
prime zeta function: 
\[
g(n) = \lambda(n) \times \sum_{d|n} \mu^2\left(\frac{n}{d}\right) C_{\Omega}(d), 
     \text{ for all } n \geq 1. 
\]
A special case of the Selberg-Delange method applied to the function 
$\widehat{G}(z) := \frac{(1+P(2)z)^{-1} \zeta(2)^{-z}}{\Gamma(1+z)}$ 
defined for any $0 \leq |z| \leq P(2)^{-1}$ 
shows that for all sufficiently large $x$ we have uniformly for 
...
The uniform asymptotics for the restricted partial sums of $C_{\Omega}(n)$ 
allow us to determine precise statistics for the average orders of 
$C_{\Omega}(n)$ and $|g(n)|$ and predict formulas for higher-order moments 
of these functions. 
%We conjecture that there are explicit functions 
%$\mu_{\Omega}(x)$ and $\sigma_{\Omega}(x)$ and a limiting probability measure on 
%$\mathbb{R}$ with cumulative density function $\Phi_{\Omega}$ such that for any real $z$ 
...
%     \Phi_{\Omega}(z) + o(1). 
%\]
%If the conjecture on the limiting distribution of $C_{\Omega}(n)$ is correct, then 
%this result provides a scaled limiting distribution for the difference between 
%$\frac{\pi^2}{6} g(n)$ and the average order of $g(n)$. 
%The existence of this limiting distribution then suggests new approaches to 
%bounding the Mertens function by considering asymptotics of the 
%distribution of $|g(n)|$ and the partial sums $L(x) := \sum_{n \leq x} \lambda(n)$ 
%along predictable infinite subsequences of large $x$. 
...
\end{equation}
where the sequence $\lambda(n) C_{\Omega}(n)$ has the 
...
The Mertens function has exact expressions by discrete convolutions of the 
classical prime counting function $\pi(x)$ and the signed partial sums of $g(n)$. 
These new formulas for $M(x)$ provide a window from which we can view classically  
...
Preliminary numerical computations of these functions and 
their partial sums suggests the intuition that 
these primitives will be easier objects to work with and hence bound in 
limiting cases as the subject of future work to extend the results in this article. 
...
\]
We use the results proved in the application of the Selberg-Delange method 
...
to prove the following theorem for the average order of $C_{\Omega}(n)$: 
...
%The next conjecture provides a complete picture of the distribution of the 
%unsigned inverse sequence 
%(see Conjecture \ref{conj_DetFormOfEKTypeThmForCOmegan_v1} and 
%Corollary \hlocalref{cor_CLT_VII}). 

\begin{conjecture*}
There are explicit functions $\mu_{\Omega}(x)$ and $\sigma_{\Omega}(x)$ and 
...
\frac{1}{x} \times \#\left\{3 \leq n \leq x: \frac{|g(n)| - 
     \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g(k)| - \mu_{\Omega}(x)}{\sigma_{\Omega}(x)}  
     \leq y\right\} & = 
     \Phi_{\Omega}\left(\frac{\pi^2 y}{6}\right) + o(1), \text{ as } x \rightarrow \infty. 
\end{align*}
\end{conjecture*}

The article is organized into sections that partition our new results by function for 
$C_{\Omega}(n)$, $g(n)$ and $|g(n)|$, and then finally the proofs of the new 
exact formulas for $M(x)$ stated in 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula}. 
The appendix sections provide a glossary of notation, 
statements of relevant known results in the literature, and short proofs of new 
formulae and lemmas that can be separated from the primary topics in the body of the article. 
...
\]
Let $\widehat{G}(z) := \widehat{F}(2, z) \times \Gamma(1+z)^{-1}$ for any 
$0 \leq |z| < P(2)^{-1}$. 
\end{definition}
...
$$\zeta(s)^{z} = \sum_{n \geq 1} \frac{d_z(n)}{n^s}, \text{ for } \Re(s) > 1.$$ The 
...
The combined sum over the interval $m \leq \frac{x}{2}$ yields the following bounds 
when $0 < |z| \leq R$: 
\begin{align*} 
...
we proved in Theorem \hlocalref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn} is 
...
The strategy behind the proof of the theorem is an extension of the 
Selberg-Delange convolution method from \cite[\S II.6.1]{TENENBAUM-PROBNUMT-METHODS}. 
Our choice of the $z$-dependent function $\widehat{F}(s, z) \zeta(s)^{z}$ 
is motivated by the exact formula for $C_{\Omega}(n)$ expanded by 
\eqref{eqn_proof_tag_hInvn_ExactNestedSumFormula_CombInterpetIdent_v3}. 
We have applied an extension of Tenenbaum's Selberg-Delange method proofs to extract 
an asymptotic formula for the coefficients of $\widehat{F}(s, z) \zeta(s)^{z}$. 
\end{remark}
...
The arguments given in the next proof is 
...
$1 \leq k \leq 2\log\log x$ when $r = \frac{k-1}{\log\log x}$. 
\begin{align*}
...
\label{eqn_AbelSummationIBPReverseFormula_stmt_v1} 
     ...
     \left(\sum_{n \leq x} \ell(n)\right) \hat{h}(x) - 
...
\]
We conclude that 
$\widehat{C}_k\left(x^{e^{-1}}\right) = o\left(\widehat{C}_k(x)\right)$ at 
all sufficiently large $x$. 
The solution to \eqref{eqn_ProofTag_HatCkx_Asymptotics_v1_v0} is then of the form 
\[
...
\end{align*} 
If $1 \leq k \leq \frac{3}{2} \log\log x$ such that $\lambda > 1$ in 
...
\end{align}
For $0 \leq z \leq 2$, the function $\widehat{G}(z)$ is monotone in 
...
that whenever $0 < |z| < P(2)^{-1}$ with $x$ sufficiently large 
\begin{align}
...
\end{align}
For large $x$ and fixed $0 < r < P(2)^{-1}$, we define 
\[
...
$\frac{1}{\Gamma(1+r)} \gg 1$ for $r \in [1, P(2)^{-1})$, 
we find that 
\[
...
whenever $\mathcal{E}[n_1] = \mathcal{E}[n_2]$ we have that 
$C_{\Omega}(n_1) = C_{\Omega}(n_2)$ and $g(n_1) = g(n_2)$. 
This property of both the unsigned functions is special and 
is a somewhat unusual observation. 
We find that the regularity with respect to the repetition of distinct values of 
each sequence inherited from this property is sufficient to arrive at the 
statement of the existence of the limiting distributions for each function 
we conjecture in 
...
\end{align} 
The latter inner sum forms the main term approximated using 
\eqref{eqn_COmeganAvgOrder_MainTermIntegralInt_v1} as $t \rightarrow \infty$. 
\end{proof} 
...
limiting probability measure on $\mathbb{R}$ with cumulative density function 
...
%         The dashed line shows the approximate fit by the CDF 
%	 of an inverse Gaussian distribution with 
...
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit1_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}\hfil
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit2_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit3_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}\hfil
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit4_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}

\caption{Histograms representing the CDF of the distribution of 
         $\sigma^{-1} \left(|g(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g(k)| - \frac{6\mu}{\pi^2}\right)$  
         for $n \leq x$ where $\mu \equiv \mu(x) = \sqrt{\log\log x}$ and 
         $\sigma \equiv \sigma(x) = \sqrt{\log\log x}$. 
         The dashed line shows the approximate fit by the CDF 
	 of a shifted log-normal distribution with 
         mean $\alpha$ and standard deviation $\lambda$. }
\label{figure_HistgInvnDistUns_Comparisons_v1}
...
\begin{align*} 
\frac{1}{x} \times \#\left\{3 \leq n \leq x: \frac{|g(n)| - 
     \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g(k)| - \mu_{\Omega}(x)}{\sigma_{\Omega}(x)} \leq y\right\} & = 
...
|g(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g(k)| & \sim \frac{6}{\pi^2} C_{\Omega}(n), 
     \text{\ as\ } n \rightarrow \infty. 
\end{align*} 
From the proof of Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2} 
we obtain that 
\begin{align*} 
...
\end{align*}  
By Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2}, 
...
Figure \hlocalref{figure_HistgInvnDistUns_Comparisons_v1} is illustrative of an apparent 
limiting distribution. 
We have arrived at the second central moment of $C_{\Omega}(n)$ 
...
\caption{A comparison of $M(x)$ and a scaled form of $G(x)$ where 
	 the absolute constant $C_2 := \zeta(2)$. A shift in $x$ of the latter plot is compared to the values of $M(x)$. 
	 The function $Q(x) := \sum_{n \leq x} \mu^2(n)$ counts the number of squarefree integers $n \leq x$ for any 
         $x \geq 1$. } 
\end{subfigure}
...
\caption{Discrete plots displaying comparisons of the scaled growth of $G(x)$ for $x \leq 4500$. }
...
suggests local information about $G(x)$ through its connection to $|G|(x)$. 
The plots shown in 
...
the values of $M(x)$ and $G(x)$ with scaled forms of related auxilliary partial sums. 
A natural question to ask in light of the numerical intuition from 
Figure \hlocalref{figure_MxAndNewAuxPartialSums_Comparison_Intro_v2_v1}(a) 
is whether the following is true: 
\[
\limsup_{x \rightarrow \infty} |G|(x) x^{-\frac{3}{2}} > 0. 
\]
A partial resolution to the question is given by the DGF of the unsigned inverse function $|g(n)|$ from 
Remark \hlocalref{remark_MiscConsequencesOfCorForFormulaOfUnsgInvnFunc_v2} which suggests that 
there are infinitely many positive integers $x$ for which $|G|(x) \geq x^{1.39943}$ 
\cite[\cf Thm.~1.3; \S 1.2]{MV}.

\subsection{Example: Expected local cancellation of 
            $G(x)$ in the new formulas for $M(x)$ along an infinite primorial subsequence} 
...
(in violation to \eqref{eqn_MertensMx_RHEquivProblem_Stmt_intro} above). 
...
the values of the Mertens function are bounded along this subsequence: 
...
The link between factorization symmetry and resulting 
...
first $x \geq 3$ values of the partial sums of $f$.  
...
For $0 < R < 2$, we have uniformly for $1 \leq k \leq R \log\log x$ 
\[
...
We have adapted the communication of his proofs to establish the next few lemmas based on 
...
\end{equation} 
The sequence $b_n(\lambda)$ satisfies $b_0(\lambda) = 1$ and 
...
%}
\[
b_n(\lambda) = \lambda(1-\lambda) b_{n-1}^{\prime}(\lambda) + \lambda(2n-1) b_{n-1}(\lambda), 
     \text{ for } n \geq 1. 
\]
\end{subequations}
\end{facts} 
...
\]
If $\lambda > 0.567142 > W(1)$, then as $z \rightarrow \infty$ 
\[
\Gamma(a, z e^{\pm\pi\imath}) = -e^{\pm \pi\imath a} \frac{z^{a-1} e^{z}}{1 + \lambda^{-1}} + 
     O_{\lambda}\left(z^{a-2} e^{z}\right). 
\]
\end{prop}
The first two estimates are only useful when $\lambda$ is bounded away from the 
