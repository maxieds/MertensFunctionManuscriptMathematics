...
\title{
       \LARGE{
       Lower bounds on the Mertens function along infinite subsequences 
       } 
}
...
standard estimates of $M(x)$ that are intimately tied to the 
...
G_E^{-1}(x) := \sum_{n \leq (\log x)^2} \lambda(n) \times \sum_{\substack{d|n \\ d > e}} 
     (\log d) (\log\log d). 
\end{equation} 
...
\begin{cor}[Unboundedness of the the Mertens function] 
\label{cor_ThePipeDreamResult_v1} 
Define the infinite increasing subsequence, 
$\{x_{0,n}\}_{n \geq 1}$, by $x_{0,n} := e^{2e^{e^{e^{e^{2n}}}}}$. 
We have that for all sufficiently large 
$y \gg \ceiling{x_{0,1}}+1$ the following bound holds: 
\begin{align*} 
\frac{|M(x_{0,y})|}{\sqrt{x_{0,y}}} & \SuccSim 
     \frac{2 C_{\ell,1} \cdot (\log \sqrt{x_{0,y}}) (\log\log\log\log \sqrt{x_{0,y}})}{ 
     (\log\log\log \sqrt{x_{0,y}})^{\frac{7}{2}} 
     (\log\log\log\log\log \sqrt{x_{0,y}})^{\frac{3}{2}}} \times 
     \exp\left(-2 (\log\log\log \sqrt{x_{0,y}})^2\right), 
     \mathrm{\ as\ } y \rightarrow \infty. 
\end{align*} 
The constant $C_{\ell,1} > 0$ in the previous equation is taken as 
\[
C_{\ell,1} := 
...
scaled Mertens function $q(x)$ grows without bound. 
...
success of the methods in our proof. Namely, 
we can draw upon the properties of strongly additive functions in place of a central reliance on 
the distributions of characteristically multiplicative structures about which we know (in general) 
much less about finite sums of their consecutive values over all $n \leq x$. 
...
where the uniform bounds hold. This fact, stated next as in 
Corollary \ref{theorem_MV_Thm7.20}, 
will be relevant when we prove 
Theorem \ref{theorem_gInv_GeneralAsymptoticsForms} using a sign-weighted 
summatory function with Abel summation that depends on the new 
lower bounds for these functions (see Lemma \ref{lemma_CLT_and_AbelSummation}). 
\end{remark} 
...
%$R \equiv 1$ in sums by the restriction that $0 \leq z < 1$) 
...
\]
The estimates above, especially the rightmost lower bound in the previous equation at $m := 1$, 
...
However, it turns out that these bounds are sufficient to prove the necessary hypotheses of 
...
\mathbb{E}\left[\sum_{\substack{d|n \\ d > e}} (\log d) (\log\log d)\right] & = 
     \frac{1}{n} \times \sum_{e < d \leq n} (\log d) (\log\log d) \Floor{n}{d} \\ 
     & \sim \int_{e}^{n} \frac{(\log t) (\log\log t)}{t} dt \\ 
     & = \frac{(\log n)^2}{2} (\log\log n) - \frac{(\log n)^2}{4} \\ 
     & \sim \frac{(\log n)^2}{2} (\log\log n). 
\end{align*} 
Now on a set of asymptotic density one, we have that since 
$\mathbb{E}|g^{-1}(n)| \not{\rightarrow} 0$ by 
...
\sum_{\substack{e \leq n \leq x \\ \lambda(n) = +1}} g^{-1}(n) & \geq 
     \phantom{-} \sum_{\substack{e \leq n \leq x \\ \lambda(n) = +1}} 
     \sum_{\substack{d|n \\ d > e}} (\log d) (\log\log d) \geq \phantom{-} 
     \sum_{\substack{e \leq n \leq (\log x)^2 \\ \lambda(n) = +1}} 
...
\sum_{\substack{e \leq n \leq x \\ \lambda(n) = -1}} g^{-1}(n) & \leq 
     ...
     \sum_{\substack{d|n \\ d > e}} (\log d) (\log\log d) \leq 
     -\sum_{\substack{e \leq n \leq (\log x)^2 \\ \lambda(n) = -1}}
     \sum_{\substack{d|n \\ d > e}} (\log d) (\log\log d). 
\end{align*} 
...
G^{-1}(x) \geq \sum_{n \leq (\log x)^2} \lambda(n) \sum_{\substack{d|n \\ d > e}} (\log d) (\log\log d) + Y. 
\end{equation} 
...
$\mathcal{G}_{-} \cap \{n \leq x\}$ in $\{n \leq x\}$ are eventually always positive as 
$x \rightarrow \infty$. 
...
\[
F_{\lambda}(x) := \sum_{\substack{u_0 < n \leq \log\log\log x}} 
...
function of $t$ for all sufficiently large $t \rightarrow \infty$. 
Then we have that 
\[
|F_{\lambda}(x)| \SuccSim \left\lvert 
     A_{\Omega}^{(\ell)}(\log\log\log x) \widehat{\tau}_{\ell}(\log\log\log x) - 
     \int_{u_0}^{\log\log\log x} 
...
Using Corollary \ref{theorem_MV_Thm7.20}, we have that 
...
\left\lvert G_E^{-1}(x) \right\rvert & \SuccSim 
     \left\lvert \sum_{e < d \leq \log\log\log x} \lambda(d) (\log d) (\log\log d) \times 
     \widehat{L}_0\left(\frac{(\log x)^2}{d}\right) 
     \right\rvert, 
\end{align*} 
where the function 
\[
...
\notag 
     ...
     =: \widehat{L}_0(x). 
\end{align} 
In particular, the precise formula for the 
...
|G_E^{-1}(x)| & \gg \left\lvert \sum_{e < d \leq (\log x)^2} 
     \lambda(d) \cdot \log d \cdot \log\log d \times 
     (-1)^{\floor{\log\log\left(\frac{(\log x)^2}{d}\right)}} \cdot 
     \widehat{L}_0\left(\frac{(\log x)^2}{d}\right) \right\rvert. 
\end{align} 
...
$\widehat{L}_0\left(\frac{(\log x)^2}{d}\right)$, or equivalently, a constant parity for 
$\floor{\log\log\left(\frac{(\log x)^2}{d}\right)}$ at such $d$. 
Consider that 
\[
\log\log\left(\frac{(\log x)^2}{d}\right) = \log\log\log x + \log\left(1 - 
     \frac{\log d}{\log\log x}\right), \mathrm{\ as\ } x \rightarrow \infty. 
\] 
If we take $d \in [e, \log\log\log x]$, we have that 
$\frac{\log d}{\log\log x} = o(1)$ as $x \rightarrow \infty$. 
...
\[
\floor{\log\log\left(\frac{(\log x)^2}{d}\right)} \sim \floor{\log\log\log x + o(1)}, 
\]
changes parity at $d_0(x)$. Then provided that the sign term involving both $d$ and $x$ 
...
decreasing function $\widehat{L}_0\left(\frac{(\log x)^2}{d}\right)$ in the variable $d$. 
We note that the function $\widehat{L}_0\left(\frac{(\log x)^2}{d}\right)$ is decreasing 
on $d \in \left[e, \frac{x}{e}\right)$, as can be viewed by computing its first 
derivative and observing that the sign on this function is negative for all sufficiently large $x$ 
for $d$ taken in this interval. 
...
Let the function $T_E(x)$ be defined for all large enough $x$ as 
\begin{equation} 
\label{eqn_proof_tag_TExFuncDefAndBounds_v1} 
T_E(x) := \frac{1}{\log\log\left(\frac{(\log x)^2}{\log\log\log x}\right)^{ 
     \frac{5}{2} + \log\log\left(\frac{(\log x)^2}{\log\log\log x}\right)}} 
     \gg \frac{1}{(\log\log\log x)^{\frac{5}{2} + \log\log\log x}}. 
\end{equation} 
Then we see that as $x \rightarrow \infty$ 
\begin{align} 
\notag 
S_{E,1}(x) & := \left\lvert \sum_{e < d \leq \log\log\log x} \lambda(d) (\log d) (\log\log d) \times 
     \widehat{L}_0\left(\frac{(\log x)^2}{d}\right) 
     \right\rvert \\ 
\notag 
     & \gg \sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} \times (\log x)^2 T_E(x) \times 
     \left\lvert \sum_{e < d \leq \log\log\log x} 
     \frac{\lambda(d) (\log d) (\log\log d)}{d} 
     \right\rvert \\ 
\label{eqn_proof_tag_SE1xFuncExp_v1} 
     ...
     \right\rvert, 
\end{align} 
where we select the functions $\widehat{\tau}_0(t) := \frac{\log t \cdot \log\log t}{t}$ and 
$-\widehat{\tau}^{\prime}_0(t) \gg \frac{\log t \cdot \log\log t}{t^2}$ in the notation from 
Lemma \ref{lemma_CLT_and_AbelSummation}. 

Let the remainder term sums be defined for all sufficiently large $x$ by 
\[
R_E(x) := \Biggl\lvert \sum_{\log\log\log x < d < \frac{(\log x)^2}{e}} 
     \lambda(d) \cdot \log d \cdot \log\log d \times 
     (-1)^{\floor{\log\log\left(\frac{\log x}{d}\right)}} \cdot 
     \widehat{L}_0\left(\frac{\log x}{d}\right) \Biggr\rvert. 
\]
...
we will provide asymptotic upper and lower bounds on $R_E(x)$ that show that the 
...
cannot match and cause a zero-tending cancellation of the terms when $x$ is large. 

First, observe that 
\begin{align*} 
R_E(x) & \ll (\log x)^2 \times \sum_{\log\log\log x < d \leq \frac{(\log x)^2}{e}} 
     \frac{\log d \cdot \log\log d}{d} \\ 
     & \sim (\log x)^2 \times \int_{\log\log\log x}^{(\log x)^2} 
     \frac{(\log t) (\log\log t)}{t} dt \\ 
     & \sim (\log x)^2 \left[\frac{(\log t)^2}{2} \log\log t - 
     \frac{(\log t)^2}{4}\right] \Biggr\rvert_{t=\log\log\log x}^{t=(\log x)^2} \\ 
     & \sim 2 \cdot (\log x)^2 (\log\log x)^2 (\log\log\log x). 
\end{align*} 
Similarly, we can bound below to show that $R_E(x)$ is at least on the order of a constant 
times $(\log x)^2$. To obtain this lower bound, note that since $\frac{\log d \cdot \log\log d}{d}$ 
is monotone decreasing for all large enough $d > e$, we obtain the smallest possible magnitude on the sum 
by alternating signs on consecutive terms in the sum. So we have some constant $C > 0$ such that 
\begin{align*} 
R_E(x) & \gg (\log x)^2 \times \left\lvert \sum_{\log\log\log x < d < \frac{(\log x)^2}{2e}} 
     \frac{\log(2d) \log\log(2d)}{2d} - \frac{\log(2d+1) \log\log(2d+1)}{2d+1} 
     \right\rvert \\ 
     & \sim (\log x)^2 \times \left\lvert \sum_{\log\log\log x < d < \frac{(\log x)^2}{2e}} 
     \frac{\log(2d) \log\log(2d)}{2d} - 
     \frac{1}{2d+1} \left(\log(2d) + \frac{1}{2d}\right) \left( 
     \log\log(2d) + \frac{1}{2d \cdot \log(2d)}\right) 
     \right\rvert \\ 
     & \approx(\log x)^2 \times  \left\lvert \sum_{\log\log\log x < d < \frac{(\log x)^2}{2e}} 
     \frac{\log(2d) \log\log(2d)}{4d^2} - 
     \frac{1}{2d} \left[ 
     \frac{\log(2d)}{2d} + \frac{1}{2d} + \frac{1}{4d^2 \cdot \log(2d)} 
     \right] \right\rvert \\ 
     & \gg C \times (\log x)^2 + o(1), \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*} 
...
smaller order terms resulting from the first sums above. 
\end{proof} 
...
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     \SuccSim 
     \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{(\log\log\log x)^{\frac{7}{2}} 
     (\log\log\log\log\log x)^{\frac{3}{2}}} \times \exp\left(-2 (\log\log\log x)^2\right). 
\end{align*} 
\end{theorem} 
\NBRef{A10-2020.04-26} 
\begin{proof} 
...
$\widehat{\tau}_0(t) = \frac{\log t \cdot \log\log t}{t}$ that satisfies 
\begin{align*} 
-\widehat{\tau}_0^{\prime}(t) & = -\frac{d}{dt}\left[ 
     \frac{\log t \cdot \log\log t}{t} 
     \right] = \frac{\log t \cdot \log\log t}{t^2} - 
     \frac{\log\log t}{t^2} - \frac{1}{t^2} 
     \SuccSim \frac{\log t \cdot \log\log t}{t^2}. 
\end{align*} 
...
G_{\ell}^{-1}(x) := \sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} \times (\log x)^2 T_E(x) \times 
     \left\lvert A_{\Omega}^{(\ell)}(\log\log\log x) \widehat{\tau}_0(\log\log\log x) - 
     \int_e^{\log\log\log x} A_{\Omega}^{(\ell)}(t) \widehat{\tau}_0^{\prime}(t) dt 
     \right\rvert. 
\end{equation} 
...
\sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} \times 
...
$I_{\ell}(t) := \sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} \times 
...
I_{\ell}\left(e^{e^{2k}}\right) e^{e^{2k}}& \SuccSim 
     \frac{2^{3/2} A_0^2}{9 \pi e^2 \log^2 2} \cdot \frac{\exp\left(e^{2k}\right)}{2^{2k} \cdot k^{2k+3/2}} 
     =: \widehat{I}_{\ell}(k). 
\end{align} 
So using the lower bound on the increasing integrand in 
\eqref{eqn_proof_thm_GInvFunc_v3_approx} (the complete terms in $x$ are decreasing), 
we find from the mean value theorem that 
\begin{align} 
\notag 
\sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} & \times (\log x)^2 T_E(x) \times 
     \left\lvert 
     \int_{\frac{\log\log\log\log\log x}{2}-\frac{1}{2}}^{\frac{\log\log\log\log\log x}{2}} 
     I_{\ell}\left(e^{e^{2k}}\right) 
     e^{e^{2k}} dk \right\rvert \\ 
...
\label{eqn_proof_thm_GInvFunc_v4_approx} 
     & \gg \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{2 \cdot 
     (\log\log\log x)^{\frac{5}{2} + \log 2 + \log\log\log x + \log\log\log\log x} 
     (\log\log\log\log\log x)^{\frac{3}{2} + \log\log\log\log\log x}}. 
\end{align} 
...
\sqrt{\frac{2}{\pi}} \frac{A_0}{3e \log 2} & \times (\log x)^2 T_E(x) \times 
     ...
     e^{e^{2k}} dk \right\rvert \\ 
     & \ll 
     \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{2e \cdot 
     (\log\log\log x)^{\frac{5}{2} + \log 2 + \log\log\log x + \log\log\log\log x} 
     (\log\log\log\log\log x)^{\frac{1}{2} + \log\log\log\log\log x}}. 
\end{align*} 
...
\label{eqn_proof_thm_GInvFunc_v5_approx} 
     & \gg \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{(\log\log\log x)^{\frac{7}{2} + \log\log\log x} 
     (\log\log\log\log\log x)^{\frac{3}{2} + \log\log\log\log\log x}}. 
\end{align} 
Hence, we see that 
\begin{align*} 
\left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     & \SuccSim 
     \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{(\log\log\log x)^{\frac{7}{2} + \log\log\log x} 
     (\log\log\log\log\log x)^{\frac{3}{2} + \log\log\log\log\log x}} \\ 
     & \gg \frac{C_{\ell,1} \cdot (\log x)^2 (\log\log\log\log x)}{(\log\log\log x)^{\frac{7}{2}} 
     (\log\log\log\log\log x)^{\frac{3}{2}}} \times \exp\left\{-\left((\log\log\log x)^2 + 
     (\log\log\log\log\log x)^2\right)\right\}. 
\end{align*} 
What is key to observe about these lower bounds is that each of them scaled by $(\log x)^{-1}$ 
monotonically increases without bound as $x \rightarrow \infty$. In particular, the remaining factor of 
$\log x$ after rescaling dominates the asymptotics of the reciprocal powers of iterated logarithms. 
\end{proof} 
...
\label{eqn_MxGInvxLowerBound_stmt_v2} 
     & \gg \frac{2 C_{\ell,1} \cdot (\log x_y) (\log\log\log\log x_y)}{(\log\log\log x_y)^{\frac{7}{2}} 
     (\log\log\log\log\log x_y)^{\frac{3}{2}}} \times \exp\left(-2 (\log\log\log x_y)^2\right). 
\end{align} 
...
Theorem \ref{theorem_CondAvgOrderGInvxSummatoryFunc_v1}. 
Then we have 
if we take $x \mapsto x_{0,y}$ with $x_{0,y} := \exp\left(2e^{e^{e^{e^{2y}}}}\right)$, 
we recover from \eqref{eqn_MxGInvxLowerBound_stmt_v2} that 
\begin{align} 
\notag 
\frac{|M(x_{0,y})|}{\sqrt{x_{0,y}}} & \gg 
     \frac{C_{\ell,1} \cdot (\log \left[(B-\varepsilon_0) \sqrt{x_{0,y}}\right]) 
     (\log\log\log\log \left[(B-\varepsilon_0) \sqrt{x_{0,y}}\right]) \times 
     \exp\left(-2 (\log\log\log \left[(B-\varepsilon_0) \sqrt{x_{0,y}}\right])^2\right)}{ 
     (\log\log\log \left[(B-\varepsilon_0) \sqrt{x_{0,y}}\right])^{\frac{7}{2}} 
     (\log\log\log\log\log \left[(B-\varepsilon_0) \sqrt{x_{0,y}}\right])^{\frac{3}{2}}} \\ 
\label{eqn_MxGInvxLowerBound_stmt_v3} 
     ...
     \exp\left(-2 (\log\log\log \sqrt{x_{0,y}})^2\right), 
\end{align} 
along this infinitely tending subsequence as $y \rightarrow \infty$. 
The scaled Mertens function is then 
unbounded in the limit supremum sense, as we have claimed, since the right-hand-side of the 
previous equation tends to positive infinity as $x_{0,y} \rightarrow \infty$. 
\end{proof} 
