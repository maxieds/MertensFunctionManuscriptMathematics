...
\subsection{Preface on notation: Unconventional notions of asymptotics} 
...
$g^{-1}(n)$ using artifacts from combinatorial analysis and elementary number theory. 
The summation methods we employ to weight sums of our arithmetic functions according to the sign of 
...
%this take on 
The identity in \eqref{eqn_Mx_gInvnPixk_formula} provides us with a powerful new method to 
...
\subsubsection{Fixing an exact expression for $M(x)$ by special arithmetic functions} 
...
As explicit lower bounds for $M(x)$ along subsequences are not obvious, and are 
historically ellusive and non-trivial to obtain as 
...
\subsubsection{Some enumerative (or counting) DGFs from Mongomery and Vaughan} 
...
We obtain lower bounds of the following form for $A_0 > 0$ an absolute constant, and 
$C_0(x)$ a function only of $x$ 
where we take $z \geq 0$ to be a real-valued parameter 
uniformly bounded in $x \geq 2$: 
\begin{align*} 
\mathcal{G}(z) \geq A_0 \cdot C_0(x)^{z}
\end{align*} 
It suffices to take 
\begin{align*}
A_0 & = \frac{4}{3 \cdot \log 2 \cdot 2^{27/4} \cdot \Gamma\left(\frac{5}{2}\right)} \approx 0.0134439 \\ 
C_0(x) & = \frac{4}{3 \log 2}. 
\end{align*} 
\end{theorem} 
...
\subsection{Cracking the classical unboundedness result, so to speak} 
...
over which the parity of both $\floor{\frac{1}{2}\log\log\log\log x}$ and 
$\floor{\frac{3}{2}\log\log\log\log x}$ are predictably signed. 
What we obtain is the following important summary corollary verifying the 
unboundedness of the scaled function $|M(x)| / \sqrt{x}$ in the limit supremum sense: 
...
\frac{|M(x_{y})|}{\sqrt{x_{y}}} & \succsim 
     ...
     (\log\log\log \sqrt{x_y})^{4 + \frac{3}{\log 2} - \frac{3}{\log 3}} + o(1),  
\end{align*} 
...
\[
C_{\ell,1} := \frac{1}{36 \cdot 2^{3/4} \sqrt{\pi} \cdot \log 2} \approx 0.000183209. 
\]
\end{cor} 
...
\subsection{Outline: Core components to the proof} 
...
\item[(1)] We prove an apparently yet undiscovered matrix inversion formula relating the summatory 
           ...
           Namely, a careful matrix and symmetry transformation based proof of 
           Theorem \ref{theorem_SummatoryFuncsOfDirCvls} is given in 
           Section \ref{Section_PrelimProofs_Config}.  
\item[(2)] This crucial step provides us with an exact formula for $M(x)$ in terms of $\pi(x)$, the seemingly 
...
\item[(3)] We tighten a result from \cite[\S 7]{MV} providing summatory functions that indicate the parity of 
           ...
           Our motivations are different than in the reference for exploiting the unique properties of this 
           construction. 
           ...
           functions $f$ using Abel summation when $x$ is large. 
\item[(4)] We then turn to the asymptotics if the quasi-periodic $g^{-1}(n)$, estimating this inverse function's 
...
\item[(6)] We remark that while this technique and approach to the classical problem at hand is certainly new, 
           ...
           $\operatorname{sgn}(f^{-1}(n)) = (-1)^{\omega(n)}$. This 
           variation in signedness tends to complicate, but still 
...
For example, 
we have notable identities of the forms \cite{CATALOG-INTDIRSERIES,CATALOG-LAMBERTSERIES} 
...
\cite[\S 2.14]{APOSTOLANUMT}. 
The classical formulation we have to work with is re-stated as follows for any 
Dirichlet invertible function $\alpha(1) \neq 0$ and any 
...
%$\chi_{\operatorname{PP}} = \Omega \ast \mu$ (\cf \cite[\S 17.3]{OPERADECRIBERO}). 
\end{prop}
\begin{proof} 
...
has average order of $\log\left[\frac{u}{\log u}\right]$ for some minimal $u \geq 2$ as $x \rightarrow \infty$. 
...
\gamma_1(z, x) & = -s\log\left(\frac{\log x}{\log 2}\right) + \frac{3}{4}s(s-1) \log(x/2) + 
...
generating function series we are after when we sum over all $p \leq u$ for some 
$u \geq 2$ corresponds to 
\begin{align*} 
\log\left[\prod_{p \leq u} \left(1-\frac{z}{p}\right)^{-1}\right] & \geq (B + \log\log u) z + 
     \sum_{j \geq 2} \left[a(u) + b(u)(j-1) + c(u) (j-1)^2\right] z^j \\ 
     & = (B + \log\log u) z - a(u) \left(1 + \frac{1}{z-1} + z\right) + b(u) \left( 
     1 + \frac{2}{z-1} + \frac{1}{(z-1)^2}\right) \\ 
     & \phantom{= (B + \log\log x) z\ } - 
     c(u) \left( 
     1 + \frac{4}{z-1} + \frac{5}{(z-1)^2} + \frac{2}{(z-1)^3}\right). 
\end{align*} 
In the previous equations, the lower bounds formed by the functions $(a,b,c)$ are 
given by the corresponding upper bounds from 
Corollary \ref{cor_PartialSumsOfReciprocalsOfPrimePowers} 
due to the leading sign on the following expansions: 
\begin{align*} 
(a_{\ell}, b_{\ell}, c_{\ell}) & := \left(-\log\left(\frac{\log u}{\log 2}\right), 
     \frac{3}{4} \log\left(\frac{u}{2}\right), \frac{11}{36} \log^2 2\right). 
\end{align*} 
Then we simplify to obtain that 
\begin{align} 
\label{eqn_proof_simpl_v1} 
\frac{e^{-Bz}}{(\log u)^{z}} \times \prod_{p \leq u} \left(1-\frac{z}{p}\right)^{-1} & 
     \geq e^{Bz} (\log u)^z 
     \left(\frac{\log u}{\log 2}\right)^{1 + \frac{1}{z-1} + z} 
     \left(\frac{u}{2}\right)^{\frac{3}{4}\left(1 + \frac{2}{z-1} + \frac{1}{(z-1)^2}\right)} \times \\ 
\notag 
     & \phantom{=} \times 
     \exp\left(-\frac{11}{36}\left(1 + \frac{4}{z-1} + \frac{5}{(z-1)^2} + 
     \frac{2}{(z-1)^3}\right) (\log 2)^2\right). 
\end{align} 
Now we need to determine which values of $u \geq 2$ minimize the expression in \eqref{eqn_proof_simpl_v1}. 
For this we will use introdutory calculus in the form of the second derivative test with respect to $u$. 
Let $f_z(u)$ denote the right-hand-side expression in the last equation. Then 
\[
f_z^{\prime}(u_0) = 0 \implies u_0 = e^{-\frac{4(z-1)}{3}}. 
\]
When we substitute this outstanding parameterized value of $u$ into the second derivative of the same function 
we obtain 
\begin{align*} 
f_z^{\prime\prime}(u_0) & = 
    2^{\frac{8 z^3-27 z^2+32 z-16}{4 (z-1)^2}} 3^{-z+\frac{1}{1-z}+1} 
    (1-z)^{z+\frac{1}{z-1}-2} z^2 (\log 2)^{\frac{z^2}{1-z}} \times \\ 
    & \phantom{=} \times 
    \exp\left(B+\frac{60
    z^4+z^3 \left(11 \log ^2(2)-312\right)+z^2 
    \left(540+11 \log ^2(2)\right)-384 z+96}{36 (z-1)^3}\right) > 0, 
\end{align*} 
which leads to a minimum value, or lower bound, at this $u_0$. 
...
Thus $(z-1)^{-m} \in [(-1)^m, 2^m]$ for integers $m \geq 1$, and we can then form the 
...
otherwise suggest. 
The key idea is that by having information about $g^{-1}(n)$ 
...
Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1}, we have that for all $n \geq 2$ 
\[
g^{-1}(n) \sim \lambda(n) \times \sum_{d|n} C_{\Omega(d)}(d). 
\]
...
%\]
Moreover, we can bound the error terms to ensure that 
\[
\left\lvert \frac{\lambda(n) \times \sum_{d|n} C_{\Omega(d)}(d)}{g^{-1}(n)} \right\rvert 
...
N_{m_3}(x) & \gg \pi_1(x) \times \sum_{k=3}^{\frac{3}{2} \log\log x} \widehat{\pi}_k(x) + 
     ...
     & \succsim \frac{4A_0 x^2}{3\sqrt{\pi} \log^{3/2}(2)} \cdot 
     \frac{(\log x)^{-\frac{1}{2}+\frac{3}{2\log 2} - \frac{3}{2\log 3}}}{\sqrt{\log\log x}} - 
     \frac{\sqrt{2} A_0 x^2}{\pi \log^{3/2}(2)} \cdot \frac{(\log x)^{1+3\log 2-3\log 3}}{\log\log x} \\ 
     & \succsim \frac{4A_0 x^2}{3\sqrt{\pi} \log^{3/2}(2)} \cdot 
     \frac{(\log x)^{-\frac{1}{2}+\frac{3}{2\log 2} - \frac{3}{2\log 3}}}{\sqrt{\log\log x}} \\ 
     & = \frac{x^2}{36 \cdot 2^{3/4} \sqrt{\pi} \log 2} \cdot 
     \frac{(\log x)^{-\frac{1}{2}+\frac{3}{2\log 2} - \frac{3}{2\log 3}}}{\sqrt{\log\log x}}. 
\end{align*} 
Now in practice, we are not summing up $n \leq x$, but rather $n \leq \log\log x$. So the 
above function evaluates to 
\[
N_{m_3}(\log\log x) \gg \frac{(\log\log x)^2}{36 \cdot 2^{3/4} \sqrt{\pi} \log 2} \cdot 
     \frac{(\log\log\log x)^{-\frac{1}{2}+\frac{3}{2\log 2} - \frac{3}{2\log 3}}}{\sqrt{\log\log\log\log x}}. 
\]
...
\[
\mathcal{S}_0(x) := 
     \frac{N_{m_3}(\log\log x)^2}{2} \succsim \frac{(\log\log x)^4}{2592 \cdot 2^{3/2} \cdot \pi \log^2 2} \cdot 
     \frac{(\log\log\log x)^{\frac{3}{\log 2} - 1 - \frac{3}{\log 3}}}{\log\log\log\log x}. 
\]
...
\end{align*} 
Thus, citing Corollary \ref{cor_Asymptotics_KeyCases_Of_Ckn_v1}, we easily see the conclusion of this 
result that we stated above. 
...
where the function 
\[
\widehat{L}_0(\log\log x) := (-1)^{\floor{\frac{3}{2} \log\log\log\log x}} \left\{ 
     \frac{2A_0}{\sqrt{\pi} \log^{\frac{3}{2}}(2)}\right\} \cdot (\log\log x) 
     (\log\log\log x)^{\frac{1}{2} + \frac{3}{2 \log 2} - \frac{3}{2 \log 3}} 
     \sqrt{\log\log\log\log x}, 
\]
with the exponent $\frac{1}{2} + \frac{3}{2 \log 2} - \frac{3}{2 \log 3} \approx 1.29868$. 
\end{cor} 
...
\notag 
     & \approx G^{-1}(x) - x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{t^2 \cdot \log(x/t)} dt. 
\end{align} 
...
This notation will come in handy to abbreviate the dominant asymptotic terms we find in 
Theorem \ref{theorem_gInv_GeneralAsymptoticsForms} proved below. 
...
\[
C_{\ell,1} = \frac{4A_0^2}{\sqrt{3} \pi^2 \log^4(2)} = 
     \frac{1}{36 \cdot 2^{3/4} \sqrt{\pi} \cdot \log 2} \approx 0.000183209.
\]
That is, we have 
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     \succsim 
     \Biggl\lvert 
     C_{\ell,1} \cdot (\log x) (\log\log x) (\log\log\log x)^{4 + \frac{3}{\log 2} - \frac{3}{\log 3}} 
     \Biggr\rvert. 
\end{align*} 
The exponent in the previous equation is numerically approximated as 
$4 + \frac{3}{\log 2} - \frac{3}{\log 3} \approx 5.59737$. 
\end{theorem} 
...
\[
\widehat{\pi}_k(x) \succsim \frac{A_0 x}{\log x} 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \left(\frac{4}{3 \log 2}\right)^{\frac{k-1}{\log\log x}}. 
\]
...
A_{\Omega}^{(\ell)}(t) & = \sum_{k \leq \frac{3}{2}\log\log t} (-1)^k \widehat{\pi}_k(x) \\ 
\notag
     & \sim (-1)^{\Floor{3\log\log t}{2}} \cdot 
     \frac{2A_0t}{\sqrt{\pi} \log^{3/2}(2)} (\log t)^{\frac{1}{2} + \frac{3}{2\log 2} - \frac{3}{2\log 3}} 
     \sqrt{\log\log\log t}. 
\end{align} 
From Corollary \ref{cor_Asymptotics_KeyCases_Of_Ckn_v1} we recover from the approximation to the 
...
\int_{u_0-1}^{\log\log x} \widehat{\tau}_0^{\prime}(t) A_{\Omega}^{(\ell)}(t) dt & \sim 
     \sum_{k=u_0+1}^{\frac{3}{4}\log\log\log\log x} \left( 
     I_{\ell}\left(e^{e^{\frac{4k+2}{3}}}\right) - 
     I_{\ell}\left(e^{e^{\frac{4k}{3}}}\right) 
     \right) \frac{e^{e^{\frac{4k}{3}}}}{\Gamma\left(\frac{4k}{3}\right)} \\ 
\notag 
     & \approx 
     C_0(u_0) + 
     (-1)^{\Floor{\log\log\log\log x}{2}} \times 
     \int_{\frac{3\log\log\log\log x}{4}-\frac{1}{2}}^{\frac{3\log\log\log\log x}{4}} 
     \frac{I_{\ell}\left(e^{e^{\frac{4k}{3}}}\right)}{\Gamma\left(\frac{4k}{3}\right)} 
     e^{e^{\frac{4k}{3}}} dk, 
\end{align} 
where we define the integrand function, 
$I_{\ell}(t) := \widehat{\tau}_0^{\prime}(t) A_{\Omega}^{(\ell)}(t)$, 
with some limiting simplifications for 
$k \in \left[\frac{3\log\log\log\log x}{4}-\frac{1}{2}, \frac{3\log\log\log\log x}{4}\right]$ as 
\footnote{ 
...
\begin{align} 
\label{eqn_proof_thm_GInvFunc_v3_approx} 
I_{\ell}\left(e^{e^{\frac{4k}{3}}}\right) \frac{e^{e^{\frac{4k}{3}}}}{ 
     \Gamma\left(\frac{4k}{3}\right)} & \succsim 
     \sqrt{\frac{2}{3}} \cdot \frac{8A_0}{3\pi \log^{5/2}(2)} \cdot 
     \frac{(\log\log x)^{\frac{4k}{3}}}{\Gamma\left(\frac{4k}{3}+1\right)} 
     (\log\log\log x)^2. 
\end{align} 
So using the lower bound on the integrand in \eqref{eqn_proof_thm_GInvFunc_v3_approx}, 
and applying Stirling's formula, we find that 
\begin{align} 
\label{eqn_proof_thm_GInvFunc_v4_approx} 
\int_{\frac{3\log\log\log\log x}{4}-\frac{1}{2}}^{\frac{3\log\log\log\log x}{4}} & 
     I_{\ell}\left(e^{e^{\frac{4k}{3}}}\right) 
     \frac{e^{e^{\frac{4k}{3}}}}{\Gamma\left(\frac{4k}{3} + 1\right)} dk \\ 
\notag 
     & \succsim 
     \frac{2A_0}{\sqrt{3} \pi^{3/2} \log^{5/2}(2)} \cdot 
     \left( 
     \frac{\log\log x}{\log\log\log\log x} 
     \right)^{\log\log\log\log x} 
     \frac{(\log\log\log x)^{\frac{7}{2} + \frac{3}{2\log 2} - \frac{3}{2\log 3}}}{ 
     \sqrt{\log\log\log\log x}}. 
\end{align} 
...
A_{\Omega}^{(\ell)}(\log\log x) & \succsim 
     (-1)^{\Floor{3\log\log\log\log x}{2}} \cdot 
     \frac{2A_0 (\log\log x)}{\sqrt{\pi} \log^{3/2}(2)} 
     (\log\log\log x)^{\frac{1}{2} + \frac{3}{2\log 2} - \frac{3}{2\log 3}} 
     \sqrt{\log\log\log\log\log x}. 
\end{align*} 
...
$\widehat{L}_0(\log\log x)$ given by 
\[
\widehat{L}_0(\log\log x) := (-1)^{\floor{\frac{3}{2} \log\log\log\log x}} \left\{ 
     \frac{2A_0}{\sqrt{\pi} \log^{\frac{3}{2}}(2)}\right\} (\log\log x) 
     (\log\log\log x)^{\frac{1}{2} + \frac{3}{2 \log 2} - \frac{3}{2 \log 3}} 
     \sqrt{\log\log\log\log x}, 
\]
as proved in Corollary \ref{cor_ASemiForm_ForGInvx_v1}. 
Moreover, it is clear that the asymptotically dominant behavior of the lower bound for 
$|G_{\ell}^{-1}(x)|$ comes from the integral term calculated in 
\eqref{eqn_proof_thm_GInvFunc_v4_approx}. We simplify the term involved in that bound by 
writing 
\[
\left( 
     \frac{\log\log x}{\log\log\log\log x} 
...
-\int_{\sqrt{x}}^{x/2} \frac{2 x}{t^2 \log(x)} \cdot G_{\ell}^{-1}(t) dt & \succsim 
     \frac{2}{\log x} \cdot \left(\max_{\sqrt{x} \leq t \leq x/2} G_{\ell}^{-1}(t)\right) \\ 
     & = 2C_{\ell,1} \cdot (\log\log \sqrt{x}) 
     (\log\log\log \sqrt{x})^{4 + \frac{3}{\log 2} - \frac{3}{\log 3}} + o(1). 
\end{align*} 
