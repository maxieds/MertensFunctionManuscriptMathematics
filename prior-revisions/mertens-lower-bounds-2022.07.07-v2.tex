...
$G_m(n) := (-1)^{m} (C_m(-) \ast g)(n)$ is expanded as 
\[
G_m(n) = - 
     \begin{cases} 
     (\omega \ast g)(n), & m = 1; \\ 
...
For $x \geq 19$, consider the following partial sums:
\begin{align*}
L_{M,\Omega}(x) & := 
...
L_{M,\Omega}(x) & = \frac{x}{\log x} \times \Biggl(
	...
	\left[\mathcal{G}\left(\frac{t-1}{\log\log x}\right) g(x, t) 
     \right]_{t=1}^{t=\log\log x} + \widehat{R}_{m}[\mathcal{G} \cdot g]\right) 
...
\left\lvert \widehat{R}_m[\mathcal{G} \cdot g] \right\rvert & = 
     O\left(\frac{1}{m!} \times \int_1^{\log\log x} B_m(\{t\}) f^{(m)}(t) dt\right). 
\end{align*}
...
$$E_m(x) := \sum\limits_{k=1}^{m} \frac{B_k}{k!} \times \frac{\partial^{(k-1)}}{\partial t^{(k-1)}} 
	\left[\mathcal{G}\left(\frac{t-1}{\log\log x}\right) g(x, t) 
     ...
     \int_1^{\log\log x} \left\lvert B_m(\{t\}) \right\rvert f_x^{(m)}(t) dt$$ 
\begin{subequations}
...
\notag
	...
	\left[\left(1 + \mathcal{G}\left(\frac{t-1}{\log\log x}\right)\right) g(x, t) 
     \right]_{t=1}^{t=\log\log x} + \widehat{R}_m[(1 + \mathcal{G}) \cdot g]\right) 
	\Biggr) \times \left(1 + O\left(\frac{1}{\log\log x}\right)\right). 
\end{align}
...
L_{E,\Omega}(x) & \ll \sum_{\substack{n \leq x \\ \Omega(n) \geq \log\log x}} 
	...
	e^{-\frac{\left(t-\log\log x\right)^2}{2\log\log x}} dt\right)^{\frac{1}{q}} \times 
