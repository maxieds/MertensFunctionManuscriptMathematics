...
\[
\mathbb{E}|g^{-1}(n)| = \frac{3\sqrt{2}}{\pi^{3/2}} (\log n)^2 \sqrt{\log\log n}(1+o(1)),  
...
in Appendix \ref{subSection_OtherFactsAndResults} for the error term bounds) 
\[
...
With the next lemma, we can accurately approximate asymptotic order of 
the sums $\mathcal{A}_{\omega}(x)$ (defined below) 
for large $x$ by only considering the truncated sums 
$\mathcal{D}_{\omega}(x)$ (defined below) 
...
\mathbb{E}\left[C_{\Omega(n)}(n)\right] = 
     \sqrt{2\pi \log\log n} (\log n) (1+o(1)). 
\] 
\end{prop} 
\begin{proof} 
...
\widehat{B}(x, r) \ll x (\log x)^{r-2r\log r} \sqrt{\log\log x} = o\left(x(\log x)\sqrt{\log\log x}\right)
\end{equation}
...
\int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = 
     \frac{\sqrt{2\pi}}{2} (\log t)^2 \sqrt{\log\log t} (1+o(1)). 
\end{align*} 
...
$$\frac{f(k, x) \sqrt{\log\log x}}{(\log x)} 
  ...
  \sigma_x(C)^2}\right),$$ 
we perform the change of variable $s \mapsto \frac{t \sqrt{\log\log x}}{(\log x)}$ to obtain the normalized 
...
$$z = \frac{6}{\pi^2} \cdot \frac{(\log(4\sqrt{2\pi}) + \alpha)}{\sigma_x(C)} - \sigma_x(C),$$ 
in Corollary \ref{cor_CLT_VII} 
for $\alpha = 0$ and with $\sigma_x(C) := \log\log x$. Note that as $\alpha \rightarrow +\infty$, 
...
\]
and where $z$ is defined on the universal covering of $\mathbb{C} \mathbin{\big\backslash} \{0\}$ 
by analytic continuation. For $a \in \mathbb{Z}^{+}$, $\Gamma(a, z)$ is an entire function of $z$. 
The following properties of $\Gamma(a, z)$ hold: 
\begin{align} 
...
\]
The main and error terms in the previous equation can also be seen by applying the asymptotic series in 
...
Note that we cannot write this expansion as $\Gamma(a, -z)$ directly unless $a \in \mathbb{Z}^{+}$ 
as the incomplete gamma function 
has a branch point at the origin with respect to its second variable. This function becomes a single-valued 
...
S_3(x) := \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-1/2}}{(2k-1) (k-1)!} = 
     \frac{(\log x)}{4\sqrt{\log\log x}} + O\left(\frac{\log x}{\log\log x}\right).       
\end{align*}
\end{lemma}
\begin{proof}
...
show that we can arrive at this bound in the form of the following inequalities by 
applying the asymptotic series formula in the last equation: 
\begin{equation}
\label{eqn_ProofTag_S3x_v1}
\frac{\log x}{2\sqrt{\log\log x}} + O\left(\frac{\log x}{(\log\log x)^2}\right) \ll S_3(x) \ll
...
near the transition point $\lambda = z/a = 1$ as $a,z \rightarrow +\infty$. 
...
\widetilde{S}_n(t) & = \int_0^1 \left(\sum_{k=1}^{n} \frac{(s^2t)^{k-1}}{(k-1)!}\right) ds \\ 
     ...
     & = \frac{1}{2t(n-1)!} \times \int_0^1 e^{y} \Gamma(n, y) dy. 
\end{align*}
Integration by parts performed one time with 
\[
     \left\{\begin{array}{ll} 
     u_x = \Gamma(n, y) & v_x^{\prime} = e^{y} dy \\ 
     u_x^{\prime} = -y^{n-1} e^{-y} dy & v_x = e^{y}
     \end{array} 
     \right\}, 
\]
implies that 
...
\notag
\widetilde{S}_n(t) & = 
     \frac{1}{2t(n-1)!} \times \left[\Gamma(n, t)e^t - \Gamma(n) + 
     \int_0^t y^{n-1} dy\right] \\ 
\label{eqn_ProofTag_AppendixLastSumLemma_IBP_resulting_exp_terms_v1}
     & = \frac{\Gamma(n, t)e^t}{2t(n-1)!} - \frac{1}{2t} + \frac{t^{n-1}}{2n!}. 
\end{align}
...
\[
\widetilde{S}_n(t) = \frac{e^t}{4t} + O\left(\frac{e^t}{nt^{3/2}}\right). 
\]
The conclusion follows taking $n = \floor{\log\log x}$, $t = \log\log x$ and 
