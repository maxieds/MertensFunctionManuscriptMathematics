...
prime zeta function. 
We prove precise formulas for the average order of both 
$\log C_{\Omega}(n)$ and $\log |g(n)|$ and conjecture a central limit theorem 
that governs the distribution of their values over $n \leq x$ as $x \rightarrow \infty$. 
...
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega}(n) & = 
     ...
     \frac{k (\log\log x)^{2k-\frac{3}{2}}}{2^k (2k-3)!!} \left( 
...
The following definition and theorem are proved in \cite[\cf \S 7.4]{MV}.
...
\]
is uniformly bounded for any $|z| \leq R \leq \frac{3}{2}$. 
...
\widehat{C}_{k,\omega}(x) = -\widehat{G}\left(\frac{2k-1}{\log\log x}\right) \frac{x}{\log x} \cdot 
     \frac{k (\log\log x)^{2k-2}}{2^k (2k-3)!!} \left( 
...
\[
\widehat{C}_{k,\omega}(x) = \frac{1}{2\pi\imath} \times \int_{|v|=r} 
...
\left\lvert x (\log x)^{-(\Re(v^2)+2)} v^{-(2k+1)} \right\rvert & \ll 
     ...
     & \ll \frac{x}{\log x} \cdot \frac{k (\log\log x)^{2k-6}}{(2k-2)!}, 
...
\widehat{C}_{k,\omega}(x) \sim  
     ...
     \times \int_{|v|=r} \frac{\widehat{F}(1, -v^2) (\log x)^{-v^2} \zeta(2)^{v^2}}{\Gamma(1-v^2) v^{2k-1}} dv. 
\end{align} 
...
\]
Observe that for any $z \geq 0$, we cite the following know asymptotic formula for the 
log-gamma function: 
\[
...
L_{\Omega}(x) & = \sum_{1 \leq k \leq \frac{3}{2}\log\log x} 
	\frac{B_0 x (\log\log x)^{k-1}}{(\log x) (k-1)!} \left(
...
A_x(u) := \frac{B_0x \Gamma\left(\frac{3u}{2}, \log\log x\right)}{\Gamma\left(\frac{3u}{2}\right)}; 
     f(u) := \frac{(3u+1)}{2} \log\left(1 + \frac{3u}{2}\right) - \frac{(3u+1)}{2}, 
     f^{\prime}(u) = \log\left(1 + \frac{3u}{2}\right) - \frac{1}{2\left(1 + \frac{3u}{2}\right)}. 
\]
Then we have by 
...
L_{\Omega}(x) & \sim A_x(\log\log x) f(\log\log x) - 
	...
	& \sim 
	\frac{3B_0x}{2}(\log\log x)(\log\log\log x) + B_0x \times \int_{\frac{2}{3}}^1 
	f^{\prime}(\alpha \log\log x) d\alpha + O\left(
	\int_0^1 \frac{(\log x)^{\frac{3\alpha}{2}-1-\frac{3\alpha}{2}\log\left(\frac{3\alpha}{2}\right)}}{ 
	(\log\log x)^{\frac{3}{2}} \left(1-\frac{3\alpha}{2}\right)} d\alpha\right) \\ 
	& \sim 
	B_0x (\log\log x)(\log\log\log x). 
\end{align*}
We will use Rankin's method to derive upper bounds on the sums 
\[
L_{\Omega}^{\ast}(x, r) := \sum_{\substack{n \leq x \\ \Omega(n) \geq r \log\log x}} 
     \log C_{\Omega}(n), 1 \leq r < 2. 
\]
Notice that it suffices to show 
\begin{equation}
\label{eqn_proof_tag_PartialSumsOver_HatCkx_EquivCond_v2}
L_{\Omega}^{\ast}\left(x, \frac{3}{2}\right) = o\left(x \log x\right), 
	\text{ as } x \rightarrow \infty. 
\end{equation}
...
$\frac{1}{\Gamma(1+r^2)} \gg 1$ for $r \in [1, 2)$, by the concavity of the 
...
As $n \rightarrow \infty$ there is an absolute constant $D_0 \geq 1$ such that 
\begin{align*} 
...
the conjecture holds. Note that for any $z \in (-\infty, \infty)$, the function 
