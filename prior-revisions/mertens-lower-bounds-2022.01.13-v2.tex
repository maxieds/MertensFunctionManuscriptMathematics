...
\title{
       \LARGE{
       Characterizations of partial sums of the M\"obius function by 
       signed sums of additively structured auxiliary 
       unsigned sequences 
       } 
}
\author{{\Large Maxie Dion Schmidt} \\ 
        {\normalsize \href{mailto:maxieds@gmail.com}{maxieds@gmail.com}} \\[0.1cm] 
        {\normalsize \href{mailto:mschmidt34@gatech.edu}{mschmidt34@gatech.edu}} \\[0.1cm] 
...
\newglossaryentry{GGTildeFHatGHatBivariateFunctions}{
    ...
    description={The functions $\mathcal{G}(z)$ and $\widetilde{\mathcal{G}}(z)$ are defined for 
                 ...
                 Section \hlocalref{subSection_TheKnownDistsOfThePrimeOmegaFunctions_IntroResults_v1}. 
...
\maketitle
\newcommand{\runtitle}{New characterizations of partial sums of the M\"obius function}
...
That is, the distinct values of the unsigned inverse function are repeated at any $n \geq 2$ 
with the precise additive configuration of the exponents in the prime factorization of $n$ regardless of 
the multiplicative products of primes that serve as the placeholders for the 
exponents of the distinct prime factors. 
We conjecture two forms of deterministic Erd\H{o}s-Kac theorem analogs that 
characterize the distributions of each of the unsigned sequences 
$$C_{\Omega}(n) := (\Omega(n))! \times \prod_{p^{\alpha}||n} \frac{1}{\alpha!}, n \geq 2,$$ and 
$|g^{-1}(n)|$ over $n \leq x$ as $x \rightarrow \infty$. 
Discrete convolutions of the partial sums 
...
summatory functions with explicit (non-centrally) normal 
probability distributions at large $x$. 
...
\subsection{Definitions and preliminary results on the Mertens function}
\label{subSection_Intro_Mx_properties} 

The \emph{Mertens function} is the summatory function of $\mu(n)$ defined by the partial sums 
...
\[
L(x) = \sum_{d \leq \sqrt{x}} M\left(\Floor{x}{d^2}\right), \text{ for } x \geq 1. 
\] 
...
\begin{figure}[ht]
     \centering
     \fbox{\includegraphics{images/MxAndLxDiscretePlots-Figure1.png}}
     \captionsetup{singlelinecheck=off} 
     \caption{A discrete plot comparing of the values of the classical summatory functions 
              $M(x)$ and $L(x)$ at integers $1 \leq x \leq 5000$. } 
     \label{figure_MxLx_discrete_plot_comparison_v1} 
\end{figure}
...
\subsection{New characterizations of $M(x)$ by partial sums of auxillary unsigned functions} 
...
summatory functions, namely, the functions $C_{\Omega}(n)$ and $|g^{-1}(n)|$, and their partial sums. 
We fix the notation for the Dirichlet invertible function $g(n) := \omega(n) + 1$ and define its 
inverse with respect to Dirichlet convolution by 
\begin{equation}
\label{eqn_gInvn_def_v1}
g^{-1}(n) := (\omega + \mathds{1})^{-1}(n), \text{ for } n \geq 1. 
\end{equation}
The Dirichlet inverse function defined in 
\eqref{eqn_gInvn_def_v1} 
exists and is unique because $g(1) = 1 \neq 0$. 
We compute the first several values of this signed inverse sequence as 
follows \cite[\seqnum{A341444}]{OEIS}: 
\[
...
The shift by the constant one in the form of $(\omega + \mathds{1}) \ast \mu$ 
in the right-hand-side convolution in \eqref{eqn_AntiqueDivisorSumIdent} is 
selected so that the resulting arithmetic function we convolve with $\mu(n)$ in constructing these 
summatory functions is Dirichlet invertible with $(\omega + \mathds{1})(1) \neq 0$ where 
$\omega(1) := 0$ (by convention). 
Based on the convolution identity given in \eqref{eqn_AntiqueDivisorSumIdent} above, 
we can prove the next formulas in \eqref{eqn_Mx_gInvnPixk_formula} 
as special cases of 
Theorem \hlocalref{theorem_SummatoryFuncsOfDirCvls} stated in 
Section \hlocalref{subSection_PrelimProofs_Config_InversionTheorem} (see 
Corollary \hlocalref{cor_CvlGAstMu}). 

We define the partial sums $G^{-1}(x)$ for integers $x \geq 1$ as follows \cite[\seqnum{A341472}]{OEIS}: 
\begin{equation}
G^{-1}(x) := \sum_{n \leq x} g^{-1}(n) = \sum_{n \leq x} \lambda(n) |g^{-1}(n)|. 
\end{equation} 
Then we have that for all $x \geq 1$ 
(see Proposition \hlocalref{prop_Mx_SBP_IntegralFormula}; 
\cf Section \hlocalref{subSection_Relating_CknFuncs_to_gInvn})
\begin{subequations}
\label{eqn_Mx_gInvnPixk_formula} 
\begin{align} 
\label{eqn_Mx_gInvnPixk_formula_v1}
M(x) & = \sum_{1 \leq k \leq x} g^{-1}(k) \left(\pi\left(\Floor{x}{k}\right) + 1\right), \\ 
\label{eqn_Mx_gInvnPixk_formula_v2} 
M(x) & = G^{-1}(x) + 
     \sum_{1 \leq k \leq \frac{x}{2}} G^{-1}(k) \left( 
     \pi\left(\Floor{x}{k}\right) - \pi\left(\Floor{x}{k+1}\right) 
     \right), \\ 
\label{eqn_Mx_gInvnPixk_formula_v3}
M(x) & = G^{-1}(x) + \sum_{p \leq x} G^{-1}\left(\Floor{x}{p}\right). 
\end{align} 
\end{subequations}
We have an identification of $G^{-1}(x)$ with the summatory function $L(x)$ given by 
\begin{align*}
G^{-1}(x) & = 
     L(x)|g^{-1}(x)| - \sum_{n < x} 
...
(see Section \hlocalref{subSection_LocalCancellationOfGInvx}). 
Suppose that the unsigned partial sums are defined for $x \geq 1$ by 
\begin{equation}
\label{eqn_SummatoryFuncOfUnsignedInverseSeq_v1} 
|G^{-1}|(x) := \sum_{n \leq x} |g^{-1}(n)|. 
\end{equation} 
Bounds on the partial sums over the unsigned inverse functions in 
\eqref{eqn_SummatoryFuncOfUnsignedInverseSeq_v1} 
provide some local information on $G^{-1}(x)$ through its connection with $|G^{-1}|(x)$. 
The plots shown in 
...
the values of $M(x)$ and $G^{-1}(x)$ with scaled forms of auxilliary partial sums 
...
\fbox{\includegraphics[width=\textwidth]{images/Figure2-MxUnsAbsPartialSumsEnvelopes-GInvFunctionSequenceCalculations.png}}
...
\caption{The displayed jagged plots show that the difference between $M(x)$ and $G^{-1}(x)$ is usually small 
         (\cf \eqref{eqn_Mx_gInvnPixk_formula_v3}). 
         The third more smoothly oscillatory plot shown in purple provides a baseline of the scaled 
         values of $G^{-1}(x)$ where the reciprocal of the absolute constant 
         $C_2^{-1} := 1-\frac{1}{\zeta(2)}$ is arbitrarily chosen so 
         as to keep a display scale that is commensurate with the first two plots. }
\end{subfigure}
...
\caption{Discrete plots displaying comparisons of the growth of $M(x)$ to the new auxillary partial sums for $x \leq 5000$. 
         ...
         $x \geq 1$. 
         Numerical computation suggests that this function is a natural scaling factor to relate the growth of the 
         auxiliary partial sums to $M(x)$. 
        }
...
\caption{Comparisons of a logarithmically scaled form of $G^{-1}(x)$ and envelopes that bound its local 
         extremum given by sign-weighted components that contribute to these partial sums. 
         Namely, we define $G^{-1}(x) := G^{-1}_{+}(x) + G^{-1}_{-}(x)$ where the functions 
         ...
         $|g^{-1}(n)|$ over $n \leq x$ such that $\lambda(n) = \pm 1$, respectively.}
\end{subfigure}
...
\caption{Comparisions of bounded envelopes for the local extremum of the logarithmically scaled values of 
         $G^{-1}(x)$ to the absolute values of the partial sums of the scaled unsigned inverse function multiplied by 
         $\zeta(2)^{-1}$ to show the relative bounds in the displayed plots. }
\end{subfigure}
...
Nonetheless, the next observation is suggestive of the quasi-periodicity of the distribution of 
...
as multisets (i.e., taking into account the multiplicity of distinct elements in each set) 
of the prime exponents, 
then $g^{-1}(n_1) = g^{-1}(n_2)$. For example, $g^{-1}(n)$ has the same values on the squarefree integers 
$n$ with exactly one, two, three (and so on) prime factors, or for example at all $n$ of the form 
$n = p_1p_2^2p_3^4$ when $p_1$, $p_2$ and $p_3$ are distinct primes. 
Hence, we find a key takeaway that 
there is an \emph{essentially additive} structure underneath the sequence 
$\{\lambda(n) g^{-1}(n)\}_{n \geq 2}$. 
\end{observation} 

The results in Proposition \ref{lemma_gInv_MxExample} below 
proves the sign of $g^{-1}(n)$ for all $n \geq 1$ 
(see Proposition \hlocalref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1}) 
and provides bounds on the extremum of the unsigned inverse function with 
respect to the local values of maximal $k := \Omega(n)$ for $n \leq x$. 
A proof of (B) follows from 
Lemma \hlocalref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} 
by a simple counting argument when all factors $d|n$ are formed by squarefree 
products of the $\omega(n)$ distinct primes $p|n$. 
The realization that the beautiful and remarkably simple combinatorial form of property (B) 
in Proposition \hlocalref{lemma_gInv_MxExample} holds for all squarefree integers later 
motivates our pursuit of simpler formulas for the inverse function $g^{-1}(n)$ 
through the sums of the unsigned auxiliary sequence 
$C_{\Omega}(n)$, that is defined in Section \hlocalref{Section_InvFunc_PreciseExpsAndAsymptotics}. 
...
\item[(C)] If $n \geq 2$ and $\Omega(n) = k$ for some $k \geq 1$, then 
     ...
     %= e \cdot k! \times \left(1 + O\left(\frac{1}{(k+1)!}\right)\right). 
     \]
\end{itemize} 
\end{prop} 

The function $C_{\Omega}(n)$ identified as a key auxiliary sequence 
...
In Corollary \hlocalref{cor_SummatoryFuncsOfUnsignedSeqs_v2}, 
...
(\cf Corollary \hlocalref{cor_SummatoryFuncsOfUnsignedSeqs_v2}; 
Proposition \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1}; and 
Conjecture \hlocalref{conj_DetFormOfEKTypeThmForCOmegan_v1}). 

In Proposition \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1}, 
...
\]
In Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2}, we prove that 
...
\frac{1}{n} \times \sum_{k \leq n} |g^{-1}(k)| = 
     ...
     \mathrm{\ as\ } n \rightarrow \infty. 
\]
The next statements provide a complete picture of the distribution of the unsigned inverse sequence whose 
values are exactly identified by the formulas in 
Proposition \ref{lemma_gInv_MxExample} along squarefree $n \geq 1$. 
Indeed, for $n \geq 1$ off of the squarefree integers, we can characterize the distrubution of the values of $|g^{-1}(n)|$ by 
the conjectured generalizations of Erd\H{o}s-Kac theorem type results that lead to 
Corollary \hlocalref{cor_CLT_VII} (assuming 
Conjecture \ref{conj_DetFormOfEKTypeThmForCOmegan_v1} holds). 
That is, in Section \hlocalref{subSection_ErdosKacTheorem_Analogs}, 
...
theorem stated in Conjecture \hlocalref{conj_DetFormOfEKTypeThmForCOmegan_v1} 
...
(see Corollary \hlocalref{cor_CLT_VII}): 
\begin{align*}
...
\end{align*}
We similarly conjecture that for any real $y$, as $x \rightarrow \infty$ 
\begin{align} 
...
\subsection{Significance of the new results and characterizations} 
...
e.g., in relation to $\omega(n)$ and (signed powers of) $\Omega(n)$, respectively. 
...
in Section \hlocalref{Section_NewFormulasForgInvn_v1} and 
Section \hlocalref{Section_NewFormulasForgInvn_v2}.
In particular, we reformulate the proofs of the results in \cite[\S 7.4; \S 2.4]{MV} 
which apply analytic methods to formulate limiting asymptotics and to 
...
$C_{\Omega}(n)$ and $|g^{-1}(n)|$ (as corollaries of the former results). 
...
via the properties of its $\lambda(n)$-sign-weighted summands, $|g^{-1}(n)|$ for $n \leq x$.
By Proposition \hlocalref{lemma_gInv_MxExample} and since 
\[
\lim_{x \rightarrow \infty} \frac{(\log\log x)^{\log\log x}}{\sqrt{x}} = 0, 
\]
we see that if the result in \eqref{eqn_EKacVariant_forUnsgInvn_Intro_v2} holds, the 
unsigned inverse function $|g^{-1}(n)|$ is centered at its average order for nearly 
...
(provided that Conjecture \hlocalref{conj_DetFormOfEKTypeThmForCOmegan_v1} holds). 
Hence, the conjectured corollary implies a decidely more regular tendency of the unsigned inverse sequence. 
In fact, as we see through the lense of the previous remarks, the values of $|g^{-1}(n)|$ for $n \leq x$ at large $x$ 
are normally centered at the average order of this unsigned sequence in a way that is only 
paralleled in the references on the canonically strongly additive number theoretic 
...
is hence suggestive of new approaches to bounding the Mertens function. 
...
\begin{align*} 
S_{r \ast h}(x) & \phantom{:}= \sum_{d \leq x} r(d) H\left(\Floor{x}{d}\right) \\ 
...
We define $S_{r \ast h}(x)$ to be the summatory function of the 
...
\]
\end{cor} 
The formula that provides $\chi_{\mathbb{P}} + \varepsilon = (\omega + 1) \ast \mu$ 
exactly when evaluated at any $n \geq 1$ from 
\eqref{eqn_AntiqueDivisorSumIdent} implies 
the key form of 
\eqref{eqn_Mx_gInvnPixk_formula_v1} and \eqref{eqn_Mx_gInvnPixk_formula_v2}
as an immediate consequence of the stated adaptation of the inversion theorem for $M(x)$. 
...
For any arithmetic function $r(n)$, let the function 
$$\operatorname{sgn}(r(n)) := \begin{cases} 
  \frac{r(n)}{|r(n)|}, & \text{ if $r(n) \neq 0$; } \\ 
  0, & \text{ otherwise, } 
  \end{cases}$$
mapping onto the set $\{-1, 0, 1\}$  
provide the signedness of $r$ at any $n \geq 1$. 
...
denotes the \emph{prime zeta function} for any $\Re(s) > 1$ where the series is convergent 
(\cf \cite{FROBERG-1968}). 
...
By a combinatorial argument related to multinomial coefficient expansions of these DGFs that 
are summed over $h^{-1}(n)$ for $n \geq 1$, 
we recover exactly that \cite[\cf \S 2]{FROBERG-1968}\footnote{
     Beginning in Section \hlocalref{Section_InvFunc_PreciseExpsAndAsymptotics}, we adopt the alternate 
...
$h^{-1}$ in powers of $P(s)$ (where $|P(s)| < 1$ whenever $\Re(s) > 1$) 
we can count that 
\begin{align*}
...
(see Section \hlocalref{Section_InvFunc_PreciseExpsAndAsymptotics}). 

\section{Auxiliary sequences related to the inverse function $g^{-1}(n)$} 
\label{Section_InvFunc_PreciseExpsAndAsymptotics} 

The computational data given as Table \hlocalref{table_conjecture_Mertens_ginvSeq_approx_values} 
...
\subsection{Definitions and properties of bivariate semi-triangular component function sequences} 
...
\subsection{Formulas relating $C_{\Omega}(n)$ to $g^{-1}(n)$} 
\label{subSection_Relating_CknFuncs_to_gInvn} 

\begin{remark}[Motivation for the next elementary results] 
...
(\cf Appendix \hlocalref{subSection_AConnectionToDistOfThePrimes}). 
\end{remark}
...
the DGF of the unsigned inverse function, $|g^{-1}(n)|$, 
...
Proposition \hlocalref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1}, we can see that 
...
\end{equation}
The proof of Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2} (below) 
shows that 
\[
...
\section{The distribution of $C_{\Omega}(n)$ and its partial sums} 
...
illustrated in Table \hlocalref{table_conjecture_Mertens_ginvSeq_approx_values}. 
...
In the remainder of this section and in the corollaries for $|g^{-1}(n)|$ in 
Section \hlocalref{Section_NewFormulasForgInvn_v2}, 
we use analytic methods, primarily in the spirit of 
\cite[\S 7.4]{MV}, to prove and conjecture new 
properties that characterize the distributions of the unsigned auxiliary functions. 
...
$\Omega(n) = k$ when $1 \leq k \leq \frac{3}{2}\log\log x$ and $x$ is large in 
Theorem \hlocalref{theorem_CnkSpCasesScaledSummatoryFuncs}. 
...
(see Lemma \hlocalref{cor_AsymptoticsForSignedSumsOfomegan_v1} and the conclusion in 
Corollary \hlocalref{cor_SummatoryFuncsOfUnsignedSeqs_v2}). 
...
\]
Let the partial sums, $\widehat{A}_z(x)$, be defined by 
\[
\widehat{A}_z(x) := \sum_{n \leq x} (-1)^{\omega(n)} 
     C_{\Omega}(n) z^{\Omega(n)}, \text{ for } x \geq 1. 
\]
...
$\widehat{A}_z(x) := \sum\limits_{n \leq x} a_z(n)$. 
Then we have that 
\begin{align} 
\notag 
...
For $x \geq 1$, let the summatory function 
\[
...
\end{align*} 
The justification for the above error term including $S_{\frac{3}{2}}(x)$ is that for 
$1 \leq k < \frac{3}{2}\log\log x$, we can show that 
$\widetilde{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) \asymp 1$ where the function 
$\widetilde{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right)$ is monotone for $k$ respectively within 
...
Remark \hlocalref{remark_MV_Pikx_FuncResultsAnnotated_v1} is decreasing in $k$ for $1 \leq k \leq \log\log x$ 
...
Remark \hlocalref{remark_MV_Pikx_FuncResultsAnnotated_v1} (in the appendix section). We then see 
...
\#\left\{n \leq x: \omega(x) \geq \frac{3}{2}\log\log x\right\} \ll 
     ...
     \qedhere 
\] 
Thus, we have obtained a correct main and error term on the partial sums $L_{\omega}(x)$. 
\end{proof}
...
\]
The argument used to justify the last equation is that clearly we find 
\[
\widehat{C}_1^{\prime}(x) \gg \widehat{\pi}_1(x). 
\]
Moreover, for any integers $k \geq 2$ we see that 
\begin{align*}
\left\lvert \widehat{C}_k^{\prime}(x) \right\rvert & \gg 
     \sum_{n \leq x} \frac{\left\lvert C_{\Omega}(n) - C_{\Omega}(n-1) \right\rvert}{n} \times 
     \Iverson{\Omega(n) = k} \\ 
     & \gg 
     \sum_{2 \leq n \leq x} \frac{C_{\Omega}(n)}{n(n-1)} \times \Iverson{\Omega(n) = k} \\ 
     & \gg 
...
\widehat{C}_k(x) = A_0\sqrt{2\pi}(\log x) \times \int_3^x 
     \frac{\sqrt{\log\log t}}{\log t} \times \widehat{C}_{k,\ast}^{\prime}(t) dt + 
     O(\log x). 
\]
...
\begin{align*}
\widehat{C}_k(x) & = \frac{\sqrt{\log\log x}}{\log x} \times \widehat{C}_{k,\ast}(x) + 
     O\left(x \times \int_3^x \frac{\sqrt{\log\log t} \times \widehat{C}_{k,\ast}(t)}{t^2 (\log t)^2} dt\right) \\ 
     & = \frac{\sqrt{\log\log x}}{\log x} \times \widehat{C}_{k,\ast}(x) + 
...
\subsection{The average order of $C_{\Omega}(n)$}
...
\subsection{Erd\H{o}s-Kac theorem analogs characterizing the distribution of $C_{\Omega}(n)$} 
...
\sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     ...
     \text{ as } x \rightarrow \infty
\end{equation*} 
A modified set of proof mechanics that draw upon the analytic methods in 
...
Proposition \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1}. 
...
The second conjectured result in (B) is effectively the statement of an Erd\H{o}s-Kac type 
central limit theorem with first and second moment statistics 
(formed as partial sums of powers of the deterministic function $C_{\Omega}(n)$) 
that grow without bound as $x \rightarrow \infty$. 
The intuition for why part (B) of the conjecture should hold is thus probabilistic in nature, 
rather than being based on the analytic proof constructions that led to the 
conjectured statement in part (A). 
Rigorous proofs of the conjectured results below fall outside of the scope of this manuscript. 
...
\mu_x(C) := \log\log x - 
     ...
     \sigma_x(C) := \sqrt{\log\log x}, 
\]
where $\widehat{G}(1) \equiv \frac{1}{\zeta(2)(1+P(2))} \approx 0.418611$. 
...
\section{The distribution of the unsigned inverse sequence $|g^{-1}(n)|$} 
...
\end{equation}
We use the formula from Proposition \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1} 
...
the results follow by a re-normalization of Conjecture \hlocalref{conj_DetFormOfEKTypeThmForCOmegan_v1}. 
\end{proof} 
...
proved in Section \hlocalref{subSection_Relating_CknFuncs_to_gInvn}, 
we recall that 
\[
M(x) = G^{-1}(x) + \sum_{p \leq x} G^{-1}\left(\Floor{x}{p}\right), 
     \text{ for } x \geq 1. 
\]
Ordinary summation by parts implies that we can also express $G^{-1}(x)$ in terms 
of the summatory function $L(x)$ and differences of the unsigned sequence 
whose distribution is given by 
Corollary \hlocalref{cor_CLT_VII}. That is, we have  
\[
G^{-1}(x) 
...
\subsection{Example: On the expectation of local cancellation of 
            $G^{-1}(x)$ in the new formulas for $M(x)$} 
...
This follows since the contributions from the summands off of the squarefree integers 
are at most a bounded multiple of $(-1)^k k!$ when $\Omega(n) = k$ by the 
cited exact factorial product formula. 
...
Lemma \hlocalref{theorem_PrimorialSeqGInvCalcs_v1} 
...
Section \hlocalref{subSection_AConnectionToDistOfThePrimes}, 
...
As in the Mertens function case, the link between strong additivity and the resulting 
sequences studied to express the partial sums of signed Dirichlet inverse functions 
are also computationally useful in more efficiently computing all of the 
first $x \geq 3$ values of these summatory functions when $x$ is large.  
The key question in formulating generalizations to the new methods we present in this 
article is in constructing the relevant strongly additive functions that fulfill the 
role of $\omega(n)$ in our new constructions that characterize $M(x)$. 
We expect that while natural extensions exist in connection with the Dirichlet inverse 
of any arithmetic $f > 0$, pinning down a strongly additive analog to the distinct prime 
counting function in the most general cases will require deep new theorems tied to the 
DGFs of the original sequences, $\{f(n)\}_{n \geq 1}$. 
...
\appendix
\cftaddtitleline{toc}{section}{Appendices and supplementary material}{}
...
\clearpage
\newpage 
\section{Results characterizing the distributions of $\omega(n)$ and $\Omega(n)$} 
...
these results imply a distinctively regular (normal) tendency 
...
\newpage
\section{Asymptotic formulas for partial sums involving the incomplete gamma function} 
...
\section{Combinatorial connections between $g^{-1}(n)$ and the distribution of the primes} 
...
The magnitudes of and spacing between the primes $p \leq n$ restricts the 
...
(\cf Observation \hlocalref{heuristic_SymmetryIngInvFuncs}). 
...
The concrete new links of the partial sums of $g^{-1}(n)$ to exact formulae characterizing $M(x)$ 
is then somewhat surprising given that the coefficients in the expansion of the Euler product 
representation of $\zeta(s)^{-1}$, or the DGF of the original signed summands, $\mu(n)$, suggests 
a fundamentally convolved and hard-to-invert 
multiplication structure that defines the components to the classical 
partial sums. In this sense, we surmise that by replacing the known hard analysis involving 
oscillatory imaginary parts in the complex plane with the reciprocal 
zeta function to define $M(x)$ we gain new information on the underlying structures that 
can be more easily recovered by working with the unsigned sequences first. 

\begin{remark}[Local extrema of the inverse sequence] 
...
\begin{example}[Combinatorial formulas involving symmetric polynomials in prime factorization exponents] 
The formula for the function $h^{-1}(n) = (g^{-1} \ast 1)(n)$ defined in the proof of 
Proposition \hlocalref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1} shows that we can express 
...
\[
\widehat{e}_k(n) := [t^k] \prod_{p|n} (1 + t \nu_p(n)) = [t^k] \prod_{p^{\alpha} || n} (1 + \alpha t). 
\]
Suppose that we set $P_k(n) := \sum_{p^{\alpha}||n} \alpha^k$ for any $n \geq 2$. 
Newton's (Newton-Girard) formulas 
relating general classes of elementary symmetric polynomials to their corresponding power-sum 
symmetric polynomials (here, in the form of $\widehat{P}_k(n)$ for $n \geq k \geq 1$) show that 
\begin{align*} 
\widehat{e}_0(n) = 1, \widehat{e}_1(n) = \widehat{P}_1(n), 
     \widehat{e}_2(n) = \frac{1}{2}\left(\widehat{P}_1(n)^2-\widehat{P}_2(n)\right), 
     \widehat{e}_3(n) = \frac{1}{6}\left(\widehat{P}_1(n)^3 - 3\widehat{P}_1(n)\widehat{P}_2(n) + 2\widehat{P}_3(n) 
     \right), n \geq 2, 
\end{align*}
though general expressions can be given in analog to the special cases in the last equation for 
$k \geq 4$ as well. 
...
g^{-1}(n) = h^{-1}(n) \times \sum_{k=0}^{\omega(n)} \binom{\Omega(n)}{k}^{-1} 
     \times \frac{\widehat{e}_k(n)}{k!}, n \geq 2. 
\]
We also notice that both of the functions 
$f_1(n) := \frac{h^{-1}(n)}{(\Omega(n))!}$ and $f_2(n) := \frac{\lambda(n) h^{-1}(n)}{(\Omega(n))!}$ are 
both multiplicative, Dirichlet invertible arithmetic functions of $n \geq 1$. 
\end{example}
...
Section 
\hlocalref{subSection_LocalCancellationOfGInvx} 
discusses limiting asymptotic properties with respect to the expected substantial 
local cancellation in the formula for $M(x)$ at most large $x \geq 1$ 
from \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} 
that is expanded exactly through the auxiliary sums $G^{-1}(x)$ as in 
\eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} above. 

\clearpage 

\newpage
\section{Tables of computations involving $g^{-1}(n)$ and its partial sums} 
