...
\[
M_{\pm}(x) := \left\lvert \sum_{\log x < d \leq \frac{(\log x)^{7/3} (\log\log x)}{e^2}} 
...
\begin{align} 
\notag 
S_{E,1}^{(\ell)}(x) & \ll \frac{(\log x)^{\frac{7}{4}} (\log\log x)^{\frac{5}{4}}}{ 
     \sqrt{\log\log\log x}} \times \int_e^{\log x} \frac{(\log t)^{3/4}}{t \cdot (\log\log t)} dt \\ 
\notag 
     & \ll \frac{(\log x)^2 (\log\log x)^{\frac{5}{4}}}{ 
...
\label{eqn_proof_tag_SE1Ellx_BoundFunc_MaximalOrder_v1} 
     & \ll (\log x)^2 (\log\log x)^{\frac{5}{4}} (\log\log\log x)^{\frac{3}{2}}. 
\end{align} 
