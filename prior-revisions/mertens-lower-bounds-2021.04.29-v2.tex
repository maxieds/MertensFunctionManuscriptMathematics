...
We have for large $x > e$ and $1 \leq k \leq 2\log\log x$ that  
\begin{align*} 
\widehat{C}_k(x) := 
...
L_{\ast}(t) & := \left\lvert \sum_{n \leq \log\log t} (-1)^{\omega(n)} \right\rvert \sim 
     ...
     \frac{t}{2\sqrt{2\pi \log\log t}} + O\left(\frac{t}{(\log\log t)^{3/2}} + E_{\omega}(t)\right). 
\end{align} 
...
from the appendix shows that for any fixed $a \in (1, 1.76323]$ we have that 
\begin{equation}
...
\mathbb{E}\left[C_{\Omega(n)}(n)\right] = 
     \frac{2 \sqrt{2\pi} (\log n)}{\sqrt{\log\log n}}. 
\] 
\end{prop} 
\begin{proof} 
...
\sum_{k=1}^{2\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & = 
     \frac{2 \sqrt{2\pi} \cdot x \log x}{\sqrt{\log\log x}} + O\left(
