...
%Along an increasing subsequence $\{x_y\}$ depending on $y$, where 
%$x_y \in \left[C_1 \sqrt{x_{0,y}}, C_2 x_{0,y}\right)$ pointwise at each $y$ 
%for some constants $C_1, C_2 > 0$, 
We have that for all sufficiently large 
$y \gg \ceiling{x_{0,1}}+1$ the following bound holds: 
\begin{align*} 
\frac{|M(x_{0,n})|}{\sqrt{x_{0,n}}} & \SuccSim 
     \frac{2 \cdot C_{\ell, 1} \cdot (\log\log \sqrt{x_{0,n}})^{3} \sqrt{\log\log\log \sqrt{x_{0,n}}}}{ 
     (\log\log\log\log \sqrt{x_{0,n}})^{\frac{5}{2}}}, \mathrm{\ as\ } y \rightarrow \infty. 
\end{align*} 
...
\end{align} 
We want this sequence $\{x_y\}_{y \geq Y}$ for $Y$ sufficiently large to correspond to 
...
$\varepsilon_0 := \frac{1}{2} \min(B, 1-C)$. Then we can take any $x$ satisfying 
\[
