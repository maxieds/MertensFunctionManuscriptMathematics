...
\label{eqn_proof_tag_Ckn_AsymptoticExp_v1} 
     & \gg \mathbb{E}\left[\sum_{p|n} \sum_{d\bigr\rvert\frac{n}{p^{\nu_p(n)}}} \left[ 
...
\begin{align*} 
\mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] - |g^{-1}(n)| & \ll 
     \mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] - \mathbb{E}|g^{-1}(n)| \\ 
     & \leq 0, \mathrm{\ as\ } n \rightarrow \infty. 
\end{align*} 
...
0 \geq \mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] - |g^{-1}(n)| \geq 
     \sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2 \log\log d - 1} - |g^{-1}(n)|, 
...
on the set $\mathcal{C}_E \cap \mathcal{S}_E$ defined as in 
...
G^{-1}(x) \geq \sum_{n \leq x} \lambda(n) \sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d)^{2\log\log d - 1}. 
\end{equation} 
...
\mathcal{A}_E := \left\{x \geq 1: 
     G^{-1}(x) \geq \sum_{n \leq x} G_E^{-1}(x) 
     \wedge G^{-1}(x) \leq 0 
     \right\}, 
\] 
...
Also, by the signedness of $g^{-1}(n)$, we expect that $|\mathbb{E}[g^{-1}(n)]| \geq 0$ for all $n \geq 1$, 
where 
\[
\left\lvert \int_{1}^{x/2} \frac{1}{t \cdot \log(x/t)} dt \right\rvert = \log\log x - \log\log 2. 
\]
Therefore, for large enough $x$ and any fixed constant $u_0$ 
\begin{align} 
\label{eqn_proof_tag_MxAbsValueConsequence_v3} 
\frac{|M(x)|}{x} & \gg \min\left\{ 
     \left\lvert \int_{u_0}^{x/2} \frac{1}{t \cdot \log(x/t)} dt \right\rvert, 
     \mathbb{E}|g^{-1}(x)|\right\} \\ 
\notag 
     & \gg \min\left\{\log\log(x/u_0) - \log\log 2, \log x + \gamma \right\} \\ 
\notag 
