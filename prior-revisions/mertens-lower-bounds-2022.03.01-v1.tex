...
$r \in [0, z_{\max}] \subseteq \left[0, 2\right)$: 
\begin{align} 
...
\widehat{B}(x, r) \ll x (\log x)^{r-1-r\log r} \sqrt{\log\log x} = 
     O\left(\frac{x \sqrt{\log\log x}}{(\log x)^{0.108197}}\right). 
\end{equation}
...
\frac{1}{x} \times 
     ...
     O\left(\frac{\sqrt{\log\log x}}{(\log x)^{0.108197}}\right), 
     \text{ as } x \rightarrow \infty. 
\end{align*} 
...
For comparison, we have the second central moment of $C_{\Omega}(n)$ 
...
\item In Figure \hlocalref{figure_MxAndNewAuxPartialSums_Comparison_Intro_v2_v2}, we compare 
      ...
      \frac{|G|(x)}{x} \sim \frac{6B_0}{\pi^2} (\log x) \sqrt{\log\log x}, 
           \text{ as } x \rightarrow \infty, 
      \]
      whereas $|G(x)| \leq |G|(x)$ for all $x \geq 1$ so that 
      \[
      \frac{|G(x)| (\log x) \sqrt{\log\log x}}{Q(x)} \ll (\log x)^2 (\log\log x). 
      \]
      Thus, the bounding envelopes on $G(x)$ plotted in (b) may hold for all sufficiently 
      large $x$ as the scaling factor is only a logarithmic function of $x$ away from the upper bound. 
      Moreover, for most large $x$, 
...
\item In Figure \hlocalref{figure_HistgInvnDistUns_Comparisons_v2}, we compare two variants of the 
      ...
      \end{align*} 
      The normalized histograms in (a) and (b) of the figure, respectively, 
      show the CDFs corresponding to $\mathcal{G}_1(x, z)$ and 
      $\mathcal{G}_2(x, z)$. The scaling of the function $G(n)$ by $n^{-1}$ shows key features of these 
      ...
      The right-hand-side functions in the numerator differences of each of the two distribution variants 
      defined in the previous equations are 
      selected for comparison with the scaled average order of $|g(n)|$ given by 
...
\item In Figure \hlocalref{figure_LogarithmicRatioDensityPlot_Comparisons_v1}, we define 
      ...
      vertical axis in the density plots shown in the figure above. 
      The plots in (a) and (c) involve the sequence of $m^{th}$ prime numbers, $\{p_m\}_{m \geq 1}$. 
      The smoother transitions featured in the density plots of (a) and  (b) 
      comparing $L(x)$ (right) to $M(x)$ (left) for the same sequences $f$ 
      show that there is more apparent correlation between $G(x)$ and the 
      former function viewed within the context of this plot type. 
      Experiments with other sequences $f$ (not shown) 
      are less visually regular than the selections displayed in the figure. 
      In general, computer simulations with $f(m)$ selected randomly from the interval $[1, m]$ 
      produce chaotic plots (not shown) with no distinctive features nor apparent correlation with the 
      denominator summatory functions.
...
These results then reflect a distinctive tendency 
