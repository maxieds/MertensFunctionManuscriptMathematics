...
\end{equation}
For any arithmetic functions $f$ and $h$, we define their Dirichlet convolution by 
\[
...
identically one assuming that for any $y > 0$ 
\begin{align*} 
...
that yield the global extremum of the function.
\end{proof} 
...
average order is large and is approximately on the order of
\[
\frac{1}{x} \times \sum_{n \leq x} C_{\Omega}(n) = A \cdot x^{Bx + C} (1 + o(1)), 
	\text{ as } x \rightarrow \infty, 
\]
for absolute constants $A,B,C > 0$. 
...
\notag 
     & \sim \sum_{d \leq n} C_{\Omega}(d) \left(\frac{6}{\pi^2 nd} + O\left(\frac{1}{\sqrt{dn}}\right) 
     \right) \\ 
...
C_p^{(n)} := \begin{cases} 0, & p \nmid N(n); \\ 
	\alpha, & p^{\alpha} || N(n), \text{ for some } \alpha \geq 1. 
	\end{cases}
\]
For integers $k \geq 1$ and primes $p$, we have limiting convergence in distribution of 
$C_p^{(n)} \overset{d}{\implies} Z_p$ where $Z_p$ is 
...
$$\frac{\log C_{\Omega}(n) - B_0 (\log\log x)(\log\log\log x)}{B_0 (\log\log x)(\log\log\log x)} 
  \overset{d}{\implies} Z_{0,1}, \text{ for any } 2 \leq n \leq x, \text{ as } x \rightarrow \infty,$$ 
...
\]
We can select $M_x \ll \sqrt{\log\log x}$ so that for large $x$ we have
...
\section{Proofs of the new exact formulas for the Mertens function} 
...
careful notes on the limiting asymptotics for the sums in this section. 
These proofs based on his work in 
\cite{NEMES2015C,NEMES2016,NEMES2019} are adapted below. 
...
We can express the inverse by a similarity transformation conjugated by shift operators by 
\begin{align*} 
...
This leads to the following alternate expression for $H(x)$: 
\[
