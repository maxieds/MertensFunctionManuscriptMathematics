...
The inverse function $g^{-1}(n) := (\omega+\mathds{1})^{-1}(n)$
...
$|g^{-1}(n)|$ that depends on the exponent patterns in the 
...
exact formulas and new characterizations of $M(x)$. 
...
The main interpretation to take away from this article is 
...
We compute the first values of this signed inverse sequence as 
...
The shift by the constant one 
in the right-hand-side convolution from \eqref{eqn_AntiqueDivisorSumIdent} is 
...
\end{align} 
\end{subequations}
There is an identification of $G^{-1}(x)$ with the summatory function $L(x)$ given by 
\begin{align*}
G^{-1}(x) & = 
     L(x)|g^{-1}(x)| - \sum_{n < x} 
...
\caption{A comparison of $M(x)$ and a scaled form of $G^{-1}(x)$ where 
...
\caption{Comparisions of bounded envelopes for the local extremum of the logarithmically scaled values of 
         $G^{-1}(x)$ to the absolute values of the partial sums of the scaled unsigned inverse function. }
\end{subfigure}

\caption{Discrete plots displaying comparisons of the growth of $M(x)$ to the new auxillary partial sums for $x \leq 4500$. 
         The scaling function $Q(x) := \sum_{n \leq x} \mu^2(n)$ counts the number of squarefree integers $n \leq x$ for any 
         $x \geq 1$. 
        }
...
(i.e., taking into account the multiplicity of distinct elements in each set), 
...
Hence, we find the key takeaway that 
...
prove the sign of $g^{-1}(n)$ for all $n \geq 1$ 
(see Proposition \hlocalref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1}) 
and provide bounds on the local extremum of the unsigned inverse function. 
...
we use an adaptation of the form of \emph{Rankin's method} from \cite[Thm.~7.20]{MV} 
to show that there is an absolute constant $B_0 > 0$ such that 
\[
...
the average order of $|g^{-1}(n)|$ is given by 
\[
...
\frac{1}{x} \times & \#\left\{3 \leq n \leq x: 
     \frac{|g^{-1}(n)|}{\sqrt{\log\log n}} - 
     \frac{6}{\pi^2 n \sqrt{\log\log n}} 
...
\frac{1}{x} \times \#\left\{3 \leq n \leq x: |g^{-1}(n)| - 
     \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \leq y\right\} & = 
     \Phi\left(\frac{\frac{\pi^2 y}{6}- B_0 \sqrt{\log\log x}}{ 
     B_0 \sqrt{x \log\log x}}\right) + o(1). 
\end{align} 
...
\eqref{eqn_GInvx_PartialSumForms_v1} are linked to 
canonical examples of strongly and completely additive functions, 
i.e., in relation to $\omega(n)$ and $\Omega(n)$. 
The definitions of the new sequences we formulate, and the 
...
In particular, we adapt the proofs of the results in \cite[\S 7.4; \S 2.4]{MV} 
which apply analytic methods to formulate limiting asymptotics 
characterizing key properties of the distribution of the completely additive 
function $\Omega(n)$. Adaptations of the key ideas in the reference 
...
$C_{\Omega}(n)$ and $|g^{-1}(n)|$ (the latter as corollaries of the former results). 

In the process, we formalize a probabilistic perspective from which to express 
...
\]
we see that if the conjectured result in \eqref{eqn_EKacVariant_forUnsgInvn_Intro_v2} holds, the 
...
Hence, the conjectured corollary implies a decidely regular tendency of the unsigned inverse sequence. 
...
distribution of $L(x)$, the explicitly identified distributions, and 
$M(x)$ as $x \rightarrow \infty$.
...
equations \eqref{eqn_Mx_gInvnPixk_formula_v1} and \eqref{eqn_Mx_gInvnPixk_formula_v2}
as an immediate consequence of the inversion theorem for $M(x)$ in the corollary. 

\subsection{The characteristic signedness property of $g^{-1}(n)$} 
\label{subSection_ProofOfSignednessOfgInvn_v1} 

\begin{prop}[The sign of $g^{-1}(n)$]
\label{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1} 
For any arithmetic function $r$, let the operator 
$$\operatorname{sgn}(r(n)) := \begin{cases} 
  \frac{r(n)}{|r(n)|}, & \text{ if $r(n) \neq 0$; } \\ 
  0, & \text{ otherwise, } 
  \end{cases}$$
provide the signedness of $r(n)$ at any $n \geq 1$. 
...
denotes the \emph{prime zeta function}. 
...
is intended to provide clear insight into the significance of the few characteristic formulas for  
...
Lemma \hlocalref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} 
implies the next results when $n$ is squarefree. 
...
M(x) = G^{-1}(x) + \sum_{p \leq x} G^{-1}\left(\Floor{x}{p}\right), \text{ for all } x \geq 1. 
\end{equation}
The proof of Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2} (below) 
shows that 
\[
\sum_{n \leq x} |g^{-1}(n)| = \sum_{d \leq x} C_{\Omega}(d) Q\left(\Floor{x}{d}\right), 
     \text{ for all } x \geq 1, 
\]
...
from the appendix provides that for any $a \in (1, 1.76321) \subset \left(1, W(1)^{-1}\right)$ the 
...
\sum_{k=1}^{\frac{3}{2} \log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega}(n) & \asymp 
     ...
     \left(1 + O\left(\frac{1}{\log\log x}\right)\right) \\ 
\notag 
     & = \frac{x \sqrt{\log\log x} \Gamma\left(\frac{3}{2} \log\log x, \log\log x\right)}{ 
...
\subsection{Conjectured Erd\H{o}s-Kac theorem analogs characterizing the distribution of $C_{\Omega}(n)$} 
\label{subSection_ErdosKacTheorem_Analogs} 

It is not difficult to prove that uniformly for $1 \leq k \leq \log\log x$ 
\begin{equation*} 
\sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     \frac{C_{\Omega}(n)}{\sqrt{\log\log n}} = 
...
\end{align*}
This calculation leads to the second conjectured result in (B). 
...
\tag{A} 
\frac{1}{x} \times \#\left\{3 \leq n \leq x: 
     \frac{\frac{C_{\Omega}(n)}{\sqrt{\log\log n}} - 
...
\tag{B} 
\frac{1}{x} \times \#\left\{3 \leq n \leq x: 
     \frac{C_{\Omega}(n) - B_0 \sqrt{\log\log x}}{ 
     B_0 \sqrt{x \log\log x}} \leq z\right\} = 
     \Phi\left(z\right) + o(1), 
     \mathrm{\ as\ } x \rightarrow \infty
\end{equation}
\end{conjecture} 
...
\subsection{Erd\H{o}s-Kac theorem analogs} 
...
\begin{align*} 
\frac{1}{x} \times \#\left\{3 \leq n \leq x: \frac{|g^{-1}(n)|}{\sqrt{\log\log n}} - 
     \frac{6}{\pi^2 n \sqrt{\log\log n}} \times \sum_{k \leq n} |g^{-1}(k)| \leq y\right\} & = 
...
We expect that while natural extensions exist in connection with the signed Dirichlet inverse 
