...
are given by the corresponding lower bounds from 
...
\exp\left(c_{\ell}(x) \left[ 
     ...
     \approx 0.637357. 
\]
We next consider the coefficients of $b_{\ell}(x)$ in our product expansion as 
\[
...
\frac{B(x, 1+\delta)}{A(x, 1)} & \ll 
     ...
     \sim 
     \frac{(\log x)^{\delta - \delta\log(1+\delta)}}{ 
     \frac{1}{2} + o(1)} 
...
\subsection{A formula for $M(x)$} 

\begin{lemma} 
\label{lemma_HatPikxSum_WeightedByFact_v1} 
As $x \rightarrow \infty$, we have that 
\[
\frac{1}{x} \times \sum_{k=1}^{\log x} \widehat{\pi}_k(x) \cdot k! \ll 
     \frac{1}{x} \times \sum_{k=1}^{\log\log x} \widehat{\pi}_k(x) \cdot k! = 
     o(\sqrt{x}). 
\]
\end{lemma} 
\begin{proof} 
...
\begin{align} 
\notag 
     & \ll \frac{1}{x} \times \sum_{k=1}^{\log\log x} \widehat{\pi}_k(x) \cdot k! 
     \ll \frac{1}{\log x} \times \sum_{k=1}^{\log\log x} k (\log\log x)^{k-1} \\ 
\notag 
     & \ll \frac{1}{(\log x) (\log\log x)} \times (\log\log x)^{\log\log x} = o(\sqrt{x}), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{align} 
Now to bound the tail end of the sum over $k \in (\log\log x, \log x]$, we use 
Theorem \ref{theorem_MV_Thm7.20-init_stmt} to show that 
we have for large $x \rightarrow \infty$ 
\begin{align*} 
\frac{1}{x} \times \sum_{k=\log\log x+1}^{\log x} \widehat{\pi}_k(x) \cdot k! & \ll 
     \sum_{k=\log\log x}^{\log x} (\log x)^{\frac{k}{\log\log x} - 1 - \frac{k}{\log\log x} \left( 
     \log k - \log\log\log x\right)} \cdot \left(\frac{k}{e}\right)^{k} \sqrt{2\pi k} \\ 
     & \ll \sum_{k=\log\log x}^{\log x} (\log x)^{\frac{\log\log\log x}{\log\log x} - 1} e^{-k \log k} 
     k^{k + \frac{1}{2}} \\ 
     & = \sum_{k=\log\log x}^{\log x} (\log x)^{\frac{\log\log\log x}{\log\log x} - 1} \sqrt{k} 
     \ll \frac{\log\log x}{\log x} \times \int_{\log\log x}^{\log x} \sqrt{t} dt \\
     & \ll (\log x)^{\frac{1}{2}} (\log\log x) = o(\sqrt{x}), 
\end{align*} 
where $\lim_{x \rightarrow \infty} (\log x)^{\frac{1}{\log\log x}} = e$. 
\end{proof} 

\begin{lemma} 
\label{lemma_gInvn_Cvl_PrimePiFunc_BoundedSum_v2} 
As $x \rightarrow \infty$, we have that 
\[
\sum_{k=1}^{x/2} g^{-1}(k) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     o(\sqrt{x}). 
\]
\end{lemma} 
\begin{proof} 
We will show two separate parts of this result: 
\begin{itemize}[itemsep=0pt,topsep=4pt,leftmargin=0.75in] 
\item[\textbf{(A)}] $\sum\limits_{k=1}^{x/2} g^{-1}(k) \frac{x}{k \cdot \log(x/k)} = o(\sqrt{x}), 
                     \mathrm{\ as\ } x \rightarrow \infty$; and 
\item[\textbf{(B)}] $\sum\limits_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} g^{-1}(k) \left[ 
                     \pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = o(\sqrt{x}), 
                     \mathrm{\ as\ } x \rightarrow \infty$. 
\end{itemize}
Then since the are absolute constants $B,C > 0$ so that whenever $x \geq 2$ we have 
\[
\frac{B \cdot x}{\log x} \leq \pi(x) - \frac{x}{\log x} \leq \frac{C \cdot x}{\log x}, 
\]
we can see that 
\[
\sum_{k=1}^{x/2} g^{-1}(k) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     O\left(\left\lvert \sum_{k=1}^{x/2} g^{-1}(k) \frac{x}{k \cdot \log(x/k)} \right\rvert + 
     \left\lvert \sum_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} g^{-1}(k) \left[ 
                     \pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] \right\rvert\right). 
\]
So in this case, \textbf{(A)} and \textbf{(B)} imply our result. \\ 
\textbf{Proof of (A):} 
By Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} and 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz} we have that 
\begin{align} 
\notag 
\sum_{k=1}^{x/2} g^{-1}(k) \frac{x}{k \cdot \log(x/k)} & \ll 
     \sum_{k=1}^{x/2} g^{-1}(k) \cdot \frac{x}{k} \\ 
\notag 
\left\lvert \sum_{k=1}^{x/2} g^{-1}(k) \cdot \frac{x}{k} \right\rvert & 
     \sim x \times \left\lvert \mathbb{E}\left[ 
     \sum_{d|\frac{x}{2}} g^{-1}(d)\right] \right\rvert 
     \ll x \times \mathbb{E}\left[C_{\Omega(x)}(x)\right] = 
     \sum_{k=1}^{x} C_{\Omega(k)}(k) \\ 
\label{eqn_proof_tag_LHSDiff_ErrorTerm_v1} 
     & \ll \frac{1}{x} \times \sum_{k=1}^{\log\log x} \widehat{\pi}_k(x) \cdot k! 
     \ll \frac{1}{\log x} \times \sum_{k=1}^{\log\log x} k (\log\log x)^{k-1} \\ 
\notag 
     & \ll \frac{1}{(\log x) (\log\log x)} \times (\log\log x)^{\log\log x} = o(\sqrt{x}), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{align} 
It suffices to only take the sum over $k \leq \log\log x$ in 
\eqref{eqn_proof_tag_LHSDiff_ErrorTerm_v1} by applying 
Lemma \ref{lemma_HatPikxSum_WeightedByFact_v1}. \\ 
\textbf{Proof of (B):} 
We know by the properties established at the end of 
Section \ref{subSection_Relating_CknFuncs_to_gInvn} that if 
$\Omega(n) = k$, then for $n \geq 2$ 
\[
|g^{-1}(n)| \leq \sum_{j=0}^{k} \binom{k}{j} \cdot j!. 
\]
It follows that for large $n \rightarrow \infty$, 
\begin{align*} 
|g^{-1}(n)| & \ll \sum_{k=1}^{\log\log n} \frac{\widehat{\pi}_k(n)}{n} \times 
     \sum_{j=0}^{k} \binom{k}{j} \cdot j! \\ 
     & \ll \frac{1}{(\log n)} \times \sum_{k=1}^{\log\log n} k \cdot (\log\log n)^{k-1} \\ 
     & \ll \frac{1}{(\log n)} (\log\log n)^{\log\log n}. 
\end{align*} 
To arrive at the last bound, we have used a crude form of the bound provided by 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz} and 
applied Lemma \ref{lemma_HatPikxSum_WeightedByFact_v1}. 

Let the summatory function 
\[
\widetilde{L}(x) := \sum_{n \leq x} \lambda(n). 
\]
By again applying a crude form of the bound given by 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz}, followed by an asymptotic approximation 
to the incomplete gamma function given by 
\eqref{eqn_IncompleteGamma_PropB}, we see from 
Corollary \ref{theorem_MV_Thm7.20} that 
\begin{align*} 
|\widetilde{L}(x)| & \ll \left\lvert \sum_{k=1}^{\log\log x} (-1)^{k} \widehat{\pi}_k(x) \right\rvert 
     \ll \frac{x}{\sqrt{\log\log x}}, 
\end{align*} 
where for this approximation $\operatorname{sgn}\left(\widetilde{L}(x)\right) = (-1)^{\floor{\log\log x}}$. 
We can also compute that 
\[
\frac{d}{dt}|\widetilde{L}(t)| \ll \frac{1}{\sqrt{\log\log t}}. 
\] 
We want to sum over the intervals where $\widetilde{L}(x) < 0$ to capture the 
contribution from the negative magnitude terms $g^{-1}(k)$ such that $\lambda(k) = -1$. 
Then by Abel summation and integration by parts 
\begin{align*} 
 & \left\lvert \sum_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} g^{-1}(k) \frac{x}{k \cdot \log(x/k)} \left[ 
                     \frac{\pi\left(\frac{x}{k}\right)}{x/k \cdot \log^{-1}(x/k)} - 1 
                     \right] \right\rvert \\ 
 & \qquad \ll \sum_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} |g^{-1}(k)| \frac{x}{k \cdot \log(x/k)} 
   \ll \sum_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} |g^{-1}(k)| \frac{x}{k} \\ 
 & \qquad \ll \int_{e}^{\frac{\log\log x}{2}} 
   \frac{x \cdot |\widetilde{L}^{\prime}(t)| (\log\log t)^{\log\log t}}{t \cdot (\log t)} 
   \Biggr\rvert_{t=e^{e^{2k}}} \times 
   e^{e^{k}} dk \\ 
 & \qquad \ll x \times \left(\max_{1 \leq c \leq \frac{\log\log x}{2}} (2c)^{2c-\frac{1}{2}} \cdot 
   \exp\left(e^{c} - e^{2c} - 2c\right)\right) \times \frac{\log\log x}{2} \\ 
 & \qquad \ll \frac{(\log\log x)^{\log\log x + \frac{1}{2}}}{\log x} e^{\sqrt{\log x}} = o(\sqrt{x}). 
   \qedhere
\end{align*} 
\textbf{Why the last proof of (B) makes sense:} 
Notice that as we have argued above, we are basically trying to prove the bound 
\[
\sum_{\substack{k=1 \\ \lambda(k)=-1}}^{x/2} \frac{x}{k} g^{-1}(k) = 
     \sum_{k=1}^{x/2} \sum_{\substack{r|k \\ \lambda(r)=-1}} g^{-1}(r) = o(\sqrt{x}). 
\]
We expect the sequence $g^{-1}(k)$ to have a random sign so that we might as well 
(in explaining this approximation) assume the the sign of $\lambda(k)$ is uniformly distibuted. 
Now we expect that on average 
\begin{align*} 
\mathbb{E}|g^{-1}(x)| & = \frac{1}{x} \times \sum_{n \leq x} C_{\Omega(n)}(n) Q\left(\frac{x}{n}\right) \\ 
     & \sim \frac{6}{\pi^2} \times \sum_{n \leq x} \frac{C_{\Omega(n)}(n)}{n} \\ 
     & \ll \int_{e}^{x} \frac{(\log\log t)^{\log\log t}}{t} dt \ll 
     (\log x) (\log\log x)^{\log\log x} = o(x^{\rho}), 
     \mathrm{\ for\ any\ } \rho > 0. 
\end{align*} 
We assume that there is a slight sign bias in the distribution of $\lambda(k)$ so that when 
$\pi(k) \equiv 1 \pmod{2}$, we expect there to be more negatively signed terms. 
At any rate, the sign bias we recover is inherent to the summatory function approximation 
to $\widetilde{L}(x)$ noted above. So because we are summing over 
$k \in \left[e^{e^{2m}}, e^{e^{2m+1}}\right)$ for $m \in \mathbb{Z}^{+}$, we obtain that 
for any small $\varepsilon > 0$
\begin{align*} 
\sum_{k=1}^{x/2} \frac{x}{k} |g^{-1}(k)| \Iverson{\lambda(k) = -1} & \ll 
    \sum_{k=1}^{x/2} \frac{x}{k^{1-\varepsilon}} \Iverson{\lambda(k) = -1} 
    \ll \int_{e}^{\frac{\log\log x}{2}} \frac{x}{e^{(1-\varepsilon) e^{2k}}} \cdot 
    e^{e^{k}} dk \\ 
    & \ll x \cdot (\log\log x) \exp\left(\sqrt{\log x} - (\log x)^{1-\varepsilon}\right) 
    \ll \lim_{\varepsilon \rightarrow 0} x^{2\varepsilon} (\log\log x) e^{\sqrt{\log x}}. 
\end{align*} 
We have somewhat oversimplified the bound in the last lines, though this rough 
back-of-the-envelope argument should make it clearer why we can expect to get this type 
of cancellation out of our formulas.  
\end{proof} 
...
M(x) & \approx G^{-1}(x) + \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
...
By Lemma \ref{lemma_gInvn_Cvl_PrimePiFunc_BoundedSum_v2}, as $x \rightarrow \infty$ 
\begin{equation} 
\label{eqn_proof_tag_LHSDiff_v0} 
\sum_{k=1}^{x/2} g^{-1}(k) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     o(\sqrt{x}). 
\end{equation} 
So by summation by parts, \eqref{eqn_proof_tag_LHSDiff_v0} implies that we have 
\begin{subequations}
\begin{align} 
\notag 
\sum_{k=1}^{x/2} g^{-1}(k) \pi(x/k) & = \sum_{k=1}^{x/2} 
    \frac{x \cdot g^{-1}(k)}{k \cdot \log(x/k)} + o(\sqrt{x}) \\ 
\notag 
    & = \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
    \sum_{k=1}^{x/2} G^{-1}(k) \left[ 
    \frac{x}{k \cdot \log\left(\frac{x}{k}\right)} - 
    \frac{x}{(k+1) \cdot \log\left(\frac{x}{k+1}\right)}\right] + o(\sqrt{x}) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v3} 
     & \sim \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) 
     -\sum_{k=1}^{x/2} G^{-1}(k) \left[ 
     \frac{x}{k \cdot \log(x/k)} - \frac{x}{(k+1) \cdot \log(x/k)}\right] + o(\sqrt{x}) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v4} 
     & \approx \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
     \sum_{k=1}^{x/2} G^{-1}(k) \frac{x}{k^2 \cdot \log(x/k)} + o(\sqrt{x}). 
\end{align} 
\end{subequations} 
Indeed, step 
...
\[
M(x) \approx G^{-1}(x) + \frac{2}{\log 2} G^{-1}\left(\frac{x}{2}\right) - 
     x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{t^2 \cdot \log(x/t)} dt + o(\sqrt{x}). 
\]
