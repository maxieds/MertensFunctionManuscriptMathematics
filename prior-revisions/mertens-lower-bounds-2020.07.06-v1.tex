...
\[
M_{\pm}(x) := \left\lvert \sum_{\log x < d < \frac{(\log x)^{5} (\log\log x)}{e}} 
     \frac{\varepsilon_{x,d} \cdot 
     \lambda(d) (\log d)^{\frac{1}{4}}}{d^{1/4} \cdot \log\log d} \right\rvert \times 
     \frac{(\log x)^{5/4}}{(\log\log x)^{1/4} \sqrt{\log\log\log x}}, 
\]
...
\] 
We will first estimate the minimal and maximal order of a sum of $k$ consecutive 
identically signed terms in the sum that defines $M_{\pm}(x)$. For $x$ large with 
$x \rightarrow \infty$ and for $1 \leq k \leq x$, we have that 
\begin{align*} 
0 \ll \sum_{d=x}^{x+k} \frac{(\log d)^{1/4}}{d^{1/4} \cdot (\log\log d)} & \sim 
     \int_x^{x+k} \frac{(\log t)^{1/4}}{t^{1/4} \cdot (\log\log t)} dt \\ 
     & = c^{3/4} \cdot \left(\operatorname{Ei}\left(\frac{5}{4} \log\log(x+k)\right) - 
     \operatorname{Ei}\left(\frac{5}{4} \log\log x\right)\right), \mathrm{\ for\ some\ } 
     c \in [x, x+k] \\ 
     & \ll \frac{25}{64} (x+k)^{3/4} (\log\log(x+k))^2 + 
     \frac{15}{16} x^{3/4} (\log\log x) \\ 
     & \sim x^{3/4} (\log\log x)^{2} \left[\frac{25}{64} \left(1+\frac{k}{x}\right)^{3/4}  
     \left(1 + \frac{k}{x (\log x) (\log\log x)}\right)^2 + 
     \frac{15}{16 (\log\log x)}\right] \\ 
     & \ll \frac{25 \cdot 2^{3/4}}{64} x^{3/4} (\log\log x)^{2} (1 + o(1)). 
\end{align*} 
So it follows that 
\begin{align*} 
M_{\pm}(x) & \gg \frac{25 \cdot 2^{3/4}}{64} (\log x)^{3/4} (\log\log\log x)^{2} \times 
     \frac{(\log x)^{5/4}}{(\log\log x)^{1/4} \sqrt{\log\log\log x}} \times \\ 
     & \phantom{\gg\ } \qquad \times 
     \frac{1}{2} \cdot \#\left\{\log x \leq d < \frac{(\log x)^{5} (\log\log x)}{e}: 
     \varepsilon_{x,d+1} \lambda(d+1) = -\varepsilon_{x,d} \lambda(d)\right\} \\ 
     & \gg \frac{25}{64 \cdot 2^{1/4}} 
     \frac{(\log x)^{2} (\log\log\log x)^{3/2}}{(\log\log x)^{1/4}}. 
\end{align*} 
This lower bound clearly exceeds the bound for $S_E(x)$ stated in 
\eqref{eqn_proof_tag_SE1xFuncExp_v1}. 
\end{proof} 
