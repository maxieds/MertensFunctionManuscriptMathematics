...
\newglossaryentry{coeffExtraction}{
    symbol={\ensuremath{[q^n] F(q)}, OGF},
    sort={coeffExtraction},
    description={The coefficient of $q^n$ in the power series expansion of $F(q)$ about zero when 
    $F(q)$ is treated as the ordinary generating function (or OGF) of some sequence, $\{f_n\}_{n \geq 0}$. 
...
\newglossaryentry{MoebiusMuFunc}{
    ...
    description={The M\"obius function defined such that $\mu^2(n)$ is the indicator function of the 
                 ...
                 The Mertens function is the summatory function of $\mu(n)$ defined for all integers 
...
\newglossaryentry{epsilonN}{
    ...
    description={The multiplicative identity with respect to Dirichlet convolution, $\varepsilon(n) := \delta_{n,1}$, 
                 defined such that for any arithmetic $f$ we have that 
                 $f \ast \varepsilon = \varepsilon \ast f = f$ where $\ast$ denotes Dirichlet convolution 
...
\newglossaryentry{fInvn}{
    ...
    sort={fInvn},
    description={
     The Dirichlet inverse $f^{-1}$ of $f$ exists 
     if and only if $f(1) \neq 0$. 
     The Dirichlet inverse of any $f$ such that $f(1) \neq 0$ 
     with respect to convolution is defined recursively by 
...
\newglossaryentry{CkngInvAuxFunc}{
    ...
    sort={CkngInvAuxFunc},
    description={The sequence is defined recursively for $n \geq 1$ as follows: 
                 \[
                 C_k(n) := \begin{cases} 
...
\newglossaryentry{PikPiHatkx}{
    ...
    description={For integers $k \geq 1$, the 
                 ...
                 $\widehat{\pi}_k(x) := \#\{2 \leq n \leq x: \Omega(n) = k\}$ for $x \geq 2$. },
    type={symbols},
...
\newglossaryentry{Nupn}{
    ...
    description={The valuation function that extracts the maximal exponent of $p$ in the prime factorization of $n$, e.g., 
                 ...
                 (e.g., when $p^{\alpha}$ exactly divides $n$) for $p$ prime, $\alpha \geq 1$ and $n \geq 2$.},
    type={symbols},
...
\newglossaryentry{LiouvilleLambdaFunc}{
     symbol={$\lambda(n), L(x), \lambda_{\ast}(n)$}, 
    ...
    description={The Liouville lambda function is the completely multiplicative function defined by 
                 ...
                 For positive integers $n \geq 2$, we define $\lambda_{\ast}(n) := (-1)^{\omega(n)}$. 
...
\newglossaryentry{AverageOrderExpectation}{
    ...
    description={We use the expectation notation of $\mathbb{E}[f(x)] = h(x)$ 
                 ...
                 of $h(x)$. This means that $\frac{1}{x} \sum_{n \leq x} f(n) \sim h(x)$. }, 
    type={symbols},
...
\newglossaryentry{GGLLRelations}{
    ...
    description={
                 ...
                 When we have that $A \ll B$ and $B \ll A$, we write $A \asymp B$. },
    type={symbols},
...
The inverse function sequence $\{g^{-1}(n)\}_{n \geq 1}$ 
...
unsigned sequence $|g^{-1}(n)|$ over $n \leq x$ with a limiting central limit theorem type 
tendency towards normal as $x \rightarrow \infty$. 
For all $x \geq 1$, discrete convolutions of 
...
(other than one when $n \geq 2$) which are 
...
\frac{1}{\zeta(s)} = \prod_{p} \left(1 - \frac{1}{p^s}\right) = 
     s \cdot \int_1^{\infty} \frac{M(x)}{x^{s+1}} dx, \text{\ for\ } \Re(s) > 1, 
\]
we obtain that 
\[
...
bounding $M(x)$ from above for large $x$ in the following form \cite{SOUND-MERTENS-ANNALS}: 
\begin{align*} 
...
equivalent characterization of $M(x)$ formed by constructing 
...
The prime-related combinatorics at hand are discussed by the remarks 
...
$G^{-1}(x) := \sum_{n \leq x} g^{-1}(n)$ with the 
prime counting function $\pi(x)$ proved in 
Section \ref{Section_KeyApplications}. 

The sequence $g^{-1}(n)$ defined precisely below and 
...
The definitions of the primary subsequences we define, and the corresponding 
parameterized bivariate DGF based proof methods that are given 
in the spirit of Montgomery and Vaughan's work, 
...
magnitude of $|g^{-1}(n)|$ that is analogous to the Erd\H{o}s-Kac theorems for 
$\omega(n)$ and $\Omega(n)$. 

Since we prove that $\operatorname{sgn}(g^{-1}(n)) = \lambda(n)$, it follows 
...
as understanding the distribution of $M(x)$ well for large $x$. 
The new results in this article then precisely 
connect the distributions of $L(x)$, a well defined 
...
through the sums of auxiliary subsequences $C_k(n)$  
in Section \ref{Section_InvFunc_PreciseExpsAndAsymptotics}. 
...
the average order of the unsigned sequence is given by 
\[
\mathbb{E}|g^{-1}(n)| \sim \frac{6}{\pi^{2}} (\log n)^2 \sqrt{\log\log n}. 
...
$\mu_x(C) := \log\log x$ and 
$\sigma_x(C) := \sqrt{\log\log x}$, 
that holds uniformly for any $-Y \leq y \leq Y$ 
as $x \rightarrow \infty$ (see Corollary \ref{cor_CLT_VII}): 
\[
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: |g^{-1}(n)| - \frac{6}{\pi^2} 
     \mathbb{E}|g^{-1}(n)| \leq y\right\} = 
     \Phi\left\{\frac{6 \sigma_x(C)}{\pi^2}\left(\frac{\pi^2 y}{6} + \sigma_x(C) - 
     \log\left(2\right)\right)\right\} + 
...
\tag{D}
     ...
     \frac{6}{\pi^2}\left(\log\left(2\right) - \sigma_x(C)\right) 
     \right) & = \frac{1}{2} + o(1) \\ 
\tag{E} 
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     \frac{6}{\pi^2}\left(\alpha + \log\left(2\right) - \sigma_x(C)\right)
     \right) & = 
...
on an infinite set of the integers with asymptotic density one. 
...
$M(x)$ along large infinite subsequences 
by sharply bounding the summatory function $G^{-1}(x)$. 
...
Theorem \ref{cor_ExprForGInvxByLx_v1} proves that for almost every sufficiently large $x$\footnote{
     By \emph{almost every} large integer $x$, we mean that the result holds for all large $x$ 
     taken within an infinite subset of $\mathbb{Z}^{+}$ with asymptotic density one. 
} there exists some $1 \leq t_0 \leq x$ such that 
\[
G^{-1}(x) = O\left(L(t_0) \cdot \mathbb{E}|g^{-1}(x)|\right).
\]
If the RH is true, then 
for any $\varepsilon > 0$ and sufficiently large $x > e$ we have that  
\[
G^{-1}(x) = O\left((\log x)^2 \sqrt{\log\log x} \sqrt{x} \times \exp\left(
     \sqrt{\log x} (\log\log x)^{\frac{5}{2}+\varepsilon}\right) 
     \right). 
\]
...
It is not difficult to prove the related identity that
\[
...
$(U)_{i,j} = \delta_{i+1,j}$ and such that 
\[
...
by a vector product with the inverse matrix from the previous equation by the formula 
\begin{align*} 
...
distinct prime factors of $n$. We can easily prove using DGFs 
...
\emph{Dirichlet generating function} (or DGF) of any 
...
\]
where we adopt the notation that for $n \geq 2$, 
$C_{\Omega(n)}(n) = (\Omega(n))! \times \prod_{p^{\alpha} || n} \frac{1}{\alpha!}$, 
where the sequence is taken to be one at $n := 1$. 
...
of these arithmetic functions towards their respective average order. 
...
For $0 < R < 2$ we have that uniformly for all $1 \leq k \leq R \cdot \log\log x$ 
\[
...
We can extend the work in \cite{MV} on the distribution of $\Omega(n)$ to see that 
for $0 < R < 2$ 
\begin{equation}
...
\pi_k(x) = \widehat{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) 
     ...
     \right], 
     \mathrm{\ unif.\ for\ } 1 \leq k \leq R\log\log x. 
\end{equation}
...
we take 
\[
\widehat{F}(s, z) := \prod_p \left(1 + \frac{z}{p^s-1}\right)^{-1} \left(1 - \frac{1}{p^s}\right)^{z}, 
...
With the next corollary, we can accurately approximate asymptotic order of 
the sums $\mathcal{A}_{\omega}(x)$ (defined below) 
for large $x$ by only considering the truncated sums 
$\mathcal{D}_{\omega}(x)$ where we have the known uniform bounds on the summands for 
$1 \leq k \leq \log\log x$. 
This result is cited in the proof of our crucial new result stated in 
Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2} of 
Section \ref{Section_NewFormulasForgInvn}. 
The careful justification of these properties is in fact essential to 
establishing several results and new theorems rigorously in 
Section \ref{Section_NewFormulasForgInvn}. 
The notation for the next sums using the subscripted $\omega$ denotes that 
we are summing over the densities $\pi_k(x)$ corresponding to the number 
of $2 \leq n \leq x$ such that $\omega(n) = k$. 
...
\mathcal{D}_{\omega}(x) & =  
     ...
     & = \frac{x}{\sqrt{2\pi \log\log x}} + O(E_{\omega}(x)), 
\end{align*}
...
$\widehat{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) \gg 1$ for all 
...
\end{align*}
The right-hand-side expression in the previous equation follows 
by applying Lemma \ref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2}. 

Next, we utilize the notation for the function $D(x, r)$ from 
Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} 
to bound the function $\mathcal{N}_{\omega}(x)$ as\footnote{
     Using a modification of the argument based on an elementary inequality for the 
     natural logarithm we cite in the proof of 
     Lemma \ref{lemma_HatCAstxSum_ExactFormulaWithError_v1} in 
     Section \ref{subSection_AvgOrdersOfTheUnsignedSequences}, 
     we can actually show a stronger bound holds for the integral in the next equations. 
     Namely, for any $\delta \in (0, 1)$ we have that 
     \[
     \int_1^2 (\log x)^{r-1-r\log r} dr \ll \frac{1}{(\log x)^{\delta+1}}. 
     \]
}
\begin{align*}
\frac{1}{x} \times \left\lvert \mathcal{N}_{\omega}(x) \right\rvert & \leq 
     ...
     & \ll \int_{1}^{2} D(x, r) dr + \#\{n \leq x: \omega(n) \geq 2\log\log x\} \\ 
     & \ll \int_1^2 (\log x)^{r-1-r\log r} dr + (\log x)^{1-2\log 2} \\ 
     & \ll \frac{1}{(\log x)^{0.3862}}. 
\end{align*} 
Then we see that 
\[
\left\lvert \frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \right\rvert \ll 
     \frac{\sqrt{\log\log x}}{(\log x)^{0.3862}} = o(1), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
Equivalently, we have shown that $\mathcal{N}_{\omega}(x) = o\left(\mathcal{D}_{\omega}(x)\right)$. 
...
\newpage
\section{Auxiliary sequences expressing the Dirichlet inverse function $g^{-1}(n)$} 
...
$C_{\Omega(n)}(n)$ to prove an Erd\H{o}s-Kac like analog that characterizes 
...
\begin{lemma}[An initial exact formula for $g^{-1}(n)$] 
...
\[
\mu(n) = g^{-1}(n) + \sum_{p|n} g^{-1}\left(\frac{n}{p}\right), n \geq 1. 
\]
\end{remark} 
...
for $h^{-1}(n) = \lambda(n) \cdot (\Omega(n))! \times \prod_{p^{\alpha} || n} (\alpha !)^{-1}$ 
we discovered in the proof of the key signedness proposition from 
Section \ref{Section_PrelimProofs_Config} 
suggests additional patterns and more regularity in the contributions of the distinct weighted 
terms in the summands of $G^{-1}(x)$. 
A preliminary analysis suggests that bounds of this type 
may improve upon those we are able to prove for $G^{-1}(x)$ in 
...
cited in Table \ref{table_conjecture_Mertens_ginvSeq_approx_values}. 
...
\subsection{Analytic proofs and adaptations of DGF methods for summing additive functions} 
...
$\widehat{F}(s, z) \cdot \zeta(s)^{z}$ correspond to 
\[
\widehat{A}_z(x) := \sum_{n \leq x} (-1)^{\omega(n)} 
     C_{\Omega(n)}(n) z^{\Omega(n)}. 
\]
We have that for all sufficiently large $x \geq 2$ and any $|z|< P(2)^{-1}$ 
\[
\widehat{A}_z(x) = \frac{x}{\Gamma(z)} \cdot \widehat{F}(2, z) \cdot (\log x)^{z-1} + 
     O_{z}\left(x \cdot (\log x)^{\Re(z) - 2}\right). 
\]
\end{theorem} 
\begin{proof} 
We can see from the proof of 
Proposition \ref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1} 
that 
\[
C_{\Omega(n)}(n) = \begin{cases} 
...
\sum_{n \geq 1} \frac{C_{\Omega(n)}(n)}{(\Omega(n))!} \cdot 
     ...
     = \exp\left(-z \cdot P(s)\right), \Re(s) \geq 2 \wedge \Re(P(s)z) > -1. 
\end{align*} 
This product based expansion is similar in construction to the parameterized bivariate 
DGF used in the reference \cite[\S 7.4]{MV}.
...
the DGF expansion of $\widehat{F}(s, z) \cdot \zeta(s)^{z}$, then 
\[
\left\lvert \sum_{n \geq 1} \frac{b_z(n) (\log n)^{2R+1}}{n^s} \right\rvert < +\infty, 
     \Re(s) \geq 2
\]
is uniformly bounded for $|z| \leq R < +\infty$. This fact follows by repeated 
...
\]
We set 
$b_z(n) := (-1)^{\omega(n)} C_{\Omega(n)}(n) z^{\Omega(n)}$, define the convolution function 
...
\label{eqn_proof_tag_Azx_FullTermsFormulaSum_v1} 
     ...
     O\left(\sum_{m \leq x} \frac{x |b_z(m)|}{m^2} \times m \cdot 
...
sum components when we take $|z| \leq R$ as follows: 
\begin{align*} 
\sum_{m \leq x/2} b_z(m) D_z(x/m) & = \frac{x}{\Gamma(z)} (\log x)^{z-1} \times 
     ...
     & = \frac{x}{\Gamma(z)} (\log x)^{z-1} \widehat{F}(2, z) + O_R\left( 
     x (\log x)^{\Re(z)-2} \times \sum_{m \geq 1} \frac{b_z(m) (\log m)^{2R+1}}{m^2} 
     \right) \\ 
     & = \frac{x}{\Gamma(z)} (\log x)^{z-1} \widehat{F}(2, z) + O_{R}\left( 
     x (\log x)^{\Re(z)-2}\right). 
     \qedhere  
\end{align*} 
\end{proof} 

It is necessary to break the next theorem into cases for the 
$1 \leq k < \log\log x$ where we have uniform bounds from 
Theorem \ref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn}. 
This necessity arises from the bounds on the incomplete gamma function 
we established in Section \ref{subSection_OtherFactsAndResults} 
to handle the subcases of the asymptotics for $\Gamma(a, z)$ at real $a, z > 0$ 
as $z \rightarrow \infty$. Namely, we are forced to account for an extra 
factor of $(1-z/a)^{-1}$ when $z = \lambda a$ for some $\lambda > 1$, whereas 
we inherit a simpler asymptotic formula to approximate 
$\Gamma(a, z)$ when $a$ is taken to be a fixed parameter that does not vary, 
or tend to infinity, when $z$ does. 

\begin{definition}
Suppose that we define two disjoint intervals, $\mathcal{I}_{1,x}$ and 
$\mathcal{I}_{2,x}$, such that $\mathcal{I}_{1,x} \cup \mathcal{I}_{2,x} = 
\{1 \leq k < \log\log x: k \in \mathbb{Z}\}$ and where the first 
interval (depending on $x$) corresponds to these $k$ in the 
uniform range such that $k \not{\alpha} \log\log x$ and the second 
corresponds to the $k$ such that $k$ is proportional to $\log\log x$ 
by a constant factor $\lambda > 1$. 
For $x > e$ and $1 \leq k \leq \log\log x$, we define 
$\lambda \equiv \lambda_{x,k} := \frac{\log\log x}{k}$. 
At sufficiently large $x \rightarrow \infty$, we consider the two cases 
\begin{align*}
\mathcal{I}_{2,x} & := \{1 \leq k < \log\log x: \lambda_{x,k} > 1\} \\ 
\mathcal{I}_{1,x} & := \{1 \leq k < \log\log x: k \in \mathbb{Z}\} \mathbin{\big\backslash} 
     \mathcal{I}_{2,x}. 
\end{align*}
Theorem \ref{theorem_CnkSpCasesScaledSummatoryFuncs} 
proves separate cases of uniform asymptotics with respect to $k < \log\log x$ 
partitioned into the two subintervals defined above as $x \rightarrow \infty$. 
\end{definition}

\begin{theorem} 
\label{theorem_CnkSpCasesScaledSummatoryFuncs} 
The next uniform asymptotics for the summatory function 
$\widehat{C}_{k,\ast}(x)$ defined below hold for all sufficiently 
large $x > e$. 
\[
...
$|z| < 1$. 
For $k \in \mathcal{I}_{1,x}$, we have that 
\[
\widehat{C}_{k,\ast}(x) = -\widehat{G}\left(\frac{k-1}{\log\log x}\right) \frac{x}{\log x} 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \left[ 
     1 + O_k\left(\frac{k}{(\log\log x)^2}\right)\right]. 
\]
On the other hand we have uniformly for all 
$\inf \mathcal{I}_{2,x} \leq k < \log\log x$ that 
\[
\widehat{C}_{k,\ast}(x) = -\widehat{G}\left(\frac{k-1}{\log\log x}\right) \frac{x}{\log x} 
     \frac{(\log\log)^{k-1}}{(k-1)! \left(1+\lambda_{x,k}^{-1}\right)} \left[ 
     1 + O_k\left(\frac{k}{(\log\log x)^2}\right)\right]. 
\]
\end{theorem} 
\begin{proof} 
...
At large $x$, the error term from this bound contributes that is bounded from above by 
\begin{align*} 
...
\end{align*} 
We find an asymptotically accurate main term approximation to the coefficients 
...
\widetilde{A}_r(x) := 
     -\int_{|v|=r} \frac{x \cdot (\log x)^{-v}}{(\log x) \Gamma(1+v) \cdot 
     v^{k} (1 - P(2) v)} dv. 
\end{align} 
The main term for the sums $\widehat{C}_{k,\ast}(x)$ 
is given by $-\frac{x}{\log x} \cdot I_k(r, x)$, where we take 
\begin{align*}
I_k(r, x) & = \frac{1}{2\pi\imath} \times \int_{|v|=r} 
     \frac{\widehat{G}(v) (\log x)^{-v}}{v^k \cdot (1 - P(2)v)} dv \\ 
...
I_{1,k}(r, x) & := \frac{\widehat{G}(r)}{2\pi\imath} \times \int_{|v|=r} 
     \frac{(\log x)^{-v}}{v^k \cdot (1-P(2)v)} dv. 
\end{align*}
We can inductively compute the remaining coefficients 
$[z^k] I_{1,k}(r, x)$ with respect to 
$x$ for fixed $k \leq \log\log x$ to apply the Cauchy integral formula. 
It is not difficult to see that for any integer $m \geq 0$, 
we have the $m^{th}$ partial derivative of the scaled integrand with respect to $z$ 
has the following limiting expansion by applying 
\eqref{eqn_IncompleteGamma_PropB} and Proposition \ref{prop_IncGammaLambdaTypeBounds_v1}, 
respectively, at fixed $m$ and large $x$ or where 
$\log\log x = \lambda k$ for some $\lambda > 1$ as $x \rightarrow \infty$: 
\begin{align*} 
\frac{1}{m!} \times \frac{\partial^{(m)}}{{\partial v}^{(m)}}\left[ 
     \frac{(\log x)^{-v}}{1 - P(2) v}\right] \Biggr\rvert_{v=0} & = 
     \sum_{j=0}^{m} \frac{(-1)^{m-j} P(2)^{j} (\log\log x)^{m-j}}{(m-j)!} \\ 
     & = 
     \frac{P(2)^{m} (\log x)^{\frac{1}{P(2)}}}{m!} \times 
     \Gamma\left(m+1, -\frac{\log\log x}{P(2)}\right) \\ 
     & = \begin{cases}
     \frac{(\log\log x)^{m}}{m!}, & \text{if $m+1 \in \mathcal{I}_{1,x}$; } \\ 
     \frac{(\log\log x)^{m}}{m! \left(1 + \lambda_{x,k}^{-1}\right)}, 
          & \text{if $m+1 \in \mathcal{I}_{2,x}$; }
     \end{cases} + O\left(\frac{(\log\log x)^{m-1}}{m!}\right). 
\end{align*} 
Note that we have restricted the asymptotic analysis of the limiting dominant terms in 
the above formula to cases of $m + 1 < \log\log x$. 
We see by taking $v = r = \frac{k-1}{\log\log x}$ that 
\[
I_{1,k}(r, x) \sim \frac{\widehat{G}(r) (\log\log x)^{k-1}}{(k-1)!}. 
\]
...
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} (\widehat{G}(v) - \widehat{G}(r)) 
     \frac{(\log x)^{-v}}{z^k \cdot (1-P(2)v)} dv. 
\]
After integrating by parts, we write that 
\[
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} (\widehat{G}(v) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(v-r)) 
     (\log x)^{-v} \left[\sum_{i \geq 0} v^{i-k} P(2)^{i}\right] dz. 
\]
Notice that 
\[
\widehat{G}(v) - \widehat{G}(r) - \widehat{G}^{\prime}(r)(v-r) = \int_{r}^{v} 
     (v-w) \widehat{G}^{\prime\prime}(w) dw \ll |v-r|^2. 
\]
We then define component integrands for $I_{2,k}(r, x)$ as follows for any integers $i \geq 0$: 
\[
T_{k,i}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} (\widehat{G}(v) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(v-r)) (\log x)^{-v} v^{i-k} dz.  
\]
With the parameterization $z = r e^{2\pi\imath\theta}$ for real $\theta \in [-1/2,1/2]$, 
we get that 
\[
T_{k,i}(r, x) \ll r^{3-k+i} \int_{-1/2}^{1/2} (\sin \pi\theta)^2 e^{(k-i-1) \cos(2\pi\theta)} d\theta. 
\]
Since $|\sin x| \leq |x|$ for all $|x| < 1$ and $\cos(2\pi\theta) \leq 1 - 8\theta^2$ whenever 
$-1/2 \leq \theta \leq 1/2$, we obtain bounds of the next forms by setting $r := \frac{k-1}{\log\log x}$. 
\begin{align*}
T_{k,i}(r, x) & \ll r^{3-k+i} e^{k-i-1} \times \int_0^{\infty} \theta^2 e^{-8(k-i-1) \theta^2} d\theta \\ 
     & \ll \frac{r^{3+i-k} e^{k-i-1}}{(k-i-1)^{3/2}} \ll 
     \frac{(\log\log x)^{k-3-i} e^{k-i-1}}{(k-1-i)^{3/2} (k-1)^{k-3-i}} \\ 
     & \ll 
     \frac{k \cdot (\log\log x)^{k-3-i}}{(k-1)!}. 
\end{align*}
Then it follows that with $r := \frac{k-1}{\log\log x}$, the sums 
\[
\sum_{i \geq 0} |T_{k,i}(r, x)| P(2)^i \ll \frac{k \cdot (\log\log x)^{k-3}}{(k-1)!} (1+o(1)). 
\]
Finally, we see that whenever $1 \leq k \leq \log\log x$, we have 
\[
\widehat{G}\left(\frac{k-1}{\log\log x}\right) = \frac{1}{\Gamma\left(1+\frac{k-1}{\log\log x}\right)} \cdot 
     \frac{\zeta(2)^{(1-k)/\log\log x}}{\left(1+\frac{(k-1)}{\log\log x}\right)} \gg 1. 
\]
...
\widehat{C}_k(x) := 
     ...
     \frac{2x}{(2k+1)} \cdot \frac{(\log\log x)^{k+1/2}}{(k-1)!}. 
\end{align*} 
\end{cor} 
We will handle the proof of the cases where $k \in \mathcal{I}_{2,x}$ as these are 
more involved in complete detail. 
After this proof is complete, the corresponding method for those 
$k \in \mathcal{I}_{1,x}$ follows as a special case. 
\begin{proof} 
We have an integral formula involving the unsigned summand 
...
in \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1} be defined precisely 
...
we note that $\widehat{G}((k-1)/\log\log x) \sim e^{o(1)}$ as 
$x \rightarrow \infty$. 
...
$\lambda_{\ast}(n) \cdot C_{\Omega(n)}(n)$ such that $\Omega(n) = k$ to approximate 
the same sum over the unsigned summands $C_{\Omega(n)}(n)$. 
The argument is based on approximating $L_{\ast}(t)$ for large $t$ 
...
$1 \leq k \leq \log\log x$\footnote{
     ...
     the function $\widehat{\mathcal{G}}(z)$ defined in 
     Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} satisfies 
     \[ 
     \widehat{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) = e^{o(1)} \xrightarrow{x \rightarrow \infty} 1. 
     \]
}:
\[
\pi_k(x) = \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} (1 + o_k(1)), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
\begin{align*} 
L_{\ast}(t) & := \left\lvert \sum_{n \leq t} (-1)^{\omega(n)} \right\rvert \sim 
     \left\lvert \sum_{k=1}^{\log\log t} (-1)^{k} \pi_k(x) \right\rvert \sim 
     \frac{t}{\sqrt{2\pi \log\log t}}, 
     \mathrm{\ as\ } t \rightarrow \infty. 
\end{align*} 
The main term for the reciprocal of the derivative of the main term approximation of this 
summatory function is given by computation as 
\[
\frac{1}{L_{\ast}^{\prime}(t)} \sim 2 \sqrt{\log\log t}. 
\]
We apply the formula from \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1},  
...
\begin{align*} 
     ...
     2\widehat{C}_{k,\ast}^{\prime}(x) \sqrt{\log\log x} (1+o(1)) 
     =: \widehat{C}_{k,\ast\ast}(x). 
\end{align*} 
There is a technical detail that complicates our analysis in so much as the reciprocal 
factor of $1 + \lambda_{x,k}^{-1}$ in the formulas for 
$\widehat{C}_{k,\ast}(x)$ from 
Theorem \ref{theorem_CnkSpCasesScaledSummatoryFuncs}, 
while essentially only impacting by a constant factor difference, depend non-trivially on $x$. 
We are careful in taking the derivatives in these cases before applying 
integration by parts below. In particular, we have that since $\lambda_{x,k}^{-1} < 1$, 
\begin{align*}
     \widehat{C}_{k,\ast\ast}(x) & \sim 
     \frac{2\sqrt{\log\log x}}{(k-1)!} \times \sum_{n \geq 0} 
     (-k)^n \left[ 
     \frac{(\log\log x)^{k-n-1}}{(\log x)}\left(1 - \frac{1}{\log x}\right) + 
     \frac{(k-n-1) (\log\log x)^{k-n-2}}{(\log x)^2}\right]. 
\end{align*} 
...
\notag
     & \sim 
     \frac{2x}{(k-1)!} \times \sum_{n \geq 0} 
     (-k)^n \Bigl[ 
     \frac{2(\log\log x)^{k-n+1/2}}{(2k-2n+1)} + 
     \Gamma\left(k-n+\frac{1}{2}, \log\log x\right) \\ 
\notag 
     & \phantom{\sim 
     \frac{2x}{(k-1)!} \times \sum_{n \geq 0} 
     (-k)^n \ } - 
     (k-n+1) \Gamma\left(k-n-\frac{1}{2}, \log\log x\right)
     \Bigr]. 
\end{align}
To obtain a main term estimate of the quality we have seen with the last few theorems, 
notice that it suffices to truncate the infinite series after $n := 0$. 
Then we obtain from Proposition \ref{prop_IncGammaLambdaTypeBounds_v1} that 
\begin{align*}
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & \sim
     \frac{2x}{(2k+1)} \cdot \frac{(\log\log x)^{k+1/2}}{(k-1)!}. 
     \qedhere 
...
\subsection{Average order of the unsigned sequences}
...
\[
\frac{1}{n} \times \mathbb{E}\left[C_{\Omega(n)}(n)\right] = 
     \frac{2(\log n) \sqrt{\log\log n}}{n} + o(1). 
\] 
\end{lemma} 
\begin{proof} 
...
Lemma \ref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v3}:
\begin{align} 
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v3} 
\sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \sim 
     2x (\log x) \sqrt{\log\log x}. 
\end{align} 
We claim that 
\begin{align} 
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v1} 
\frac{1}{x^2} \times \sum_{n \leq x} C_{\Omega(n)}(n) & = \frac{1}{x^2} \times 
     \sum_{k \geq 1} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) \\ 
\notag 
     & = 
     \frac{1}{x^2} \times \sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} 
     C_{\Omega(n)}(n) + o(1), \text{ as } x \rightarrow \infty. 
\end{align} 
...
\frac{1}{x^2} \times 
     \sum\limits_{\log\log x < k \leq \log x} \sum\limits_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n)
     = o\left(\frac{(\log x)^{3.5093} \sqrt{\log\log x}}{\sqrt{x}}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{equation} 
We know from Theorem \ref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn} 
that for all sufficiently large $x$ 
\[
\sum_{n \leq x} (-1)^{\omega(n)} C_{\Omega(n)}(n) z^{\Omega(n)} = 
     x \frac{\widehat{F}(2, z)}{\Gamma(z)} (\log x)^{z-1} + O_z\left( 
     x (\log x)^{\Re(z)-2}\right). 
\]
By Lemma \ref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2}, 
we have that the summatory function 
\[
\left\lvert \sum_{n \leq x} (-1)^{\omega(n)} \right\rvert \asymp 
     \frac{x}{\sqrt{\log\log x}}. 
\]
We can argue as in the proof of Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2} 
by integration by parts in the Abel summation formula that whenever $z > 1$ 
and $x > e$ is sufficiently large 
\begin{align*}
\sum_{n \leq x} C_{\Omega(n)}(n) z^{\Omega(n)} & \ll \frac{\widehat{F}(2, z)}{\Gamma(z)} \times 
     ...
     \Biggr] \\ 
     & \ll \frac{x \widehat{F}(2, z)}{\Gamma(1+z)} \sqrt{\log\log x} (\log x)^{z}. 
\end{align*}
For all large enough $x > e$, we define 
\[
\widehat{B}(x, r) := \#\{n \leq x: C_{\Omega(n)}(n) \geq r \cdot \log\log x\}. 
\]
We argue as in the proof from the reference \cite[Thm.\ 7.20; \S 7.4]{MV} that for 
$1 \leq r < P(2)^{-1}$ 
\begin{align*}
\widehat{B}(x, r) & \ll x (\log x)^{r \log r} \times \sum_{n \leq x} C_{\Omega(n)}(n) r^{\Omega(n)} \\ 
     & \sim \frac{x \widehat{F}(2, z)}{\Gamma(1+z)} \sqrt{\log\log x} (\log x)^{r-r\log r}. 
\end{align*}
Since $\widehat{F}(2, r) = \frac{\zeta(2)^{-r}}{1+P(2)r} \ll 1$ for $r \in [1, P(2)^{-1})$, and 
similarly we have that $\frac{1}{\Gamma(1+r)} \gg 1$ for $r$ taken within this same range, 
we get that 
\[
\widehat{B}(x, r) \ll x \sqrt{\log\log x} \times (\log x)^{r-r\log r}, \text{ for all } 1 \leq r < P(2)^{-1}. 
\]
We wish to evaluate the limiting asymptotics of the sum 
\begin{align*}
S_2(x) & := \frac{1}{x \sqrt{\log\log x}} \times \sum_{k \geq \log\log x} \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     C_{\Omega(n)}(n) \\ 
     & \ll \frac{1}{x \sqrt{\log\log x}} \times \bigl[ 
     \#\left\{n \leq x: \log\log x \leq C_{\Omega(n)}(n) < P(2)^{-1} \log\log x\right\} \times 
     3 \log\log x \\ 
     & \phantom{\ll\frac{1}{x \sqrt{\log\log x}} \times \bigl[\ } + 
     \#\left\{n \leq x: C_{\Omega(n)}(n) \geq P(2)^{-1} \log\log x\right\} 
     \times \Gamma(P(2)^{-1} \log\log x + 1)\bigr] \\ 
     & \ll \int_1^{P(2)^{-1}} (\log x)^{r-r\log r} dr + \lim_{r \rightarrow P(2)^{-1}} \left[
     (\log x)^{r-r\log r} \times \left(\frac{P(2)^{-1} \log\log x}{e}\right)^{P(2)^{-1} \log\log x} 
     \right] \\ 
& \ll \int_1^{P(2)^{-1}} (\log x)^{r-r\log r} dr + o\left(\sqrt{x} \cdot (\log x)^{3.5093}\right). 
\end{align*} 
The $o(1)$ term in the previous equation follows from the fact that 
\[
\lim_{x \rightarrow \infty} \frac{1}{\sqrt{x}} \times (\log\log x)^{3\log\log x} = 0. 
\]
We claim that the integral term in the previous equation is $\ll (\log x)^{15/16}$. 
Indeed, for any $n > 0$ and $r > 0$, we have by an elementary inequality for the natural logarithm 
function that $-r \log r \geq -nr(r^{1/n}-1)$. 
Hence, we can integrate the following series termwise for $(a, b) := (1, P(2)^{-1})$: 
\begin{align*}
\int_a^b (\log x)^{r-r\log r} dr & \leq \int_a^b \left[\sum_{k \geq 0} \sum_{s=0}^{k} 
     \binom{k}{s} \frac{(\log\log x)^{k}}{k!} \times (-n)^s (1+n)^{k-1} r^{k-s+\left(1+\frac{1}{n}\right)s}\right] dr \\ 
     & = \frac{1}{r \cdot (\log x)^{r(r^{1/n}-1-n)}} \Biggr\rvert_{r=a}^{r=b} \\ 
     & \ll (\log x)^{15/16}. 
\end{align*} 
The last step is obtained by evaluating $r$ at the lower limit of $r = a = 1$ and setting 
$n := \frac{15}{16}$. 
In conclusion, we have proved that $\frac{S_2(x)}{x} = o(1)$ as $x \rightarrow \infty$. 
\end{proof} 
...
\begin{align*} 
\mathbb{E}|g^{-1}(n)| & = \frac{6}{\pi^{2}} (\log n)^2 \sqrt{\log\log n} + 
     O\left(\frac{\log n}{\sqrt{\log\log n}}\right). 
\end{align*} 
\end{cor} 
\begin{proof} 
We use the formula from Lemma \ref{lemma_HatCAstxSum_ExactFormulaWithError_v1} 
to find $\mathbb{E}[C_{\Omega(n)}(n)]$ as $n \rightarrow \infty$.
This result implies that for sufficiently large $t$ 
\begin{align*} 
\int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = 
     (\log t)^2 \sqrt{\log\log t} + 
     O\left(\frac{\log t}{\sqrt{\log\log t}}\right). 
\end{align*} 
Recall that the summatory function of the 
squarefree integers is approximated for large $x$ by 
\[
...
\notag 
     ...
     \right] \\ 
\notag 
     & \sim \frac{6}{\pi^2} \left(\mathbb{E}[C_{\Omega(n)}(n)] + \sum_{d<n} 
...
Set the mean and variance parameter analogs be defined by 
\[
\mu_x(C) := \log\log x, - \log(2), 
...
\] 
We assert that it suffices to consider the distribution of $\Phi_2(x, z)$ as 
...
\[
|\mu_n(C) - \mu_x(C)| \leq \log\log x - \log\log\sqrt{x} \leq \log 2, 
\]
and 
\[
|\sigma_n(C) - \sigma_x(C)| \leq \sqrt{\log\log x} - 
     \sqrt{\log\log\sqrt{x}} \leq \frac{\log 2}{\sqrt{\log\log x}}. 
\]
...
densities corresponding to these alternate terms. 
The rest of our argument follows the method in the proof of the related theorem in 
\cite[Thm.\ 7.21; \S 7.4]{MV} closely. Readers familiar with the 
reference will see many parallels to those constructions. 
The crux of the remainder of the proof emulates the methods from 
Montgomery and Vaughan. After a change of variable we obtain the limiting 
CLT statement in analog to their analytic proof of the Erd\H{o}s-Kac theorem 
for the distributions of $\omega(n)$ and $\Omega(n)$. 
...
$t_x := \frac{t}{(\log x) \sqrt{\log\log x}}$. 
...
\frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) & 
     \sim 
     \frac{2 (\log\log x)^{k+\frac{1}{2}}}{(2k+1)(k-1)!} 
     \sim 
     \frac{(\log x)}{\sqrt{2\pi}  
     \left(1+\frac{1}{2k}\right)} \cdot e^{t_x} (1+o(1))^{k+\frac{1}{2}} \times 
     \left(1+\delta_{t,x}\right)^{-\mu_x(C)(1+\delta_{t,x})-\frac{1}{2}}. 
\end{align*} 
Notice that 
\begin{align*}
\frac{1}{1+\frac{1}{2k}} & \sim \sum_{m \geq 0} \frac{(-1)^m}{(2 \mu_x(C))^m (1+\delta)^m} 
...
\frac{(\log\log x)^{k+\frac{1}{2}}}{(2k+1)(k-1)!} & \sim 
     \frac{(\log x) \sqrt{\log\log x}}{\sqrt{2\pi} \cdot \sigma_x(C)} 
     \cdot \exp\left(-\frac{t_x^2}{2\sigma_x(C)^2}\right) \times 
     \left[1 + \widetilde{E}(x, t_x)\right]. 
\end{align*} 
It follows that for $1 \leq k \leq \log\log x$ 
\begin{align*}
f(k, x) & = 
     \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \\ 
     & \sim 
     \frac{(\log x) \sqrt{\log\log x}}{\sqrt{2\pi} \cdot \sigma_x(C)} 
     \cdot \exp\left(-\frac{(k-\mu_x(C))^2}{2(\log x)^2 (\log\log x) \sigma_x(C)^2}\right) \times 
     \left[1 + \widetilde{E}\left(x, \frac{|k - \mu_x(C)|}{(\log x) \sqrt{\log\log x}}\right)\right]. 
\end{align*}
Since our target probability density function approximating the PDF (in $t$) of the 
normal distribution is given by 
$$\frac{f(k, x)}{(\log x) \sqrt{\log\log x}} 
  \rightarrow \frac{1}{\sqrt{2\pi} \cdot \sigma_x(C)} \times \exp\left(-\frac{t^2}{2
  \sigma_x(C)^2}\right),$$ 
we perform the change of variable $s \mapsto (\log x) \sqrt{\log\log x} \cdot t$ to obtain the normalized 
...
limiting formula corresponds to a main term given by the standard normal CDF at $z$. 
\end{proof} 
...
For $Y > 0$ and all $-Y \leq z \leq Y$ 
we have uniformly that as $x \rightarrow \infty$
\begin{align*} 
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq z\right\} & = 
     \Phi\left\{\frac{6 \sigma_x(C)}{\pi^2}\left(\frac{\pi^2 z}{6} + \sigma_x(C) - 
     \log\left(2\right)\right)\right\} + o(1). 
\end{align*} 
\end{cor} 
\begin{proof} 
We claim that 
...
The result in (A) follows since $\Phi(0) = \frac{1}{2}$. 
\end{proof} 
...
\[
\lim_{x \rightarrow \infty}\ \frac{1}{x} \cdot \#\left\{n \leq x: 
...
The most recent known upper bound on $L(x)$ (assuming the RH) is established by 
...
$$G^{-1}(x) = O\left(L(t_0) \cdot \mathbb{E}|g^{-1}(x)|\right).$$ 
If the RH is true, then 
for any $\varepsilon > 0$ and all large integers $x > e$ 
\[
G^{-1}(x) = O\left((\log x)^2 \sqrt{\log\log x} \sqrt{x} \times \exp\left( 
     \sqrt{\log x} (\log\log x)^{\frac{5}{2} + \varepsilon}\right)\right). 
\]
\end{theorem} 
\begin{proof} 
...
\label{proof_tag_GInvx_LxAbelSummationIntFormula_v1} 
     & = O\left(|L(t_0)| \cdot \mathbb{E}|g^{-1}(x)|\right), \mathrm{\ for\ some\ } t_0 \in [1, x]. 
\end{align} 
...
\end{align*}
Since we have deduced that the set of $n \leq x$ on which $|g^{-1}(n)|$ is substantially larger 
than its average order is asymptotically thin, 
we find the bounds 
\begin{align*}
...
\int_{x-o(1)}^{x} \left(\frac{\log t}{\log\log t}\right)^{\frac{\log t}{\log\log t} + 
     ...
     m, r = o\left(\frac{(\log x) (\log\log\log x)}{\log\log x}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*}
...
\lim_{x \rightarrow \infty} x^{-\frac{(\log x)(\log\log\log x)}{\log\log x}} 
     (\log x)^{m+r} \\ 
     & = \lim_{x \rightarrow \infty} \exp\left((m+r) \log x - (\log x)^2 
     \frac{\log\log\log x}{\log\log x}\right) 
     = \lim_{t \rightarrow \infty} e^{-t} = 0. 
\end{align*} 
For large $x$, let $\mathcal{R}_x := \{t \leq x: |g^{-1}(t)| \ggg \mathbb{E}|g^{-1}(t)|\}$ such that 
$|\mathcal{R}_x| = o(1)$. 
The formula from \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1} implies 
that for large $x$ and any $m, r = o\left(\frac{(\log x)(\log\log\log x)}{\log\log x}\right)$ 
\begin{align*}
G^{-1}(x) & = O\left(\int L^{\prime}(x) |g^{-1}(x)| dx\right) 
     = O\left(\mathbb{E}|g^{-1}(x)| \times \int L^{\prime}(x) dx + 
     \int_{x-|\mathcal{R}_x|}^x |L^{\prime}(t)| \cdot |g^{-1}(t)| dt\right) \\ 
     & = O\left(\mathbb{E}|g^{-1}(x)| \cdot |L(x)| + 
     o\left(\frac{x}{(\log x)^m (\log\log x)^r}\right)\right). 
\end{align*} 
If the RH is true, by applying Humphries' result in 
\eqref{eqn_LxBigOhAsymptotics_Humphries_v1} in tandem with 
Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2}, 
then for any $\varepsilon > 0$, 
$m,r = o\left(\frac{(\log x) (\log\log\log x)}{\log\log x}\right)$ and 
almost every large integer $x \geq 1$ we have that 
\begin{align*}
G^{-1}(x) & = O\left( 
     (\log x)^2 \sqrt{\log\log x} \cdot \sqrt{x} \times 
     \exp\left(\sqrt{\log x} \cdot (\log\log x)^{\frac{5}{2} + \varepsilon}\right) + 
     o\left(\frac{x}{(\log x)^m (\log\log x)^r}\right) 
     \right), \\ 
     & = O\left((\log x)^2 \sqrt{\log\log x} \cdot x^{\frac{1}{2} + 
     \frac{(\log\log x)^{5/2+\varepsilon}}{\sqrt{\log x}}} + 
     o\left(x^{1-\log\log\log x}\right)\right). 
\end{align*}
To obtain the conclusion of the second result, we take limits as $x \rightarrow \infty$ 
to see that the dominant term is given by the leftmost term in the last equation. 
\end{proof} 
...
\end{lemma} 
\begin{proof} 
The proof of (A) is obvious since for $k_0 \in \left[1, \frac{x}{2}\right]$ we have that 
\[
...
\end{align*}
We assert by the asymptotic formulas for the 
...
the proposition and apply the bound above for the difference of the 
...
S_2(x) & = O\left(G^{-1}\left(\frac{x}{2}\right) + \int_{\sqrt{x}}^{\frac{x}{2}} 
     ...
     \int_{\sqrt{x}}^{\frac{x}{2}} \frac{dt}{t \cdot \log\left(\frac{x}{t}\right)}\right) \\ 
     & = O\left(G^{-1}\left(\frac{x}{2}\right) + (\log\log x) \times 
     \max_{\sqrt{x} < k < \frac{x}{2}} \frac{|G^{-1}(k)|}{k}\right). 
\end{align*} 
The rightmost maximum term in the previous equation is 
known to satisfy $\frac{|G^{-1}(k)|}{k} \ll \mathbb{E}|g^{-1}(k)|$ as 
$k \rightarrow \infty$. 
The conclusion follows since the average order of $|g^{-1}(n)|$ is 
increasing for sufficiently large $n$. 
\end{proof} 
...
Much in the same way that variants of the Erd\H{o}s-Kac theorem are proved 
...
Paul Pollack, Steven J.~Miller, Ernie Croot, Bruce Reznick and Michael Lacey. 
...
\newpage
\section{Appendix: Asymptotic formulas for the incomplete gamma function} 
...
\]
The following properties of $\Gamma(a, x)$ hold at $z, a > 0$: 
\begin{align} 
\label{eqn_IncompleteGamma_PropA} 
\Gamma(a, z) & = (a-1)! \cdot e^{-z} \times \sum_{k=0}^{a-1} \frac{z^k}{k!}, a \in \mathbb{Z}^{+}, z > 0, \\ 
\label{eqn_IncompleteGamma_PropB} 
\Gamma(a, z) & \sim x^{a-1} \cdot e^{-z}, \mathrm{\ for\ fixed\ } a > 0, \mathrm{\ as\ } z \rightarrow +\infty. 
\end{align}
...
\end{equation} 
If $z,a \rightarrow \infty$ with $z = \lambda a$ for some $\lambda > 0$ such that 
...
$b_0(\lambda) = 1$ and\footnote{
     ...
     \[
     b_n(\lambda) = \sum_{k=1}^{n} \gkpEII{n}{k-1} \lambda^k. 
     \]
}
\[
...
Suppose that $z$ and $a > 0$ are real parameters. 
If $|z| = \lambda a$ for some $\lambda > 1$, then as $z \rightarrow +\infty$ we have that 
\[
\Gamma(a, z) = \frac{z^{a-1} e^{-z}}{1-\lambda^{-1}} + O_{\lambda}\left(z^{a-2} e^{-z}\right), 
\]
and if $a \geq 1$ is integer-valued, then as $z \rightarrow +\infty$ we have 
\[
\Gamma(a, -z) \sim \frac{z^{a-1} e^{z}}{1+\lambda^{-1}} + 
      O_{\lambda}\left(z^{a-2} e^{-z}\right). 
\]
\end{prop}
\begin{proof}
We can see that for $\lambda > 1$ and $n \geq 1$ 
\cite[\S 6.2]{GKP} 
$$\sum_{k=1}^{n} [\lambda^k] b_n(\lambda) = \frac{(2n)!}{2^n n!} \sim 
  \sqrt{2} \left(\frac{2n}{e}\right)^n.$$ 
Thus we conclude that for all large enough $n$ 
$$b_n(\lambda) \ll (2n)! \cdot \lambda^{n}.$$ 
It follows from \eqref{eqn_IncompleteGamma_PropD} that 
for $N := z\left(1-\frac{\log \lambda}{\lambda} - \frac{1}{\lambda}\right)$ 
(\cf \cite[\S A.1]{NEMES2015C})
\begin{align}
\notag
\Gamma(a, z) & \sim z^{a-1} e^{-z} \times \sum_{0 \leq n < N} 
     \frac{(-1)^n b_n(\lambda)}{z^n \cdot (1-\lambda^{-1})^{2n+1}} 
     = z^{a-1} e^{-z} (1+E(a, z; \lambda)), 
\end{align}
where 
$$|E(a, z; \lambda)| \ll \sum_{1 \leq n < N} \frac{(2\lambda^2 n)^n}{((\lambda-1)^2 ez)^{n}}.$$
For all $\lambda > 1$, we have that the $n^{th}$ summand in the previous bound 
is given by $c_n^n$ where $0 < c_n < 1$. We conclude that since 
$z \rightarrow \infty$, we get $E(a, z; \lambda) = o(1)$. 
This argument justifies the main term for $\Gamma(a, z)$ stated as our proposition. 

We obtain the following expansions using a similar justification on which 
terms in the sum are main terms as above for $a \in \mathbb{Z}^{+}$ and 
for $N := z\left(1+\frac{\log \lambda}{\lambda} + \frac{1}{\lambda}\right)$ 
as $z \rightarrow -\infty$: 
\begin{align*}
\Gamma(a, z) & = z^a e^{z} \times \sum_{n \geq 0} 
     \frac{(-a)^n b_n(\lambda)}{(z-a)^{2n+1}} 
     \sim z^{a-1} e^{z} \times \sum_{n \geq 0} \frac{(-1)^n b_n(\lambda)}{z^n 
     \left(1+\frac{1}{\lambda}\right)^{2n+1}} \\ 
     & \sim 
     \frac{z^{a-1} e^{z}}{\left(1+\frac{1}{\lambda}\right)} + 
     O\left(\sum_{1 \leq n < N} \frac{\lambda^{2n} (2ne^{-1})^n}{\left(1 + \lambda\right)^{2n+1}}\right). 
     \qedhere 
\end{align*}
\end{proof}
...
\begin{align}
\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnA}
S_1(x) & := \frac{x}{\log x} \times \left\lvert \sum_{1 \leq k \leq \log\log x} 
     \frac{(-1)^k (\log\log x)^{k-1}}{(k-1)!} \right\rvert 
     \sim \frac{x}{\sqrt{2\pi \log\log x}}, \\ 
\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnC}
S_2(x) & := x \times \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} \sim 
     \frac{x(\log x)}{\sqrt{2\pi}}. 
\end{align}
\end{lemma}
\begin{proof}[Proof of \eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnA}]
We set $t := \log\log x$ and let $t \rightarrow +\infty$. 
We can write 
\[
\frac{\log x}{x} \cdot S_1(x) = \left\lvert \sum_{o \leq k < t} \frac{(-t)^k}{k!} \right\rvert. 
\]
The argument is to bound the scaled form of the function in the previous equation from above and 
below by the limiting asymptotic main term, $\frac{\log x}{\sqrt{2\pi \log\log x}}$. 
By Taylor's theorem for the exponential function with bounded remainder terms, 
we have that for some $0 < s < t$ 
\begin{align} 
\label{eqn_LemmaProofTag_ExpMinust_TaylorsRemainderThm_v1} 
e^{-t} & = \sum_{0 \leq k < t} \frac{(-t)^k}{k!} + \frac{(-s)^t}{t!} e^{-s}. 
\end{align}
Clearly, we have that the left-hand-side of 
\eqref{eqn_LemmaProofTag_ExpMinust_TaylorsRemainderThm_v1} 
corresponds to an $O\left(\frac{1}{\log x}\right)$ error term. 
We can also compute that as $t \rightarrow \infty$ the remainder term in the previous 
equation satisfies 
\[
\left\lvert \frac{(-s)^t}{t!} e^{-s} \right\rvert \leq \frac{t^t}{t!} \sim 
     \frac{\log x}{\sqrt{2\pi \log\log x}}, 
\]
by applying Stirlings formula to approximate $t!$ when $t$ is sufficiently large. 
Hence, we get that 
\[
\left\lvert \sum_{o \leq k < t} \frac{(-t)^k}{k!} \right\rvert \leq 
     \frac{\log x}{\sqrt{2\pi \log\log x}} + O\left(\frac{1}{\log x}\right). 
\]
On the other hand, as $t \rightarrow +\infty$, again applying Stirling's formula, 
we have that 
\begin{align*}
\left\lvert \sum_{o \leq k < t} \frac{(-t)^k}{k!} \right\rvert & \geq 
     \frac{t^t}{t!} + O\left(\frac{1}{\log x}\right) 
     \geq \frac{e^t}{\sqrt{2\pi t}} + O\left(\frac{1}{\log x}\right) \\ 
     & = \frac{\log x}{\sqrt{2\pi \log\log x}} + O\left(\frac{1}{\log x}\right).
\end{align*}
We conclude that 
\[
S_1(x) = \frac{x}{\sqrt{2\pi \log\log x}} + O\left(\frac{x}{\log x}\right). 
     \qedhere
\]
\end{proof}
\begin{proof}[Proof of \eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnC}]
Let $t := \log\log x$ and suppose that $t \rightarrow +\infty$. 
By \eqref{eqn_IncompleteGamma_PropA}, 
\eqref{eqn_IncompleteGamma_PropC} and 
Stirling's formula we see that 
\begin{align*}
\sum_{0 \leq k \leq t} \frac{t^k}{k!} & \sim \frac{e^t}{(t-1)!} \Gamma(t, t) 
     \sim \frac{t^{t+1/2}}{t!} 
     \sim \frac{e^t}{\sqrt{2\pi}}. 
\end{align*}
We have technically made a simplifying assumption in adjusting the bounds of summation on the 
left-hand-side of the previous equation. This turns out to be valid in so much as 
with $t := \log\log x$, as $x \rightarrow +\infty$ we have that $t \sim t \pm 1$ so that the 
distinction in this adjustment is asymptotically negligible in our formula. 
\end{proof}
...
For $x > e$, we have that 
\begin{align}
\label{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2__subeqnD} 
S_3(x) := \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k+1/2}}{(2k+1) (k-1)!} \sim 
     \frac{1}{2} (\log x) \sqrt{\log\log x} + O\left(\frac{\log x}{\sqrt{\log\log x}}\right).       
\end{align}
\end{lemma}
\begin{proof}
We can sum this series symbolically with \emph{Mathematica} to find that 
\begin{align}
\label{eqn_ProofTag_S3xThreeTermSum_Exp_v1} 
S_3(x) & = \frac{1}{2} (\log x) \sqrt{\log\log x} - \frac{\sqrt{\pi}}{4} \operatorname{erfi}\left( 
     \sqrt{\log\log x}\right) \\ 
\notag 
     & \phantom{=\ } - 
     \frac{_2F_2\left(1, \frac{3}{2}+\log\log x; 1 + \log\log x, 
     \frac{5}{2} + \log\log x; \log\log x\right) (\log x) (\log\log x)}{2\sqrt{2\pi} (2\log\log x + 3)}. 
\end{align}
We will bound each component term in the above expansion of $S_3(x)$ to see that 
the dominant asymptotic order of this function is given by the leading term. 

As $|z| \rightarrow \infty$, the \emph{imaginary error function}, denoted by 
$\operatorname{erfi}(z)$, has the following asymptotic expansion 
\cite[\S 7.12]{NISTHB}: 
\[
\operatorname{erfi}(z) = -\imath + \frac{e^{x^2}}{\sqrt{\pi}} \left(z^{-1} + \frac{z^{-3}}{2} + 
     \frac{3z^{-5}}{4} + \frac{15 z^{-7}}{8} + O(z^{-9})\right). 
\]
It follows that 
\[
\frac{\sqrt{\pi}}{4} \operatorname{erfi}\left(\sqrt{\log\log x}\right) \sim 
     \frac{(\log x)}{4} \left(\frac{1}{\sqrt{\log\log x}} + O\left(\frac{1}{(\log\log x)^{3/2}}\right)\right). 
\]
By bounding the remaining convergent hypergeometric series term in the expansion of 
$S_3(x)$, we see that 
\begin{align}
\label{eqn_proofTag_H11x_HypGeomSeriesExp_v1}
     & \frac{_2F_2\left(1, \frac{3}{2}+\log\log x; 1 + \log\log x, 
     \frac{5}{2} + \log\log x; \log\log x\right)}{ 
     (2\log\log x+3)} \\ 
\notag 
     & = \frac{1}{2 \log\log x} \times \sum_{k \geq 0} 
     \frac{1}{\left(1 + \frac{2k+3}{2\log\log x}\right)} \prod_{i=1}^{k} \left(1 + \frac{i}{\log\log x}\right)^{-1} \\ 
\notag 
     & =\frac{1}{2 \log\log x} \times 
     \sum_{\substack{k \geq 0 \\ 2k+3 < \log\log x}} \left(1+\frac{2k+3}{\log\log x}\right)^{-1} \times 
     \prod_{i=1}^k \left(1 + \frac{i}{\log\log x}\right)^{-1} + O\left( \frac{1}{\log\log x} \times 
     \prod_{i=1}^{\log\log x-1} \left(1 + \frac{i}{\log\log x}\right)^{-1}\right). 
\end{align}
The rightmost sum corresponds to the error term. Indeed, we see that the inner sum in the 
second to last line is convergent to some constant. The leading product factors remaining 
in the last equation satisfy 
\[
\prod_{i=1}^{\log\log x - 1} \left(1+\frac{i}{\log\log x}\right) \geq 
     \frac{1}{2} + \frac{\log\log x}{2}, 
\]
by appealing to a polynomial expansion of the factorial product by the Stirling numbers of the 
first kind where each component term $\frac{j}{\log\log x} < 1$ whenever 
$1 \leq j < \log\log x$. It follows that the error term in 
\eqref{eqn_proofTag_H11x_HypGeomSeriesExp_v1} is $O\left((\log\log x)^{-1}\right)$. 
The main term in \eqref{eqn_proofTag_H11x_HypGeomSeriesExp_v1}
is expanded as 
\begin{align*}
     \sum_{\substack{k \geq 0 \\ 2k+3 < \log\log x}} & 
     \left(1 + \frac{2k+3}{\log\log x}\right)^{-1} \times 
     \prod_{i=1}^{k} \left(1+\frac{i}{\log\log x}\right)^{-1} \\ 
     & \ll 
\sum_{\substack{k \geq 0 \\ 2k+3 < \log\log x}} 
     \prod_{i=1}^{k} \left(1+\frac{i}{\log\log x} + O\left(\frac{i^2}{(\log\log x)^2}\right)\right) \times 
     \left(1 + \frac{2k+3}{\log\log x} + O\left(\frac{(2k+3)^2}{(\log\log x)^2}\right)\right) \\ 
     & = \sum_{\substack{k \geq 0 \\ 2k \leq \log\log x-3}} \left[ 
          1 + \frac{(2k+1)(2k+2)}{2 \log\log x} + o\left(\frac{k^2}{\log\log x}\right)\right] \\ 
     & \sim  
     \frac{(\log\log x)^2}{12} + \frac{3(\log\log x)}{8} - \frac{19}{12} + 
     \frac{1}{8\log\log x}. 
\end{align*}
This justifies taking the claimed main term resulting from the 
expansion of $S_3(x)$ in \eqref{eqn_ProofTag_S3xThreeTermSum_Exp_v1}. 
\end{proof}
\end{subequations}
