...
We prove a related weaker property of $|M(x)| \log x /\sqrt{x}$, e.g., showing that 
...
is the following important summary corollary that partially 
...
\begin{align*} 
\frac{|M(x_{0,y})| \log(x_{0,y})}{\sqrt{x_{0,y}}} & \SuccSim 
     \frac{C_{\ell,1} \cdot (\log\log \sqrt{x_{0,y}})}{(\log\log\log\log \sqrt{x_{0,y}})^{\frac{9}{4}} 
     (\log\log\log\log\log \sqrt{x_{0,y}})^{\frac{7}{2}}} \times 
...
scaled Mertens function $q(x) \log x$ grows without bound. 
...
\[
\widehat{G}_E^{-1}(x) := \sum_{\substack{e \leq n \leq (\log x)^2}} \lambda(n) \times 
     \sum_{\substack{d|n \\ d > e}} \frac{(\log d)^{\frac{1}{4}}}{\log\log d}, 
\]
...
decreasing function $\widehat{L}_0\left(\frac{\log\log x}{d}\right)$ in the variable $d$. 
We note that the function $\widehat{L}_0\left(\frac{\log\log x}{d}\right)$ is decreasing 
on $d \in \left[e, \frac{\log\log x}{e}\right)$, as can be viewed by computing its first 
...
We can bound below to show that $R_E(x)$ is at least on the order of a constant 
times $(\log x)^2$. To obtain this lower bound, note that since $\frac{(\log d)^{\frac{1}{4}}}{d \cdot \log\log d}$ 
...
terms in the above sum are given by 
\begin{align*} 
R_E(x) & \gg (\log\log x) \times \left\lvert \sum_{\log\log\log x < d < \frac{\log\log x}{2e}} 
     \frac{\log(2d)^{1/4}}{\log\log(2d)} \left[ 
     ...
     \right] \right\rvert \\ 
     & \approx (\log\log x) \times \left\lvert O(1) + \sum_{\log\log\log x < d < \frac{\log\log x}{2e}} 
     \frac{1}{16 \cdot d \cdot \log(2d)^{3/4} \log\log(2d)^2} \right\rvert \\ 
     & \gg (\log\log x) \times \left\lvert \frac{1}{64} 
     \operatorname{Ei}\left(\frac{\log\log\log\log x}{4}\right) \right\rvert \\ 
     & \gg \frac{3}{1024} (\log\log x) (\log\log\log\log x). 
\end{align*} 
...
sums over $d \in [e, \log\log\log\log x]$. 
...
G_{\ell}^{-1}(x) := \sqrt{\frac{2}{\pi}} \frac{2 A_0}{3e \log 2} \times (\log\log x) \times 
     T_E(x) \times 
     \left\lvert A_{\Omega}^{(\ell)}(\log\log\log x) \widehat{\tau}_0(\log\log\log x) - 
     \int_e^{\log\log\log x} A_{\Omega}^{(\ell)}(t) \widehat{\tau}_0^{\prime}(t) dt 
     \right\rvert. 
