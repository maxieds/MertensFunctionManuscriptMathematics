...
To be absolutely clear about notation, we are explicit and not re-use the $\sim$ relation by 
...
An intentionally constructed mock example motivating this usage of these 
relations clarifies the point of making this subtle distinction in notation appears below. 
...
for the asymptotically dominant main term in the lower bounds stated above. Note that this main term contribution 
...
a conjecture due to Gonek (among others) that in fact the limiting behavior of 
$M(x)$ satisfies 
...
encapsulate the parity of $\lambda(n)$ through the summatory functions $\widehat{\pi}_k(x)$. 
...
where we must take $0 \leq z \leq 1$: 
\begin{align*} 
...
\subsection{Almost cracking the classical unboundedness barrier} 
...
$y \geq \max\left(\ceiling{e^{e^{e^{e^{e}}}}}, u_0+1\right)$: 
\begin{align*} 
\frac{|M(x_y)|}{\sqrt{x_y}} & \SuccSim 
     2C_{\ell,1} \cdot \frac{(\log\log \sqrt{x_y})}{\log \sqrt{x_y}} 
     ...
     \log_{\ast}^{6}(\sqrt{x_y})^{\frac{5}{2}}} + o(1),  
\end{align*} 
...
\]
but also set a minimal rate (along a large subsequence) at which this form of the 
...
$M(x)$ that we develop in this article. The full listing of ``routine'', or so-called cut-and-dry, results we need as glue 
...
\item[\textbf{Step C:}] \textit{Re-writing the exact formula for $M(x)$}: Key interpretations used in 
     ...
     \end{itemize} 
\item[\textbf{Step D:}] \textit{The Holy Sub-Grail}: A big leap twoards proving that 
...
Then we have using the recurrence relation for $h^{-1}$ with $\chi_{\mathbb{P}} = \omega \ast \mu$ that 
\begin{align} 
\notag
...
\label{eqn_proof_tag_hInvnAstOne_explFormulaExp_v1} 
     & = \begin{cases} 
     \lambda(n) \times (\Omega(n) - 1)!, & n \geq 2; \\ 
     \lambda(n), & n=1. 
     \end{cases} 
\end{align} 
We need to compute the sign of the function $h^{-1} \ast \mu$, where $g^{-1} = h^{-1} \ast \mu$ by the 
DGF in \eqref{eqn_DGF_of_gInvn} since the DGF of $\mu(n)$ is well-known to be $1 / \zeta(s)$ for $\Re(s) > 1$
(the DGF of a convolution is a product of the component DGFs). 

First, by M\"{o}bius inversion and the 
formula for $h^{-1} \ast 1$ we proved above, for each $n \geq 2$, we have that 
there exist constants $C_{1,n},C_{2,n} > 0$ so that 
\[
C_{1,n} \cdot (\lambda \ast \mu)(n) \leq h^{-1}(n) \leq C_{2,n} \cdot (\lambda \ast \mu)(n). 
\] 
This observation follows from the non-negativity of the factorial function in the formula we just proved in 
\eqref{eqn_proof_tag_hInvnAstOne_explFormulaExp_v1}. 
Since both $\lambda,\mu$ are multiplicative, $\lambda \ast \mu$ is multiplicative, where we know that the values of 
any multiplicative function are uniquely determined by their action at prime powers. 
So we can compute that for any prime $p$ and 
integer exponents $\alpha \geq 1$, 
\[
(\lambda \ast \mu)(p^{\alpha}) = \lambda(p^{\alpha}) - \lambda(p^{\alpha-1}) = 2 \lambda(p^{\alpha}). 
\]
Then by the multiplicativity of $\lambda(n)$, the previous inequalities are re-stated in the form of 
\[
2 C_{1,n} \cdot \lambda(n) \leq h^{-1}(n) \leq 2 C_{2,n} \cdot \lambda(n). 
\] 
Now to bound $h^{-1} \ast \mu$, we similarly can argue again that by multiplicativity we have 
\[
4 C_{1,n} \cdot \lambda(n) \leq (h^{-1} \ast \mu)(n) \leq 4 C_{2,n} \cdot \lambda(n). 
\] 
Since the absolute constants (for each $n$) are positive, we clearly recover the signedness of 
$g^{-1}(n)$ as $\lambda(n)$. 
\end{proof} 
...
as $x \rightarrow \infty$ (see Remark \ref{remark_FutureImprovementsOnMVGzLowerBound}). 
...
\begin{remark}[Future improvements on the lower bounds for $\mathcal{G}(z)$] 
\label{remark_FutureImprovementsOnMVGzLowerBound} 
In some sense, what we have accomplished in proving the lower bound on $\mathcal{G}(z)$ using 
elementary calculus in the proof of Theorem \ref{theorem_GFs_SymmFuncs_SumsOfRecipOfPowsOfPrimes} is 
sufficient, but still an overall very weak bound. The weakness of this bound contributes to the 
missing logarithmic factor when we show that 
\begin{equation} 
\label{eqn_LimSup_ClassicalScaledByLogx} 
\limsup_{x \rightarrow \infty} \frac{|M(x)| \log x}{\sqrt{x}} = +\infty, 
\end{equation} 
in Corollary \ref{cor_ThePipeDreamResult_v1}. 
To actually tighen up the ship of our proof (so to speak) to show what is widely believed, and 
classically conjectured, as 
\[
\limsup_{x \rightarrow \infty} \frac{|M(x)|}{\sqrt{x}} = +\infty, 
\] 
my best intuition suggests that finding a better (asymptotically higher order) lower bound in this 
step is crucial. 
Based on prior calculations, we want to (minimally) in fact show that the parameter $u$ from the 
previous proof of Theorem \ref{theorem_GFs_SymmFuncs_SumsOfRecipOfPowsOfPrimes} can allow us to 
take partial prime products indexed as high as $p \leq x$ (for $x$ as in 
Theorem \ref{theorem_HatPi_ExtInTermsOfGz}), and have that this scheme still obtains an 
accurate limiting lower bound on $\mathcal{G}(z)$ for large $x$. 
If this assumption is true for all large enough $x$, then 
the troublesome factor of $\log x$ in \eqref{eqn_LimSup_ClassicalScaledByLogx} 
vanishes when we proceed through the subsequent steps 
remaining in this proof using the new bound type of this improved form. 
\end{remark} 
...
Conjecture \ref{lemma_gInv_MxExample} is not the most accurate 
...
Thus by mathematical induction, we have proved that the claimed limiting asymptotic behavior holds for 
$C_k(n)$ whenever $k \geq 1$ as $n \rightarrow \infty$. 
\end{proof} 
...
Thus if we let 
\[
\widetilde{G}^{-1}(x) := \sum_{n \leq x} \sum_{d|n} C_{\Omega(d)}(d), 
\]
denote the summatory function defined by approximating $g^{-1}(n)$ by 
$\sum_{d|n} C_{\Omega(d)}(d)$, we obtain a lower bound in the form of 
$$|G^{-1}(x)| \SuccSim \left\lvert \widetilde{G}^{-1}(x) \right\rvert.$$
\end{cor} 
\begin{proof} 
...
\mathbb{E}\left[\frac{\sum\limits_{d|n} C_{\Omega(d)}(d)}{|g^{-1}(n)|}\right] & = 
     \left\lvert \frac{g^{-1}(n) - \lambda(n) \left(1 - \frac{6}{\pi^2}\right) 
...
\label{eqn_proof_tag_v1_ExpectationFormulagInvnVersisSRn} 
     & = 1 - \frac{\log n + 2\gamma-1 + o(1)}{\mathbb{E}\left[|g^{-1}(n)|\right]}. 
\end{align} 
...
\[
\mathbb{E}\left[|g^{-1}(n)|\right] \leq \mathbb{E}\left[C_{\Omega(n)}(n)\right] = 
...
the signedness of the summatory function term in the integral formula above is a moot point entirely. 
\end{proof} 
...
interesting matter of bounding $M(x) \log x / \sqrt{x}$ in the next subsection. 
...
$\lambda(n)$ are captured by the terms in the range over $k$ summed by 
$A_{\Omega}^{(\ell)}(t)$ for 
...
\left\lvert \frac{\sum\limits_{r \cdot \log\log x < k \leq x} (-1)^k \cdot \widehat{\pi}_k(x)}{A_{\Omega}(x)} \right\rvert & 
     \PrecSim 
     \left\lvert \frac{B(x, r)}{\log\log x + B(x, 1)} \right\rvert 
     \ll 
     \left\lvert \frac{(\log x)^{r-1-r\log r}}{x(1 + o(1))} \right\rvert = o(1), 
\end{align} 
as $x \rightarrow \infty$. 
Recall that the function $B(x, r)$ is defined and bounded 
as in the cited theorem from \cite{MV} given as it appears in the reference 
on page \pageref{theorem_MV_Thm7.20-init_stmt}. 
...
\[
C_{\ell,1} = 4A_0^2  = 
     \frac{128 \cdot 2^{1/8}}{6561 \cdot e^6 \pi \log^6(2)} 
     \exp\left(-\frac{55}{2} \log^2(2)\right) 
     \approx 2.76631 \times 10^{-10}.
\]
That is, we have 
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     ...
     (\log\log\log\log x)^{\frac{5}{2}}} \cdot \frac{\log_{\ast}^5(x)^{2\log 2 + \frac{1}{3\log 2} + 1}}{ 
...
\notag 
     & \SuccSim 
     \frac{4A_0^2}{\pi} (\log\log x) \frac{(\log\log\log x)^{2\log 2+\frac{1}{3\log 2} - 2}}{ 
...
\end{align*} 
\textit{Remarks. } 
What we have shown in total is a logarithmically scaled lesser form of the classically conjectured 
...
$|M(x)| / \sqrt{x}$ towards unboundedness along an infinite subsequence, e.g., progress on the problem. 
\end{proof} 
...
\newpage
\subsection{Table: Computations with a signed Dirichlet inverse function and its summatory function} 
