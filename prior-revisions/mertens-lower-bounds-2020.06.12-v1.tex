...
corresponding to squarefree integers, and prime powers. Namely, if for $k \geq 1$ we set 
\begin{align*} 
M_k := \underset{{\substack{2 \leq n \leq p_1^k \cdots p_k^k \\ \Omega(n) = k}}}{\operatorname{argmax}} 
     |g^{-1}(n)|, \qquad 
m_k := \underset{{\substack{2 \leq n \leq p_1^k \cdots p_k^k \\ \Omega(n) = k}}}{\operatorname{argmin}} 
     |g^{-1}(n)|, 
\end{align*} 
then $M_k$ is squarefree and $m_k$ is a prime power. In particular, we have that 
\[
M_k = \sum_{j=0}^{k} \binom{k}{j} j!, m_k = 2 \cdot (-1)^{k}. 
\]
