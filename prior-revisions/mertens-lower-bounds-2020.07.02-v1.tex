...
\left\lvert G_E^{-1}(x) \right\rvert & \SuccSim 
     \frac{2^{\frac{1}{4}} (\log 2)^{\frac{1}{2}}}{2 \sqrt{\pi}} 
...
\left\lvert \sum_{k \leq \log\log x} (-1)^k \cdot \widehat{\pi}_k(x) \right\rvert 
     & \gg \frac{2^{\frac{1}{4}} (\log 2)^{\frac{1}{2}}}{\sqrt{\pi}} 
...
T_E(x) := \frac{1}{\log\left[(\log x)^{4} (\log\log x)\right]^{\frac{1}{2}} 
...
the bounding function $|\widehat{\tau}_{\ell}(t)|$ is a non-negative monotone 
...
\[
|G_E^{-1}(x)| \gg \frac{e\widehat{C}_0}{2 \sqrt{2\pi}} \cdot 
...
we have 
\begin{align*} 
\left\lvert \widehat{\tau}_{\ell}\left(x\right) - 
     \widehat{\tau}_{\ell}\left(\frac{x}{2} - \frac{1}{2}\right) \right\rvert & = 
     \widehat{\tau}_{\ell}\left(x\right) \times \left\lvert 1 - 
     ...
     \right\rvert \\ 
     & =  \widehat{\tau}_{\ell}\left(x\right) \times \left\lvert 
...
G_{\ell}^{-1}(x) & := \frac{\widehat{C}_0}{2 \sqrt{2\pi}} \cdot 
     \frac{(\log x)^{5/4}}{(\log\log x)^{1/4} \sqrt{\log\log\log x}} \times \\ 
\notag 
     & \phantom{:=\qquad\ } \times 
...
\eqref{eqn_proof_thm_GInvFunc_v4v2_approx}: 
\begin{align} 
...
\frac{\widehat{C}_0}{2 \sqrt{2\pi}} & \cdot 
     ...
     \sqrt{\log\log x} \cdot (\log\log\log x)^2}
\end{align} 
Hence, we conclude that we can take $\left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert$ 
...
Proposition \ref{prop_Mx_SBP_IntegralFormula} 
over $t \in [u_0, x/2]$ into two disjoint subintervals: 
...
\notag 
     ...
     \right) \times \int_{\sqrt{x}}^{\frac{x}{2}} \frac{2 \sqrt{x}}{ 
