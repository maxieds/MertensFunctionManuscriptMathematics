...
\subsection{Rigorous proof that average order lower bounds are meaningful with respect to our problem} 
...
its proof is not a deep result of classical order. The basic argument uses properties of the DGF of $g^{-1}(n)$ 
to show that a convolution it necessarily forms as its coefficients has sign given by $\lambda(n)$. 
...
Then by Proposition \ref{prop_AntiqueDivisorSumIdent} and the known property that the DGF of $f^{-1}(n)$ is 
the reciprocal of the DGF of the original arithmetic function $f$, for all $\Re(s) > 1$ 
\begin{align} 
...
If 
\[
\frac{1}{x} \cdot \#\left\{n \leq x: |G^{-1}(n)| - |\mathbb{E}[G^{-1}(n)]| \leq 0\right\} = 
     C + o(1), 
\]
