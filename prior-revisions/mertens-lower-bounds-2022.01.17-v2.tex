...
inverse with respect to Dirichlet convolution by \cite[\seqnum{A341444}]{OEIS} 
\begin{equation}
...
\frac{1}{x} \times & \#\left\{3 \leq n \leq x: 
     ...
     = 
     \Phi_{\Omega}\left(\frac{\frac{\pi^2 y}{6}-\mu_x(C)}{\sigma_x(C)}\right) + 
...
\frac{1}{x} \times \#\left\{3 \leq n \leq x: |g^{-1}(n)| - 
     ...
     \Phi_{\Omega}\left(\frac{\frac{\pi^2 y}{6}- B_0 \sqrt{\log\log\log x}}{ 
...
$|g^{-2}(n)|$ and asymptotics for $L(x)$. 
In particular, there is an identification of $G^{-1}(x)$ with the summatory function $L(x)$ given by 
\begin{align*}
G^{-1}(x) & = 
     L(x)|g^{-1}(x)| - \sum_{n < x} 
...
proof methods given in the spirit of Montgomery, Vaughan and Tenenbaum's work, 
...
There is a limiting probability measure with CDF $\Phi_{\Omega}(z)$ such that 
...
$\mu_x(C)$, $\sigma_x(C)$ and $\Phi_{\Omega}(z)$ are defined as in 
...
\frac{1}{x} \times \#\left\{3 \leq n \leq x: \frac{|g^{-1}(n)|}{\sqrt{\log\log n}} - 
     ...
     \Phi_{\Omega}\left(\frac{\frac{\pi^2 y}{6}-\mu_x(C)}{\sigma_x(C)}\right) + o(1). 
\end{align*} 
...
summatory function $G^{-1}(x)$, which relates the distribution of $M(x)$, 
