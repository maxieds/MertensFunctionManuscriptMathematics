...
\sqrt{\operatorname{Var}\left[\log C_{\Omega}(n)\right]} \sim 
	B_0 \cdot (\log\log n) (\log\log\log n), 
	\mathtext{ as } n \rightarrow \infty. 
\]
\end{prop}
\begin{proof}
...
\label{eqn_proof_tag_COmeganVariance_InitEqn_v2}
	& \sim B_0^2 \left((\log\log n)^2 (\log\log\log n)^2 + I_A(n)\right). 
\end{align} 
...
function $(-1)^{\omega(n)} C_{\Omega}(n)$ requires long and technical analyses
...
\lim_{x \rightarrow \infty} 
     \mathbb{P}\left[\frac{\log C_{\Omega}(N(x)) - \mu_x}{\sigma_x} \leq z\right] = 
...
\]
Therefore, for any $1 \leq k \leq \log_2(x)$ as $x \rightarrow \infty$
\[
...
should be governed by a probabilistic model is heuristic in character. 
...
predicts \'a priori the regularity by limiting distribution we find in 
...
\subsection{Discussion of the new results involving the auxiliary unsigned functions}
...
\subsubsection{Adaptive strategies leveraging the unpredictability of $\mu(n)$ versus $\lambda(n)$}
...
$\ll_R$ in the reference. We omit the extra annotation of the notation since 
$R \in [1, 2)$ is at most a bounded constant. 
\end{proof}
...
We define the regularized incomplete gamma function by 
...
\[
(r_{x,j})_{1 \leq x,j \leq N} = \hat{R} \left(I - U^{T}\right), 
...
\end{align*}
The $p$-degree remainder term is bounded by 
\begin{align*}
...
we denote the $p$-degree EM formula error term by 
...
\int_1^{\log\log x} 
	\mathcal{G}\left(\frac{t-1}{\log\log x}\right) g(x, t) dt & = 
	\mathcal{G}\left(\frac{c(x)-1}{\log\log x}\right) \times 
...
given by equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v2}. 
...
monotone on the positive integers $k$ within this range. 
...
Next, we can differentiate both sides of equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v2} 
to form a first-order ordinary differential equation (ODE) for the function $\mathcal{G}(v)$ 
by performing the change of variable $v = \frac{c(x)-1}{\log\log x}$. 
The solution to this ODE in the variable $v$, subject to the 
initial condition $\mathcal{G}(0) := 1$, is given by 
$\mathcal{G}(v) \equiv 1$. 
This shows that $B_0^{\ast}(x) \equiv 1$, which completes step \textbf{(ii)}. 
Combining this step with equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v4} 
shows that equation \eqref{eqn_proof_tag_LOmegax_MainTerm_v1} holds. 

We must now bound the error term to complete the proof. To this end, we claim that 
\begin{equation}
...
\log C_{\Omega}(n) \ll \Omega(n) \log \Omega(n), \mathtext{ for } n \leq x, 
     \mathtext{ as } x \rightarrow \infty. 
\end{equation}
The assertion that the upper bound for $\log C_{\Omega}(n)$ in 
\eqref{eqn_proof_tag_LogCOmegan_ll_FuncOfOmegan_v1} holds 
for all $n \leq x$, 
even though the right-hand-side terms involving $\Omega(n)$ 
oscillate in magnitude over the integers $1 \leq n \leq x$, is justified by 
maximizing (minimizing) the ratio of the right-hand-side of 
\eqref{eqn_proof_tag_LogCOmegan_ll_FuncOfOmegan_v1} to 
Binet's log-gamma formula with $z \equiv \Omega(n)$. 
This procedure is completed numerically with 
computer algebra software to find 
approximations to explicit bounded real 
$1 \leq \Omega(n_{\ell}), \Omega(n_u) \leq 12$ at which we attain 
the respective global extrema of the ratio.
...
L_{E,\Omega}(x) & \ll \sum_{\substack{n \leq x \\ \Omega(n) \geq \log\log x}} 
	...
	\#\left\{n \leq x: \Omega(n) \geq \log\log x\right\}^{\frac{q-1}{q}} \\ 
     & \ll x (\log\log x)^{\frac{5}{2} - \frac{1}{2q}} (\log\log\log x), 
     \mathtext{ for some } q \in (0, \infty).
\end{align*} 
The exponent of $\log\log x$ on the right-hand-side of the 
last equation is less than the $\frac{2}{3}$ required by 
equation \eqref{eqn_proof_tag_PartialSumsOver_HatCkx_EquivCond_v2}
for all $q \in \left(0, \frac{1}{3}\right)$. 
We can perform the change of variable 
$u := \frac{t-\log\log x}{\sqrt{\log\log x}}$, noting that the non-negative integral 
$q^{th}$ moment of any normal random variable is finite for all $q > 0$, and 
...
For example, we can select $q := \frac{3}{11} + \varepsilon$ for any 
$\varepsilon \in \left(0, \frac{2}{33}\right)$ to obtain the desired bound). 
Finally, equations \eqref{eqn_proof_tag_LOmegax_MainTerm_v1} and 
\eqref{eqn_proof_tag_PartialSumsOver_HatCkx_EquivCond_v2} 
yield the conclusion of the theorem since 
\[
