...
\frac{|M(x_y)|}{\sqrt{x_y}} & \SuccSim 
     2C_{\ell,1} \cdot \frac{(\log\log \sqrt{x_y})}{(\log\log\log\log \sqrt{x_y})^{7/2}} \cdot 
     \left(\log\log\log \sqrt{x_y}\right)^{2\log 2 + \frac{1}{3\log 2} - 2} + o(1),  
\end{align*} 
...
\[
C_{\ell,1} := \frac{4 \cdot 2^{\frac{1}{16}}}{81 \cdot e^3 \log^3(2)} 
     \exp\left(-\frac{55}{4} \log^2(2)\right) 
     \approx 0.0000104227. 
\]
\end{cor} 
...
$1 < R < 2$ at $R := \frac{3}{2}$ (practically, truncated as though $R \equiv 1$ in sums) so that 
...
\eqref{eqn_proof_tag_Ckn_AsymptoticExp_v1}. So we obtain that \footnote{ 
     Here, we simplify the iterated logarithm expansions as 
     \begin{align*} 
...
provably asymptotically sufficient in form expression for $g^{-1}(n)$. 
...
we have proved a lower bound on $\widehat{\pi}_k(x)$. We use that result in the sums 
...
\end{align*} 
Thus we easily see the conclusion of the 
...
\left\lvert \frac{\lambda(n) \times \sum\limits_{d|n} C_{\Omega(d)}(d)}{g^{-1}(n)} \right\rvert & = 
     ...
     \widetilde{S}_{\operatorname{odd}}(m_1, m_2, m_3; n)}{C_{\log\log n}(n)} \right\rvert 
     \xrightarrow{n \rightarrow \infty} 1, 
\end{align*} 
and since 
\begin{align*} 
...
sign of $g^{-1}(n)$ as $n \rightarrow \infty$. All the formalism required to rigorously go through this 
result boils down to a simple verification that the main terms contributing to the form of 
$g^{-1}(n)$ when $n$ is large are easier to work with than an exact result. 
\end{proof} 
...
\[
C_{\ell,1} = A_0 \sqrt{\frac{2}{\pi}} = 
     \frac{4 \cdot 2^{\frac{1}{16}}}{81 \cdot e^3 \log^3(2)} \exp\left(-\frac{55}{4} \log^2(2)\right) 
     \approx 0.0000104227.
\]
That is, we have 
\begin{align*} 
 & \left\lvert G_{\ell}^{-1}\left(x\right) \right\rvert
     \SuccSim 
     C_{\ell,1} \cdot \frac{(\log\log x)}{(\log\log\log\log x)^{7/2}} \cdot 
     \left(\log\log\log x\right)^{2\log 2 + \frac{1}{3\log 2} - 2}. 
\end{align*} 
...
\widehat{\tau}_0^{\prime}(t) & = \frac{d}{dx}\left[ 
     (\log\log t)^{2k-1} 
     \right] = 
     \frac{(2k-1) (\log\log t)^{2k-2}}{t \cdot \log t}. 
\end{align*} 
...
I_{\ell}\left(e^{e^{2k}}\right) \frac{e^{e^{2k}}}{ 
     \Gamma\left(2k\right)} & \SuccSim 
     \frac{A_0 (4k-1) (2k)^{2k}}{16\sqrt{\pi} k^{9/2}} \exp\left( 
     e^{2k} + 2k\left(2\log 2 + \frac{1}{3\log 2} - 2\right) 
     \right). 
\end{align} 
...
\int_{\frac{\log\log\log\log x}{2}-1}^{\frac{\log\log\log\log x}{2}} & 
     I_{\ell}\left(e^{e^{2k}}\right) 
     \frac{e^{e^{2k}}}{\Gamma\left(2k\right)} dk \\ 
\notag 
     & \SuccSim 
     A_0 \sqrt{\frac{2}{\pi}} \cdot \frac{(\log\log x)}{(\log\log\log\log x)^{7/2}} \cdot 
     \left(\log\log\log x\right)^{2\log 2 + \frac{1}{3\log 2} - 2}. 
\end{align} 
...
-\int_{\sqrt{x}}^{x/2} \frac{2 x}{t^2 \log(x)} \cdot G_{\ell}^{-1}(t) dt & \SuccSim 
     ...
     & = 2C_{\ell,1} \sqrt{x} \cdot \frac{(\log\log \sqrt{x})}{(\log\log\log\log \sqrt{x})^{7/2}} \cdot 
     \left(\log\log\log \sqrt{x}\right)^{2\log 2 + \frac{1}{3\log 2} - 2} + o(1). 
\end{align*} 
...
\frac{|M(x)|}{\sqrt{x}} & \SuccSim 
     2C_{\ell,1} \cdot \frac{(\log\log \sqrt{x})}{(\log\log\log\log \sqrt{x})^{7/2}} \cdot 
     \left(\log\log\log \sqrt{x}\right)^{2\log 2 + \frac{1}{3\log 2} - 2} + o(1), 
\end{align*} 
