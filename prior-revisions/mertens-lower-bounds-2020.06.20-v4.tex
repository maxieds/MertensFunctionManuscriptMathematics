...
$$\limsup_{x \rightarrow \infty} \frac{|M(x)| (\log\log x)^{\frac{5}{2}} (\log\log\log x)^{2}}{ 
  \sqrt{x} \cdot (\log x)^{\frac{1}{4}}} > 0.234145.$$ 
...
over the function $A_{-z}(x)$. It turns out that the construction fails if we do not weight the 
signs of these coefficients at this stage of computation. 
In particular, a subtlety that arises in naive computations with these uniform bounds 
it is not enough to obtain quality estimates by 
summing over $(-1)^k \widehat{\pi}_k(x)$ when the asymptotic formulas 
for these summatory functions correspond to the coefficient cases of 
$A_z(x)$ for $z \geq 0$ in \eqref{eqn_remark_MV_AzxCoeffFormlaIntegral_v1}. 
...
\exp\left(-a_{\ell}(x) \left[1 - \frac{1}{1-z} + z\right]\right) & \geq 
     ...
     \left(\frac{\log x}{\log 2}\right)^{z} \gg 
...
\end{equation} 
By our restrictions that $1 \leq k \leq \log\log x$, we can see that the previous 
contour formula holds when we set $r := \frac{k-1}{\log\log x}$. 
We now first bound the main term from \eqref{eqn_MV_Azx_formula} as 
\begin{align*} 
...
\begin{align*} 
|A_{-z}(x)| & \gg \frac{\widehat{C}_0 x^{\frac{1}{4}}}{(\log x)^{\frac{5}{2}}} 
...
Next, we begin work on bounding the error term as in the reference: 
\begin{align*} 
...
\end{equation} 
The choice of the upper bound on the summation in \eqref{eqn_GEInvxSummatoryFuncDef_v1} 
is subtle and key for a couple of technical reasons that arise in our proof. 
We require from the results in \eqref{eqn_proof_tag_GEInvxLowerBound_v1} of 
Corollary \ref{cor_ASemiForm_ForGInvx_v1} and in the final 
proof of Corollary \ref{cor_ThePipeDreamResult_v1} 
given in Section \ref{subSection_TheCoreResultProof} that 
the input $t$ in the following equation satisfies: 
\[
t^{\frac{1}{4}} \gg (\log\log x)^{\frac{1}{4}} (\log x)^{\frac{5}{4}}. 
\]
As a practical matter, this ensures that our lower bounds obtained on 
$\frac{|M(x)|}{\sqrt{x}}$ based on the proof of 
Theorem \ref{theorem_gInv_GeneralAsymptoticsForms} utilized in proving 
Corollary \ref{cor_ThePipeDreamResult_v1} 
are unbounded as $x \rightarrow \infty$. 
We also require a restriction on this upper bound of summation so that 
it does not grow polynomially in $x$. This technical requirement is seen by 
working through the details in the proof of the next result given in 
...
$o\left((\log x)^{\frac{5}{4}} (\log\log x)^{\frac{1}{4}}\right)$, though still grows 
...
\frac{R_E(x)}{(\log x)^{\frac{5}{4}} (\log\log x)^{\frac{1}{4}}} & \gg 
     \left\lvert 
     \sum_{\log x < d < \frac{(\log x)^{5} (\log\log x)}{2e}} 
     O\left(\frac{\log(2d)^{1/4}}{(2d)^{1/4} \log\log(2d)}\right) 
     \right\rvert = 
     O\left(1\right). 
\end{align*} 
...
$(-1)^{\floor{\log\log t}}$ in the form of\footnote{ 
     ...
     according to 
     \[
     \left\{e^e \leq t \leq (\log x)^{5} (\log\log x): 
     ...
     $t \in \left[e^e, (\log x)^{5} (\log\log x)\right]$. 
} 
\begin{align} 
\notag 
\frac{2^{\frac{1}{4}} e (\log 2)^{\frac{1}{2}}}{\sqrt{\pi}} & 
