...
there is an absolute constant $\hat{G}_{\ast} > 0$ such that 
the average order of the unsigned sequence is given by 
\[
\mathbb{E}|g^{-1}(n)| \sim \frac{2\sqrt{2}}{\pi^{3/2}} (\log n)^2 \sqrt{\log\log n}. 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: |g^{-1}(n)| - \frac{6}{\pi^2} 
     ...
     \log\left(\frac{2\sqrt{2\pi}}{3}\right)\right)\right\} + 
...
\tag{D}
     ...
     \frac{6}{\pi^2}\left(\log\left(\frac{2\sqrt{2\pi}}{3}\right) - \sigma_x(C)\right) 
     \right) & = \frac{1}{2} + o(1) \\ 
\tag{E} 
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq 
     \frac{6}{\pi^2}\left(\alpha + \log\left(\frac{2\sqrt{2\pi}}{3}\right) - \sigma_x(C)\right)
     \right) & = 
...
Readers please note our use of the asymptotic notation $A \asymp B$. 
...
\begin{align*} 
\mathcal{D}_{\omega}(x) & \asymp  
     \sum_{1 \leq k \leq \log\log x} \frac{(-1)^k \cdot x}{\log x} \cdot 
     \frac{(\log\log x)^{k-1}}{(k-1)!} + O(E_{\omega}(x)) \\ 
     & \sim -\frac{3x}{2\sqrt{2\pi} \sqrt{\log\log x}} + O(E_{\omega}(x)), 
\end{align*}
...
For any $\delta_{x,k} > 0$ when we define 
$r\log\log x \leq k =: \log\log x + \delta_{x,k}$, we obtain the bounds that 
$1 \leq r \leq \frac{\log x}{\log\log x}$ for all large $x > e$ since 
$\omega(n) \leq \log_2(n)$ for any $n \geq 2$. 
Expanding logarithms while utilizing the notation for $D(x, r)$ from 
Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} leads to 
\[
\frac{D(x, r)}{x} \ll (\log x)^{r - 1 - r \log r} \ll 
     \frac{x^{1+\log\log\log x}}{(\log x)^{1+\log x}}. 
\]
Then we see that 
\[
\left\lvert \frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \right\rvert \ll 
     \frac{x^{\log\log\log x} \sqrt{\log\log x}}{(\log x)^{1 + \log x}} = o(1), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
Equivalently, we have shown that $\mathcal{N}_{\omega}(x) = o\left(\mathcal{D}_{\omega}(x)\right)$. 
The following bounds result from the triangle inequality for large $x$: 
\[
...
We see by taking $z = r = \frac{k-1}{\log\log x}$ that 
\[
...
\[
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|z|=r} (\widehat{G}(z) - \widehat{G}(r)) 
     \frac{(\log x)^{z}}{z^k \cdot (1+P(2)z)} dz. 
\]
After integrating by parts, we write that 
\[
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|z|=r} (\widehat{G}(z) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(z-r)) 
     (\log x)^{z} \left[\sum_{i \geq 0} (-1)^i z^{i-k} P(2)^{i}\right] dz. 
\]
Notice that 
\[
\widehat{G}(z) - \widehat{G}(r) - \widehat{G}^{\prime}(r)(z-r) = \int_{r}^{z} 
     (z-w) \widehat{G}^{\prime\prime}(w) dw \ll |z-r|^2. 
\]
We then define component integrands for $I_{2,k}(r, x)$ as follows for any integers $i \geq 0$: 
\[
T_{k,i}(r, x) := \frac{1}{2\pi\imath} \times \int_{|z|=r} (\widehat{G}(z) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(z-r)) (\log x)^{z} z^{i-k} dz.  
\]
With the parameterization $z = re^{2\pi\imath\theta}$ for real $\theta \in [-1/2,1/2]$, 
we get that 
\[
...
\widehat{C}_k(x) := 
     ...
     \frac{4\sqrt{2\pi} \cdot x}{3(2k+1)} \cdot \frac{(\log\log x)^{k+1/2}}{(k-1)!}. 
\end{align*} 
\end{cor} 
...
let $L_{\ast}(x) := \left\lvert \sum_{n \leq x} \lambda_{\ast}(n) \right\rvert$ for $x \geq 1$. 
...
\[
\pi_k(x) \asymp \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} (1 + o_k(1)), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
L_{\ast}(t) & := \left\lvert \sum_{n \leq t} (-1)^{\omega(n)} \right\rvert \asymp 
     ...
     -\frac{3t}{2 \sqrt{\log\log t}} + O\left(\frac{t}{\log\log t}\right), 
...
\[
\frac{1}{L_{\ast}^{\prime}(t)} \sim 
      -\frac{2\sqrt{2\pi}}{3} \sqrt{\log\log t}. 
\]
...
\begin{align*} 
     ...
     \asymp \frac{\widehat{C}_{k,\ast}^{\prime}(x)}{L_{\ast}^{\prime}(x)} \qquad\quad \implies \\ 
     C_{\Omega(x)}(x) \Iverson{\Omega(x) = k} & \sim -
     \frac{2\sqrt{2\pi}}{3} \widehat{C}_{k,\ast}^{\prime}(x) \sqrt{\log\log x} (1+o(1)) 
...
\begin{align*}
     \widehat{C}_{k,\ast\ast}(x) & \sim \frac{2\sqrt{2\pi}}{3} 
     \frac{\sqrt{\log\log x}}{(k-1)!} \times \sum_{n \geq 0} 
     (-k)^n \left[ 
...
notice that it suffices to truncate the infinite series at $n < 1$. 
...
\[
\mathbb{E}\left[C_{\Omega(n)}(n)\right] \sim \frac{2 \sqrt{2\pi} (\log n) \sqrt{\log\log n}}{3}. 
\] 
\end{lemma} 
\begin{proof} 
...
\sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \sim 
     \frac{2\sqrt{2\pi} \cdot x (\log x) \sqrt{\log\log x}}{3}. 
\end{align} 
We claim that 
\begin{equation} 
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v1} 
\sum_{n \leq x} C_{\Omega(n)}(n) = 
     \sum_{k \geq 1} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) \asymp 
     \sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n). 
\end{equation} 
If equation \eqref{eqn_proof_tag_PartialSumsOver_HatCkx_v1} holds, 
...
\frac{\sum\limits_{\log\log x < k \leq \log x} \sum\limits_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n)}{ 
     \sum\limits_{k=1}^{\log\log x} \sum\limits_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n)} 
     = o\left(x (\log x) \sqrt{\log\log x}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{equation} 
...
\begin{align*} 
S_{2,\varepsilon}(x) & := \sum_{\log\log x < k \leq 
     (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n). 
\end{align*} 
Then 
\[
\sum_{k=\log\log x}^{\log(x)} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) 
     \asymp \lim_{\varepsilon \rightarrow 1^{-}} S_{2,\varepsilon}(x),  
\]
with equality in asymptotic order 
as $\varepsilon \rightarrow 1$ because the upper bound of summation tends to 
$\log x$. 

Observe that whenever $\Omega(n) = k$, we have that $C_{\Omega(n)}(n) \leq k!$, with 
equality at the upper bound precisely when $\mu^2(n) = 1$, e.g., when $n$ is squarefree. 
We can then bound the sums defined above using 
Theorem \ref{theorem_MV_Thm7.20-init_stmt} with 
$r := \frac{k}{\log\log x} + o(1)$ for large $x$ by 
\begin{align*} 
S_{2,\varepsilon}(x) & = 
     \sum_{\log\log x \leq k \leq 
     (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \\ 
     ...
     & \ll \sum_{k=\log\log x}^{(\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     (\log x)^{\frac{k}{\log\log x} - 1} e^{-k} \sqrt{k} \\ 
     & \ll \frac{1}{(\log x)} \times \int_{\log\log x}^{ 
     (\log\log x)^{\frac{\varepsilon \log\log x}{\log\log\log x}}} 
     (\log x)^{\frac{t}{\log\log x}} e^{-t} \sqrt{t} \cdot dt \\ 
     & = o(x). 
\end{align*} 
...
\begin{align*} 
\mathbb{E}|g^{-1}(n)| & = \frac{2\sqrt{2}}{\pi^{3/2}} (\log n)^2 \sqrt{\log\log n} + 
...
\int \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = 
     \frac{\sqrt{2\pi} (\log t)^2 \sqrt{\log\log t}}{3} + 
...
\[
\mu_x(C) := \log\log x - \log\left(\frac{2\sqrt{2\pi}}{3}\right), 
...
The difference of the two auxiliary variables is negligible as 
...
\frac{1}{1+\frac{1}{2k}} & \sim \sum_{m \geq 0} \frac{(-1)^m}{(2 \mu_x(C))^m (1+\delta)^m} 
     \asymp 1 - \frac{1}{2 \mu_x(C)} \left(1+\delta+O(\delta^2)\right) \\ 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     ...
     \log\left(\frac{2\sqrt{2\pi}}{3}\right)\right)\right\} + o(1). 
\end{align*} 
\end{cor} 
\begin{proof} 
We claim that 
...
to see that the dominant term is given by the rightmost term in the last equation. 
\end{proof} 
...
\]
The following properties of $\Gamma(a, x)$ hold at positive real $z, a > 0$: 
\begin{align} 
...
\[
\Gamma(a, z) \sim \frac{z^{a-1} e^{-z}}{1-\lambda} + O_{\lambda}\left(z^{a-2} e^{-z}\right), 
\]
and if $a \geq 1$ is integer-valued, then as $z \rightarrow +\infty$ we have 
\[
\Gamma(a, -z) \sim \frac{(-z)^{a-1} e^{z}}{\left(1+\frac{1}{\lambda}\right)}. 
\]
\end{prop}
\begin{proof}
...
\begin{align*}
(-1)^a \cdot \Gamma(a, -z) & = -z^a e^{z} \times \sum_{n \geq 0} 
...
S_1(x) & := \frac{x}{\log x} \times \sum_{1 \leq k \leq \log\log x} \frac{(-1)^k (\log\log x)^{k-1}}{(k-1)!} 
     \sim -\frac{3x}{2\sqrt{2\pi}\sqrt{\log\log x}} + O\left(\frac{x}{\log\log x}\right), \\ 
...
Let the component sums $T_{1,1}(x)$ and $T_{1,2}(x)$ be defined by 
\begin{align*}
T_{1,1}(x) & := \sum_{\substack{k \leq \log\log x \\ k\mathrm{\ odd}}} 
     \frac{(\log\log x)^{k-1}}{(k-1)!} = \sum_{k=1}^{\frac{\log\log x}{2}} 
     \frac{(\log\log x)^{2k-1}}{(2k-1)!} \\ 
T_{1,2}(x) & := \sum_{\substack{k \leq \log\log x \\ k\mathrm{\ even}}} 
     \frac{(\log\log x)^{k-1}}{(k-1)!} = \sum_{k=0}^{\frac{\log\log x-2}{2}} 
     \frac{(\log\log x)^{2k}}{(2k)!}. 
\end{align*}
Then we have that 
\[
S_1(x) = \frac{x}{\log x} \left(T_{1,2}(x) - T_{1,1}(x)\right). 
\]
We find that for all sufficiently large $x$, 
the difference of the component sums can be expressed as a sum of 
convergent generalized hypergeometric functions 
so that we have the following expansions for large $x$: 
\begin{align*}
\frac{T_{1,2}(x) - T_{1,1}(x)}{\log x} & \sim \frac{1}{(\log x)^2} 
     + \frac{1}{\sqrt{2\pi}\sqrt{\log\log x}} \left[-1 + 
     \sum_{k \geq 0} \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1} 
     - \sum_{k \geq 1} \prod_{i=1}^{2k} \left(1+\frac{i}{\log\log x}\right)^{-1} 
     \right]. 
\end{align*}
We bound each of the two hypergeometric sums in the previous equation by 
summing each over a pair of disjoint intervals. The difference between these 
two series provides the main term for $S_1(x)$. 

Consider the first hypergeometric series term above. It is expanded as follows: 
\begin{align} 
\notag
H_{1,1}(x) & := \sum_{k \geq 0} \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1} \\ 
\notag
     & = \sum_{\substack{k \geq 0 \\ 2k+1 < \log\log x}} 
     \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1} + 
     \prod_{i=1}^{\log\log x - 1} \left(1+\frac{i}{\log\log x}\right)^{-1} \left[
     \sum_{\substack{2k+1 \geq \log\log x}} 
     \prod_{i=\log\log x}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1}
     \right] \\ 
\label{eqn_proofTag_H11x_HypGeomSeriesExp_v1} 
     & = \sum_{\substack{k \geq 0 \\ 2k+1 < \log\log x}} 
     \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1} + 
     O\left(\prod_{i=1}^{\log\log x - 1} \left(1+\frac{i}{\log\log x}\right)^{-1}\right). 
\end{align} 
...
\]
by appealing to an expansion of the factorial product by the Stirling numbers of the 
first kind where each component term $\frac{j}{\log\log x} < 1$ for all 
...
\sum_{\substack{k \geq 0 \\ 2k+1 < \log\log x}} 
     \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x}\right)^{-1} & = 
\sum_{\substack{k \geq 0 \\ 2k+1 < \log\log x}} 
     \prod_{i=1}^{2k+1} \left(1+\frac{i}{\log\log x} + O\left(\frac{i^2}{(\log\log x)^2}\right)\right) \\ 
     & = \sum_{\substack{k \geq 0 \\ 2k \leq \log\log x-2}} \left[
          1 + \frac{(2k+1)(2k+2)}{2 \log\log x} + o\left(\frac{k^2}{\log\log x}\right)\right] \\ 
     & \sim 
     \frac{(\log\log x)^2}{12} + \frac{5(\log\log x)}{8} - \frac{13}{12}. 
\end{align*}
A similar argument allows us to bound the second hypergeometric series term in the difference 
formula for $S_1(x)$. In particular, we see that 
\begin{align*}
H_{1,2}(x) & := \sum_{k \geq 1} \prod_{i=1}^{2k} \left(1+\frac{i}{\log\log x}\right)^{-1} 
     = \frac{(\log\log x)^2}{12} + \frac{5(\log\log x)}{8} - \frac{7}{12} 
     + O\left(\frac{1}{\log\log x}\right). 
\end{align*}
The difference of the asymptotic formulas for these series leads to the formula 
\[
\frac{x}{\log x} \left(T_{1,2}(x) - T_{1,1}(x)\right) = \frac{x}{(\log x)^2} - 
     \frac{3x}{2\sqrt{2\pi} \sqrt{\log\log x}} + O\left(\frac{x}{\log\log x}\right). 
     \qedhere 
\]
\end{proof}
\begin{proof}[Proof of \eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2_subeqnC}]
We can sum $S_2(x)$ exactly to arrive at the following expression: 
\[
S_2(x) = \frac{x \cdot \Gamma(\log\log x, \log\log x) (\log x) (\log\log x)}{\Gamma(\log\log x + 1)}. 
\]
When we apply \eqref{eqn_IncompleteGamma_PropC} with Stirlings formula to show that 
$\Gamma(\log\log x + 1) \sim \sqrt{2\pi (\log\log x)} \left(\frac{\log\log x}{e}\right)^{\log\log x}$
for all $x$ sufficiently large, we can then conclude the result. 
\end{proof}
...
\]
Bounding the remaining convergent hypergeometric series term in the expansion of 
$S_3(x)$ is similar to the task from 
Lemma \ref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2}. 
Instead of repeating that argument, we see that 
\begin{align*}
& \frac{_2F_2\left(1, \frac{3}{2}+\log\log x; 1 + \log\log x, 
