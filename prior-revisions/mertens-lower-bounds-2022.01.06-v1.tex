...
\newpage
\renewcommand{\contentsname}{Article Table of Contents}
...
\] 
The first figure in Appendix \hlocalref{appendix_figures_SeqsAndPartialSumsPlots} 
compares the values of the two classical summatory functions, $M(x)$ and $L(x)$, 
as discrete plots over the integers $1 \leq x \leq 2500$. 
...
(see Theorem \hlocalref{theorem_SummatoryFuncsOfDirCvls}; 
Corollary \hlocalref{cor_CvlGAstMu}; and  
Corollary \hlocalref{cor_Mx_gInvnPixk_formula}). 
This procedure identifies sequences of new signed and unsigned auxiliary functions 
that are deeply tied to the strong additivity of $\omega(n)$ and whose 
partial sums characterize $M(x)$ through the discrete convolution formulas 
stated as the prime indexed sums 
proved in Section \hlocalref{subSection_KeyApplications_NewExactFormulasForMx}. 

\subsection{Motivation for the new results in the article} 
...
Section \hlocalref{Section_InvFunc_PreciseExpsAndAsymptotics}. 
These identities inspire the deep connection between the 
unsigned inverse function, $|g^{-1}(n)|$, 
and the resulting additive prime counting combinatorics we formulate precisely in 
...
This observation restricts our ability to immediately state better, or tighter than the 
best previously known, upper bounds on $M(x)$ that hold as $x \rightarrow \infty$. 
The challenge of establishing tight lower bounds on $|M(x)|$ along infinite sequences of 
positive integers remains an open, classically hard problem that we wish to tackle by 
extensions of the work in this article. 
...
\subsection{Preliminary results on the Mertens function}
...
Section \hlocalref{subSection_PrelimProofs_Config_InversionTheorem}. 
The matrix-based arithmetic we cite to show that each of 
these inversion theorems holds provides an intuitive, and constructive, proof methodology 
that is usually only tenuously motivated in conventional references that state similar results 
(e.g., see \cite[Thm.~2.21--2.22; \S 2.14]{APOSTOLANUMT}). 
This observation motivates our exposition in the next section where we spend a short time proving 
each of the sub-results in the theorem carefully from first principles. 
...
Key consequences of Theorem \hlocalref{theorem_SummatoryFuncsOfDirCvls} 
...
An elementary proof of the key identity offering a 
curious Dirichlet convolution based formula for the 
indicator function of the primes is established at the start of 
Section \hlocalref{subSection_ProofOfSignednessOfgInvn_v1}. 
The formula that provides $\chi_{\mathbb{P}} + \varepsilon = (\omega + 1) \ast \mu$ 
exactly when evaluated at any $n \geq 1$ implies 
Corollary \hlocalref{cor_Mx_gInvnPixk_formula} 
...
inverse with respect to Dirichlet convolution by $g^{-1}(n)$, which exists and is unique because 
$g(1) = 1 \neq 0$. 
...
Namely, we observe a familiar formula for $g^{-1}(n)$ 
on an asymptotically dense infinite subset of integers (with limiting density of $\frac{6}{\pi^2}$), i.e., 
...
These regular properties are conjectured to distribute the distinct values of these unsigned 
sequences over $n \leq x$ each according to a non-centrally normal tending CDF with 
negligible error terms that vanishing as $x \rightarrow \infty$. 
In fact, as we see in the remarks below, the values of $|g^{-1}(n)|$ for $n \leq x$ at large $x$ 
...
These canonically additive prime omega functions and their special properties 
are characterized by the original celebrated form of the Erd\H{o}s-Kac theorem and 
related theorems that show asymptotically negligible density in the values 
of these sequences diverging substantially 
from their respective average order at nearly all sufficiently large $x$ 
(see Section \hlocalref{subSection_TheKnownDistsOfThePrimeOmegaFunctions_IntroResults_v1}). 
...
\frac{1}{n} \times \sum_{k \leq n} |g^{-1}(k)| = 
     ...
     \mathrm{\ as\ } n \rightarrow \infty. 
\]
In Section \hlocalref{subSection_ErdosKacTheorem_Analogs}, 
...
We prove that (see Proposition \hlocalref{prop_Mx_SBP_IntegralFormula}) 
\begin{align} 
...
M(x) & = G^{-1}(x) + 
     ...
     \right), x \geq 1, 
\end{align} 
and that (\cf Section \hlocalref{subSection_Relating_CknFuncs_to_gInvn}) 
\[
...
Bounds on the partial sums over the unsigned inverse functions as summands provide some local 
...
When combined with Corollary \hlocalref{cor_CvlGAstMu}, 
...
otherwise onto zero if the function $r(n)$ vanishes at this $n \geq 1$. 
...
(see Section \hlocalref{subSection_ErdosKacTheorem_Analogs}), 
...
Proposition \hlocalref{lemma_HatCAstxSum_ExactFormulaWithError_v1} and 
Corollary \hlocalref{cor_ExpectationFormulaAbsgInvn_v2} below. 
...
\newpage
\section{Appendix: Asymptotic formulas for partial sums involving the incomplete gamma function} 
...
\newpage
\section{Figures: Discrete plots of the classical summatory functions and 
         scaled partial sums of the new auxiliary sequences}
\label{appendix_figures_SeqsAndPartialSumsPlots} 

\begin{figure}[ht]

\centering
\fbox{\includegraphics[width=0.925\textwidth]{GInvFunctionSequenceCalculations-Figure1.png}}

\captionsetup{singlelinecheck=off} 
\caption*{{\large{\rm \textbf{\rm \bf Plot \thesection.1:}} 
          A comparison of the plots of the classical summatory functions, $M(x)$ and $L(x)$, 
          versus the scaled functions $|G^{-1}(x)|$ and 
          $|G^{-1}|(n) := \sum_{k \leq n} |g^{-1}(k)|$. 
          In the displayed figure, the absolute constants are given by 
          $C_1 := \left(1-\frac{6}{\pi^2}\right)^{-1}$ and 
          $C_2 := 1-\frac{6}{\pi^2}$ where the non-constant reciprocal weight on the values 
          of the last two functions is given by the summatory function that counts the number 
          of squarefree $n \leq x$, $Q(x) := \sum_{n \leq x} \mu^2(n)$ for any $x \geq 1$. 
          }}
\end{figure}
...
\centering
\fbox{\includegraphics[width=0.925\textwidth]{GInvFunctionSequenceCalculations-Figure2.png}}

\captionsetup{singlelinecheck=off} 
\caption*{{\large{\rm \textbf{\rm \bf Plot \thesection.2:}} 
          Four discrete plots comparing the (scaled) values of the partial sums of the 
          signed and unsigned inverse sequence and the sign-weighted components that 
          contribute to the signed sums, $G^{-1}(x)$. 
          Namely, $G^{-1}(x) := G^{-1}_{+}(x) + G^{-1}_{-}(x)$ where the functions 
          $G^{-1}_{+}(x) > 0$ and $G^{-1}_{-}(x) < 0$ for all $x \geq 1$ 
          and these components denote the unsigned contributions of only those summands 
...
\newpage
\section{Tables: Computations involving $g^{-1}(n)$ and its partial sums} 
