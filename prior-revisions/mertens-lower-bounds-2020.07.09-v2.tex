...
We claim that as $x \rightarrow \infty$ 
\begin{equation} 
\label{eqn_proof_tag_LHSDiff_v0} 
\sum_{k=1}^{x/2} g^{-1}(x) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     o(\sqrt{x}). 
\end{equation} 
To prove this assertion, we observe that 
\[
0 \leq \pi(x) - \frac{x}{\log x} \leq \frac{C \cdot x}{\log x}, 
\]
for some absolute $C > 0$ whenever $x \geq 2$. This implies that 
\[
\sum_{k=1}^{x/2} g^{-1}(x) \left[\pi\left(\frac{x}{k}\right) - \frac{x}{k \cdot \log(x/k)}\right] = 
     O\left(\sum_{k=1}^{x/2} g^{-1}(k) \cdot \frac{x}{k}\right), 
\]
where by Lemma \ref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} and 
...
\eqref{eqn_proof_tag_LHSDiff_ErrorTerm_v1} since by 
Theorem \ref{theorem_MV_Thm7.20-init_stmt} 
we have that for large $x \rightarrow \infty$ 
\begin{align*} 
