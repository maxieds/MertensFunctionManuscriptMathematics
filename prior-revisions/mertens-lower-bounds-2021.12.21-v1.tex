...
\newglossaryentry{GammaIncompleteGamma}{
    ...
    description={The incomplete gamma function is defined as $\Gamma(a, z) := \int_z^{\infty} t^{a-1} e^{-t} dt$ for 
                 ...
                 are discussed in Appendix \ref{subSection_OtherFactsAndResults}. },
    type={symbols},
...
\item[(B)] For squarefree integers $n \geq 1$, we have that 
     ...
     \]
\item[(C)] If $n \geq 1$ and $\Omega(n) = k$ for some $k \geq 1$, then 
     \[
...
\]
%Since $(\log x)^{\frac{1}{\log\log x}} = e$, 
...
from the appendix provides that for any $a \in (1, 1.76321) = \left(1, W(1)^{-1}\right)$ 
\begin{align}
\notag 
S_a(x) := 
...
\[
\widehat{C}_k^{\prime}(x) \asymp \widehat{\pi}_k(x) \asymp 
...
We can also show that there is an absolute constant $D_0 > 0$ such that the 
variance type sums 
\[
...
the integers $x = (4m+1)\#$ 
that each precisely the product of the first $4m+1$ primes for $m \geq 1$. 
