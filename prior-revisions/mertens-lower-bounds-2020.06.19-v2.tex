...
\[
\widehat{\pi}_k(x) \SuccSim \frac{x}{\log x} \left(\frac{\log 2}{\log x}\right)^{ 
...
$R := 1$ so that 
$$z \equiv z(k, x) = \frac{k-1}{\log\log x} \in [0, 1),$$ 
in the notation of Theorem \ref{theorem_HatPi_ExtInTermsOfGz} 
for all $x > e$ as $x \rightarrow \infty$. 
Singular values of the weight functions in $z$ of the 
terms $(a_{\ell}(x), b_{\ell}(x), c_{\ell}(x))$ correspond to the 
case when $z := 1$. By elementary calculus, the only other extremity in the 
range $z \in [0, 1)$ occurs at the lower bound of $z := 0$. 
Because we restrict the indices of $1 \leq k \leq \log\log x$ in the theorem, 
we have that $z \neq 1$ for all $x$ as $x \rightarrow \infty$. 
However, we need to consider the limiting contributions of these terms at the 
right-hand boundary as $z \rightarrow 1^{-}$. 
In particular, we have that $z$ is closest to $1$ when 
\[
z(k, x) \equiv z(\log\log x, x) = 1 - \frac{1}{\log\log x}, 
\]
so that $1 \leq (1-z)^{-1} \leq \log\log x$. 

The extremal values of the coefficients of $c_{\ell}(x)$ thus occur when 
$z := 0$ or as $x \rightarrow \infty$ by contributing 
\[
\exp\left(c_{\ell}(x) \left[ 
     1 - \frac{4}{1-z} + \frac{5}{(1-z)^2} - \frac{2}{(1-z)^3} 
     \right]\right) \leq \exp\left(c_{\ell}(x) \left[ 
     1 - 4 \log\log x + 5 (\log\log x)^{2} - 2 (\log\log x)^{3}\right]\right). 
\]
So we can assume that as $x \rightarrow \infty$, a lower bound on the factors 
asserted by the form of these terms is taken by effectively setting $z := 0$. 
Next, we consider the coefficients of $b_{\ell}(x)$ in our product expansion: 
\[
1 \leq \exp\left(b_{\ell}(x) \left[1 - \frac{2}{1-z} + \frac{1}{(1-z)^2}\right]\right) 
     \leq \left(\frac{x}{2}\right)^{1 - 2\log\log x + (\log\log x)^2}. 
\]
Then it follows that a minimum on the exponential of the function in 
\eqref{eqn_proof_tag_PHatFiniteTruncProdFactorOfGz_v3} again corresponds to 
setting $z := 0$. The contribution resulting from this factor in the product is 
one. Lastly, we will bound the contributions to the product from the coefficients of 
$a_{\ell}(x)$ as follows: 
\begin{align*} 
\exp\left(-a_{\ell}(x) \left[1 - \frac{1}{1-z} + z\right]\right) & \geq 
     \left(\frac{\log 2}{\log x}\right)^{z} \\ 
\exp\left(-a_{\ell}(x) \left[1 - \frac{1}{1-z} + z\right]\right) & \leq 
     \left(\frac{\log 2}{\log x}\right)^{z+1} \left(\frac{\log x}{\log 2}\right)^{ 
     \log\log x}. 
\end{align*} 
So we select the bound from below in the first of the previous two equations in 
constructing our result. In summary, we have arrived at a proof that 
...
its interval that $\Gamma(z+1) \leq \Gamma(2) = 1$. 
\end{proof} 
...
\left\lvert G_E^{-1}(x) \right\rvert & \SuccSim 
     \frac{e \log 2}{\sqrt{2\pi}} \times 
     \frac{(\log x)^{\frac{3}{2}}}{\sqrt{\log\log\log x}} \times 
...
\notag 
     & \gg \frac{e \log 2}{\sqrt{2\pi}} \cdot 
...
T_E(x) := \frac{1}{\log\left[(\log x)^{\frac{3}{2}} (\log\log x)\right]
     \sqrt{\log\log\left[(\log x)^{\frac{3}{2}} (\log\log x)\right]}} \gg 
...
\label{eqn_proof_tag_SE1xFuncExp_v1} 
     & \gg \frac{e \log 2}{\sqrt{2\pi}} \times (\log x)^{\frac{3}{2}} (\log\log x) 
     T_E(x) \times 
     \left\lvert \sum_{e < d \leq \log x} 
     \frac{\lambda(d) (\log d)^{\frac{1}{4}}}{d \cdot \log\log d} 
     \right\rvert \\ 
...
\[
C_{\ell,1} = \frac{e^2 (\log 2)^2}{2 \pi} \approx 0.565015.  
\]
...
G_{\ell}^{-1}(x) := \frac{e \log 2}{\sqrt{2\pi}} \cdot 
...
\frac{e \log 2}{\sqrt{2\pi}} \times 
     ...
     \right\rvert & \gg \frac{e \log 2}{\sqrt{2\pi}} \times 
...
$$I_{\ell}(t) := \frac{e \log 2}{\sqrt{2\pi}} \times 
...
I_{\ell}\left(e^{e^{2k}}\right) e^{e^{2k}}& \SuccSim 
     \frac{e^2 (\log 2)^2}{4\sqrt{2} \pi} \cdot \frac{e^{-\frac{3k}{2}}}{k^{\frac{3}{2}}} 
     =: \widehat{I}_{\ell}(k). 
\end{align} 
...
\begin{align} 
\notag 
\frac{e^2 (\log 2)^2}{2\pi} & \times (\log x)^{\frac{3}{2}} (\log\log x) \times T_E(x) \times 
     \left\lvert 
     ...
     e^{e^{2k}} dk \right\rvert \\ 
\notag 
     & \gg \frac{e^2 (\log 2)^2}{2\pi}  \times (\log x)^{\frac{3}{2}} (\log\log x) \times T_E(x) \times 
     \Biggl\lvert 
...
\label{eqn_proof_thm_GInvFunc_v4v2_approx} 
     & \ll 
     \frac{e^{\frac{3}{4}} \cdot C_{\ell,1} \cdot (\log x)^{\frac{3}{2}}}{2 \cdot  
     (\log\log x)^{\frac{3}{4}} (\log\log\log x)^2}. 
\end{align} 
