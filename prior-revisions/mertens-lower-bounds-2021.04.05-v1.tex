...
\newglossaryentry{Nupn}{
    ...
    description={The valuation function that extracts the maximal exponent of $p$ in the prime factorization of $n$, e.g., 
                 ...
                 That is, when $p^{\alpha}$ exactly divides $n$) for $p$ prime, $\alpha \geq 1$ and $n \geq 2$.},
    type={symbols},
...
prime counting function $\pi(x)$ we prove in 
...
magnitude of $|g^{-1}(n)|$ that is analogous to the established Erd\H{o}s-Kac theorems for 
$\omega(n)$ and $\Omega(n)$. 
...
the average order of the unsigned sequence is bounded by 
\[
\mathbb{E}|g^{-1}(n)| = \frac{6}{\pi} \cdot \frac{(\log n)^2}{\sqrt{\log\log n}} (1+o(1)),  
...
on an infinite set of the integers with asymptotic density one over the integers. 
...
for every $\varepsilon > 0$ and all sufficiently large $x > e$ we have that  
\[
...
can be sindexed over only the primes $p|n$ given our definition of $h$ from above. 
We can inductively 
...
where the same function is taken to be one for $n := 1$. 
...
defined by $\widehat{\mathcal{G}}(z) := \widetilde{F}(1, z) / \Gamma(1 + z)$ where 
we take 
\[
...
\[
\{\lambda(n) C_{\Omega(n)}(n) \}_{n \geq 1} \mapsto \{
...
maximal depth in the previous equation) we get the relation 
\begin{equation} 
...
\subsection{Analytic proofs and adaptations of bivariate DGF methods for summing additive functions} 
...
\[
\widehat{A}_z(x) = \frac{x}{\Gamma(z)} \times \widehat{F}(2, z) \times (\log x)^{z-1} + 
     O_{z}\left(x (\log x)^{\Re(z) - 2}\right). 
\]
\end{theorem} 
\begin{proof} 
We can again see from the proof of 
Proposition \ref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1} 
that 
\[
C_{\Omega(n)}(n) = \begin{cases} 
...
By computing a Laplace transform on the right-hand-side of the above equation, we obtain that 
\begin{align*} 
...
the DGF expansion of $\widehat{F}(s, z) \zeta(s)^{z}$, then 
\[
...
sum components when we take $|z| \leq R$ in the following form: 
\begin{align*} 
...
\widetilde{A}_r(x) := 
     -\int_{|v|=r} \frac{x (\log x)^{-v}}{(\log x) \Gamma(1+v) 
     v^{k} (1 - P(2) v)} dv. 
\end{align} 
...
I_k(r, x) & = \frac{(-1)^{k}}{2\pi\imath} \times \int_{|v|=r} 
     \frac{\widehat{G}(v) (\log x)^{-v}}{v^k (1 - P(2)v)} dv \\ 
...
I_{1,k}(r, x) & := \frac{\widehat{G}(r)}{2\pi\imath} \times \int_{|v|=r} 
     \frac{(\log x)^{-v}}{v^k (1-P(2)v)} dv. 
\end{align*}
We can inductively compute the remaining coefficients 
$I_{1,k}(r, x)$ with respect to 
$x$ for fixed $k \leq \log\log x$ and apply the Cauchy integral formula. 
...
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} (\widehat{G}(v) - \widehat{G}(r) - 
     ...
     (\log x)^{-v} \left[\sum_{q \geq 0} v^{q-k} P(2)^{q}\right] dz. 
\]
Notice that 
\[
...
\]
We can define component integrands for $I_{2,k}(r, x)$ as follows for any integers $q \geq 0$: 
\[
T_{k,q}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} (\widehat{G}(v) - \widehat{G}(r) - 
     \widehat{G}^{\prime}(r)(v-r)) (\log x)^{-v} v^{q-k} dz.  
\]
With the parameterization $z = r e^{2\pi\imath\theta}$ for real $\theta \in [-1/2,1/2]$, 
we get that 
\[
T_{k,q}(r, x) \ll r^{3-k+i} \int_{-1/2}^{1/2} (\sin \pi\theta)^2 e^{(k-i-1) \cos(2\pi\theta)} d\theta. 
\]
...
setting $r := \frac{k-1}{\log\log x}$. 
\begin{align*}
T_{k,i}(r, x) & \ll r^{3-k+q} e^{k-q-1} \times \int_0^{\infty} \theta^2 e^{-8(k-q-1) \theta^2} d\theta \\ 
     & \ll \frac{r^{3+q-k} e^{k-q-1}}{(k-q-1)^{3/2}} \ll 
     \frac{(\log\log x)^{k-3-q} e^{k-q-1}}{(k-1-q)^{3/2} (k-1)^{k-3-q}} \\ 
     & \ll 
     \frac{k (\log\log x)^{k-3-i}}{(k-1)!}. 
\end{align*}
Then it follows that with $r := \frac{k-1}{\log\log x}$, the sums 
\[
\sum_{q \geq 0} |T_{k,q}(r, x)| P(2)^q \ll \frac{k (\log\log x)^{k-3}}{(k-1)!} (1+o(1)). 
\]
...
\end{cor} 
\begin{proof} 
Suppose that $h$ is an arithmetic function. 
We have an integral formula that results by 
...
The argument is based on first approximating $L_{\ast}(t)$ for large $t$ 
...
\sum_{n \leq x} (-1)^{\omega(n)} C_{\Omega(n)}(n) z^{\Omega(n)} = 
     \frac{x \widehat{F}(2, z)}{\Gamma(z)} (\log x)^{z-1} + O_z\left( 
     x (\log x)^{\Re(z)-2}\right). 
\]
...
\widehat{B}(x, r) \ll x (\log x)^{r-2r\log r} \sqrt{\log\log x} = 
...
The rest of our argument parallels the method in the proof of the related theorem in 
\cite[Thm.\ 7.21; \S 7.4]{MV} from Montgomery and Vaughan closely. 
After a change of variable we obtain the limiting 
...
$$\frac{f(k, x) \sqrt{\log\log x}}{(\log x)} 
  ...
  \sigma_x(C)^2}\right),$$ 
we perform the change of variable $s \mapsto \frac{t \sqrt{\log\log x}}{(\log x)}$ to obtain the 
...
For $Y > 0$ and we have uniformly at all $-Y \leq z \leq Y$ 
as $x \rightarrow \infty$ that 
\begin{align*} 
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq z\right\} & = 
     \Phi\left\{\frac{6 \sigma_x(C)}{\pi^2}\left(\frac{\pi^2 z}{6} + \sigma_x(C)\right) - 
...
The most recent known upper bound on $L(x)$ (assuming the RH) is proved by 
...
G^{-1}(x) & = O\left( 
     ...
     \right), \\ 
     & = O\left((\log x)^2 \sqrt{\log\log x} \times x^{\frac{1}{2} + 
...
The third formula above follows from summation by parts. 
\end{proof} 
...
\]
The function $\Gamma(a, z)$ can be continued to an analytic function of $z$ on 
$\mathbb{C} \mathbin{\big\backslash} \{0\}$. 
For $a \in \mathbb{Z}^{+}$, $\Gamma(a, z)$ is an entire function of $z$. 
...
For $x \rightarrow +\infty$, we have that 
\begin{align*}
S_3(x) := \sum_{1 \leq k 2\leq \log\log x} \frac{(\log\log x)^{k-1/2}}{(2k-1) (k-1)!} = 
