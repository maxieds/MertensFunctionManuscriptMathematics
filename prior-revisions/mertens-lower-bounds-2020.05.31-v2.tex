...
\sum_{\substack{e \leq n \leq x \\ n \in \mathcal{S}_E \\ \lambda(n) = +1}} g^{-1}(n) & \geq 
     ...
     \sum_{\substack{e \leq n \leq (\log x)^2 \\ n \in \mathcal{S}_E \\ \lambda(n) = +1}} 
...
\left\lvert G_E^{-1}(x) \right\rvert & \SuccSim 
     ...
     \frac{(\log x)^2}{(\log\log\log x)^{\frac{5}{2} + \log\log\log x}} \times 
...
$-\widehat{\tau}^{\prime}_0(t) \gg \frac{\log t \cdot \log\log t}{t^2}$ in the notation of 
Lemma \ref{lemma_CLT_and_AbelSummation}. 

Let the remainder term sums be defined for all sufficiently large $x$ by 
\[
R_E(x) := \left\lvert \sum_{\log\log\log x < d < \frac{(\log x)^2}{e}} 
     \lambda(d) \cdot \log d \cdot \log\log d \times 
...
cannot match in order to cause a zero-tending cancellation of the terms when $x$ is large. 
...
R_E(x) & \ll (\log x)^2 \times \sum_{\log\log\log x < d \leq \frac{(\log x)^2}{e}} 
     ...
     & \sim (\log x)^2 \times \left[\frac{(\log t)^2}{2} \log\log t - 
...
Let $\varepsilon_0 := \frac{1}{2} \min(B, 1-C)$ for $B, C \in (0, 1)$ given by the hypotheses of 
Theorem \ref{theorem_CondAvgOrderGInvxSummatoryFunc_v1}. 
Then we have 
