...
\begin{definition}
For any $z \in (-\infty, \infty)$, the cumulative density function of the 
...
$c(t)$ is defined as in equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v2} 
for all real $t \geq 19$. 
It is not difficult to see that $\mathcal{C}(t)$ is continuous and differentiable at all $t \in (1, \infty)$. 
We can see by computation from equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v2} that 
for all sufficiently large $t > 1$, the derivative of this function satisfies 
$\mathcal{C}^{\prime}(t) < 0$. 
Moreover, we have that $1 \leq \mathcal{C}(t) < 2$ for all $t \geq 1$ where 
$0 \leq \frac{c(x)-1}{\log\log x} \leq 1$ for all integers $x \geq 19$ and where 
$\mathcal{G}(0) = \mathcal{G}(1) = 1$. This means that 
\[
\lim_{x \rightarrow \infty} \mathcal{C}(x) = 1, 
\]
and so $B_0^{\ast}(x) \xrightarrow{x \rightarrow \infty} 1$, 
which completes step \textbf{(ii)}. 
This step combined with equation \eqref{eqn_ProofTag_MainTermFunc_LMOmegax_v4} 
...
are both attained at finite $e^e < n_{\ell}, n_u < +\infty$ and 
