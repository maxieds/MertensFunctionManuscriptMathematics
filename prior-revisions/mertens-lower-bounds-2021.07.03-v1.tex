...
with itself at $n$, that we prove by elementary methods in 
...
\widehat{C}_{k,\ast}(x) := \sum_{\substack{n \leq x \\ \Omega(n) = k}} 
     (-1)^{\omega(n)} C_k(n) 
\]
Let the function $\widehat{G}(z) := \widehat{F}(2, z) / \Gamma(1+z)$ for 
...
\[
\left\lvert \widehat{G}\left(\frac{1-k}{\log\log x}\right) \right\rvert = 
     \left\lvert \frac{\left(1-\frac{k-1}{\log\log x}\right)}{ 
     \Gamma\left(2-\frac{k-1}{\log\log x}\right)} \cdot 
     \frac{\zeta(2)^{\frac{k-1}{\log\log x}}}{\left(1-\frac{P(2)(k-1)}{\log\log x}\right)} 
     \right\rvert 
     \gg \frac{\left(1+\frac{1}{\log\log x}\right)}{\Gamma\left(1+\frac{1}{\log\log x}\right)} 
     \gg 1. 
\]
...
using the following uniform asymptotics\footnote{
     ...
     $\widetilde{\mathcal{G}}(2) \approx 0.607927$. 
} 
...
Proposition \ref{prop_IncGammaLambdaTypeBounds_v1} with $\lambda = \frac{1}{2}$ 
from the appendix section, respectively, with 
