...
We can also show that there is an absolute constant $D_0 > 0$ such that 
\[
\frac{1}{n} \times \left(\sum_{k \leq n} C_{\Omega}(k)^2 - 
     \left(\sum_{k \leq n} C_{\Omega}(k)\right)^2\right) = 
     D_0 n (\log n)^2 (\log\log n) (1 + o(1)), \text{ as } n \rightarrow \infty. 
\]
...
In particular, since we have for sufficiently large $n$ that 
