...
\epsilon(q) := \frac{1}{2}\left(1 + q - \frac{1}{q}\right) < \frac{2}{3},
     \mathtext{ for all } q \in \left(\frac{1 + \sqrt{37}}{6}, \infty\right). 
\]
For example, we can select $q := \frac{6}{5}$ to obtain the desired bound\footnote{
     This choice of $q$ is made by rounding the lower range 
     $\frac{1 + \sqrt{37}}{6} \approx 1.18046$ upwards to one decimal place of precision. 
     This yields the exponent 
     $\epsilon\left(\frac{6}{5}\right) = \frac{41}{60} \approx 0.683333$. 
}. 
Theorem \hlocalref{theorem_MV_Thm7.20-init_stmt} then shows that 
