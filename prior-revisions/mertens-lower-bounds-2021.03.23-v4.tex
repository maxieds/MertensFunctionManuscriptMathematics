...
these results imply a regular, normal tendency 
...
analogous results bounding the distribution of $\omega(n)$. We have that 
for $0 < R < 2$ 
\begin{equation}
...
This result is cited in the proof of our new result stated in 
Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2} of 
Section \ref{Section_NewFormulasForgInvn} 
(see Appendix \ref{subSection_OtherFactsAndResults}). 
...
\end{align*} 
As $x \rightarrow \infty$, we have that $\mathcal{D}_{\omega}(x) / \mathcal{N}_{\omega}(x) = o(1)$ 
...
Next, we utilize the notation for and bounds on the function $D(x, r)$ from 
...
Then we see that 
\[
\left\lvert \frac{\mathcal{D}_{\omega}(x)}{\mathcal{N}_{\omega}(x)} \right\rvert = 
     O\left(\frac{1}{\sqrt{\log\log x}}\right) = o(1), 
...
\[
1+o(1) = \left(\frac{\mathcal{D}_{\omega}(x)- 
     \mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)}\right)^{-1} \ll 
     \frac{\mathcal{D}_{\omega}(x)}{\mathcal{A}_{\omega}(x)} \ll 
     \left(\frac{\mathcal{D}_{\omega}(x)+\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)}\right)^{-1} = 
     1 + o(1). 
\]
...
S_2(x) & := \frac{1}{x \sqrt{\log\log x}} \times 
     \sum_{k \geq \log\log x} \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     C_{\Omega(n)}(n) \ll \widehat{B}(x, 1) = o(1). 
\end{align*} 
We have proved that 
...
\begin{align*} 
\frac{2 (\log\log x)^{k+\frac{1}{2}}}{(2k+1)(k-1)!} & \sim 
...
A tight bound on the main term of our sum is argued in cases on the 
sign of the first sum on the right-hand-side of 
\eqref{eqn_LemmaProofTag_ExpMinust_TaylorsRemainderThm_v1}.
Let $s_t := \operatorname{sgn}\left(\sum_{0 \leq k < t} \frac{(-t)^k}{k!}\right)$. 
If $s_t = -1$, then we get that 
\[
\left\lvert \sum_{0 \leq k < t} \frac{(-t)^k}{k!} \right\rvert \leq 
     \frac{\log x}{\sqrt{2\pi \log\log x}} + O\left(\frac{1}{\log x}\right). 
\]
When $s_t = +1$, then for all large $x$ we must have that the sign on the 
remainder term from Taylor's theorem in 
\eqref{eqn_LemmaProofTag_ExpMinust_TaylorsRemainderThm_v1} is negative. 
Reversing the corresponding inequality yields the symmetric bound that 
\[
\left\lvert \sum_{0 \leq k < t} \frac{(-t)^k}{k!} \right\rvert \geq 
     \frac{\log x}{\sqrt{2\pi \log\log x}} + O\left(\frac{1}{\log x}\right). 
     \qedhere 
\]
\end{proof}
...
It follows that 
\[
\frac{\sqrt{\pi}}{4} \operatorname{erfi}\left(\sqrt{\log\log x}\right) = 
...
\end{align}
The rightmost sum corresponds to another error term. Indeed, we see that the inner sum for the 
error term in the 
...
\sum_{\substack{k \geq 0 \\ 2k+3 < \log\log x}} 
     ...
     \frac{1}{8\log\log x}. 
\end{align*}
Hence, the main term is the first leading term in the 
expansion of $S_3(x)$ from \eqref{eqn_ProofTag_S3xThreeTermSum_Exp_v1}. 
\end{proof}
\end{subequations}
