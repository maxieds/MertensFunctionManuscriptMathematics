...
$x \geq 3$ we can approximate\footnote{
     Since $\pi(1) = 0$, the actual range of summation on the left-hand-side 
     corresponds to $k \in \left[1, \frac{x}{2}\right]$. 
}
\begin{subequations}
\begin{align} 
\notag 
...
\textbf{Part D.} 
The lower bound on $R_E(x)$, consider 
...
equation are bounded by an absolute positive constant 
\begin{align*} 
\frac{R_E(x)}{(\log x)^{\frac{5}{4}} (\log\log x)^{\frac{1}{4}}} & \gg 
     \left\lvert 
     \sum_{\log x < d < \frac{(\log x)^{5} (\log\log x)}{2e}} 
...
\end{lemma} 
\begin{proof} 
By the first condition $\widehat{\pi}_k^{(\ell)}(x) = o\left(\widehat{\pi}_k(x)\right)$ we find that 
\begin{align*} 
