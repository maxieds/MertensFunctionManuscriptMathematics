...
\title{
       \LARGE{
       New exact formulas for partial sums of the M\"obius function expressed by 
...
\newpage
\section{Introduction (TODO -- Needs work)} 
...
We use the notation $|g^{-1}(n)|$ to denote the absoute value of $g^{-1}(n)$. 
...
Hence, we have a key takeaway that 
there is an \emph{essentially additive}, rather than multiplicative, structure underneath the 
unsigned sequence $\{|g^{-1}(n)|\}_{n \geq 2}$. 
\end{observation} 
...
i.e., the prime sums defined as the DGF of $\chi_{\mathbb{P}}(n)$. 
...
\end{align} 
TODO: Pull out in actual Theorem statement ... 
...
(see Theorem \hlocalref{prop_Mx_SBP_IntegralFormula}; 
...
\section{An extension of the Selberg-Delange method}

We see that for $\Re(s) > 1$, the function $\widehat{F}(s, z) = \zeta(s)^{-z} (1+P(s)z)^{-1}$, 
which involves a complex power of the Riemann zeta function. 
...
In the reference, the author 
considers asymptotics of the partial sums of the coefficients in the DGF expansion of 
functions of the form $F(s, z) := G(s, z) \zeta(s)^{z}$. 
%Tenenbaum's reference 
%provides a proof of asymptotic formulae (main and error terms) when $G(s, z) \equiv H(s)$, 
%which is an analytic function of $s$ only that does not depend on the parameter $z$. 
Our choice of the $z$-dependent function $\widehat{F}(s, z)$ given in the next definition 
...
We then apply the extension of Tenenbaum's Selberg-Delange method proof to extract 
...
\section{Properties of the function $g^{-1}(n)$} 
\label{Section_NewFormulasForgInvn_v2} 

\begin{definition}
For integers $n \geq 1$, we define the function $g(n) := \omega(n) + 1$ and 
take its Dirichlet inverse to be 
\[
g^{-1}(n) = (\omega + \mathds{1})^{-1}(n), \text{ for } n \geq 1. 
\]
The function $|g^{-1}(n)|$ denotes the unsigned inverse function, i.e., the 
absolute value of the signed function $g^{-1}(n)$. 
\end{definition}
...
selected so that the resulting arithmetic function we convolve with $\mu(n)$ in constructing the 
summatory functions in 
Theorem \ref{prop_Mx_SBP_IntegralFormula} (below) 
is Dirichlet invertible with $(\omega + \mathds{1})(1) \neq 0$. 
...
The $\{\pm 1\}$-valued sign of the non-zero function $g^{-1}(n)$ is given by 
...
\caption{Histograms representing the CDF of the distribution of the distinct absolute values of 
         $|g^{-1}(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)|$ for $n \leq x$. 
         The dashed line shows the CDF of a normally distributed random variable, $X \sim \mathcal{N}(\mu, \sigma^2)$, 
         scaled by $\zeta(2)^{-1}$ where $\mu \equiv \mu(x) = 2B_0 \sqrt{\log\log\log x}$ and 
         $\sigma \equiv \sigma(x) = 4 B_0 \sqrt{(\log x) (\log\log\log x)}$. } 
\label{figure_HistgInvnDistUns_Comparisons_v1}
...
we can prove the next formulas as special cases of 
Corollary\hlocalref{cor_CvlGAstMu}. 

\begin{definition}
\begin{subequations}
The summatory function of $g^{-1}(n)$ is defined for all $x \geq 1$ by the partial sums 
\begin{equation}
G^{-1}(x) := \sum_{n \leq x} g^{-1}(n) = \sum_{n \leq x} \lambda(n) |g^{-1}(n)|. 
\end{equation}
...
\end{subequations}
\end{theorem} 
\begin{proof}[Proof of \eqref{prop_Mx_SBP_IntegralFormula_PartA} and \eqref{prop_Mx_SBP_IntegralFormula_PartB}] 
...
\end{proof} 
\begin{proof}[Proof of \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1}]
Lemma \hlocalref{lemma_AnExactFormulaFor_gInvByMobiusInv_v1} 
...
\section{Conclusions (TODO -- Need to edit)}
