...
$$C_{\Omega}(n) := (\Omega(n))! \times \prod_{p^{\alpha}||n} \frac{1}{\alpha!}, \text{ for } n \geq 2,$$ and 
...
summatory functions with explicit distributions at large $x$. 
...
is unique to our work to establish 
...
through the sums of the unsigned auxiliary sequence $C_{\Omega}(n)$. 
...
For $n \geq 1$ off of the squarefree integers, 
...
%which leads to the conclusion of the following result 
...
selected so that the resulting arithmetic function we convolve with $\mu(n)$ 
...
the M\"obius function, $\mu^2(n) = |\mu(n)|$. This result allows us to see that up to 
the leading sign weight by $\lambda(n)$ on the values of this function, there is an 
essentially additive structure beneath the distinct values of $g^{-1}(n)$ for $n \leq x$. 
