...
\newglossaryentry{gInvn}{
    ...
    description={The Dirichlet inverse function, $g^{-1}(n) = (\omega+1)^{-1}(n)$ with corresponding 
                 summatory function $G^{-1}(x) := \sum\limits_{n \leq x} g^{-1}(n)$. },
    type={symbols},
...
our new characterization of $M(x)$ using two primary 
auxiliary sequences and their 
...
\] 
The previous two formulas lead us to the 
...
following forms for any fixed $\epsilon > 0$ 
...
\subsubsection{An exact expression for $M(x)$ via strongly additive functions -- TODO UPDATE} 
...
(see Table \ref{table_conjecture_Mertens_ginvSeq_approx_values} in the appendix section) 
\[
...
Hence, there is clear additive structure that underlies the sequence 
$\{g^{-1}(n)\}_{n \geq 2}$. 
\end{observation} 
...
provides that there are arbitrarily large $x$ such that 
\[
...
D_{(\omega+1)^{-1}}(s) = \frac{1}{\zeta(s) (P(s)+1)}. 
\end{align} 
...
$|z| < P(2)^{-1}$ where $\widehat{F}(s, z)$ is defined as 
...
using the following uniform asymptotics\footnote{
     We can in fact show that for any $1 \leq k \leq 2\log\log x$, 
     the function $\widetilde{\mathcal{G}}(z)$ defined in 
     Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} satisfies 
     \[ 
     \widetilde{\mathcal{G}}(0) \geq \widetilde{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) \geq 
...
\[
\pi_k(x) = \widetilde{\mathcal{G}}\left(\frac{k-1}{\log\log x}\right) 
     \frac{x}{\log x} \cdot \frac{(\log\log x)^{k-1}}{(k-1)!} \left[1 + 
...
\notag
     ...
     \frac{1}{2} \int_3^x \widehat{G}\left(\frac{k-1}{\log\log t}\right) 
     (\log\log t)^{k-\frac{3}{2}} dt \\ 
\notag
     & = 2A_0 \sqrt{2\pi \log\log x} \widehat{C}_{k,\ast}(x) 
     - \frac{x}{2} \widehat{G}\left(\frac{k-1}{\log\log b}\right) \times 
     \int_3^x \frac{(\log\log t)^{k-\frac{3}{2}}}{t (\log t)^2} dt, 
     \text{ some } 3 \leq b \leq x \\ 
\notag 
     & = 2A_0 \sqrt{2\pi \log\log x} \widehat{C}_{k,\ast}(x) + 
...
\notag 
     ...
     1 + O\left(\frac{1}{\log\log x}\right)\right]. 
\end{align}
The last line once again follows by seeing that 
...
\sum_{k=1}^{2\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \asymp 
     ...
     \left[1 + O\left(\frac{1}{\log\log x}\right)\right] \\ 
\notag 
     & = \frac{x \sqrt{\log\log x} \Gamma(2\log\log x, \log\log x)}{\Gamma(2\log\log x)} 
     \left[1 + O\left(\frac{1}{\log\log x}\right)\right] \\ 
\notag 
...
\subsection{Analogs to the Erd\H{o}s-Kac theorem for the distributions of the unsigned auxiliary functions} 
...
\frac{2A_0\sqrt{2\pi}}{\log x} & \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     ...
     \times \exp\left(-\frac{t^2}{2\sigma_x(C)^2}\right) \left( 
     1 + \widetilde{E}(x, t)
     \right). 
\end{align*} 
...
\subsection{Some asymptotics of $G^{-1}(x)$} 
...
\end{lemma}
\begin{proof} 
By Proposition \ref{lemma_gInv_MxExample}, we have that for all squarefree integers 
$n \geq 1$ 
\begin{align*} 
...
\begin{align*} 
\left\lvert \sum_{k=2}^{\log_2\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4m-1}} 
     g^{-1}(p_1^k t) \right\rvert & \leq \left\lvert 
     \sum_{k=2}^{\log_2\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4m-1}} 
...
we get that 
\begin{align*}
\left\lvert \sum_{k=2}^{\log_{p_s}\left((4m+1)\#\right)} \sum_{\substack{t \\ \omega(t)=4m+1-r}} 
...
$G^{-1}(x)$ infinitely often, there is a preponderance of evidence leading us to 
expect that there is usually a large amount of cancellation between the successive 
...
Then \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} 
shows that we indeed get substantial cancellation from the 
summatory function terms involving $G^{-1}(x)$ in the formula for $M(x)$ 
along the sequence of $x = (4m+1)\#$. 
\end{remark}
