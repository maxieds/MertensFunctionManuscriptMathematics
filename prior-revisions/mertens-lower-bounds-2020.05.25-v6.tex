...
\mathbb{E}|g^{-1}(n)| & \geq \frac{1}{n} \times \sum_{\substack{m \leq n \\ \mu^2(m) = 1}} \sum_{d|m} C_{\Omega(d)}(d) \\ 
     ...
     & \sim \frac{1}{n} \times \left((\log n)^3 (\log\log n) Q(n) - \int Q(n) \frac{d}{dn}\left[ 
...
G^{-1}(x) \geq \sum_{n \leq x} \lambda(n) \sum_{\substack{d|n \\ d > e^e}} (\log d) (\log\log d). 
\end{equation} 
...
\mathcal{A}_E := \left\{x \geq 1: 
     G^{-1}(x) \geq G_E^{-1}(x) \wedge G^{-1}(x) \leq 0 
     \right\}, 
\] 
we have that $|G^{-1}(x)| - |G_E^{-1}(x)| \leq 0$, $\forall x \in \mathcal{A}_E$. 
