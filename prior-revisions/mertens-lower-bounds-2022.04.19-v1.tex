...
\subsection{Discussion of the new formulas for $M(x)$}
...
\begin{remark}[Prospective applications]
...
Note that using the distribution of $|g(n)|$ predicted by the proposition to 
evaluate the new formulas for $M(x)$ in 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} 
asymptotically still requires more information 
...
\subsection{Discussion of the significance of the new results}
...
exact expressions for $M(x)$ in terms of $L(x)$, e.g., than the 
identity from equation \eqref{eqn_MxInTermsOfLx_v1}. 
...
of $\mu(n)$ predicted by models of this function by Sarnak's conjecture. 
On the other hand, arguments as to why the new representations for the 
Mertens function in 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} are more desirable to explore than 
classical formulae for $M(x)$ boil down to three counter points:
\begin{itemize}
\item[(1)] Break through work in recent years due to 
	Matom\"aki, Radziwi\l\l and Soundararajan 
	bounding multiplicative functions 
...
\item[(2)] The squarefree $n \geq 1$ on which $\lambda(n)$ and $\mu(n)$ must identically agree 
	are in some senses the ``easier'' integer cases insomuch as we can prove very regular properties 
...
\item[(3)] A final counter point is to note that $\lambda(n)$ is completely 
...
\end{itemize}
The discussions summarized above serve to motivate more study on the unsigned 
