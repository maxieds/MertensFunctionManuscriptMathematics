...
through the Erd\H{o}s-Kac theorem 
...
distribution of $\omega(n)$ from the last equation 
...
(\cf \cite{ELLIOTT-V1,ELLIOTT-V2} -- TODO). 
\end{remark}

%\fbox{\textbf{??? TODO (ask JL): Is it overkill to go into this much detail here (rest of this section) ??? }}
%
%It is no surprise that we have a CLT to govern the distinct values of any 
%non-negative real-valued additive arithmetic function $f(n)$ in the form of 
%\[
%\frac{1}{x} \times \#\left\{n \leq x: \frac{f(n) - E_f(x)}{D_f(x)} \leq z\right\} = 
%     \int_{-\infty}^z d\nu_f(t) + o(1), 
%     \text{ for } z \in (-\infty, \infty), 
%     \text{ as } x \rightarrow \infty, 
%\]
%for a limiting probability measure $\nu_f$ and where 
%(\cf \cite{ELLIOTT-V1,ELLIOTT-V2,IWANIEC-KOWALSKI}) 
%\textbf{ -- ... TODO ... -- }
%\begin{align*} 
%E_f(x) & := \sum_{p^{\alpha} \leq x} f\left(p^{\alpha}\right) 
%	p^{-\alpha} \left(1 - p^{-1}\right), 
%        \text{ for } x \geq 1, \\ 
%D_f^2(x) & := \sum_{p^{\alpha} \leq x} f^2\left(p^{\alpha}\right) p^{-\alpha}, 
%        \text{ for } x \geq 1. 
%\end{align*}
%Unsigned functions that satisfy the special property in 
%equation \eqref{eqn_FactSymmPropertyOfgn_v1} should satisfy a similar  
%CLT type result that governs the distribution of their values on the 
%integers $n \leq x$ as $x \rightarrow \infty$. 
%Recall the notation for the expectation and variance of an arithmetic function from 
%Definition \hlocalref{def_AvgOrder_FirstAndSecondMomentsOfFuncs_v1}.
%
%\begin{conjecture}
%\label{conj_COmegaFuncTypeDistsInsights_v1}
%Suppose that the following prime product is absolutely convergent for all 
%$\Re(s) > \widehat{\sigma}_{f,a}$ whenever $|z| < R_f$ and 
%where $\widehat{f}(n)$ is a multiplicative function: 
%\[
%\widehat{\mathcal{D}}_f(s, z) := \prod_p \left(1 - \sum_{r \geq 1} 
%	\frac{\widehat{f}(p^r) z^r}{p^{rs} r!}\right)^{-1}. 
%\] 
%For any fixed arithmetic function $f(n)$, if 
%\begin{equation}
%\label{eqn_ConjStmtTag_fnCoeffsDoubleInt_v1}
%f(n) = \int_0^{\infty} \left(\lim_{T \rightarrow \infty}\ \frac{1}{2T} \times 
%	\int_{-T}^{T} \widehat{\mathcal{D}}_f\left(\sigma + \imath v, t\right) 
%	n^{\sigma + \imath v} dv\right) e^{-t} dt, 
%	\text{ for } n \geq 1 
%	\text{ and } \sigma > \widehat{\sigma}_{f,a}, 
%\end{equation}
%then for any $z \in (-\infty, \infty)$ 
%\[
%\frac{1}{x} \times \#\left\{n \leq x: \frac{\log\left\lvert f(n) \right\rvert - 
%	\mathbb{E}\left[\log \left\lvert f(x) \right\rvert\right]}{ 
%	\sqrt{\operatorname{Var}\left[\log \left\lvert f(x) \right\rvert\right]}} \leq z\right\} = 
%	\Phi(z) + o(1), 
%	\text{ as } x \rightarrow \infty. 
%\]
%\end{conjecture}
%
%The hypothesis in equation \eqref{eqn_ConjStmtTag_fnCoeffsDoubleInt_v1} 
%is equivalent to the requirement that there is some arithmetic function 
%$\widehat{f}(n)$ such that $\widehat{\mathcal{D}}_f(s, 1)$ is the DGF of 
%$\frac{f(n)}{(\Omega(n))!}$ for $\Re(s) > \widehat{\sigma}_{f,a}$ 
%\cite[Thm.~11.17; \S 11.11]{APOSTOLANUMT}. 
