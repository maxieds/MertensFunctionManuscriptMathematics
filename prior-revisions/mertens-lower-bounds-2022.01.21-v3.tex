...
summatory functions, namely, the functions $C_{\Omega}(n)$ and $|g^{-1}(n)|$, and 
their partial sums defined below. 
...
to prove the next corollary for some absolute constant $A_0 > 0$ where the 
function 
\[
...
determined by the formula in \eqref{eqn_gInvn_ExactDivisorSumFormula_WithSgnWeight_v1} 
that holds for all $n \geq 1$. 
...
In fact, the next conjecture provides a complete picture of the distribution of the 
...
the generalization of Erd\H{o}s-Kac theorem type result that leads to 
Corollary \hlocalref{cor_CLT_VII} (under the assumption that  
...
which then leads to the conclusion of the following result 
(\cf Figure \hlocalref{figure_HistgInvnDistUns_Comparisons_v1}): 
...
\frac{1}{x} \times \#\left\{3 \leq n \leq x: \frac{|g^{-1}(n)| - 
     \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| - B_0 \sqrt{\log\log\log x}}{ 
...
Theorem \ref{prop_Mx_SBP_IntegralFormula} 
