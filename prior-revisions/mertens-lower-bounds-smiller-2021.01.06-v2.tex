...
by formulas involving discrete convolutions of $G^{-1}(x) := \sum_{n \leq x} g^{-1}(n)$ with the 
...
In Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2}, we prove that the approximate average order 
...
$\hat{a}$ an absolute constant, and for all $-Y \leq y \leq Y$ (see 
...
to in the remarks above are actually 
...
\end{equation*} 
Hence when we integrate over a set of wide enough 
disjoint consecutive intervals containing large enough integer values, 
we can assume that an asymptotic lower bound on the 
contribution of $|g^{-1}(n)|$ is given by the function's average 
order, and a limiting upper bound is given by the same inequality 
above for any fixed $\delta > 0$. Moreover, observe that 
the exceptional values of the sequence do not exceed these bounds as 
\begin{align*} 
...
Theorem \ref{prop_GInvx_EQ_BigOOfx_v1} 
proves that like the known elementary 
bound for $M(x)$, we have that $G^{-1}(x) = o(x)$ as $x \rightarrow \infty$. 
The results in 
...
\begin{align*}
     M(x) & = O\left(G^{-1}(x) + G^{-1}\left(\Floor{x}{2}\right) + 
     \frac{\sqrt{x} \cdot G^{-1}\left(\floor{\sqrt{x}}\right)}{\log x} 
     + \sqrt{x} \cdot (\log\log x)\right). 
\end{align*} 
...
\label{eqn_proof_tag_ExpAbsgInvn_ExpandedByExpsOfCkn_v1} 
     & \asymp \frac{6 \sqrt{2}}{\pi^{\frac{3}{2}}} (\log n)^2 \sqrt{\log\log n} + O(1). 
     \qedhere 
\end{align} 
\end{proof} 
...
|g^{-1}(n)| & = \Delta_n(n \cdot \mathbb{E}|g^{-1}(n)|) 
     ...
     \mathrm{\ as\ } n \rightarrow \infty. 
\end{align*} 
Since $\mathbb{E}|g^{-1}(n-1)| \sim \mathbb{E}|g^{-1}(n)|$ at all sufficiently large $n$, by 
Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2}, 
...
\begin{align*}
     ...
     \frac{\sqrt{x} \cdot G^{-1}(\sqrt{x})}{\log x} 
     + \sqrt{x} \cdot (\log\log x)\right). 
\end{align*} 
\end{cor}
\begin{proof}
...
\end{align*} 
The rightmost term in the previous bound is combined with 
Theorem \ref{prop_GInvx_EQ_BigOOfx_v1} to find 
\[
S_2(x) = O\left(G^{-1}\left(\frac{x}{2}\right) + \sqrt{x} \cdot (\log\log x)\right). 
     \qedhere
\]
\end{proof} 
