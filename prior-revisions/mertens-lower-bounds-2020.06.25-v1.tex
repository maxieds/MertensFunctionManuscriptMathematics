...
\item[$\blacktriangleright$ \textbf{Step E:}] \textit{The Holy Grail. } 
     ...
     \begin{itemize}[noitemsep,topsep=0pt,leftmargin=0.35in]
     \item[--] \small{Theorem \ref{cor_ThePipeDreamResult_v1} (on page \pageref{proofOf_cor_ThePipeDreamResult_v1})} 
     \end{itemize} 
...
We indeed obtain a lower bound by applying the contour integral 
in \eqref{eqn_MV7.61_CIF} to the bounded function on the right-hand-side of 
\eqref{eqn_AbsAmzx_LowerBoundFormula_v2} to formulate the main term in 
\eqref{eqn_HatPikx_LowerBoundFormula_v3}. In particular, by \eqref{eqn_MV7.61_CIF} 
and \eqref{eqn_MV_Azx_formula} we have that 
\begin{align*} 
\widehat{\pi}_k(x) & \sim \frac{1}{2\pi\imath} \int_{|z|=r} \frac{|A_{-z}(x)|}{z^{k+1}} dz \\ 
...
\min\limits_{t \in [0, 2\pi]} \left\lvert 
     ...
     & \gg F(1, -r). 
\end{align*} 
Then since $1 \leq k \leq \log\log x$, with $|z| = r$ we have that $0 \leq r < 1$, so that the bound on these 
terms we obtained above holds here as well. Hence, the main term for $\widehat{\pi}_k(x)$ 
given by \eqref{eqn_HatPikx_LowerBoundFormula_v3} is accurate. \\ 
...
proof of Theorem \ref{cor_ThePipeDreamResult_v1} 
