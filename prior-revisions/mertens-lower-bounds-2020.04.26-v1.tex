...
\pagestyle{empty}
\pagestyle{fancy}
\fancyhead[RO,RE]{M. D. Schmidt -- Prepared for Prof. Kevin Ford -- \today} 
\fancyhead[LO,LE]{}
...
complicated exact 
distribution of the M\"obius function over the natural numbers -- which since 
$\sum_{n \geq 1} \frac{\mu(n)}{n^s} = \frac{1}{\zeta(s)} = \prod_p \left(1-p^{-s}\right)$ 
for $\Re(s) > 1$, is also closely 
tied to precise statements of the distribution of the prime numbers via the RHS Euler product 
formula indexed over only primes $p$ expressing this DGF of the multiplicative $\mu(n)$. 
...
so-called ``low lying'', non-trivial zeta function zeros in conjunction with integral formulas expressing $M(x)$. 
...
We seek to concretely prove that this is indeed the case. Our proof, perhaps unsurprisingly, is not based on 
standard estimates of $M(x)$ by Mellin inversion, which are intimately tied to the distribution of the non-trivial 
zeros of the Riemann zeta function. As the problem of unboundedness of $q(x)$ is a weaker statement than the RH, 
we are encouraged that our new results do not assume the RH as a precondition for rigorous accuracy of the claimed 
statements. 
In particular, we focus on obtaining new lower bounds 
...
found in Chapter 7 of Montogomery and Vaughan. There is a distinct stylistic 
...
each function has a long known log--logarithmic average order, to establish our crucially 
log-loglog-logloglog-logarithmically (etc.) thin 
new statements -- a choice that allows us to distinguish and partially resolve conjectured statements of 
how close to $\sqrt{x}$ the unboundedness of $M(x)$ actually is in limiting forms, e.g., as in the 
$(\log\log x)^{5/4}$ conjectured behavior from Gonek's famous limiting conjecture on the growth of $M(x)$. 
%We make surprising new claims about a classical conjecture on the boundedness of 
%$|M(x)| / \sqrt{x}$ along an asymptotically large growing infinite subsequence of reals as a 
%concluding corollary to the lower bounds on components of our updated formulas for $M(x)$ 
%proved within the article. 

%Note that a related summatory function, with typically better known bounds and known sign bias 
%properties, is defined by $L_{\alpha}(x) := \sum_{n \leq x} \lambda(n) n^{-\alpha}$ for 
%$\lambda(n) = (-1)^{\Omega(n)}$ the standard completely multiplicative 
%arithmetic Liouville lambda function. We use a more combinatorial relationship of 
%exact expressions for $M(x)$ by summatory functions of the Dirichlet inverse functions, $g^{-1}(n)$, of the 
%Dirichlet invertible arithmetic function $g(n) = \omega(n) + 1 = 1 + \sum_{p|n} 1$ with $g(1) = 1$. 
%The sign, or even signed parity, of $g^{-1}(n)$ is given by $\lambda(n)$ -- a property deeply, and apparently 
%inextricably linked to the additivity of $\omega(n)$. Thus we seemingly draw upon the properties of $\lambda(n)$ and 
%related additive arithmetic function constructions, rather than the more puzzling brand of 
%tradition M\"obius function sums -- along intervals, or other wise --  
%in obtaining new results to bound $M(x)$ from below along prescribed subsequences. 
...
\large{\textit{Draft prepared for Professors Kevin Ford, Michael Lacey and Bruce Reznick.}} \\[0.3cm] 
\small{ 
        I (MDS) have checked these 
        results by careful reading, and re-reading at least twice myself with input from others, before 
        emailing this document. I have a semi-organized \texttt{Mathematica} notebook 
        references to help with verfiying the claims and performing some of the more involved 
        integration and summation procedures. This supplementary notebook reference is tenatively provided over email. 
        Please let me know if there is any other information or 
        clarifications I can provide to help you with understanding my argument and the new constructions. \\[0.25cm] 
        For the time being, please do not share the results in this draft article, nor in any of the 
        supplementary resources I have prepared, with anyone else. I hesitate to say that in the event of 
        catastrophic mental block, logarithmically thin errors, or boasting about solving classically 
        relevant results using non-standard, untested methodology could make me look foolish. \\[0.25cm]
        Thank you for being willing to help me debug this ship first! 
}
       
\bigskip\hrule\bigskip
\noindent
\textbf{(The game plan eventually) } 
Note that this draft has been checked and verified as mostly (e.g., almost surely) 
error free by the following distinguished full professors of mathematics 
(in addition to the Ph.D. candidate author): 
\begin{itemize} 
...
\newpage
\section{Key notes: Core components to the proof} 
...
\prod_{p \leq x} \left(1 - \frac{1}{p}\right)^{z} = \frac{e^{-Bz}}{(\log x)^{z}} \left[ 
     1 - \frac{z}{B} (\log x)^{\frac{1}{14}} + o_{z,R}\left(z^2 \cdot (\log x)^{\frac{1}{14}}\right)\right]. 
\]
\end{cor} 
\begin{proof} 
...
\subsubsection{Ideas towards why the proposed generalization could be useful (not even jokingly suggested)} 

Consider the following construction in the direction of proving that the set of \emph{twin primes} 
\[
\mathbb{TP} := \left\{n \in \mathbb{Z}^{+}: (n, n+2) \in \mathbb{P}^2\right\}, 
\]
contains infinitely many elements. In particular, if we can use the setup from the previous subsection 
using characteristic functions, and write the \emph{twin prime counting function} as 
\[
\pi_{\operatorname{TP}}(x) := \#\{n \leq x: n \in \mathbb{TP}\}, 
\]
then for the Mertens-style function variant, $M_{\operatorname{TP}}(x) := \sum_{n \leq x} \mu_{\operatorname{TP}}(n)$, 
we obtain bounds of the form 
\[
-\frac{d}{dx}\left[M_{\operatorname{TP}}(x) + G_{\operatorname{TP}}^{-1}(x)\right] \geq 
     \left(\min\limits_{1 \leq t \leq \frac{x}{2}} \left\lvert \frac{G_{\operatorname{TP}}^{-1}(t)}{t^3} 
     \right\rvert \right) \cdot \pi_{\operatorname{TP}}(x). 
\]
Provided the generalized Mertens set function $M_{\operatorname{TP}}(x)$ can be shown to satisfy certain 
limiting bounds along a nicely defined convenient infinite subsequence, 
this trick is an obvious way to attack the problem of infinitude (or lack thereof) of the twin primes 
$\mathbb{TP}$. We suggest that proving the existence of sufficient lower bounds on $M_{\operatorname{TP}}(x)$ 
along an infinite subsequence ought be a substantially difficult problem, 
though our new more combinatorial analytic approaches to the classical $M(x)$ bounds certainly suggest new 
ways to challenge what is known on this problem type. 
