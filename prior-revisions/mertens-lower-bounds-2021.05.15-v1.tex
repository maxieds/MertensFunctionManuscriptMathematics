...
\newglossaryentry{fCvlg}{
    ...
    sort={fg},
    description={The Dirichlet convolution of $f$ and $g$, 
...
\begin{align} 
\notag 
A_z(x) & = \sum_{m \leq x/2} b_z(m) D_z\left(\frac{x}{m}\right) + \sum_{x/2 < m \leq x} b_z(m) \\ 
...
\widehat{C}_k(x) := 
     ...
     \frac{4A_0\sqrt{2\pi} x}{(2k-1)} \cdot \frac{(\log\log x)^{k-1/2}}{(k-1)!}, 
\end{align*} 
...
\notag
     & \sim 
     \frac{4A_0\sqrt{2\pi} x (\log\log x)^{k-1/2}}{(2k-1) (k-1)!} + 
     ...
     & \sim \frac{4A_0\sqrt{2\pi} x (\log\log x)^{k-1/2}}{(2k-1) (k-1)!} 
     \qedhere 
\end{align}
\end{proof}
...
\end{align*} 
It follows that uniformly for $1 \leq k \leq 2\log\log x$ 
\begin{align*}
f(k, x) & := 
     \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \\ 
     & \sim 
...
for $\alpha = 0$. Note that as $\alpha \rightarrow +\infty$, 
...
\[
n! = \Gamma(1 + t - \xi) = \sqrt{2\pi} t^{t-\xi+1/2} e^{-t} \left(1 + O(t^{-1})\right) = 
     \sqrt{2\pi} t^{n+1/2} e^{-t} \left(1 + O(t^{-1})\right). 
\]
