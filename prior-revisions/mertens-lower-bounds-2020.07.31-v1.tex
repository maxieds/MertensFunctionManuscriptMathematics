...
It also leads to very regular properties of component sequences in the new formula for $M(x)$ that include 
generalizations to Erd\"os-Kac like theorems satisfied by the distributions of the special auxiliary 
function sequences. 
...
through auxiliary function sequences\footnote{
     Using the sequences $C_k(n)$ defined in 
     Section \ref{Section_InvFunc_PreciseExpsAndAsymptotics}, we can however write for 
     primes $p$, $\alpha \geq 1$ and $n \geq 1$ such that $(p, n) = 1$ that 
     \[
     |g^{-1}(p^{\alpha} \cdot n)| = |g^{-1}(n)| + \sum_{d|n} \left[ 
          C_{\Omega(d) + \alpha}\left(d \cdot p^{\alpha}\right) - 
          C_{\Omega(d) + \alpha-1}\left(d \cdot p^{\alpha-1}\right) 
          \right]. 
     \]
}. 
...
\subsection{Uniform asymptotics from certain enumerative bivariate DGFs} 
...
\begin{align*} 
I_{1,x} & := \frac{(-1)^{k-1} G(-r)}{2\pi\imath} \int_{|v|=r} (\log x)^{-v} v^{-k} dv \\ 
    & \phantom{:}= \frac{G(-r) (\log\log x)^{k-1}}{(k-1)!} \\ 
I_{2,x} & := \frac{(-1)^{k-1}}{2\pi\imath} \int_{|v|=r} (G(-z) - G(-r)) (\log x)^{-v} v^{-k} dv \\ 
    & \phantom{:}= \frac{(-1)^{k-1}}{2\pi\imath} \int_{|v|=r} (G(-z) - G(-r) + G^{\prime}(-r) (v+r)) 
    (\log x)^{-v} v^{-k} dv. 
\end{align*} 
We have by taking a power series expansion of $G^{\prime\prime}(-w)$ about $-v$ and integrating 
the resulting series termwise with respect to $w$ that 
\[
\left\lvert G(-z) - G(-r) + G^{\prime}(-r) (v+r) \right\rvert = 
     \left\lvert \int_{-r}^{v} (v+w) G^{\prime\prime}(-w) dw \right\rvert \ll 
     G^{\prime\prime}(-r) \times |v+r|^2 \ll |v+r|^2. 
\] 
...
\]
Additionally, since 
$$\lambda(d) C_{\Omega(d)}(d) = (g^{-1} \ast 1)^{-1}(d) = (\chi_{\mathbb{P}} + \varepsilon)(d),$$ 
...
\newpage
\section{The precise distribution of 
...
limiting normal distribution as $n \rightarrow \infty$ 
\cite{ERDOS-KAC-REF,BILLINGSLY-CLT-PRIMEDIVFUNC,RENYI-TURAN}. This extremely regular 
...
to obtain a related result using \eqref{eqn_proof_tag_FsEQ2z_justification_formula} below. 
...
\]
Then since $\sigma_x(C) \xrightarrow{x \rightarrow \infty} +\infty$ and for $z \geq 0$ we have the 
reflection identity $\Phi(z) = 1 - \Phi(-z)$, we have by an asymptotic approximation 
to the error function as 
\begin{align*} 
\Phi(z) & = \frac{1}{2}\left(1 + \operatorname{erf}\left(\frac{z}{\sqrt{2}}\right)\right) \\ 
     & = 1 - \frac{e^{-z^2/4}}{\sqrt{2\pi}}\left[ 
     z^{-1} - z^{-3} + 3z^{-5} - 15z^{-7} + \cdots 
     \right], \mathrm{\ for\ } |z| \rightarrow \infty, 
\end{align*} 
that 
\[
\gamma_0(x) = \Phi\left(-\sigma_x(C)\right) \asymp \frac{1}{\sigma_x(C) \exp(\mu_x(C)/4)} = o(1). 
\]
...
given by $(-1)^{1+\ceiling{\log\log x}}$ for large $x$. 
...
\caption*{\textbf{\rm \bf Expected observations in the table.} \\ 
