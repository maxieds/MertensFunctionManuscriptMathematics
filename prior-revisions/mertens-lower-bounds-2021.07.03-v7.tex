...
\notag
     & \sim 2A_0 \sqrt{2\pi \log\log x} \cdot \widehat{C}_{k,\ast}(x) - 
     A_0\sqrt{2\pi} \times \int_3^x \widehat{G}\left(\frac{k-1}{\log\log t}\right) 
     \frac{(\log\log t)^{k-\frac{3}{2}}}{(\log t)^2} dt \\ 
\notag
     & = 2A_0 \sqrt{2\pi \log\log x} \cdot \widehat{C}_{k,\ast}(x) 
...
\notag 
     ...
     1 + O\left(\frac{1}{\log\log x}\right)\right). 
\end{align}
The error term in them previous equation follows by seeing that 
...
$p_s \leq (4m+1)\#$ for $2 \leq s \leq 4m+1$. If we have that 
...
\]
Then the corresponding sums $G_L^{-1}((4m+1)\#)$ are bounded above by 
\begin{align*} 
