...
\item[(C)] We can write the inverse function at squarefree $n$ as 
     \[
     g^{-1}(n) = \lambda(n) \times \sum_{m=0}^{\omega(n)} \binom{\omega(n)}{m} \cdot m!. 
     \]
\end{itemize} 
...
exponents in the prime factorization of any $n \geq 2$ (see the proof of property (C) given on 
page \pageref{proofOf_propCFromConj_lemma_gInv_MxExample}). 
...
\subsubsection{A basis for our intuition} 
...
\subsection{Establishing the summatory function identities} 
...
$B - 2\varepsilon < C, 0 < B - \varepsilon < 1, 0 < C + \varepsilon < 1$, e.g., 
...
we require the following fact of our notation for average order: 
...
\end{prop} 
\begin{proof} 
We must first argue that the set of $n > e^e$ on which $\Omega(n)$ differs substantially 
...
large $n \rightarrow \infty$ that \footnote{ 
     In particular, exactly we obtain the definite integral formula 
     \[
     \int_{e^e}^n \frac{(\log\log t)^{m}}{t} dt = (-1)^m \cdot \Gamma(m+1, -\log\log n). 
     \]
}
\begin{align*} 
\frac{1}{n} \times \int_{e^e}^{n} (\log\log t)^m dt & = \frac{1}{n}\left[ 
...
\begin{align*} 
\frac{1}{n} \times \int_{e^e}^{n} (\log\log t)^{2\log\log t - 1} dt & \approx 
     \frac{1}{n} \times \int_{e^e}^{n} \frac{(1 + \log\log t)^{2\log\log t}}{\log\log t} dt \\ 
     & = \frac{1}{n} \times \int_{e^e}^{n} 
...
Let $F \in C^{1}(1, \infty)$ be a monotone non-decreasing function such that 
...
\[
\sum_{n \leq x} f(n) \gg x \cdot F(x), \mathrm{\ as\ } x \rightarrow \infty. 
\]
...
For sufficiently large $x \geq 1$, let $\gamma_{-}^{\ast} := \min\left(\gamma_{-}, \frac{1}{x}\right)$.  
By the positivity of $f(n)$, we know that $F(x)$ is positive for all sufficiently large $x$. 
Moreover, by the monotonicity of $F$ we have that for large enough $x$ 
\begin{align*} 
\sum_{\substack{n \leq x \\ n \in \mathcal{F}_{-}}} F(n) & \leq \sum_{n \leq \gamma_{-}^{\ast} x} F(n) \\ 
     & \leq \max\left(0, (\gamma_{-} x) F(\gamma_{-}^{\ast} x) - \int_1^{\gamma_{-}^{\ast}x} t \cdot F^{\prime}(t) dt\right) \\ 
     & \leq \max\left(0, (\gamma_{-} x) F(\gamma_{-}^{\ast} x) - F(\gamma_{-}^{\ast} x)\right). 
\end{align*} 
Thus we have that 
\begin{align*} 
\sum_{n \leq x} f(n) & \leq \sum_{\substack{n \leq x \\ n \notin \mathcal{F}_{-}}} f(n) + 
     \sum_{\substack{n \leq x \\ n \in \mathcal{F}_{-}}} F(n) \\ 
     & \leq (1-\gamma_{-}) x F((1-\gamma_{-}) x) + 
     \max\left(0, (\gamma_{-} x) F(\gamma_{-}^{\ast} x) - F(\gamma_{-}^{\ast} x)\right), 
\end{align*} 
by the monotonicity of $F$. So if $\gamma_{-} \in (0, 1)$, i.e., if $\gamma_{-} \neq 0, 1$, then 
\begin{align} 
\label{eqn_proof_tag_GammaMinusDensityIsZero_v3} 
\sum_{n \leq x} f(n) & \ll x \cdot F\left(\max(1-\gamma_{-}, \gamma_{-}) x\right) \\ 
     & < x \cdot F(x), 
\end{align} 
again by the monotonicity of $F$. The bound in \eqref{eqn_proof_tag_GammaMinusDensityIsZero_v3} 
contradicts our hypothesis that the summatory function of $f(n)$ is asymptotically greater than 
$x \cdot F(x)$. Notice also that the limiting density cannot be one since if $\gamma_{-} = 1$, then 
\begin{align*} 
\sum_{n \leq x} f(n) & < \sum_{n \leq x} F(n) \\ 
     & \leq x \cdot \max_{1 \leq j \leq x} F(j) = x \cdot F(x), 
\end{align*} 
since $F$ is monotone non-decreasing on $[1, \infty)$ by assumption. 
...
This implies that at any such large $x$, we have 
\begin{align*} 
\sum_{n \leq x} f(n) & < M_x \cdot x \times F(M_x \cdot x) + (1-M_x) x \times F((1-M_x) \cdot x) \\ 
     & \leq x \cdot F\left(\max(M_x, 1-M_x) \cdot x\right) \\ 
     & \ll x \cdot F(x), 
\end{align*} 
by the monotonicity of $F$. So infinitely often, our hypothesis on the asymptotic lower bound on 
...
\mathbb{E}|g^{-1}(n)| & \geq \frac{1}{n} \times \sum_{\substack{m \leq n \\ \mu^2(m) = 1}} \sum_{d|m} C_{\Omega(d)}(d) \\ 
     & \approx \frac{1}{n} \times \sum_{d \leq n} C_{\Omega(d)}(d) Q\left(\frac{n}{d}\right) \\ 
     ...
     & \gg \frac{6}{\pi^2} \int_{e^e}^n \frac{(\log t)^2 (\log\log t)}{t} dt + 
     O\left(\frac{1}{n} \times \int_{e^e}^n (\log t)^2 (\log\log t) dt\right) \\ 
...
\end{cor} 
\begin{proof} 
First, we see that for all large enough $n$ on a set of asymptotic density one, 
\[
...
\notag 
     & \geq \frac{1}{x} \times \left[\sum_{n \leq \frac{x}{2}} |g^{-1}(n)|\right] (1 + o(1)) \\ 
\label{eqn_proof_tag_MxAbsValueConsequence_v4} 
     & = \frac{1}{2} \cdot \mathbb{E}\left\lvert g^{-1}\left(\frac{x}{2}\right) \right\rvert (1 + o(1)), 
\end{align} 
where the factor of $\frac{1}{2}$ in the upper limit of summation above 
...
\eqref{eqn_proof_tag_MxAbsValueConsequence_v4}, we obtain that for almost every 
sufficiently large $x$ 
\begin{align*} 
\frac{|M(x)|}{x} \gg \frac{3}{\pi^2} \log\left(\frac{x}{2}\right) (1 + o(1)) 
     \xrightarrow{x \rightarrow \infty} + \infty. 
\end{align*} 
So we recover a contradiction to the known property that $|M(x)| \leq x$ 
for all $x \geq 1$. 
...
First, we can take the first chunk 
of the interval of integration and bound it using \eqref{eqn_pf_tag_v2-restated_v2} as 
\begin{align*} 
-\int_{u_0}^{\sqrt{x}} \frac{x}{t^2 \log(x/t)} |G_{\ell}^{-1}(t)| dt & 
     \geq 
     -\int_{u_0}^{\sqrt{x}} \frac{x}{t^2 \log(x/u_0)} |G_{\ell}^{-1}(t)| dt \\ 
     & \SuccSim 
     \frac{\sqrt{x}}{\log(x/u_0)} \cdot \left(\min\limits_{u_0 \leq t \leq \sqrt{x}} 
     |G_{\ell}^{-1}(t)| 
     \right) \\ 
     & = o\left(\sqrt{x}\right). 
\end{align*} 
...
\begin{align*} 
-\int_{\sqrt{x}}^{x/2} \frac{2x}{t^2 \log(x)} \cdot |G_{\ell}^{-1}(t)| dt & 
     \geq 
     -\int_{\sqrt{x}}^{x/2} \frac{2x}{t^2 \log(x)} \cdot |G_{\ell}^{-1}(t)| dt \\ 
     & \SuccSim 
     ...
     (\log\log\log\log \sqrt{x})^{\frac{5}{2}}} + o(\sqrt{x}). 
\end{align*} 
Finally, since $G_{\ell}^{-1}(x) = o(\sqrt{x})$, we obtain in total dropping constants that 
\begin{align} 
\label{eqn_MxGellInvxLowerBound_stmt_v1} 
\frac{1}{\sqrt{x}} \left\lvert G_{\ell}^{-1}(x) - x \cdot \int_1^{x/2} \frac{G_{\ell}^{-1}(t)}{ 
     t^2 \cdot \log(x/t)} dt \right\rvert & \gg 
     \frac{\left\lvert G_{\ell}^{-1}(\sqrt{x}) \right\rvert}{\log\left(\sqrt{x}\right)}. 
\end{align} 
...
The existence of this infinite subsequence 
shows that there is some $x_y$ for each large enough $y \rightarrow \infty$ 
such that $|G^{-1}(x_y)| \gg |G_E^{-1}(x)| \gg |G_{\ell}^{-1}(x_y)|$ where 
$x_y \rightarrow \infty$ as $y \rightarrow \infty$. 
We also know from 
Corollary \ref{lemma_BddExpectationOfgInvn} and 
Lemma \ref{lemma_AsymptoticDensitiesOfExceptionalSets_v1} with 
\begin{align*} 
\mathbb{E}\left[\sum_{d|n} C_{\Omega(d)}(d)\right] & \sim 
     \sum_{d \leq n} \frac{C_{\Omega(d)}(d)}{d} \\ 
     & \sim \mathbb{E}[C_{\Omega(n)}(n)] + \sum_{d < n} \frac{\mathbb{E}[C_{\Omega(d)}(d)]}{d} \\ 
     & \gg |g^{-1}(n)|, 
\end{align*} 
so that by Proposition \ref{prop_factA_stmt_v1}, $G^{-1}(x) = o(\sqrt{x})$. 
Then given these observations, by \eqref{eqn_MxGellInvxLowerBound_stmt_v1} we must have that 
\begin{align*} 
\frac{|M(x_y)|}{\sqrt{x_y}} & \SuccSim 
     \frac{(\log\log \sqrt{x_y})^{3} \sqrt{\log\log\log \sqrt{x_y}}}{ 
     (\log\log\log\log \sqrt{x_y})^{\frac{5}{2}}} 
