...
$|z| < 1$. 
For $1 \leq k \leq \log\log x$, we have that 
\[
...
\begin{align*}
I_k(r, x) & = \frac{(-1)^{k}}{2\pi\imath} \times \int_{|v|=r} 
...
\eqref{lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2} at large $x$: 
\begin{align*} 
...
L_{\ast}(t) & := \left\lvert \sum_{n \leq t} (-1)^{\omega(n)} \right\rvert \sim 
     \left\lvert \sum_{k=1}^{\log\log t} (-1)^{k} \pi_k(x) \right\rvert = 
...
\begin{align*} 
     ...
     2\sqrt{2\pi \log\log x} \cdot \widehat{C}_{k,\ast}^{\prime}(x) (1+o(1)) 
...
\notag
     & \sim 
     \frac{4\sqrt{2\pi} (\log\log x)^{k-1/2}}{(2k-1) (k-1)!} + 
     \frac{2\sqrt{2\pi} \Gamma\left(k - \frac{1}{2}, \log\log x\right)}{(k-1)!} - 
     \frac{2\sqrt{2\pi} \Gamma\left(k - \frac{3}{2}, \log\log x\right)}{(k-1)!} \\ 
     & \sim \frac{4\sqrt{2\pi} (\log\log x)^{k-1/2}}{(2k-1) (k-1)!}. 
     \qedhere 
\end{align}
\end{proof}
...
\]
In particular, for 
$\sqrt{x} \leq n \leq x$ and $C_{\Omega(n)}(n) \leq 2 \cdot \mu_x(C)$ we can show 
...
Paul Pollack, Steven J.~Miller, Gerg\H{o} Nemes, 
Ernie Croot, Bruce Reznick and Michael Lacey. 
...
We have adapted his proofs to establish the next few lemmas. 
...
\[
\Gamma(a, z) = \int_{z}^{\infty} t^{a-1} e^{-t} dt, a \in \mathbb{R}, \Re(z) > 0. 
\]
...
Suppose that $a,z,\lambda > 0$ are such that $z = \lambda a$. If $\lambda > 1$, then as 
$z \rightarrow +\infty$ 
\[
\Gamma(a, z) = \frac{z^{a-1} e^{-z}}{1-\lambda^{-1}} + O_{\lambda}\left(z^{a-2} e^{-z}\right). 
\]
If $\lambda > 0.567142 > W(1)$ where $W(x)$ denotes the principal branch of the Lambert $W$-function, 
...
\]
From the bounds in \cite[\S 3.1]{NEMES2016}, we get 
\[
\left\lvert z^{a} e^{-z} R_1(a, \lambda) \right\rvert \leq 
     z^a e^{-z} \times \frac{a \cdot b_1(\lambda)}{(z-a)^{3}} = 
     \frac{z^{a-2} e^{-z}}{(1-\lambda^{-1})^{3}}
\]
The main term in the previous equation can also be seen by applying the asymptotic series in 
\eqref{eqn_IncompleteGamma_PropD} directly. 

The proof of the second equation above follows from the following asymptotics 
\cite[Eq.\ (1.1)]{NEMES2015C}
\[
\Gamma(-a, z) \sim z^{-a} e^{-z} \times \sum_{n \geq 0} \frac{a^n b_n(-\lambda)}{(z+a)^{2n+1}}, 
\]
by setting $a \mapsto a e^{\pm \pi\imath}$ and $z \mapsto z e^{\pm \pi\imath}$. 
Note that we cannot this expansion as $\Gamma(a, -z)$ directly as the incomplete gamma function 
has branch points with respect to its second variable. This function becomes a single-valued 
analytic function of its second input on the universal covering of $\mathbb{C} \mathbin{\big\backslash} \{0\}$. 
...
\end{lemma}
\begin{proof}
We have that for $n \geq 1$ and any $t > 0$ \cite[\cf \S 8.4]{NISTHB}
\[
\sum_{1 \leq k \leq n} \frac{(-1)^k t^{k-1}}{(k-1)!} = -e^{-t} \times 
     \frac{\Gamma(n, te^{\pi\imath})}{(n-1)!}. 
\]
Suppose that $t = n + \xi$ with $\xi = O(1)$. By the second formula 
...
\Gamma(n, t e^{\pi\imath}) = (-1)^{n+1} \times \frac{t^n e^{t}}{t+n} + 
     O\left(\frac{n t^n e^{t}}{(t+n)^3}\right) = 
     (-1)^{n+1} \frac{t^n e^t}{2n} + O\left(\frac{t^n e^t}{nt}\right). 
\end{equation}
...
S_3(x) := \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-1/2}}{(2k-1) (k-1)!} = 
     \frac{(\log x)}{4\sqrt{\log\log x}} (1+o(1)) + O\left(\frac{\log x}{\log\log x}\right).       
\end{align*}
\end{lemma}
\begin{proof}
Notice that we can write 
\[
\sum_{k \geq 1} \frac{(\log\log x)^{k-1}}{(2k+1) (k-1)!} - \sum_{k \geq 0} \frac{(\log\log x)^k}{(2k+1) k!} \leq 
     S_3(x) \leq 
     \sum_{k \geq 1} \frac{(\log\log x)^{k-1}}{(2k-1) (k-1)!} - 
     \sum_{k \geq 0} \frac{(\log\log x)^{k-1}}{2 \cdot k!}(1+o(1)). 
\]
The symbolic summation procedures in \emph{Mathematica} \cite{SCHMIDT-MERTENS-COMPUTATIONS}, 
show that we can arrive at this bound in the form of 
\begin{equation}
\label{eqn_ProofTag_S3x_v1}
     \frac{\log x}{2\sqrt{\log\log x}} + O\left(\frac{\log x}{\sqrt{\log\log x}}\right) \ll S_3(x) \ll
     \frac{\log x}{2\sqrt{\log\log x}} + O\left(\frac{\log x}{\log\log x}\right). 
\end{equation}
...
\]
This implies the bounds given in \eqref{eqn_ProofTag_S3x_v1} match the stated formula 
up to a factor of $\frac{1}{2}$. 
We will give an exact proof of the result directly using the bounds for the incomplete 
gamma function established in the recent reference \cite{NEMES2019}. The reference takes into 
account the behavior of the incomplete gamma function $\Gamma(a, z)$ 
near the transition point $\lambda = z/a = 1$ when $a,z \rightarrow +\infty$. 

For $n \geq 1$ and any $t > 0$, let 
\[
\widetilde{S}_n(t) := \sum_{1 \leq k \leq n} \frac{t^{k-1}}{(2k-1) (k-1)!}. 
\]
By the formula in \eqref{eqn_IncompleteGamma_PropB} and a change of variable, we get that 
\begin{align*}
\widetilde{S}_n(t) & = \int_0^1 s^2 \left(\sum_{k=1}^{n} \frac{(s^2t)^{k-1}}{(k-1)!}\right) ds \\ 
     & = \frac{1}{(n-1)!} \times \int_0^1 s^2 e^{s^2t} \Gamma(n, s^2t) ds \\ 
     & = \frac{1}{2(n-1)!} \times \int_0^1 \sqrt{x} e^{xt} \Gamma(n, xt) dx. 
\end{align*}
Integration by parts performed one time with 
\[
     \left\{\begin{array}{ll} 
     u_x = \Gamma(n, xt) & v_x^{\prime} = \sqrt{xt} e^{xt} dx \\ 
     u_x^{\prime} = -t^n x^{n-1} e^{-xt} dx & v_x = 
     \frac{\sqrt{x} e^{tx}}{\sqrt{t}} - \frac{\sqrt{\pi} \operatorname{erfi}(\sqrt{tx})}{2t}
     \end{array} 
     \right\}, 
\]
implies that 
\begin{align}
\notag
     \widetilde{S}_n(t) & = \frac{\Gamma(n, t)}{2(n-1)!} \cdot \frac{e^t}{t}\left(1 - 
     \frac{\sqrt{\pi}}{2} \cdot \frac{e^{-t}}{\sqrt{t}} \operatorname{erfi}(\sqrt{t})\right) + 
     \frac{t^{n-1}}{(n-1)!} \cdot \frac{1}{(2n+1)} \\ 
\notag
     & \phantom{=\ } - 
     \frac{\sqrt{\pi}}{4} \cdot \frac{t^{n-3/2}}{(n-1)!} \times \int_0^1 x^{n-1} e^{-xt} 
     \operatorname{erfi}(\sqrt{tx}) \\ 
\label{eqn_ProofTag_AppendixLastSumLemma_IBP_resulting_exp_terms_v1}
     & = \frac{\Gamma(n, t)}{2(n-1)!} \cdot \frac{e^t}{t}\left(1 - 
     \frac{\sqrt{\pi}}{2} \cdot \frac{e^{-t}}{\sqrt{t}} \operatorname{erfi}(\sqrt{t})\right) + 
     \frac{t^{n-1}}{(n-1)!} \cdot \frac{1}{(2n+1)} \\ 
\notag
     & \phantom{=\ } - 
     \frac{\sqrt{\pi}}{4 t^{3/2} (n-1)!} \times \int_0^{t} s^{n-1} e^{-s} \operatorname{erfi}(\sqrt{s}). 
\end{align}
Using the asymptotic series for $\operatorname{erfi}(z)$, we can see that as $t \rightarrow +\infty$ 
\[
\frac{\sqrt{\pi}}{2} \cdot \frac{e^{-t}}{\sqrt{t}} \operatorname{erfi}(\sqrt{t}) = O(t^{-1}). 
\]
Suppose that $t = n + \xi$ where $\xi = O(1)$. According to the reference \cite[Eq.\ (2.4)]{NEMES2019}, 
we have that as $t \rightarrow +\infty$ 
\[
\Gamma(n, t) = t^n e^{-t} \sqrt{\frac{\pi}{2t}} \left(1 + O(t^{-1/2})\right). 
\]
Similarly, we have that as $t \rightarrow +\infty$ 
\[
\frac{t^{n-1}}{(n-1)!} \cdot \frac{1}{(2n+1)} = \frac{t^{n-2}}{2 (n-1)!} \left(1 + O(t^{-1})\right) (1+o(1)). 
\]
Additionally, from the definition of $\operatorname{erfi}(z)$ and 
\cite[\S 7.12(i)]{NISTHB}, we get 
\begin{align*}
\int_0^1 s^{n-1} e^{-s} \operatorname{erfi}(\sqrt{s}) ds & = 
     \int_0^1 s^{n-1} e^{-s} \operatorname{erfi}(\sqrt{s}) ds + 
     \int_{1}^{t} s^{n-1} e^{-s} \operatorname{erfi}(\sqrt{s}) ds \\ 
     & = O\left(1 + \int_1^{t} s^{n-3/2} ds\right) = 
     O\left(\frac{t^{n-1/2}}{n}\right), \text{ as } n \rightarrow t, t \rightarrow + \infty. 
\end{align*}
When we combine each of these estimates for the terms in 
\eqref{eqn_ProofTag_AppendixLastSumLemma_IBP_resulting_exp_terms_v1} and let $t \rightarrow +\infty$ 
\[
\widetilde{S}_n(t) = \frac{1}{2} \sqrt{\frac{\pi}{2}} \cdot \frac{t^{n-3/2}}{(n-1)!} \left( 
     1 + O\left(\frac{1}{\sqrt{t}}\right)\right). 
\]
By the variant of Stirling's formula expressed in \cite[\S 5.11(i)]{NISTHB}, we again have that 
\[
(n-1)! = \sqrt{2\pi} \cdot t^{n-1/2} e^{-t} \left(1 + O(t^{-1})\right). 
\]
Whence, with $t = n + O(1)$ and as $n \rightarrow +\infty$, we obtain 
\[
\widetilde{S}_n(t) = \frac{e^t}{4t} + O\left(\frac{e^t}{t^{3/2}}\right). 
\]
The conclusion follows taking $n = \log\log x$ (floor function thereof), $t = \log\log x$ and 
