...
\begin{cor}[Unboundedness of the the Mertens function over $\sqrt{x}$] 
\label{cor_ThePipeDreamResult_v1} 
Let $u_0 := e^{e^{e^{e^2}}}$ and define the infinite increasing subsequence, 
$\{x_{0,n}\}_{n \geq 1}$, by $x_{0,n} := e^{e^{e^{e^{4n}}}}$. 
...
The inner summation in the lower bound stated for $|G_E^{-1}(x)|$ is indexed only for 
...
\begin{align*} 
\widehat{L}_0(\log\log x) \widehat{\tau}_0(\log x) A_{\Omega}^{\ell}(\log x) & \SuccSim 
     C_{\ell,1} \cdot (\log x) (\log\log x)^{5/2} \cdot \frac{(\log\log\log x)^2}{ 
     (\log\log\log\log x)^{5/2}}. 
\end{align*} 
...
It suffices to take $u_0 = e^{e^{e^{e^2}}}$. 
...
\end{align*} 
where $B_{\ell,2}(u_0)$ can be taken as an indefinite, but still some absolute constant with respect to $u_0$. 
