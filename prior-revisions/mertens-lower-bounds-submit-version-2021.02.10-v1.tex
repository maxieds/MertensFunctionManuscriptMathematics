...
\newglossaryentry{NormalCDFFunc}{
    symbol={$\Phi(z)$},
    sort={NormalCDFFunc},
    description={For $x \in \mathbb{R}$, we define the standard normal distribution CDF by 
...
This new perspective offers neww exact characterizations of $M(x)$ for all $x \geq 1$ 
...
parameterized contours as described below. 
...
We can then numerically evaluate the bounded extremal values of this function as 
\begin{align*} 
...
\] 
We next inductively compute the remaining coefficients 
...
\end{align*} 
Finally, by parameterizing the countour around $|z| = r := \frac{k-1}{\log\log x} < 1$, we 
