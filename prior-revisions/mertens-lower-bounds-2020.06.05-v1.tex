...
R_E(x) & \gg (\log\log x) \times \left\lvert \sum_{\log\log\log x < d < \frac{\log\log x}{2e}} 
     ...
     \right] \right\rvert \\ 
     & = 
     (\log\log x) \times \left\lvert \sum_{\log\log\log x < d < \frac{\log\log x}{2e}} 
     O\left(\frac{\log(2d)^{1/4}}{2d (2d+1) \cdot \log\log(2d)}\right) \right\rvert \\ 
     & = O\left(\log\log x\right). 
\end{align*} 
...
$\{x_{0,y}\}_{y \geq Y_0}$, by $x_{0,n} := e^{2e^{e^{e^{e^{2y}}}}}$ for some sufficiently 
large finite $Y_0 \gg 1$. 
