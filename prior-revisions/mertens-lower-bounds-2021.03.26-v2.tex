...
\[
\pi_k(x) = \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} \left(1 + 
...
resulting from an application of Stirling's formula, \eqref{eqn_IncompleteGamma_PropA} and 
\eqref{eqn_IncompleteGamma_PropC}. 
\begin{align*} 
\frac{x}{\log x} \times \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-2}}{(k-1)!} & = 
     \frac{\Gamma(\log\log x, \log\log x)}{\Gamma(\log\log x + 1)} 
     %& \sim \sqrt{\frac{\pi}{2}} \times \frac{x (\log\log x)^{\log\log x-1/2}}{(\log x)} \cdot 
     %\frac{(\log x)}{\sqrt{2\pi} (\log\log x)^{\log\log x+1/2}} \\ 
...
\mathbb{E}\left[C_{\Omega(n)}(n)\right] = 
     \frac{\sqrt{2\pi} (\log n)}{\sqrt{\log\log n}} (1+o(1)). 
\] 
\end{prop} 
\begin{proof} 
...
\sum_{k=1}^{\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \sim 
     \frac{\sqrt{2\pi} \cdot x (\log x)}{\sqrt{\log\log x}}. 
\end{align} 
We claim that 
\begin{align} 
...
\end{align*}
An error term for the last formula follows from the asymptotic series for 
$\operatorname{erfi}(z)$ in \eqref{eqn_Erfix_KnownAsymptoticSeries_v1} below. 
...
\widehat{B}(x, r) \ll x (\log x)^{r-2r\log r} \sqrt{\log\log x} = o\left(\frac{x}{\sqrt{\log\log x}}\right)
\end{equation}
...
\begin{align*} 
\mathbb{E}|g^{-1}(n)| & = \frac{6}{\sqrt{2} \cdot \pi^{3/2}} 
...
This result implies that for all sufficiently large $t \rightarrow +\infty$ 
\begin{align*} 
...
\eqref{eqn_AbsValueOf_gInvn_FornSquareFree_v1} and 
applying \eqref{eqn_Erfix_KnownAsymptoticSeries_v1} 
we find that  
\begin{align} 
\notag 
...
Bruce Reznick and Michael Lacey. 
...
$z$ is defined on the universal covering of $\mathbb{C} \mathbin{\big\backslash} \{0\}$ 
...
\end{lemma}
\begin{proof}
We have that for $n \geq 1$ and any $t > 0$ by 
\eqref{eqn_IncompleteGamma_PropA} and \cite[\S 8.4]{NISTHB}
\[
...
\end{lemma}
\begin{proof}
Notice that by \eqref{eqn_Erfix_KnownAsymptoticSeries_v1} we can write 
\begin{align*}
S_3(x) & = \sum_{k \geq 1} \frac{(\log\log x)^{k-1/2}}{(2k-1) (k-1)!} + O\left(
     \sum_{k \geq 0} \frac{(\log\log x)^{\log\log x + k + 1/2}}{(2k+2\log\log x + 1) 
     \Gamma(k+\log\log x+1)}\right) \\ 
     & = \frac{\sqrt{\pi}}{2} \operatorname{erfi}\left(\sqrt{\log\log x}\right)\left(1 + O\left( 
     \frac{1}{\log\log x}\right)\right) \\ 
     & = \frac{\log x}{2\sqrt{\log\log x}} + O\left(\frac{\log x}{(\log\log x)^{3/2}}\right). 
\end{align*} 
The second formula involving the imaginary error function in the previous equations is 
obtained by applying the symbolic summation procedures in \emph{Mathematica}. 
The last result nearly implies the stated formula in this lemma with less tight error term 
