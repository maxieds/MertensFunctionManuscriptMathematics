...
\newglossaryentry{aStarConstant}{
    symbol={$a^{\ast}$},
    sort={aStarConstant},
    description={We define the absolute constant 
                 $a^{\ast} := \frac{3}{2}\log\left(\frac{3}{2e}\right)+1 \approx 0.108198$. },
    type={symbols},
    name={The Absolute Constant $a^{\ast}$}
    }
\glsaddall[types={symbols}]
...
\begin{align*} 
\mathcal{N}_{\omega}(x) & := \left\lvert \sum_{k > \frac{3}{2}\log\log x} (-1)^{k} \pi_k(x) \right\rvert, \\ 
\mathcal{D}_{\omega}(x) & := \left\lvert \sum_{k \leq \frac{3}{2}\log\log x} (-1)^{k} \pi_k(x) \right\rvert, \\ 
...
\begin{align*} 
\mathcal{D}_{\omega}(x) & =  
\left\lvert \sum_{1 \leq k \leq \frac{3}{2}\log\log x} \frac{(-1)^k \cdot x}{\log x} \cdot 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \right\rvert + O(E_{\omega}(x)) \\ 
     & = \frac{8x}{65\sqrt{2\pi} (\log x)^{a^{\ast}} \sqrt{\log\log x}} + 
     O\left(\frac{x}{(\log x)^{a^{\ast}} (\log\log x)^{3/2}} + E_{\omega}(x)\right), 
\end{align*}
where the absolute constant $a^{\ast} := -\frac{3}{2}\log\left(\frac{3}{2e}\right) - \frac{1}{2} \approx 0.391802$. 
...
$1 \leq k \leq \log\log x$ and large $x$ as 
\begin{align*}
E_{\omega}(x) & := \sum_{k \leq \frac{3}{2}\log\log x} \frac{x}{\log x} 
     \cdot \frac{k (\log\log x)^{k-3}}{(k-1)!} 
     \leq \frac{x}{\log x} \times \sum_{1 \leq k \leq \frac{3}{2}\log\log x} 
     \frac{(\log\log x)^{k-2}}{(k-1)!} \\ 
     & \leq \frac{x}{(\log x)(\log\log x)} e^{\log\log x} \leq \frac{x}{\log\log x}. 
\end{align*}
Next, we utilize the notation for and bounds on the function $D(x, r)$ for $r := 3/2$ from 
Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} 
to bound the function $\mathcal{N}_{\omega}(x)$ as follows: 
\begin{align*}
\frac{1}{x} \times \left\lvert \mathcal{N}_{\omega}(x) \right\rvert & \leq 
     \sum_{k \geq \frac{3}{2}\log\log x} \frac{\pi_k(x)}{x} = 
     \frac{1}{x} \times \sum_{k \geq \frac{3}{2}\log\log x} \#\left\{2 \leq n \leq x: \omega(n)=k\right\} 
     \ll \frac{1}{(\log x)^{0.108198}}. 
\end{align*} 
Then we see that 
\[
\left\lvert \frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \right\rvert = 
     O\left(\frac{(\log\log x)^{1-a^{\ast}}}{(\log x)^{0.108198}}\right) = o(1), 
...
Let the absolute constant $a^{\ast} := \frac{3}{2}\log\left(\frac{3}{2e}\right)+1 \approx 0.108198$. 
For $x \rightarrow +\infty$, we have that 
\begin{align*}
S_1(x) & := \frac{x}{\log x} \times \left\lvert \sum_{1 \leq k \leq \floor{\frac{3}{2}\log\log x}} 
     \frac{(-1)^k (\log\log x)^{k-1}}{(k-1)!} \right\rvert 
     = \frac{8x}{65\sqrt{2\pi} (\log x)^{a^{\ast}} \sqrt{\log\log x}} + 
     O\left(\frac{x}{(\log x)^{a^{\ast}} (\log\log x)^{3/2}}\right). 
\end{align*}
\end{lemma}
\begin{proof}
...
\sum_{1 \leq k \leq n} \frac{(-1)^k t^{k-1}}{(k-1)!} = -e^{-t} \times 
     \frac{\Gamma(n, -t)}{(n-1)!}. 
\]
Suppose that $t = \frac{2n}{3} + \xi$ with $\xi = O(1)$ (e.g., so we can take the floor of the input $n$ to 
truncate the last sum). By the third formula 
in Proposition \ref{prop_IncGammaLambdaTypeBounds_v1} 
with the parameters $(a, z, \lambda) \mapsto \left(n, t, \frac{2}{3} + \frac{\xi}{n}\right)$, 
we deduce that as $n,t \rightarrow +\infty$. 
\begin{equation}
\label{eqn_ProofTag_lemma_ConvenientIncGammaFuncTypePartialSumAsymptotics_v2}
     \Gamma(n, -t) = (-1)^{n+1} \times \frac{4t^n e^{t}}{5(3n+2t)} + 
     O\left(\frac{n t^n e^{t}}{\left(3n+2t\right)^3}\right) = 
     (-1)^{n+1} \times \frac{8t^n e^t}{65n} + O\left(\frac{t^{n-1} e^t}{n}\right). 
\end{equation}
Accordingly, we see that 
\[
\sum_{1 \leq k \leq n} \frac{(-1)^k t^{k-1}}{(k-1)!} = 
     (-1)^{n} \times \frac{8t^n}{65n!} + O\left(\frac{t^{n-1}}{n!}\right). 
\]
By the variant of Stirling's formula in \cite[\cf Eq.\ (5.11.8)]{NISTHB}, we have 
\[
n! = \Gamma\left(1 + \frac{2t}{3} - \frac{2\xi}{3}\right) = 
     \sqrt{2\pi} \cdot \left(\frac{3t}{2}\right)^{\frac{3}{2}(t-\xi)+1/2} e^{-t} \left(1 + O(t^{-1})\right) = 
     \sqrt{2\pi} \cdot e^{-t} t^{n+1/2} \left(\frac{3}{2e}\right)^{\frac{3t}{2}} \left(1 + O(t^{-1})\right). 
\]
Hence, as $n \rightarrow +\infty$ with $t := n + \xi$ and $\xi = O(1)$, we obtain that 
\[
\sum_{k=1}^{n} \frac{(-1)^k t^{k-1}}{(k-1)!} = (-1)^n \times \frac{8e^t}{65 \sqrt{2\pi t}} 
     \left(\frac{3}{2e}\right)^{\frac{3t}{2}}\left(1+ O\left(t^{-1}\right)\right). 
\]
The conclusion follows by taking $n := \floor{\frac{3}{2}\log\log x}$, 
