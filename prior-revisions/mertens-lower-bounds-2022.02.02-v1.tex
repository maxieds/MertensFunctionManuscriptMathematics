...
One can use the relation in \eqref{eqn_gInvn_ExactDivisorSumFormula_WithSgnWeight_v1} 
to derive the average order of $|g(n)|$ from the last result. 
...
The arguments given in the next proof are 
...
\end{align}
For large $x$ and fixed $1 \leq r < P(2)^{-1}$, we define 
\[
...
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit1_GnOvern_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}\hfil
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit2_GnOvern_v2-GInvFunctionSequenceCalculations.png}}
\end{subfigure}
\captionsetup{justification=centering}
\caption{The distribution of $\frac{G(n)n^{-1} - \log n \sqrt{\log\log n}}{\log x \sqrt{\log\log x}}$ for $3 \leq n \leq x$. } 
\end{subfigure}
...
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit1_GnOvern_v1-GInvFunctionSequenceCalculations.png}}
\end{subfigure}\hfil
\centering
\begin{subfigure}{0.47\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/FigureHistogramFit2_GnOvern_v1-GInvFunctionSequenceCalculations.png}}
\end{subfigure}
\captionsetup{justification=centering}
	\caption{The distribution of $\frac{G(n)n^{-1} - \log x \sqrt{\log\log x}}{\log x \sqrt{\log\log x}}$ for $3 \leq n \leq x$. } 
\end{subfigure}

\caption{Distributions of the auxiliary summatory function $G(x)$ 
	 in Definition \hlocalref{def_GInvAndGInvAbs_SummFuncs_v2}. }
\label{figure_HistgInvnDistUns_Comparisons_v1}
...
\centering
\begin{subfigure}{0.375\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/Figure8-GInvCOmegamLxUns.png}}
\end{subfigure}\hfil
\centering
\begin{subfigure}{0.375\textwidth}
\fbox{\includegraphics[width=\textwidth]{images/Figure8-GInvPrimeOverLogPrimemLxUns.png}}
\end{subfigure}
\captionsetup{justification=centering}
\caption{$G[C_{\Omega}][L](m, x)$ (left) and $G\left[\frac{p_m}{\log p_m}\right][L](m, x)$ (right)} 
\end{subfigure}

\caption{We define $R[f][S](m, x) := R\left(\floor{\frac{x}{f(m)} \log\left(1 + \frac{x}{f(m)}\right)^{-1}}\right) S(x)^{-1}$  
	 where we shift the denominator by positive one whenever $S(x) = 0$. 
	 The variable $2 \leq m \leq 64$ is shown along the horizontal axis with $1 \leq x \leq 4500$ along the 
...
contain local information about $G(x)$ through its connection to $|G|(x)$. 
...
shows that there is more correlation between $G(x)$ and the former function viewed within this context. 
Experiments with other sequences $f$ are less regular than the selections shown in the figure. 
...
We credit the correspondence with Gerg\H{o} Nemes 
