...
$\min(-G_{\ell}^{-1}(x/2), -G_{\ell}^{-1}(\sqrt{x})) = -G_{\ell}^{-1}(\sqrt{x})$. 
Then \eqref{eqn_pf_tag_v2-restated_v3} becomes (TODO: Check / refine ... ) 
\begin{align*} 
M(x) & \gg G_{\ell}^{-1}(x) - G_{\ell}^{-1}(x) A_{\Omega}^{(\ell)}(x) + \int_{\sqrt{x}}^{x/2} 
      \frac{x}{t^2 \log(x/t)} \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt + o(\sqrt{x}) \\ 
     & \gg G_{\ell}^{-1}(x) - G_{\ell}^{-1}(x) A_{\Omega}^{(\ell)}(x) + 
     \int_{\sqrt{x}}^{x/2} 
     \frac{x}{t^2 \log(x/t)} \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt + o(\sqrt{x}) \\ 
     & \succsim \left(\min_{\sqrt{x} \leq t \leq x/2} \frac{x}{t^2 \log(x/t)}\right) \cdot 
     G_{\ell}^{-1}\left(x^{1/2}\right) + o\left(\sqrt{x}\right) \\ 
     & = \frac{2}{\log x} \times G_{\ell}^{-1}\left(\sqrt{x}\right) + o\left(\sqrt{x}\right). 
\end{align*} 
...
\begin{align*} 
\frac{|M(x)|}{\sqrt{x}} & \gg 
     \frac{4}{3 \log(x)} G_{\ell}^{-1}\left(x^{1/2}\right) + o(1) \\ 
