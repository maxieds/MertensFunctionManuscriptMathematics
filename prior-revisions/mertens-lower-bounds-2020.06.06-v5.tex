...
\renewcommand{\thefootnote}{$\ast$\underline{F\thesection.\Alph{footnote}}}
\makeatletter
...
$n \leq (\log x)^{\frac{3}{2}} (\log\log x) = o(x)$. 
...
We have that for almost every sufficiently large $x$, that as $x \rightarrow \infty$ (TODO) 
\begin{align*} 
...
\item[\textbf{(A)}] We identify and extract an initial subinterval of our full bounds 
     on the summation defined by 
     \eqref{eqn_GEInvxSummatoryFuncDef_v1}. 
     On this subinterval we prove that we can expect 
     constant sign term contributions from the inputs to the function $\widehat{L}_0$ 
     involving (a priori) both $d,x$. 
...
\item[\textbf{(C)}] We define and determine additional characteristic formulas we will 
     ...
     to just this initial subinterval range. 
...
proofs of our previous results so far. 
Since further disassembly of these distinct parts to proving the bound into 
external makes 
...
$\widehat{L}_0\left((\log x)^{\frac{3}{2}} (\log\log x) / d\right)$, or equivalently a constant parity for 
$\floor{\log\log\left((\log x)^{\frac{3}{2}} (\log\log x) / d\right)}$ at such $d$. 
...
$\left\{(\log x)^{\frac{3}{2}} (\log\log x)\right\}$ and 
...
\]
changes in parity from its value modulo $2$ from $d_0(x)-1$ to $d_0(x)$. 
...
integers $x \rightarrow \infty$. 
Consider the following points for sufficiently large $x$ and any 
$\log d \in \mathcal{R}_x$: 
\begin{itemize}[itemsep=0pt,topsep=0pt,leftmargin=0.35in] 
\item[\textbf{(1)}] If the fractional part 
     $\left\{\log\log\left((\log x)^{\frac{3}{2}} (\log\log x)\right)\right\} = 0$, then (TODO) 
     \begin{align*} 
     \floor{\log\log\left(\frac{(\log x)^{\frac{3}{2}} (\log\log x)}{d}\right)} & = 
          \floor{\log\log\log\log x} + 
          \floor{-\frac{\log d}{(\log\log x)(\log\log\log x)} + 
          o\left(\frac{1}{(\log\log x)^2}\right)}. 
     \end{align*} 
...
\textbf{Part C.} 
We require a couple of key arguments to ensure that we have covered all 
problematic corner cases that can arise 
in forming the lower bound based on a truncation of the interval of summation from the 
original definition of $G_E^{-1}(x)$ to summing only over $d \in \mathcal{R}_x$: 
\begin{itemize}[itemsep=0pt,topsep=4pt,leftmargin=0.35in] 
\item[\textbf{(i)}] 
Our bounds make sense as stated when we remove the signed contributions from the 
so-termed extraneous terms added by summing over 
$\widebar{\mathcal{R}}_x := \left(\log\log\log x, \frac{\log\log x}{e}\right)$. 
To make this point we will define several shorthands for component functions to the 
bound below. 
\item[\textbf{(ii)}] We cannot 
...
$d > \log\log\log x$ (for $d$ on $\widebar{\mathcal{R}}_x$). 
This argument is not sophisticated in light of the intentional lack of tightness 
(or optimality in the largest possible order sense) of the 
upper bound we have chosen on the interval $\mathcal{R}_x$. 
In fact, we can evaluate a routine sum for the 
absolute contribution of the unsigned remainder terms to show that it cannot non-trivially 
interfere with the primary terms in the bound when $x$ is large. 
\end{itemize} 
...
\newpage
\section{Appendix: Notation to express asymptotic relations} 
