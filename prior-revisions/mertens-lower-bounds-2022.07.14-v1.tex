...
\subsection{Local cancellation in the formulas along a primorial subsequence} 
\label{subSection_LocalCancellationOfGInvx} 

\begin{definition}
For integers $x \geq 1$ we define the squarefree and non-squarefree 
restricted summatory functions of $g(n)$ respectively by the partial sums 
\begin{align*}
G_{\sq}(x) & := \sum_{\substack{n \leq x \\ \mu^2(n)=1}} g(n), \\ 
G_{\nsq}(x) & := \sum_{\substack{n \leq x \\ \mu(n)=0}} g(n). 
\end{align*}
For any $x \geq 1$, let the following two functions define the 
componentwise decomposition of 
equation \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} in 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} 
be defined in the sense of the last equations as follows: 
\begin{align*}
\widehat{G}_{\sq}(x) & := G_{\sq}(x) + \sum_{p \leq x} G_{\sq}\left(\Floor{x}{p}\right), \\ 
\widehat{G}_{\nsq}(x) & := G_{\nsq}(x) + \sum_{p \leq x} G_{\nsq}\left(\Floor{x}{p}\right). 
\end{align*}
\end{definition}
...
\begin{lemma}
\label{lemma_MertensPrimorialSubsequenceLemma_v1}
As $m \rightarrow \infty$ each of the following holds:
\begin{align}
\tag{A.1}
G_{\sq}\left((4m+1)\#\right) & = -(4m+1)! + O\left(\frac{1}{m}\right), \\ 
\tag{A.2}
G_{\sq}\left(\frac{(4m+1)\#}{p_k}\right) & = (4m)! + O\left(\frac{1}{m}\right), 
     \mathtext{ for all } 1 \leq k \leq 4m+1, \\ 
\tag{B.1}
-G_{\nsq}\left((4m+1)\#\right) & \gg (4m+1)! + O(1), \\ 
\tag{B.2}
G_{\nsq}\left(\frac{(4m+1)\#}{p_k}\right) & \gg (4m)! + O\left(\frac{1}{4m+1}\right), 
     \mathtext{ for all } 1 \leq k \leq 4m+1.
\end{align}
\end{lemma}
\begin{proof}[Proof of Equations (A.1) and (A.2)]
We prove (A.1) by summing over the squarefree $n \leq (4m+1)\#$ partitioned 
by the number of prime divisors of $n$ using the formula from 
equation \eqref{eqn_PropB_lemma_gInv_MxExample} 
to express $|g(n)|$ in the following forms:
\begin{align*}
G_{\sq}\left((4m+1)\#\right) & =  
     \sum_{0 \leq j \leq 4m+1} (-1)^j \times 
     \#\left\{n \leq (4m+1)\#: \omega(n)=j\right\} \times 
     \sum_{0 \leq m \leq j} \binom{j}{m} m! \\ 
     & = \sum_{0 \leq j \leq 4m+1} \binom{4m+1}{j} (-1)^j j! \left(e + 
     O\left(\frac{1}{(j+1)!}\right)\right) \\ 
     & = -(4m+1)! + O\left(\frac{1}{2m+1}\right), 
     \mathtext{ as } m \rightarrow \infty. 
\end{align*}
The same counting argument is used to sum the left-hand-side function in (A.2) for 
each $1 \leq k \leq 4m+1$ as 
\begin{align*}
G_{\sq}\left(\frac{(4m+1)\#}{p_k}\right) & = \sum_{0 \leq j \leq 4m} \binom{4m}{j} (-1)^j j! \left(
     e + O\left(\frac{1}{(j+1)!}\right)\right) \\ 
     & = (4m)! + O\left(\frac{1}{m}\right), 
     \mathtext{ as } m \rightarrow \infty. 
     \qedhere
\end{align*}
\end{proof}
\begin{proof}[Proof of Equations (B.1) and (B.2)]
\textbf{TODO ... Current argument is off ... May need to adapt the proof of the lemma from the appendix section later ... }
\end{proof}

Lemma \hlocalref{lemma_MertensPrimorialSubsequenceLemma_v1} 
allows us to prove that assuming the 
Riemann Hypothesis (RH) is true, then 
along the subsequence of primorial integers, 
$\{(4m+1)\#\}_{m \geq 1}$, the Mertens function is at most a bounded constant. 
This is a remarkable result that has not been proven in the references to date! 

\begin{cor}
If the RH is true, then 
\[
M\left((4m+1)\#\right) = O(1), \mathtext{ as } m \rightarrow \infty. 
\]
\end{cor}
\begin{proof}
Equations (A.1) and (A.2) of 
Lemma \hlocalref{lemma_MertensPrimorialSubsequenceLemma_v1} show that 
\begin{equation}
\label{eqn_ProofTag_GHatSqCancellationSumBound_v2}
\widehat{G}_{\sq}((4m+1)\#) = G_{\sq}((4m+1)\#) + \sum_{1 \leq k \leq 4m+1} 
     G_{\sq}\left(\frac{(4m+1)\#}{p_k}\right) = O(1). 
\end{equation}
We need to show that $\left\lvert \widehat{G}_{\nsq}((4m+1)\#) \right\rvert = O(1)$ 
for large $m$. Suppose that this is not the case. 
Then equations (B.1) and (B.2) from the lemma require the constant factors on the 
main terms of these lower bounds to differ in absolute value. 
This shows that for large $m$ we have
\begin{equation}
\label{eqn_ProofTag_MertensAlongPrimorialsLowerBound_v3}
M((4m+1)\#) \gg \left\lvert \widehat{G}_{\nsq}((4m+1)\#) \right\rvert \gg (4m+1)!. 
\end{equation}
We cite that \cite{DUSART-1999,DUSART-2010} 
\[
n\# %\sim e^{\vartheta(p_n)} 
     \asymp n^n (\log n)^n e^{-n(1+o(1))}, 
     \mathtext{ as } n \rightarrow \infty. 
\]
This result and equation \eqref{eqn_ProofTag_MertensAlongPrimorialsLowerBound_v3} show that 
as $m \rightarrow \infty$ 
\begin{equation}
\label{eqn_ProofTag_MertensAlongPrimorialsLowerBound_v4}
\frac{M((4m+1)\#)}{\sqrt{(4m+1)\#}} \gg \frac{\sqrt{(4m+1)\#}}{\log^{4m+1}(4m+1)} \gg 
     \left[(4m+1)\#\right]^{\frac{1}{6}}. 
\end{equation}
The RH is equivalent to showing that 
\begin{equation} 
\label{eqn_MertensMx_RHEquivProblem_Stmt_intro} 
M(x) = O\left(x^{\frac{1}{2}+\varepsilon}\right), 
     \mathtext{ for all } \varepsilon \in \left(0, \frac{1}{2}\right).
\end{equation}
If equation \eqref{eqn_ProofTag_MertensAlongPrimorialsLowerBound_v4} is correct, we have a 
contradiction to the RH for large $m$. 
In other words, under the assumption of the RH we must have that at most
$\left\lvert \widehat{G}_{\nsq}((4m+1)\#) \right\rvert = O(1)$.
Using this bound with equation \eqref{eqn_ProofTag_GHatSqCancellationSumBound_v2} and 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} proves the absolute boundedness 
along the primorial subsequence since 
\[
\left\lvert M((4m+1)\#) \right\rvert = \left\lvert \widehat{G}_{\sq}((4m+1)\#) + 
     \widehat{G}_{\nsq}((4m+1)\#) \right\rvert. 
     \qedhere
\]
\end{proof}
