...
\newglossaryentry{chiPrimeP}{
    ...
    description={The indicator function of the primes equals one if and only if 
                 ...
                 For any $s \in \mathbb{C}$ with $\Re(s) > 1$, 
...
limiting Erd\H{o}s-Kac type theorem for both 
...
\begin{align*} 
M(x) & \ll \sqrt{x} \times \exp\left((\log x)^{\frac{1}{2}} (\log\log x)^{14}\right), \\ 
...
$\frac{1}{n} \times \sum_{k \leq n} \Omega(k) = \log\log n + B_2$ for $B_1,B_2 \in (0, 2)$ 
absolute constants in each case, 
%\footnote{
%     More precisely, we have that \cite[\S 22.8; \S 22.10]{HARDYWRIGHT} 
%     \begin{align*} 
%     B_1 & = \gamma + \sum_p \left(\log\left(1-\frac{1}{p}\right) + \frac{1}{p}\right) 
%          \approx 0.261497, \\ 
%     B_2 & = B_1 + \sum_p \frac{1}{p(p-1)} \approx 1.03465, 
%     \end{align*} 
%     where $\gamma \approx 0.577216$ is the \emph{Euler-Mascheroni constant}. 
%}, 
...
\end{align*} 
Then we have upper bounds given by 
\begin{align*} 
...
product identity of the following form using the results from the proof of 
Proposition \ref{prop_SignageDirInvsOfPosBddArithmeticFuncs_v1}: 
\begin{align*} 
...
\end{align*} 
It follows from the Euler product for $\zeta(s)$ that 
\[
...
\sum_{m \leq \frac{x}{2}} b_z(m) D_z\left(\frac{x}{m}\right) & = \frac{x}{\Gamma(z)} (\log x)^{z-1} \times 
     ...
     \right) \\ 
     & = \frac{x \widehat{F}(2, z)}{\Gamma(z)} (\log x)^{z-1} + O_{z}\left( 
...
\]
Since $(\log x)^{\frac{1}{\log\log x}} \rightarrow e$ as $x \rightarrow \infty$, 
the error term in the formula 
...
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} 
     ...
     \frac{(\log x)^{v}}{v^k} dv. 
\]
After integrating by parts \cite[\cf Thm.\ 7.19; \S 7.4]{MV}, we have that 
\[
I_{2,k}(r, x) := \frac{1}{2\pi\imath} \times \int_{|v|=r} 
...
Remark \ref{remark_MV_Pikx_FuncResultsAnnotated_v1} is decreasing with 
...
E_{\omega}(x) & \ll \frac{x}{\log x} \times 
     \sum_{1 \leq k \leq \log\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} \\ 
     & = 
     \frac{x \cdot \Gamma(\log\log x, \log\log x)}{\Gamma(\log\log x)} 
...
\end{equation} 
We notice that $O\left(\widehat{C}_k^{\prime}(x)\right) = O(1)$ so that since 
$\widehat{C}_k(x)$ is a non-decreasing function of $x$, we have 
$O\left(\widehat{C}_k^{\prime}(x)\right) = o\left(\widehat{C}_k(x)\right)$ for all 
large enough $x$. 
...
\widehat{C}_k(x) \asymp \frac{(\log\log x)^{k-\frac{1}{2}}}{(k-1)!} \times 
     \sum_{k=0}^{\log\log x} x^{e^{-k}} \sim 
     \frac{(\log\log x)^{k-\frac{1}{2}}}{(k-1)!} \times 
     \sum_{j \geq 0} \frac{e^j (\log x)^j}{e^j-1} \sim  
     \frac{x (\log\log x)^{k-\frac{1}{2}}}{(k-1)!}. 
\]
...
\widehat{C}_k(x) - \widehat{C}_k\left(x^{e^{-1}}\right) = 
...
\sum_{k=1}^{2\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & \asymp 
     ...
     \left(1 + O\left(\frac{1}{\log\log x}\right)\right) \\ 
\notag 
     & = \frac{x (\log x) \sqrt{\log\log x} \cdot \Gamma(2\log\log x, \log\log x)}{\Gamma(2\log\log x)} 
     \left(1 + O\left(\frac{1}{\log\log x}\right)\right) \\ 
\notag 
...
\sum_{k=1}^{2\log\log x} \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) & = 
     B_0 x (\log x) \sqrt{\log\log x} \left(1 + O\left(\frac{1}{\log\log x}\right)\right). 
\end{align} 
We claim that 
\begin{align} 
\notag 
...
that whenever $0 < |z| < P(2)^{-1}$ and when $x$ is sufficiently large we have that 
\begin{align}
\notag
...
\end{align*} 
We have shown by the above argument that 
$S_2(x) = O\left((\log x)^{0.613706} \sqrt{\log\log x}\right)$ 
as $x \rightarrow \infty$. This implies that 
...
asymptotic series given in \eqref{eqn_Erfix_KnownAsymptoticSeries_v1}. 
...
\begin{ansatz}
We require a probabilistic ansatz that the functions 
...
\cite[\S 2.4]{MV} as 
\[
\#\{n \leq x: \Omega(n) - \omega(n) = m\} \sim d_m x, m \geq 0, 
\]
where $\sum_{m \geq 0} d_m \sim 1$ as $x \rightarrow \infty$. 
...
when there is overlap in the values $r_1,r_2$ assumed by $X_{n_1,k},X_{n_2,k}$, we still 
see that these variables are (approximately) independent as $x \rightarrow \infty$: 
\[
\mathbb{P}(X_{n_1,k} = r_1 \mid X_{n_2,k} = r_2) = \begin{cases} 
     \mathbb{P}(X_{n_1,k} = r_1), & r_1 \neq r_2; \\ 
          \mathbb{P}(X_{n_1,k} = r_1) + O\left(\frac{1}{x}\right), & r_1 = r_2, 
     \end{cases} 
\]
and vice versa. 
\end{ansatz}
...
\begin{prop}
We have that each of the following holds: 
\begin{itemize} 
\item[(A)] For $1 \leq k \leq 2\log\log x$, let 
     \[
     \mu_k(x) := \frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
          \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}}. 
     \]
     Then as $x \rightarrow \infty$, we have that 
     \[
     \mu_k(x) = \frac{A_0 \sqrt{2\pi}}{\log x} \times 
          \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
          \frac{(\log\log x)^{k-1}}{(k-1)!}\left(1 + O\left(\frac{1}{\log\log x}\right)\right). 
     \]
\item[(B)] For $1 \leq k \leq 2\log\log x$, let 
     \[
     ...
     \]
     Then $s_x^2 = o(1)$ as $x \rightarrow \infty$. 
     Moreover, we have that 
     \[
     \lim_{x \rightarrow \infty} \max_{\substack{1 \leq k \leq 2\log\log x}} 
          \frac{\sigma_k^2(x)}{s_x^2} = 0. 
     \]
\item[(C)] For $n \geq 1$, let $T_n := X_{n,\Omega(n)}$. The sequence of random variables 
     $\{T_n\}_{n \geq 1}$ satisfies Lindeberg's condition, i.e., for all fixed $\epsilon > 0$ 
     \[
     \lim_{x \rightarrow \infty} \frac{1}{s_x^2} \times \sum_{n \leq x} \left(\frac{1}{n} \times 
          \sum_{m \leq n} 
          (T_m-\mu_{\Omega(m)}(m))^2 \mathds{1}_{\{|T_m-\mu_{\Omega(m)}(m)| > \epsilon s_n\}} 
          \right) = 0. 
     \]
\end{itemize} 
\end{prop}
...
that uniformly for $\frac{1}{2}\log\log x \leq k \leq \log\log x$ 
\begin{align} 
\notag 
\sum_{\substack{n \leq x \\ \Omega(n)=k}} \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}} & = 
     \frac{\widehat{C}_k(x)}{(\log x) \sqrt{\log\log x}} + O\left( 
     \int_3^x \frac{dt}{t (\log t) (\log\log t)}\right) \\ 
\label{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v0} 
     & = \frac{A_0 \sqrt{2\pi} x}{\log x} \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!}\left(1 + O\left(\frac{1}{\log\log x}\right)\right), 
     \text{ as } x \rightarrow \infty. 
\end{align}
The main term of the right-hand-side bound in 
\eqref{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v0} 
is maximized for $\frac{1}{2}\log\log x \leq k \leq \log\log x$ 
at the upper end of the range for $k$ by 
$A_0 (\log\log x)^{-\frac{1}{2}}$ as $x \rightarrow \infty$.
\end{proof}
\begin{proof}[Proof of (B)]
We have for large $x$ and any $1 \leq k \leq 2\log\log x$ that 
$\sigma_k^2(x) \leq \mu_k^2(x) \leq x \sigma_k^2(x)$. 
It follows that 
\[
s_x^2 = \sum_{1 \leq k \leq 2\log\log x} \widehat{\pi}_k(x) \sigma_k^2(x) \ll 
     \sum_{1 \leq k \leq 2\log\log x} \frac{1}{(\log\log x)^{\frac{3}{2}}} \ll 
     \frac{1}{\sqrt{\log\log x}}. 
\]
In the previous equation, we have used that for $1 \leq k \leq 2\log\log x$ 
both $\widehat{\pi}_k(x), \mu_k(x) \ll \frac{x}{\sqrt{\log\log x}}$. 
Similiarly, we see that $s_x^2 \gg \frac{1}{x \sqrt{\log\log x}}$ using the first 
inequality above. 
Then we see that 
\[
\max_{1 \leq k \leq 2\log\log x} \frac{\sigma_k^2(x)}{s_x^2} \ll 
     \max_{1 \leq k \leq 2\log\log x} \sqrt{\log\log x} \mu_k^2(x) \ll 
     \frac{1}{\sqrt{\log\log x}}. 
     \qedhere 
\]
\end{proof}
...
\[
\frac{1}{x} \times \sum_{1 \leq k \leq \mu_x(C) + z\sigma_x(C)} 
     \mu_k(x)  
     \implies \mathcal{N}\left(\mu_x(C), \sigma_x(C)^2\right) + 
     O\left(\frac{1}{\sqrt{\log\log x}}\right). 
\]
\end{prop} 
\begin{proof}
...
of the ansatz. For $1 \leq k \leq 2\log\log x$, let 
\[
...
By integration by parts applied to 
Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2}, we have 
that uniformly for $1 \leq k \leq \log\log x$ 
\begin{align} 
\notag 
...
satisfies \emph{Lindeberg's condition} so that for the Lindeberg variance parameter 
$s_x^2$, we have 
\[
\frac{1}{x \cdot s_x} \times \sum_{1 \leq k \leq 2\log\log x} \left( 
     \sum_{\substack{n \leq x \\ \Omega(n)=k}} \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}} - 
     x \cdot \widehat{\mu}_k(x)\right) \implies \mathcal{N}(0, 1), 
     \text{ as } x \rightarrow \infty. 
\]
...
It is straightforward to extend the analytic methods used in 
\cite[Thm.~7.21; \S 7.4]{MV} to prove that for large $x$ 
\[
\sum_{k} \widehat{\mu}_k(x) \implies \mathcal{N}\left(\mu_x(C), \sigma_x(C)^2\right), 
\]
where the error term in the difference from normal is 
$O\left((\log\log x)^{-\frac{1}{2}}\right)$. 
Hence, we also have that 
\[
\frac{1}{x} \times \sum_{n \leq x} \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}}
     \implies \mathcal{N}\left(\mu_x(C), \sigma_x(C)^2\right), 
\]
with the same error term. We conclude that 
\[
\frac{1}{x} \times \#\left\{2 \leq n \leq x: 
     \frac{\frac{C_{\Omega(n)}(n)}{(\log n)\sqrt{\log\log n}} - 
     \mu_x(C)}{\sigma_x(C)} \leq z\right\} = 
...
\frac{1}{x} \times \#\left\{2 \leq n \leq x: 
     \frac{\frac{C_{\Omega(n)}(n)}{(\log n)\sqrt{\log\log n}} - 
     \mu_x(C)}{\sigma_x(C)} \leq z\right\} = 
     \Phi\left(z\right) + O_z\left(\frac{1}{\sqrt{\log\log x}}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\] 
\end{theorem} 
\begin{proof} 
...
for the target probability density function of the standard normal distribution in the 
...
\frac{1}{x} \times \sum_{\substack{n \leq x \\ \Omega(n)=k}} 
     \frac{C_{\Omega(n)}(n)}{(\log n) \sqrt{\log\log n}} & \sim 
     \frac{A_0 \sqrt{2\pi}}{\log x} \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \\ 
     & \sim 
     \frac{e^{t} (1+\delta_{t,x})^{\frac{1}{2}-t-\mu_x(C)}}{\sqrt{2\pi \log\log x} \cdot \widehat{G}(1)} 
     \times \widehat{G}\left(\frac{k-1}{\log\log x}\right). 
\end{align*} 
...
uniformly for $|t| \leq \mu_x(C)^{\frac{2}{3}}$ 
\begin{align*} 
\frac{A_0 \sqrt{2\pi}}{\log x} & \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!} \\ 
     & = \frac{1}{\sqrt{2\pi} \sigma_x(C)} \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \times \exp\left(-\frac{t^2}{2\sigma_x(C)^2}\right) \times \left( 
     1 + \widetilde{E}(x, t)
     \right). 
\end{align*} 
For $|t| \leq \mu_x(C)^{\frac{2}{3}}$, we also have that 
\[
...
is the DGF of $g^{-1}(n)$ evaluated at $s = 2$. 
Hence, we have that whenever $|t| \leq  \mu_x(C)^{\frac{2}{3}}$ 
\begin{align*}
\frac{A_0 \sqrt{2\pi}}{\log x} & \times \widehat{G}\left(\frac{k-1}{\log\log x}\right) 
     \frac{(\log\log x)^{k-1}}{(k-1)!} 
     & = 
     \frac{1}{\sqrt{2\pi} \sigma_x(C)} \times 
     \exp\left(-\frac{(k-\mu_x(C))^2}{2\sigma_x(C)^2}\right) \times 
...
We then sum over a corresponding range of 
\[
...
each of the following statements holds for $\alpha \in \mathbb{R}$: 
\begin{align*} 
\tag{A}
...
We recover the next theorem about bounds for almost all 
...
\]
Then if we let $y \mapsto \frac{y}{\delta (\log x) \sqrt{\log\log x}}$ in 
...
\frac{1}{x} \times \#\left\{x^{\delta} \leq n \leq x: 
     ...
     \Phi\left(\frac{\frac{\pi^2 y}{6\delta(\log x) \sqrt{\log\log x}} - \mu_x(C)}{\sigma_x(C)}\right) 
     + O\left(\frac{1}{\sqrt{\log\log x}}\right). 
\]
Now suppose that we set $\delta := \frac{1}{\log\log x}$ and let $x \rightarrow \infty$. 
Since $(\log x)^{\frac{1}{\log\log x}} \rightarrow e$ as $x \rightarrow \infty$, 
we obtain for large enough $y$ that  
\[
\frac{1}{x} \times \#\left\{e \leq n \leq x: 
     |g^{-1}(n)| - \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \leq y\right\} \leq  
     \Phi\left(\frac{\frac{\pi^2 y}{6(\log x) (\log\log x)^{\frac{3}{2}}} - \mu_x(C)}{\sigma_x(C)}\right) 
     + O\left(\frac{1}{\sqrt{\log\log x}}\right). 
\]
Suppose that for all large $x$, we take 
$y \geq \frac{12}{\pi^2} (\log x) (\log\log x)^{\frac{5}{2}}$. 
Then we have 
\[
\Phi\left(\frac{\frac{\pi^2 y}{6(\log x) (\log\log x)^{\frac{3}{2}}} - \mu_x(C)}{\sigma_x(C)}\right) \leq 
     \Phi\left(\log\log x\right) = 
     \frac{1}{2}\left(1 + \operatorname{erf}\left(\frac{\log\log x}{\sqrt{2}}\right)\right) = 
     1 + O\left(\frac{1}{(\log x)^{\frac{\sqrt{2}}{2}} (\log\log x)}\right). 
\]
By Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2} with 
$\delta = \frac{1}{\log\log x}$, we have that for $x^{\delta} \leq n \leq x$ 
\begin{align*}
y + \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| & \geq 
     \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \times \left( 
     \frac{2(\log\log x)^2}{B_0 \delta^2 (\log x)} \left(1 + 
     O\left(\frac{1}{\log\log x}\right)\right) + 1 + O\left(\frac{\delta}{\log x}\right)\right) \\ 
     & = \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \times \left( 
     1 + \frac{2}{B_0 (\log x)} + O\left(\frac{1}{(\log x) (\log\log x)}\right)\right) \\ 
     & = \frac{6}{\pi^2 n} \times \sum_{k \leq n} |g^{-1}(k)| \times \left( 
     1 + O\left(\frac{1}{\log x}\right)\right). 
     \qedhere 
\end{align*} 
\end{proof} 
...
\item[(iii)] $M((4m+1)\#) = o\left(\sum\limits_{\substack{n \leq (4m+1)\# \\ \omega(n)=\Omega(n)}} 
     g^{-1}(n) \left(1+\pi\left(\frac{(4m+1)\#}{n}\right)\right)\right)$. 
\end{itemize} 
