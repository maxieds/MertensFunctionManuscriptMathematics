...
\pagestyle{empty}
\pagestyle{fancy}
\fancyhead[RO,RE]{Maxie's Pipe Dream Proof Attempt -- \today} 
\fancyhead[LO,LE]{}
...
\subsection{Organization of this document (TODO)} 
...
\begin{prop}[Sign of Dirichlet inverses of positive and bounded arithmetic functions] 
...
where the function 
\[
\widehat{L}_0(x) := (-1)^{\floor{\frac{3}{2} \log\log\log\log x} + 1} \left\{ 
...
$C_{\ell,1}, C_{\ell,2}$ are an absolute constants defined by 
\[
C_{\ell,1} = \frac{3}{16} \sqrt{\frac{3}{2}} \frac{A^2 (2e+3)^2}{\pi e B^2 (\log 2)^3}, 
     C_{\ell,2} = \frac{27 A^2 (2e+3)^3}{128 \pi^{3/2} B^2 (\log 2)^3}, 
\]
...
\label{proof_thm_GInvFunc_v0} 
     ...
     \frac{3A}{4eB \log^{\frac{3}{2}}(2) \Gamma\left(1 + \frac{3}{2}\log\log t\right)} \left( 
...
Stirling numbers of the first kind that \cite[\S 7.4]{GKP}\footnote{ 
     ...
     \[
     \sum_{k \geq 0} \gkpSI{k}{j} \frac{z^k}{k!} = \frac{(-1)^j}{j!} \operatorname{Log}^j(1-z). 
     \]
} 
\begin{align*}
...
\notag 
     & \phantom{\succsim\ - } 
     \widehat{T}\left(\frac{\log\log\log\log x}{2}; \frac{\log\log\log\log x}{2}\right) \left( 
     \operatorname{cf}_{+}\left(\frac{\log\log\log\log x-1}{2}\right) - 
     \operatorname{cf}_{-}\left(\frac{\log\log\log\log x}{2}\right)
     \right) \\ 
\notag 
     & \phantom{\succsim\ } - 
     \widehat{T}\left(\frac{\log\log\log\log x-1}{2}; \frac{\log\log\log\log x-1}{2}\right) \left( 
     \operatorname{cf}_{+}\left(\frac{\log\log\log\log x}{2}\right) - 
     \operatorname{cf}_{-}\left(\frac{\log\log\log\log x-1}{2}\right)
     \right) 
     \Biggr]. 
...
\notag 
     & \succsim 
      ...
      9 \sqrt{3} A (2e+3)^2}{32 B\pi e \log^{3/2}(2)} 
...
Since $\pi(1) = 0$ and $\pi(j) = \pi(\sqrt{x})$ for all $\sqrt{x} \leq j < x$, 
...
\notag 
      ...
      \int_{u_0}^{\sqrt{x}} 
      \frac{2\sqrt{x}}{t^2 \log(x)} \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt + 
      \int_{\sqrt{x}}^{x/2} 
      \frac{x}{t^2 \log(x/t)} \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt. 
\end{align} 
We have that one of these integrals contributes a term of $o(\sqrt{x})$ in the 
form of 
\begin{align*} 
\left\lvert \int_{u_0}^{\sqrt{x}} \frac{2\sqrt{x}}{t^2 \log x} 
     \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt \right\rvert & \leq 
     \left\lvert 
     \frac{2 \sqrt{x}}{\log x} \left(\frac{1}{\sqrt{x}} - \frac{1}{u_0}\right) 
     \frac{d}{dx}\left[|G_{\ell}^{-1}(t)|\right] \Bigr\rvert_{t=\sqrt{x}} \right\rvert 
     = o\left(\sqrt{x}\right). 
\end{align*} 
...
M(x) & \gg G_{\ell}^{-1}(x) - G_{\ell}^{-1}(x) A_{\Omega}^{(\ell)}(x) + \int_{u_0}^{\sqrt{x}} 
     ...
     \int_{u_0}^{\sqrt{x}} 
     \frac{x}{t^2 \log(x/t)} \frac{d}{dt}\left[G_{\ell}^{-1}(t)\right] dt + o(1) \\ 
     & \succsim \left(\min_{u_0 \leq t \leq x^{1/4}} \frac{x}{t^2 \log(x/t)}\right) \cdot 
     G_{\ell}^{-1}\left(x^{1/4}\right) + o\left(\sqrt{x}\right) \\ 
     & = \frac{4\sqrt{x}}{3\log x} \times G_{\ell}^{-1}\left(x^{1/4}\right) + o\left(\sqrt{x}\right). 
\end{align*} 
...
\begin{align*} 
\frac{|M(x)|}{\sqrt{x}} & \gg 
     \frac{4}{3 \log(x)} G_{\ell}^{-1}\left(x^{1/4}\right) + o(1) \\ 
     & \succsim \frac{8 C_{\ell,1}}{3} \cdot \left(\frac{\log x}{4}\right)^{ 
     \frac{3}{2} \log\log\log\log x - \frac{3}{7}} 
     \log\log\log x \cdot \sqrt{\log\log\log\log x} \cdot 
     (\log\log x)^{\frac{71}{14} + \frac{3}{2\log 2} - \frac{3}{2\log 3}} \times \\ 
     & \phantom{\succsim\ \frac{8 C_{\ell,1}}{3} \cdot } \times 
     \left(\frac{\log\log\log x \cdot \log\log\log\log x}{\log\log x}\right)^{\log\log\log\log x} \\ 
     & \succsim 
     \frac{8 C_{\ell,1}}{3} \cdot \left(\frac{\log x}{4}\right)^{ 
     \frac{3}{2} \log\log\log\log x - \frac{3}{7}} 
     \log\log\log x \cdot \sqrt{\log\log\log\log x} \cdot 
     (\log\log x)^{\frac{71}{14} + \frac{3}{2\log 2} - \frac{3}{2\log 3}}, 
\end{align*} 
where the constant power $\frac{71}{14} + \frac{3}{2\log 2} - \frac{3}{2\log 3} \approx 5.87011$. 
The transition in the last step of the previous equations is justified by establishing the limit 
\[
...
\begin{center}
\frame{\includegraphics[width=0.45\textwidth]{images/graph-100.png}} 
\frame{\includegraphics[width=0.45\textwidth]{images/graph-500.png}} 
\end{center} 
\end{minipage} 
...
\begin{minipage}{\textwidth} 
\begin{center}
\frame{\includegraphics[width=0.45\textwidth]{images/graph-1500.png}} 
\frame{\includegraphics[width=0.45\textwidth]{images/graph-10000.png}} 
\end{center}
\end{minipage} 
...
\caption*{\textbf{\rm \bf Table \thesubsection:} 
          ...
          columns show the summatory function of $g^{-1}(n)$, $G^{-1}(x) := \sum_{n \leq x} g^{-1}(n)$, borken down into its 
