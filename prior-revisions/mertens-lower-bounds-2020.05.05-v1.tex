...
We obtain lower bounds of the following form for $A_0$ an absolute constant, and 
...
prior sections. Namely, we prove the form of an explicit limiting lower bound for the 
summatory function, $G^{-1}(x) := \sum_{n \leq x} g^{-1}(n)$, along a specific subsequence 
along which the parity of both $\floor{\frac{1}{2}\log\log\log\log x}$ and 
$\floor{\frac{3}{2}\log\log\log\log x}$ are predictable. 
What we obtain is the following corollary verifying the unboundedness of the scaled function 
...
\end{align*} 
as $y \rightarrow \infty$. In the previous equation, we borrow the notation for the 
...
and then piece-by-piece in the next sections of the article in the next listing. This outline is provided to help 
the reader see our logic and proof methodology as easily as possible. 
\begin{itemize} 
...
\subsection{Proving precise bounds on the inverse functions} 
...
A_{\Omega}^{(\ell)}(\log\log x) & \succsim 
     ...
     \frac{2A_0x}{\sqrt{\pi} \log^{3/2}(2)} (\log\log\log x)^{\frac{1}{2} + \frac{3}{2\log 2} - \frac{3}{2\log 3}} 
