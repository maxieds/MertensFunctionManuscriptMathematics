...
\] 
%The plots in Figure \ref{figure_MxLx_discrete_plot_comparison_v1} compare the values of 
%the two summatory functions $M(x)$ and $L(x)$ at integers $1 \leq x \leq 5000$. 
...
\begin{figure}[ht]
     ...
     \caption{A comparison of the values of the summatory functions 
...
\caption{Bounded envelopes for the local extremum of $M(x)$ expressed in terms of the partial sums of the unsigned 
...
\caption{The displayed jagged plots show that the difference between $M(x)$ and $G^{-1}(x)$ is usually small 
         ...
         values of $G^{-1}(x)$ where the absolute constant 
...
\caption{Comparisons of a logarithmically scaled form of $G^{-1}(x)$ and envelopes that bound its local 
         ...
         $g^{-1}(n)$ for $n \leq x$ such that $\lambda(n) = \pm 1$, respectively.}
\end{subfigure}
...
\caption{Comparisions of bounded envelopes for the local extremum of the logarithmically scaled values of 
         ...
         $\zeta(2)^{-1}$ to show the relative bounds in the displays. }
\end{subfigure}
...
in Proposition \hlocalref{lemma_gInv_MxExample} holds for all squarefree integers 
...
Proposition \ref{lemma_gInv_MxExample}. 
...
in Section \hlocalref{Section_NewFormulasForgInvn}.
We reformulate the proofs of the results in \cite[\S 7.4; \S 2.4]{MV} 
that apply analytic methods to formulate limiting asymptotics and to 
...
are centered at the average order of this unsigned sequence in a way that is only 
...
denotes the \emph{prime zeta function} defined as in the glossary on 
page \pageref{Section_NotationAndConventions} for any $\Re(s) > 1$ where the series is convergent 
...
$h^{-1}$ geometrically in powers of $P(s)$ (where $|P(s)| < 1$ whenever $\Re(s) > 1$) 
we can count that 
\begin{align*}
...
The discrete plots of related summatory functions given in the figures from 
Appendix \hlocalref{appendix_figures_SeqsAndPartialSumsPlots} 
also serve to visually motivate why we are interested in the distribution of this 
sequence, and the component function $C_{\Omega}(n)$ we explicitly connect to it, in the 
next results. 
...
(see Section \hlocalref{subSection_AConnectionToDistOfThePrimes}). 
...
In the remainder of this section and in the corollaries on $|g^{-1}(n)|$ in 
...
\]
Let the partial sums of the coefficients of the DGF 
$\widehat{F}(s, z) \zeta(s)^{z}$ be denoted by 
\[
...
\[
\widehat{C}_1^{\prime}(x) \gg \widehat{\pi}_1(x), \widehat{C}_2^{\prime}(x) \gg \widehat{\pi}_2(x). 
\]
Moreover, for any integers $k \geq 3$ we see that 
\begin{align*}
...
(\cf Facts \hlocalref{facts_ExpIntIncGammaFuncs} for $k$ of 
...
second moment type partial sums of the function $C_{\Omega}(n)$ satisfy 
\begin{align*}
...
The second conjecture is effectively the statement of an Erd\H{o}s-Kac type 
...
Then we have for any $z \in (-\infty, +\infty)$ that 
\begin{equation} 
\tag{A} 
...
\end{prop} 
\begin{proof} 
We know by applying Corollary \hlocalref{cor_Mx_gInvnPixk_formula} that 
\begin{align} 
\notag
...
\subsection{Example: Local cancellation of $G^{-1}(x)$ in the new formulas for $M(x)$} 
