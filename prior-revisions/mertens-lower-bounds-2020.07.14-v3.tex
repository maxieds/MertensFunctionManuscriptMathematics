...
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v3} 
     ...
     \left[1 + O\left(\frac{1}{(\log\log x)^2}\right)\right]. 
\end{align} 
We define the following two component sums for large $x$ and $0 < \varepsilon < 1$ so that 
$(\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}} = o(\log x)$: 
\begin{align*} 
S_1(x) & := \sum_{P(2) \log\log x < k \leq (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} \widehat{C}_k(x) \\ 
S_{2,\varepsilon}(x) & := \sum_{\log\log x \leq k \leq \log\log x} \widehat{C}_k(x). 
\end{align*} 
Then 
\[
\sum_{P(2)\log\log x < k \leq (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} \widehat{C}_k(x) 
     \gg S_1(x) + S_{2,\varepsilon}(x),  
\]
with equality as $\varepsilon \rightarrow 1$. 
...
We bound each of the sums defined above using 
Theorem \ref{theorem_MV_Thm7.20-init_stmt} for large $x \rightarrow \infty$ as 
\begin{align*} 
S_1(x) & \leq \sum_{P(2) \log\log x < k \leq (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} 
     \sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) 
     \ll \sum_{k=P(2)\log\log x+1}^{\log\log x-1} \frac{\widehat{\pi}_k(x)}{x} \cdot k! \\ 
     & \ll 
     \sum_{k=P(2)\log\log x+1}^{\log\log x} (\log x)^{P(2)-1-P(2)\log P(2)} (\log\log x)^{k} \sqrt{k} \\ 
     & \ll 
     (\log x)^{P(2)-1-P(2)\log P(2)} \int_{P(2)\log\log x}^{\log\log x} (\log\log x)^{t} \sqrt{t} \cdot dt \\ 
     & \ll (\log x)^{P(2)-1-P(2)\log P(2)} \sqrt{\log\log x} (\log\log x)^{\log\log x} = o(x), \\ 
...
$P(2)-1-P(2)\log P(2) \approx -0.188883$. 
...
We have that for large $x \geq 2$ and $1 \leq k \leq P(2) \log\log x$ 
\begin{align*} 
...
\end{align*} 
So by Abel summation (again integrating by parts), we obtain 
\begin{align*} 
...
\begin{align*} 
\mathbb{E}|g^{-1}(n)| & = \frac{9}{\pi^2 \widehat{c} \sqrt{2\pi P(2)}} \cdot 
...
\label{eqn_proof_tag_SE1xFuncExp_v1} 
     & \gg 
     (\log x)^{\frac{7}{4}} (\log\log x)^{\frac{3}{4}} 
     T_E(x) \times 
     \left\lvert \sum_{e < d \leq \log x} 
     \frac{\lambda(d) (\log d)^{\frac{3}{4}}}{d^{3/4} \cdot \log\log d} 
     \right\rvert \\ 
...
\widehat{I}_{\ell}(x) & = \left\lvert A_{\Omega}^{(\ell)}\left(e^{e^{2c}}\right) 
     ...
     & = O\left(A_{\Omega}^{(\ell)}(\log\log x) 
      \widehat{\tau}_{\ell}(\log\log x)\right). 
\end{align*} 
Notice that the monotonicity of $|A_{\Omega}^{(\ell)}(x)|$ is natural to our application, 
and is only essential in so much as it allows us to choose one of the bounds of integration 
as the explicit input to our functions from the mean value theorem. 
It does not reflect the signedness of the terms it is summed over. 
