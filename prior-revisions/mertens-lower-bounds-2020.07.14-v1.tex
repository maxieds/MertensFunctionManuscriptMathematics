...
\] 
%The time and space complexity needed to compute $M(x)$ by the best known algorithms make obtaining 
%larger order bounds computationally intractable. 
...
\item[\textbf{(5)}] In Section \ref{}, we transform the new formula for $M(x)$ in 
           \eqref{eqn_Mx_gInvnPixk_formula} 
           into an integral representation with small bounded error term. 
           To prove that this error term in correct as $x \rightarrow \infty$, we require 
           proofs of Erd\"os-Kac like theorems providing semi-distributions on the values of 
           key component functions and their sums. 
...
\[
M(x) \approx G^{-1}(x) + \left(1 - \frac{2}{\log 2}\right) G^{-1}\left(\frac{x}{2}\right) - 
     \sum_{k=1}^{x/2} G^{-1}(k) \cdot \frac{x}{k^2 \log(x/k)} + o(\sqrt{x}). 
\]
This formula 
...
$|A_{\Omega}^{(\ell)}(t)|$ is monotone increasing for $t > u_0$ large, and that 
...
\newpage
\section{New formulas, bounds and error terms for $M(x)$} 
\label{Section_NewFormulasForMx} 

\subsection{Proving new formulas that identify main and error terms} 

\begin{lemma} 
\label{lemma_EMxFunc_ErrorTermStmt_v1} 
Let the error term function be defined as follows for all sufficiently large $x \geq 2$: 
\[
E_M(x) := \sum_{k=1}^{x/2-1} G^{-1}(k) \left[ 
     \pi\left(\Floor{x}{k}\right) - \pi\left(\Floor{x}{k+1}\right) 
     \right]. 
\]
Then there is an infinite sequence $\mathbb{X}_E \subseteq \mathbb{Z}^{+}$ such that 
for all $x \in \mathbb{X}_E$, $E_M(x) = o(\sqrt{x})$ as $x \rightarrow \infty$. 
\end{lemma} 

We will require something more in the form of an Erd\"os-Kac like theorem providing 
a limiting semi-distribution form on the values of $|g^{-1}(n)|$ to prove the lemma. 
These theorems are established in the next subsection. 
The next result is our stated as our central new formula for $M(x)$. 

\begin{prop} 
\label{prop_Mx_SBP_IntegralFormula} 
For all sufficiently large $x$, we have that (TODO) 
\begin{align} 
\label{eqn_pf_tag_v2-restated_v2} 
M(x) & \approx G^{-1}(x) +\left(1-\frac{2}{\log 2}\right) 
     G^{-1}\left(\frac{x}{2}\right) + 
     x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{t^2 \cdot \log(x/t)} dt + 
     E_M(x). 
\end{align} 
\end{prop} 
\begin{proof} 
...
We seek to approximate the function $\pi(j) \sim \frac{j}{\log j}$ by the main 
term in its limiting asymptotic approximation. To make this substitution useful 
in our formulas, we need not only that $\pi(j) - \frac{j}{\log j}$ is small, but 
also that the summatory functions over the difference of these terms weighted 
by $g^{-1}$ are small enough in order. In short, we need to make sure that the 
non-negligible error terms in our approximation are not too unruly as to make 
our main term results imprecise as $x \rightarrow \infty$. 

Now observe that 
\begin{align} 
\label{eqn_proof_tag_LHSDiff_v0} 
D_g^{-1}(x) & := \sum_{k=1}^{x/2} g^{-1}(k) \left[ 
     \pi\left(\frac{x}{k}\right) -  \frac{x}{k \cdot \log\left(\frac{x}{k}\right)} 
     \right] \\ 
\notag 
     & = \left(1 - \frac{2}{\log 2}\right) G^{-1}\left(\frac{x}{2}\right) + 
     \sum_{k=1}^{x/2-1} G^{-1}(k) \left[ 
     \pi\left(\frac{x}{k}\right) - \pi\left(\frac{x}{k+1}\right) + 
     \frac{x}{k \cdot \log\left(\frac{x}{k}\right)} - 
     \frac{x}{(k+1) \cdot \log\left(\frac{x}{k+1}\right)} 
     \right]. 
\end{align} 
By applying summation by parts, \eqref{eqn_proof_tag_LHSDiff_v0} implies that we have 
\begin{subequations}
\begin{align} 
\notag 
\sum_{k=1}^{x/2-1} G^{-1}(k) \left[ 
     ...
     \sum_{k=1}^{x/2-1} G^{-1}(k) \left[ 
     \frac{x}{k \cdot \log\left(\frac{x}{k}\right)} - 
     \frac{x}{(k+1) \cdot \log\left(\frac{x}{k+1}\right)}\right] + E_M(x) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v3} 
     & \sim 
     \sum_{k=1}^{x/2-1} G^{-1}(k) \left[ 
     \frac{x}{k \cdot \log(x/k)} - \frac{x}{(k+1) \cdot \log(x/k)}\right] + E_M(x) \\ 
\label{eqn_proof_tag_MxSumApprox_Step_v4} 
     & \approx 
     \sum_{k=1}^{x/2-1} G^{-1}(k) \frac{x}{k^2 \cdot \log(x/k)} + E_M(x). 
\end{align} 
\end{subequations} 
Indeed, step 
\eqref{eqn_proof_tag_MxSumApprox_Step_v4} is justified by writing 
\begin{align*} 
...
M(x) \approx G^{-1}(x) + \left(1 - \frac{2}{\log 2}\right) 
     G^{-1}\left(\frac{x}{2}\right) + 
     x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{t^2 \cdot \log(x/t)} dt + E_M(x). 
\]
...
does not require more restrictive attention in constructing limiting cases 
as $x \rightarrow \infty$. 
\end{proof} 
...
\widehat{A}_z(x) = \frac{x \cdot F(2, z)}{\Gamma(z)} (\log x)^{z-1} + 
     O_{R,z}\left(x \cdot (\log x)^{\Re(z) - 2}\right), 
\]
\end{prop} 
\begin{proof} 
\textbf{(TODO)} 
...
of its possible values. The form of this assumption we draw upon is that for large $x$, 
\[
\#\{n \leq x: \Omega(n) - \omega(n) = k\} \sim d_k. 
\] 
For a more rigorous treatment of this underlying principle see \cite{TODO,TODO}. 
\end{remark} 

\begin{theorem} 
\label{theorem_CnkSpCasesScaledSummatoryFuncs} 
We have uniformly for 
$1 \leq k < P(2) \cdot \log\log x$ 
that as $x \rightarrow \infty$ 
\[
\widehat{C}_k(x) := 
     \sum_{\substack{n \leq x \\ \Omega(n) = k}} \lambda(n) (-1)^{\omega(n)} 
     C_k(n) \asymp \frac{x}{\log x} \cdot 
     \frac{(-1)^{k} \cdot P(2)^{k-1} \cdot (\log\log x)^{k-1}}{(k-1)!}\left[ 
     1 + O\left(\frac{k}{(\log\log x)^3}\right)\right]. 
\]
\end{theorem} 
\begin{proof} 
...
$r := \frac{k-1}{\log\log x}$ with $r \leq P(2) \approx 0.452247$: 
\begin{align*} 
...
\end{align*} 
By induction we can compute the coefficients $[z^k] \widehat{A}_z(x)$ with respect to 
$x$ for fixed $k \leq P(2) \cdot \log\log x$ 
using the Cauchy integral formula for integers $m \geq 0$: 
\[
...
\] 
Now by parameterizing the countour around $|z| = r := \frac{k-1}{\log\log x}$ we 
find the the main term of our approximation corresponds to 
\begin{align*} 
-\int_{|z|=r} \frac{x \cdot \exp(-P(2) z) (\log x)^{-z}}{(\log x) z^{k} (1 + P(2) z)} dz & \sim 
     \frac{x}{\log x} \cdot \frac{(-1)^{k} P(2)^{k-1} (\log\log x)^{k-1}}{ 
     \Gamma\left(1 - \frac{k}{\log\log x}\right) (k-1)!} \\ 
     & \asymp \frac{x}{\log x} \cdot \frac{(-1)^{k} P(2)^{k-1} (\log\log x)^{k-1}}{(k-1)!}. 
     \qedhere 
\end{align*} 
\end{proof} 

The signs of the functions estimated in the next theorem are 
again dictated by the differences of the prime omega functions as 
$(-1)^{\Omega(n) - \omega(n)}$. It happens, as we have summarized above, that 
this distribution is fairly regular. This signedness property, in place of 
...
In particular, an exact DGF expression for 
$\lambda(n) C_{\Omega(n)}(n)$ is complicated by the need to estimate the asymptotics 
of the coefficients of the product 
\[
...
\left\lvert \mathbb{E}\left[ 
     ...
     \right] \right\rvert 
     \asymp \frac{1}{\sqrt{2\pi P(2)}} \cdot \frac{(\log x)^{P(2)}}{\sqrt{\log\log x}}. 
\] 
\end{lemma} 
\begin{proof} 
We observe that 
...
\sum_{k=1}^{\log_2(x)} \widehat{C}_k(x) \asymp 
     \sum_{k=1}^{P(2) \log\log x} \widehat{C}_k(x). 
\end{equation} 
...
\left\lvert \frac{\sum\limits_{P(2) \log\log x < k \leq \log_2(x)} \widehat{C}_k(x)}{ 
     \sum\limits_{k=1}^{P(2) \log\log x} \widehat{C}_k(x)} \right\rvert = o(1), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{equation} 
...
\begin{align} 
\notag 
\left\lvert \sum_{k=1}^{P(2) \log\log x} \widehat{C}_k(x) \right\rvert & = 
     \left\lvert \frac{x}{\log x} \times \sum_{k=1}^{P(2) \log\log x} 
     \frac{(-1)^{k} P(2)^{k-1} (\log\log x)^{k-1}}{(k-1)!} \right\rvert \left[
     1 + O\left(\frac{k}{(\log\log x)^3}\right)\right] \\ 
\notag 
     & = \left\lvert \frac{x}{\log x} \times \sum_{k=1}^{P(2) \log\log x} 
     \frac{(-1)^{k} P(2)^{k-1} (\log\log x)^{k-1}}{(k-1)!} \right\rvert \left[
...
\notag 
     & = \left\lvert \frac{x \cdot \Gamma(P(2) \log\log x, -P(2) (\log\log x)) (\log x)^{P(2)-1}}{ 
     \Gamma(P(2) \log\log x)} \right\rvert \left[
     1 + O\left(\frac{1}{(\log\log x)^2}\right)
     \right] \\ 
\notag 
     & = \frac{x}{\log x} \cdot \frac{(P(2) \log\log x)^{P(2) \log\log x - 1}}{\Gamma(P(2) \log\log x)} 
     \left[1 + O\left(\frac{1}{(\log\log x)^2}\right)\right] \\ 
\label{eqn_proof_tag_PartialSumsOver_HatCkx_v3} 
     & = \frac{x}{\sqrt{2\pi P(2)}} \cdot \frac{(\log x)^{P(2)}}{\sqrt{\log\log x}} 
     \left[1 + O\left(\frac{1}{(\log\log x)^2}\right)\right]. 
\end{align} 
...
Theorem \ref{theorem_MV_Thm7.20-init_stmt} to show that for large $x \rightarrow \infty$ we have 
for $0 < \varepsilon < 1$ 
\begin{align*} 
 & \sum_{P(2)\log\log x < k \leq (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} \widehat{C}_k(x) \leq 
    \sum_{P(2) \log\log x < k \leq (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} 
    \sum_{\substack{n \leq x \\ \Omega(n) = k}} C_{\Omega(n)}(n) \\ 
    & \quad \ll \sum_{k=P(2)\log\log x+1}^{\log\log x-1} \frac{\widehat{\pi}_k(x)}{x} \cdot k! + 
      \sum_{k=\log\log x}^{(\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} 
      \frac{\widehat{\pi}_k(x)}{x} \cdot k! \\ 
    & \quad \ll 
     \sum_{k=P(2)\log\log x+1}^{\log\log x} (\log x)^{P(2)-1-P(2)\log P(2)} (\log\log x)^{k} \sqrt{k} \\ 
    & \quad \phantom{\ll\quad\ } + 
     \sum_{k=\log\log x}^{(\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}}} 
     (\log x)^{\frac{k}{\log\log x} - 1 - \frac{k}{\log\log x} \left( 
     \log k - \log\log\log x\right)} \cdot \left(\frac{k}{e}\right)^{k} \sqrt{2\pi k} \\ 
     & \quad \ll 
     \sum_{k=P(2)\log\log x+1}^{\log\log x} (\log x)^{P(2)-1-P(2)\log P(2)} (\log\log x)^{k} \sqrt{k} + 
     \sum_{k=\log\log x}^{\varepsilon \frac{\log\log x}{\log\log\log x}} 
     (\log x)^{k \frac{\log\log\log x}{\log\log x} - 1} \sqrt{k} \\ 
     & \quad \ll 
     (\log x)^{P(2)-1-P(2)\log P(2)} \int_{P(2)\log\log x}^{\log\log x} (\log\log x)^{t} \sqrt{t} dt 
     \frac{1}{(\log x)} \times \int_{\log\log x}^{\varepsilon \frac{\log\log x}{\log\log\log x}} (\log\log x)^t \sqrt{t} dt \\
     & \quad \ll (\log x)^{P(2)-1-P(2)\log P(2)} \sqrt{\log\log x} (\log\log x)^{\log\log x} + 
     \frac{1}{(\log x)} \sqrt{\varepsilon \frac{\log\log x}{\log\log\log x}} 
     (\log\log x)^{\varepsilon \frac{\log\log x}{\log\log\log x}} = o(x), 
\end{align*} 
where $\lim_{x \rightarrow \infty} (\log x)^{\frac{1}{\log\log x}} = e$ and 
$P(2)-1-P(2)\log P(2) \approx -0.188883$. By \eqref{eqn_proof_tag_PartialSumsOver_HatCkx_v3} this 
...
\eqref{eqn_proof_tag_hInvn_ExactNestedSumFormula_CombInterpetIdent_v3}, so this contribution is negligible. 
...
\begin{align*} 
\sum_{n \leq x} C_{\Omega(n)}(n) & = \frac{3x}{2\widehat{c} \sqrt{2\pi} P(2) (1 + P(2))} \cdot \frac{(\log x)^{1 + P(2)}}{ 
     \sqrt{\log\log x}}\left[
...
\end{cor} 
\begin{proof} 
We handle transforming our previous results for the first sum. 
Let 
\begin{align*} 
\widehat{C}_{\ast}(x) & := \left\lvert \sum_{n \leq x} \lambda(n) (-1)^{\omega(n)} C_{\Omega(n)}(n) \right\rvert \\ 
     & \phantom{:} = 
...
\begin{align*} 
L_{\ast}(t) & := \sum_{n \leq x} \lambda(n) (-1)^{\omega(n)} = 
     \sum_{k=0}^{\log_2(t)} (-1)^{k} \cdot \#\{n \leq t: \Omega(n) - \omega(n) = k\} \\ 
     & \sim \sum_{k=0}^{\log_2(t)} \widehat{c}t \cdot \frac{(-1)^{k}}{2^k} = 
     \frac{2\widehat{c} t}{3} + o(1), \mathrm{\ as\ } t \rightarrow \infty. 
\end{align*} 
Then integrating by parts in the Abel summation formula, we obtain that 
\begin{align*} 
\widehat{C}_{\ast}(x) & = \int_1^{x} L_{\ast}^{\prime}(t) C_{\Omega(t)}(t) dt \qquad \implies \\ 
C_{\Omega(x)}(x) & \sim \frac{\widehat{C}_{\ast}^{\prime}(x)}{L_{\ast}^{\prime}(x)} \\ 
     & \sim \frac{3}{4\widehat{c} \sqrt{2\pi} P(2)} \cdot \frac{(\log x)^{P(2) - 1}}{ 
...
\end{align*} 
So by Abel summation (again integrating by parts), we obtain (TODO) 
\begin{align*} 
...
\begin{align*} 
\mathbb{E}|g^{-1}(n)| & = \frac{9}{\pi^2 \sqrt{2\pi} \widehat{c} P(2) (1+\log P(2)) (2+P(2))} 
     \cdot \frac{(\log n)^{2+P(2)}}{\sqrt{\log\log n}}, 
\end{align*} 
where $2 + \log P(2) \approx 1.20647$. 
\end{cor} 
\begin{proof} 
Because of the first formula in Corollary \ref{cor_SummatoryFuncsOfUnsignedSeqs_v2}, 
we have that as $n \rightarrow \infty$ 
\[
\mathbb{E}[C_{\Omega(n)}(n)] = \frac{3}{2\widehat{c} \sqrt{2\pi} P(2) (1 + P(2))} \cdot 
     \frac{(\log n)^{1 + P(2)}}{\sqrt{\log\log n}}. 
\]
This implies that 
\begin{align*} 
\int_2^x \frac{\mathbb{E}[C_{\Omega(t)}(t)]}{t} dt & = \frac{3}{2\sqrt{2} \widehat{c} P(2) (1+P(2))} \cdot 
     \frac{\operatorname{erfi}\left(\sqrt{2+P(2)} \cdot \sqrt{\log\log x}\right)}{\sqrt{2+P(2)}} \\ 
     & = \frac{3}{2\sqrt{2\pi} \widehat{c} P(2) (1+P(2)) (2+P(2))} \cdot \frac{(\log x)^{2+P(2)}}{ 
     \sqrt{\log\log x}}\left[1 + O\left(\frac{1}{\log\log x}\right)\right]. 
\end{align*} 
...
\mathbb{E}|g^{-1}(n)| & = \frac{6}{\pi^2}\left[\mathbb{E}[C_{\Omega(n)}(n)] + 
     ...
     & = \frac{9}{\pi^2 \sqrt{2\pi} \widehat{c} P(2) (1+P(2)) (2+P(2))} \cdot \frac{(\log n)^{2+P(2)}}{ 
     \sqrt{\log\log n}}. 
     \qedhere 
\end{align*} 
\end{proof} 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x: \frac{C_{\Omega(n)}(n) - 
     ...
     \frac{3}{2\widehat{c} \cdot (\log x)^{2-P(2)} (\log\log x)} \cdot \Phi(z) + 
     O\left(\frac{1}{(\log x)^{2-P(2)} (\log\log x)^{3/2}}\right). 
\] 
...
\frac{1}{x} \cdot \#\left\{2 \leq n \leq x:|g^{-1}(n)| - 
     ...
     \frac{3}{2\widehat{c} \cdot (\log x)^{2-P(2)} (\log\log x)} \cdot 
...
\subsection{Proof of the error term bound} 

\begin{proof}[Proof of Lemma \ref{lemma_EMxFunc_ErrorTermStmt_v1} (Part I)] 
First of all, we notice that 
\[
\frac{x}{k} - \frac{x}{k+1} = \frac{x}{k(k+1)} \sim \frac{x}{k^2}, 
\]
so that $\frac{x}{k^2} \geq 1$ $\implies$ $k \leq \sqrt{x}$ \textbf{(TODO)}. 
Thus we can re-write the error term sum as 
\begin{align} 
\label{eqn_EMx_SumReWritten_v2} 
E_M(x) & \sim \sum_{k=1}^{\sqrt{x}} G^{-1}(k) \left[\pi\left(\frac{x}{k}\right) - 
     \pi\left(\frac{x}{k+1}\right)\right] \\ 
     & = \sum_{k=1}^{\sqrt{x}} g^{-1}(k) \pi\left(\frac{x}{k}\right). 
\end{align} 
Since $\pi\left(\frac{x}{k}\right) \leq \pi(\sqrt{x})$ for $1 \leq k \leq \sqrt{x}$, 
we have that 
\begin{align} 
\label{eqn_EMx_SumReWritten_v3} 
E_M(x) = O\left(\frac{\sqrt{x}}{\log x} G^{-1}(\sqrt{x})\right). 
\end{align} 
So to show that $E_M(x) = o(\sqrt{x})$ infinitely often along an infinite subsequence of 
positive integers, it suffices to show that 
$G^{-1}(\sqrt{x}) = o(\log x)$ for infinitely many positive integers. 
We will require a few more results before we can prove this current conjectured property. 
\end{proof} 
...
\end{align} 
We compute that for almost every sufficiently large $x \rightarrow \infty$: 
\begin{align*} 
\frac{|G^{-1}(x)|}{x} & = \frac{1}{x} \times \left\lvert 
     ...
     \right\rvert. 
\end{align*} 
Let the summation in the previous equation be defined by 
\[
...
S_{-}(x) \ll \sum_{1 \leq k \leq \log_2(x)} \widehat{\pi}_k(x) \times \sum_{j=0}^{k} \binom{k}{j} \cdot j!. 
\end{equation} 
...
\end{align*} 
By an argument with conditional probabilities of set densities, we find 
\begin{align} 
\notag 
...
to find that 
\begin{align*} 
S_{-}(x) & \ll \frac{\pi^2}{3} \times \sum_{1 \leq k \leq \log_2(x)} \pi_k(x) \times \sum_{j=0}^{k} 
     \binom{k}{j} \cdot j! \\ 
     & \ll \frac{\pi^2}{3} \times 
...
\begin{align*} 
\mathcal{L}_{+}(x) & := \frac{1}{x} \cdot \#\{n \leq x: \lambda(n) = +1\} 
...
\]
This implies that $\frac{S_{-}(x)}{x} = O(1)$. In either of these extreme 
bounds on $S_{-}(x)$, we have by 
...
happen when $G^{-1}(x) = 0$. 
This singularity in the distribution of $G^{-1}(x)$ can only occur when 
\[
G^{-1}(x) = \frac{g^{-1}(x)}{2} + 
     \frac{1}{2\pi\imath} \int_{c-\imath\infty}^{c+\imath\infty} 
     \frac{x^s}{s \cdot (P(s) + 1) \zeta(s)} ds = 0, \mathrm{\ for\ } c > 1. 
\] 
It suffices to assume that $G^{-1}(x) \neq 0$ on an asymptotically 
dense subset of the integers for the bounds we need to prove 
Corllary \ref{cor_ThePipeDreamResult_v1} 
in the last subsection. In particular, we only require that 
\[
...
\[
M_{\pm}(x) := \left\lvert \sum_{\log x < d \leq \frac{(\log x)^{5} (\log\log x)}{e^2}} 
...
\begin{align} 
\notag 
S_{E,1}^{(\ell)}(x) & \ll \frac{(\log x)^{\frac{5}{4}}}{(\log\log x)^{\frac{1}{4}} \cdot 
     \sqrt{\log\log\log x}} \times \int_e^{\log x} \frac{(\log t)^{1/4}}{t \cdot (\log\log t)} dt \\ 
\notag 
     & \ll \frac{(\log x)^2}{(\log\log x)^{\frac{1}{4}} \cdot 
     \sqrt{\log\log\log x}} \times \operatorname{Ei}\left(\frac{5}{4} \log\log\log x\right) \\ 
\label{eqn_proof_tag_SE1Ellx_BoundFunc_MaximalOrder_v1} 
     & \ll \frac{(\log x)^2 (\log\log\log x)^{\frac{3}{2}}}{(\log\log x)^{\frac{1}{4}}}. 
\end{align} 
...
we obtain that 
\[
\widehat{L}_0\left(\frac{(\log x)^{5} (\log\log x)}{d}\right) \gg 
     \frac{(\log x)^{5/4} (\log\log x)^{1/4}}{d}, d \in \overline{\mathcal{R}}_x. 
\]
So we need to find a global lower bound on the sum 
\[
S_{\pm}(x) := 
     (\log x)^{5/4} (\log\log x)^{1/4} \times 
...
\begin{align*} 
 & \left\lvert \frac{(\log d)^{\frac{1}{4}}}{d^{\frac{1}{4}} \cdot (\log\log d)} - 
     ...
     \right\rvert \\ 
     & \qquad \gg \frac{\delta}{(d + \delta)^{\frac{1}{4}} \log(d + \delta)^{\frac{3}{4}} 
     \log\log(d + \delta)}. 
\end{align*} 
...
\] 
Define the maximum number of consecutively signed terms on this interval to be 
$\delta_{\max} \geq 1$. 
Then by the difference property we noted above, we have that 
\begin{align*} 
\frac{S_{\pm}(x)}{(\log x)^{5/4} (\log\log x)^{1/4}} & 
     \gg \sum_{k=0}^{\frac{N_x}{2}} \frac{\delta_{\max}}{(\log x + (2k+1) \delta_{\max})^{\frac{1}{4}} 
     \log(\log x + (2k+1) \delta_{\max})^{\frac{3}{4}} \log\log(\log x + (2k+1) \delta_{\max})} \\ 
     & 
     \gg \int_{\log x + \delta_{\max}}^{\frac{(\log x)^{5} (\log\log x) - e^2 \delta_{\max}}{2e^2 N_x}} 
     \frac{2\delta_{\max}^2 \cdot dt}{t^{1/4} \cdot (\log t)^{3/4} (\log\log t)} \\ 
     & \gg \delta_{\max} \cdot \left(\log x + \delta_{\max}\right)^{\frac{3}{4}} \times 
     \operatorname{Ei}\left(\frac{1}{4} \log\log t\right) \Biggr\rvert_{t=\log x + \delta_{\max}}^{ 
     t=\frac{(\log x)^{5} (\log\log x) - e^2 \delta_{\max}}{2e^2 N_x}} \\ 
     & \gg \left\lvert \left(\log x + \delta_{\max}\right)^{\frac{3}{4}} \times \left( 
     \log\left(\frac{1}{4}\right) + \log\log\log x\right) \right\rvert \\ 
     & \gg (\log x)^{\frac{3}{4}}. 
\end{align*} 
Therefore, we have that 
\[
|R_E(x)| \gg S_{\pm}(x) \gg (\log x)^{2} (\log\log x)^{1/4}. 
\]
Clearly, the right-hand-side lower bound in the previous equation is asymptotically larger than the 
...
\item[\textbf{(1)}] 
     $\int_{u_0}^{x} 
     |A_{\Omega}(t) f^{\prime}(t)| dt \gg \widehat{I}_{\ell}(x)$; and 
\item[\textbf{(2)}] 
...
We next select this non-negative arithmetic function 
...
and the other that will conveniently give us our slow-growing tendency towards 
...
\notag 
\frac{|M(x)|}{\sqrt{x}} & = 
     \frac{1}{\sqrt{x}} \left\lvert G^{-1}(x) +  \frac{2}{(\log 2)} G^{-1}\left(\frac{x}{2}\right) - 
     x \cdot \int_1^{x/2} \frac{G^{-1}(t)}{ 
     t^2 \cdot \log(x/t)} dt + o(\sqrt{x})\right\rvert \\ 
\label{eqn_MxGInvxLowerBound_stmt_v0} 
     & \gg 
     \left\lvert \left\lvert \frac{G^{-1}(x)}{\sqrt{x}} + 
     \frac{2}{(\log 2)} G^{-1}\left(\frac{x}{2}\right) \right\rvert - 
     \sqrt{x} \int_1^{x/2} \frac{|G^{-1}(t)|}{ 
     t^2 \cdot \log(x/t)} dt + o(1)\right\rvert \\ 
\notag 
     & \gg 
...
claim is accurate as the integral bound does not trivially vanish at large $x$. 
 
To complete the logic to the formula we arrived at in \eqref{eqn_MxGInvxLowerBound_stmt_v1}, 
...
\begin{align*} 
\left\lvert \frac{G^{-1}(x)}{\sqrt{x}} + 
     \frac{2}{(\log 2)} G^{-1}\left(\frac{x}{2}\right) \right\rvert & \gg 
     \frac{6\sqrt{x}}{\pi^2} \left\lvert (\log x) (\log\log x) - \frac{2}{\sqrt{2} (\log 2)} 
     (\log x - \log 2)(\log\log x + o(1)) \right\rvert \\ 
     & \gg 
     \frac{12\sqrt{x}}{\pi^2 \sqrt{2}} (\log\log x), 
...
only at $x \in \mathbb{Z}^{+}$, what we in fact precisely arrive at 
...
by applying Theorem \ref{theorem_gInv_GeneralAsymptoticsForms} to 
...
\end{align} 
We evaluate the following limit to conclude unboundedness 
...
Theorem \ref{theorem_gInv_GeneralAsymptoticsForms} 
with $\hat{x}_{0,y} \geq \sqrt{x_{0,y}}$ for all $y \geq Y_0$. 
\end{proof} 
