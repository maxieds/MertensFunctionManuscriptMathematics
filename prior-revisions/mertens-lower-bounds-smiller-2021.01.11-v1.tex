...
\newglossaryentry{fInvn}{
    ...
    sort={fInvn},
    description={
    The Dirichlet inverse of $f$ exists 
    if and only if $f(1) \neq 0$. 
    ...
    This inverse function, denoted by $f^{-1}$ when it exists, 
...
\newglossaryentry{PikPiHatkx}{
    ...
    sort={PikPiHatkx},
    description={The prime counting function variant $\pi_k(x)$ denotes the number of integers $1 \leq n \leq x$ for $x \geq 2$ with 
                 exactly $k$ distinct prime factors: $\pi_k(x) := \#\{n \leq x: \omega(n) = k\}$. Similarly, the function 
                 $\widehat{\pi}_k(x) := \#\{n \leq x: \Omega(n) = k\}$ for $x \geq 2$. },
    type={symbols},
...
\newglossaryentry{QxSummatoryFunc}{
    ...
    description={For $x \geq 1$, we define $Q(x)$ to be the summatory function indicating the number of 
                 squarefree integers $n \leq x$. More precisely, this function is summed and identified with its 
                 limiting asymptotic formula as $x \rightarrow \infty$ in the following form: 
                 $Q(x) := \sum_{n \leq x} \mu^2(n) \sim \frac{6x}{\pi^2} + O(\sqrt{x})$. },
    type={symbols},
...
unsigned sequence $|g^{-1}(n)|$ with a limiting central limit theorem type 
...
exact formulas and asymptotic bounds for $M(x)$. 
In this way, we prove another concrete link between the distributions of both 
$L(x) := \sum_{n \leq x} \lambda(n)$ and the Mertens function and connect these classical 
...
direct recursion between the distinct values of $g^{-1}(n)$. 
...
This leads us to conclude the following uniform statement for any fixed $Y > 0$, 
$\mu_x(C) := \log\log x + \log 2 - \log(\hat{C}_{\ast}(C))$, 
$\sigma_x(C) := \sqrt{\log\log x}$, 
and for all $-Y \leq y \leq Y$ (see 
...
\tag{B} 
     \mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq \frac{
     \alpha \mathbb{E}|g^{-1}(x)| \sqrt{\log\log x}}{\log x}\right) & = 
\Phi\left(\frac{\alpha \pi^2 \hat{G}_{\ast}}{6}\right) + o(1), \alpha \in \mathbb{R}. 
\end{align*} 
...
$$|g^{-1}(n)| \leq \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)|,$$
...
by taking powers of a reciprocal Euler type product. 
A proof constructed from this type of bivariate power series 
...
\[
G^{-1}(x) = O\left(\max_{1 \leq t \leq x} |L(t)| \times \mathbb{E}|g^{-1}(x)|\right). 
\]
If the RH is true, then we have the following result for any 
$\varepsilon > 0$ and almost every integer $x \geq 1$: 
\[
G^{-1}(x) = O\left( 
     \sqrt{x} \cdot (\log x)^2 \sqrt{\log\log x} \times 
...
precise details needed to make our claims based on this intuition rigorous. 
...
\end{align*} 
Next, for any $\delta_{x,k} > 0$ when we define 
$r\log\log x \leq k := \log\log x + \delta_{x,k}$, we obtain the bounds that 
$1 \leq r \leq \frac{\log x}{\log\log x}$ for all large $x > e$. 
...
\frac{\mathcal{N}_{\omega}(x)}{\mathcal{D}_{\omega}(x)} \ll 
     ...
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
Now we have the following bounds for large $x$: 
\[
...
$C_{\Omega(n)}(n)$ to prove an Erd\"os-Kac like analog for the distribution of the 
unsigned function $|g^{-1}(n)|$. 
...
\]
This connection between the summatory function of $g^{-1}(n)$ and the primes is also conveyed 
...
corresponding to squarefree integers and prime powers. Namely, if for integers 
...
\[
\left\lvert \sum_{n \geq 1} \frac{b_z(n) (\log n)^{2R+1}}{n^2} \right\rvert < +\infty, 
\]
...
\cite[Thm.\ 7.17; \S 7.4]{MV} shows that for any $z \in \mathbb{C}$ such that $0 < |z| < 2$ 
...
corresponding to \eqref{eqn_AbelSummationIBPReverseFormula_stmt_v1} be defined 
for large $x > e$ and integers $k \geq 1$ by 
\begin{align*} 
...
using the following uniform asymptotics for $\pi_k(x)$ 
when $1 \leq k \leq \log\log x$: 
\[
\pi_k(x) \asymp \frac{x}{\log x} \frac{(\log\log x)^{k-1}}{(k-1)!} (1 + o_k(1)), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\]
We have by an asymptotic approximation to the incomplete gamma function and 
...
\end{align*} 
The main term for the reciprocal of the derivative of the main term approximation to this 
...
equality at the upper bound precisely when $\mu^2(n) = 1$, or equivalently when $n$ is squarefree. 
...
\notag 
     ...
     \right] \\ 
\notag 
     & \asymp \frac{6}{\pi^2} \left[\mathbb{E}[C_{\Omega(n)}(n)] + \sum_{d<n} 
...
\]
Let the mean and variance parameter analogs be denoted by 
\[
...
We define the small parameter $\delta_{t,x} := \frac{t}{\mu_x(C)}$. 
...
Since our target probability density function approximating the PDF of the 
normal distribution is given by 
$$f(k, x) \rightarrow \frac{1}{\sqrt{2\pi} \cdot \sigma_x(C)} \times e^{\frac{-(t-\mu_x(C))^2}{2\sigma_x^2}},$$ 
we perform the change of variable $t \mapsto (\log x) \sqrt{\log\log x}$ to obtain the normalized 
...
Uniformly for $Y > 0$ and all $-Y \leq z \leq Y$ 
we have that 
\begin{align*} 
...
\end{align*} 
Since $|g^{-1}(n)| = \sum_{d|n} C_{\Omega(d)}(d)$ for all 
squarefree $n \geq 1$, we obtain that 
\begin{align*} 
...
|g^{-1}(n)| & = \Delta_n(n \cdot \mathbb{E}|g^{-1}(n)|) 
     ...
     \mathrm{\ as\ } n \rightarrow \infty. 
\end{align*} 
Since $\mathbb{E}|g^{-1}(n-1)| \sim \mathbb{E}|g^{-1}(n)|$ for all sufficiently large $n$, by 
Corollary \ref{cor_ExpectationFormulaAbsgInvn_v2}, 
...
\mathbb{P}\left(|g^{-1}(n)| - \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)| \leq \frac{\alpha \cdot 
     \mathbb{E}|g^{-1}(x)| \cdot \sqrt{\log\log x}}{\log x}\right) & = 
     \Phi\left(\frac{\alpha \pi^2 \hat{G}_{\ast}}{6}\right) + o(1), \alpha \in \mathbb{R}. 
\end{align*} 
\end{lemma} 
\begin{proof} 
...
\end{align*}
Since we have a MacLaurin series for the error function for $|z| < 1$ given by 
\begin{align*}
\Phi(-z) & = 1 - \Phi(z) = \frac{1}{2} - \frac{z}{\sqrt{2\pi}}\left(1 - \frac{z^2}{12} + O(|z|^5)\right), 
\end{align*}
the result in (A) clearly holds as $x \rightarrow \infty$. 
The result in (B) follows easily by rearrangement and 
...
It follows from the lemma that as $n \rightarrow \infty$, 
$$|g^{-1}(n)| \leq \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)|,$$
on a set of asymptotic density one. 
That is, we can see that 
\[
\lim_{x \rightarrow \infty}\ \frac{1}{x} \cdot \#\left\{n \leq x: 
     |g^{-1}(n)| \leq \frac{6}{\pi^2} \mathbb{E}|g^{-1}(n)|\right\} = 1. 
\]
\end{remark} 
...
We need to bound the sums of the maximal values of $|g^{-1}(n)|$ over $n \leq x$
as $x \rightarrow \infty$. We know by a result of Robin that \cite{ROBIN-PRIMEOMEGAFUNC-BOUNDS}
\[
...
\end{align*}
Now because we have deduced that the set of $n$ on which $|g^{-1}(n)|$ is substantially larger 
than its average order is asymptotically thin, we can see that the contributions at these maximal 
values such that the values are of order greater than $\mathbb{E}|g^{-1}(x)|$ are bounded by 
\begin{align*}
     \left\lvert \int_{x-o\left(\frac{1}{x}\right)}^{x} L^{\prime}(t) |g^{-1}(t)| dt\right\vert & \ll 
\int_{x-o\left(\frac{1}{x}\right)}^{x} \left(\frac{\log t}{\log\log t}\right)^{\frac{\log t}{\log\log t} + 
     \frac{1}{2}} dt \\ 
     & \ll x \left(\frac{\log x}{\log\log x}\right)^{\frac{\log x}{\log\log x} + \frac{1}{2}} \times 
     \int_{1-o\left(\frac{1}{x^2}\right)}^{1} dv \\ 
     & \ll o\left(\frac{(\log x)^{\frac{3}{2}}}{\sqrt{\log\log x}}\right), 
     \mathrm{\ as\ } x \rightarrow \infty. 
\end{align*}
...
G^{-1}(x) & = O\left(\int L^{\prime}(x) |g^{-1}(x)| dx\right) \\ 
     & = O\left(\mathbb{E}|g^{-1}(x)| \times \int 
     L^{\prime}(x) dx\right). 
\end{align*} 
Thus we obtain the conslusion of the second result by applying Humpries' result in 
...
\end{align*} 
The rightmost term in the previous bound is 
known to satisfy $\frac{|G^{-1}(k)|}{k} \ll \mathbb{E}|g^{-1}(k)|$ as 
$k \rightarrow \infty$. 
The conclusion follows since the average order of $|g^{-1}(n)|$ is 
non-decreasing with sufficiently large $n$. 
\end{proof} 

\newpage
\section{Conclusions}

We have identified a key sequence, 
...
preserving the original asymptotic order of $|G^{-1}(x)|$. 
