...
\begin{align*}
M(x) & = O\left(\left\lvert G^{-1}(x) + G^{-1}\left(\frac{x}{2}\right) \right\rvert + 
     x^{\frac{3}{4}} \sqrt{\log\log x} (\log x)^{3} + 
     \frac{x}{\log x} \times \left\lvert 
     \sum_{k \leq \sqrt{x}} \frac{G^{-1}(k)}{k^2} \right\rvert 
     \right). 
\end{align*} 
...
\[
S_1(x) = O\left(\frac{x}{\log x} \times \left\lvert \sum_{k \leq \sqrt{x}} 
     \frac{G^{-1}(k)}{k^2} \right\rvert \right). 
\]
To bound the second sum, we perform summation by parts as in the proof of 
the proposition to obtain that 
\begin{align*} 
S_2(x) & = O\left(G^{-1}\left(\frac{x}{2}\right) + \left\lvert \int_{\sqrt{x}}^{\frac{x}{2}} 
     \frac{x G^{-1}(t)}{t^2 \log\left(\frac{x}{t}\right)} dt \right\rvert \right). 
\end{align*} 
...
\begin{align*}
\left\lvert \int_{\sqrt{x}}^{\frac{x}{2}} \frac{x G^{-1}(t)}{t^2 \log\left(\frac{x}{t}\right)} dt \right\rvert 
     & \ll 
...
|M(x)| \gg \left\lvert \left\lvert G^{-1}(x) + G^{-1}\left(\frac{x}{2}\right) \right\rvert - I(x) \right\rvert, 
\end{equation}
where 
\[
I(x) := \left\lvert \int_2^{\frac{x}{2}} 
     \frac{x G^{-1}\left(t\right)}{t^2 \log\left(\frac{x}{t}\right)} dt 
     \right\rvert. 
\]
...
I(x) & \asymp \frac{x}{\log x} \times \left\lvert \int_1^{\sqrt{x}} \frac{G^{-1}(t)}{t^2} dt 
     \right\rvert \asymp \frac{x}{\log x}, 
\end{align}
...
positive at $\frac{6}{\pi^2}$ allow us to state the next key lemma 
...
\begin{align*}
G^{-1}((4m+1)\#) & \asymp \sum_{0 \leq k \leq 4m+1} \binom{4m+1}{k} (-1)^{k} k! 
     \left(e + O\left(\frac{1}{(k+1)!}\right)\right) \sim -(4m+1)! \\ 
G^{-1}\left(\frac{(4m+1)\#}{2}\right) & \asymp \sum_{0 \leq k \leq 4m} \binom{4m+1}{k} (-1)^{k} k! 
...
\[
(n\#) \sim e^{\vartheta(p_n)} \asymp n^n (\log n)^n e^{-n(1+o(1))}, \text{ as } n \rightarrow \infty. 
\]
...
\frac{\log x}{W_0(\log x) W_0\left(\frac{\log x}{W_0(\log x)}\right)} = 
     ...
     \text{ as } x \rightarrow \infty. 
\]
We also have from Lemma \ref{theorem_PrimorialSeqGInvCalcs_v1} and 
\eqref{eqn_RmksOnFamousConj_MxLowerBound_v2} that 
\[
\frac{G^{-1}((4m+1)\#) + G^{-1}\left(\frac{(4m+1)\#}{2}\right)}{I((4m+1)\#)} \sim 
     \frac{\sqrt{2\pi}(e-2)\sqrt{4m+1} \cdot \vartheta(p_{4m+1})}{\log(4m+1)^{4m+1}} = o(1), 
...
\limsup_{x \rightarrow \infty} \frac{M(x)}{\sqrt{x}} & \gg \limsup_{m \rightarrow \infty} 
     ...
     & \gg \lim_{x \rightarrow \infty} 
     \frac{\sqrt{x\log x}}{\exp\left(\sqrt{\frac{\log x}{\log\log x}}\right) (\log\log x)^{1+
     \sqrt{\frac{\log x}{\log\log x}}}} = + \infty. 
\end{align*}
The theorem statement follows from 
\eqref{eqn_RmksOnFamousConj_MxLowerBound}. 
\end{proof}
