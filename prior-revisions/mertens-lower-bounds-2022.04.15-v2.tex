...
\end{align} 
Some probabilistic models of the M\"obius function lead us to consider the behavior of $M(x)$ 
...
\mathbb{E}\left[\sum_{1 \leq n \leq x} X_n\right] = 0, 
     \operatorname{Var}\left(\sum_{1 \leq n \leq x} X_n\right) = \frac{6n}{\pi^2}, 
     \text{ and } 
...
in the last theorem denotes the classical prime counting function.

\subsection{Significance of the new formulas for $M(x)$}
...
exact expressions for $M(x)$ in terms of $L(x)$ cited in equation \eqref{eqn_MxInTermsOfLx_v1}. 
...
It stands to reason that $\lambda(n)$ must then inherit the pseudo-randomized quirks 
of $\mu(n)$ predicted by certain models of the behavior of this function. 
On the other hand, several lengthy discussions on why the new representations for the 
Mertens function in 
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} are more desirable to explore than 
classical formulae for $M(x)$ boil down to three primary counter points:
\begin{itemize}
\item[(1)] Break through work in recent years bounding multiplicative functions 
...
\item[(2)] The squarefree $n \geq 1$ on which $\lambda(n)$ and $\mu(n)$ must identically agree 
	are in some senses the ``easy'' integer cases insomuch as we can prove very regular properties 
	that govern the distributions of the distinct values of 
	$\omega(n)$, $\Omega(n)$ and their difference for $n \leq x$ as $x \rightarrow \infty$ 
	\citep[\cf \S 2.4; \S 7.4]{MV}; 
\item[(3)] A final counter point is to note that $\lambda(n)$ is \emph{completely} 
	multiplicative. Hence, it stands to reason that eventually $\lambda(n)$ may be shown to be 
	a ``better behaved'' cousin to the multiplicative $\mu(n)$ along the 
	integers $n \geq 4$ for which $\mu(n) = 0$. 
	This notion is intentionally left imprecise at the time of writing this manuscript.
\end{itemize}
These exchanges are summarized above to motivate more study on the unsigned 
functions that are central to the new results in the article. 

\subsection{Key results and formulas}
...
In this section, we define $C_{\Omega}(n)$ precisely and explore its properties. 
...
\end{align}
By the argument given in \cite[Thm.~7.21; \S 7.4]{MV}, if we define 
...
\eqref{eqn_proof_tag_SumLogCOmegan_P0_exp_v2} is one. 
...
and $\log C_{\Omega}(n) \ll \Omega(n) \log \Omega(n)$ for $n \leq x$, 
...
Notice that the assertion on the upper bound for $\log C_{\Omega}(n)$ in the last few 
inline equations holds 
for all $n$ even though the right-hand-side terms involving $\Omega(n)$ 
oscillate in magnitude as $n$ ranges between $1$ and $x$. This is justified by 
maximizing (minimizing) the ratio of the right-hand-side above to Binet's log-gamma formula cited above 
numerically to find explicit bounded real $z \equiv \Omega(n) \equiv k \in [1, 11)$ 
...
The $\pm 1$-valued sign of the function $g(n)$ is $\lambda(n)$ for all $n \geq 1$. 
\end{prop} 
\begin{proof} 
...
\]
\end{proof} 
The function $|h^{-1}(n)|$ from the last proof is the same as $C_{\Omega}(n)$ for all $n \geq 1$. 
...
model under which we can prove the conjectured convergence in the next conjecture.
...
\begin{prop}
\label{cor_CLT_VII} 
Assume that Conjecture \hlocalref{conj_DetFormOfEKTypeThmForCOmegan_v1} is true. 
...
\[
0 < z \leq \left(\frac{\Gamma(\log\log x + 1) (\log x)}{\sqrt{2\pi \log\log x}}\right)^{M+1} 
     (1 + o(1)), \text{ as } x \rightarrow \infty. 
\]
We may consider finding a $M_x \geq 1$ such that for large $x$ we have that
$\Phi(M_x) = 1 + o(1)$ and then apply the last bound to evaluate 
the non-trivial cases of $z$ that contribute only negligible weight to sums over 
the difference in Proposition \hlocalref{cor_CLT_VII}. 
That is, those differences where $|g(n)|$ diverges from the asymptotic bound for its average 
order with substantial weight. 

\section{Proofs of the exact formulas for $M(x)$} 
...
positive unsigned arithmetic functions whose summands are sign-weighted by $\lambda(n)$. 
...
asymptotics of the summatory function of $g(n)$, the function $G(x)$, 
may yield new insights into the behavior of 
$M(x)$, we take the time to explore its properties somewhat in this section as well. 
...
A consequence of Theorem \hlocalref{theorem_SummatoryFuncsOfDirCvls} 
(proved in Appendix \hlocalref{Section_PrelimProofs_Config}) 
...
\item In Figure \hlocalref{figure_MxAndNewAuxPartialSums_Comparison_Intro_v2_v2}, we compare 
      ...
      \[
      \frac{|G(x)|}{x} \ll \frac{|G|(x)}{x (\log x) \sqrt{\log\log x}} = 
           \frac{1}{x (\log x) \sqrt{\log\log x}} \times \sum_{n \leq x} |g(n)|.
      \]
...
If the last equation holds, we obtain a contradiction to the 
condition required by equation \eqref{eqn_MertensMx_RHEquivProblem_Stmt_intro} above. 
We can actually give a statement of a stronger asymptotic bound for the 
Mertens function along this subsequence (assuming the RH) by considering the 
error terms in the proof of 
Proposition \hlocalref{theorem_PrimorialSeqGInvCalcs_v1}. 
\end{remark}
...
is suggests numerically by inspection that the distribution of $G(x)$ is easier to work with 
...
\cite[\cf \S 2.14; \S 3.10; \S 3.12; \S 4.9, p.\ 95]{APOSTOLANUMT}. 
...
r_{x,j} := R\left(\floor{\frac{x}{j}}\right) - R\left(\floor{\frac{x}{j+1}}\right) 
     \equiv R_{x,j} - R_{x,j+1}, \text{ for } 1 \leq j \leq x, 
\] 
where 
\[
