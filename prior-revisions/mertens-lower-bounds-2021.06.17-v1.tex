...
\usepackage[total={6.5in, 10in},tmargin=0.75in,headsep=8pt,footskip=30pt,footnotesep=0.5in]{geometry}
...
$\lambda(n) = (-1)^{\Omega(n)}$, denoted by $L(x) := \sum_{n \leq x} \lambda(n)$ 
for any $x \geq 1$ via the following relation \cite{HUMPHRIES-JNT-2013,LEHMAN-1960}: 
\[
...
equivalent characterization of $M(x)$ using two new auxiliary sequences and their 
summatory functions. This characterization is formed by constructing 
combinatorially significant sequences related to the distribution of the primes 
through convolutions of strongly additive functions. 
The new characterizations of $M(x)$ by the methods in this article stem from a 
standing curiosity about an under utilized elementary identity from 
the list of exercises in \cite[\S 2; \cf \S 11]{APOSTOLANUMT}. 
The indicator function of the primes is given by M\"obius inversion as the 
Dirichlet convolution $\chi_{\mathbb{P}} = \omega \ast \mu$, or 
equivalently in the form of $\chi_{\mathbb{P}} + \varepsilon = (\omega + 1) \ast \mu$. 
We form the summatory function of $(\omega + 1) \ast \mu(n)$ over $n \leq x$ for 
any $x \geq 1$ and then apply classical style inversion theorems to exactly relate 
$M(x)$ to the partial sums of $g^{-1}(n) := (\omega+1)^{-1}(n)$. 

We are informed by the modern results in \cite[\S 7.4; \S 2.4]{MV} that 
apply traditional analytic methods to formulate limiting asymptotics and to 
directly give a rigorous proof of an Erd\H{o}s-Kac theorem for the completely additive 
function $\Omega(n)$. Adaptations of the key ideas in the 
expository proofs from the reference 
provide us with a foundation for analytic proofs of several limiting 
...
Section \ref{Section_NewFormulasForgInvn} are as such 
predictable given that strong additivity leads to explicit 
probability measures and central limit type theorems 
that predict the distributions of functions of this type 
over $n \leq x$ as $x \rightarrow \infty$ 
\cite[\cf Thm.~1.5; \S 1.7]{IWANIEC-KOWALSKI}. 
In effect, we make progress on the initially elementary problem at hand by 
leveraging the connection of $C_{\Omega(n)}(n)$ and $|g^{-1}(n)|$ with the 
canonically additive functions to obtain our results proved 
in Section \ref{Section_NewFormulasForgInvn}. 
We expect that an outline of the method behind the collective proofs we 
provide with respect to the Mertens function special case can be generalized to identify 
another associated additive function with the role of $\omega(n)$ here to 
express asymptotics for partial sums of other multiplicative functions. 
...
These identities inspire a deep connection between the 
unsigned inverse function and additive prime counting combinatorics. 
...
While we primarily rely on the analytic proof methodology presented in 
\cite{MV}, and require the statements 
of several asymptotic formulas and bounds related to 
$\omega(n)$ and $\Omega(n)$ from the reference, 
most, if not all, of the results given in this manuscript are new. 
The function $C_{\Omega(n)}(n)$ was considered under alternate notation 
by the work of Fr\"oberg (circa 1968) on the series expansions of the 
\emph{prime zeta function}, $P(s)$, 
e.g., the prime sums defined as the DGF of $\chi_{\mathbb{P}}(n)$. 
The clear interpretation of the function in connection with $M(x)$ 
is unique to our work on the properties this auxiliary function. 
Explicit references to uniform asymptotics for restricted partial sums of 
$C_{\Omega(n)}(n)$ and the probabilistic features of the limiting distribution 
of this function are missing in number theoretic literature. 
We also utilize techniques from recent work over the last few years in the 
appendix section to evaluate asymptotics of several special partial sums we require in 
terms of the upper incomplete gamma function $\Gamma(a, z)$ as both parameters 
$a,z$ simultaneously tend to infinity at a rate proportional to $\log\log x$ 
\cite{NEMES2015C,NEMES2016,NEMES2019}. 

Since we prove that $\operatorname{sgn}(g^{-1}(n)) = \lambda(n)$ for all $n \geq 1$, 
we have an updated probabilistic perspective from which to express 
our intuition about features of the distribution of 
the summatory function $G^{-1}(x)$ at large $x$. 
The partial sums defined by $G^{-1}(x)$ are related precisely to the properties of 
...
by considering properties of the latter partial sums. 
These results motivate future work that will provide new upper (and possibly lower) bounds 
on $M(x)$ through the additive combinatorial properties of the repeated distinct 
...
\sum_{\substack{n \leq x \\ \Omega(n)=k}} C_{\Omega(n)}(n) \sim 
     \frac{4 A_0 \sqrt{2\pi} \cdot x}{(2k-1)} \cdot \frac{(\log\log x)^{k-\frac{1}{2}}}{(k-1)!}, 
...
on an infinite set of the integers with asymptotic density of one over the 
positive integers. 
...
The motivation for considering this auxiliary sequence representing the $k$-fold 
Dirichlet convolution of $\omega(n)$ with itself follows from our definition of 
$g^{-1}(n) := (\omega+1)^{-1}(n)$. 
We show several precise relations of the function $C_{\Omega(n)}(n)$ to the inverse 
...
\{\lambda(n) C_{\Omega(n)}(n) \}_{n \geq 1} = \{
     ...
     -3, 2, 2, -1, 4, 1, 2, \ldots \}. 
\]
We can see that $C_{\Omega(n)}(n) \leq (\Omega(n))!$ for all $n \geq 1$ with 
...
\cite[\S 7.4]{MV} to establish proofs of 
...
provide new characterizations of $M(x)$ by its relationship to the summatory function 
$G^{-1}(x)$ (\cf equation \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1}). 
...
\eqref{eqn_ConvenientIncGammaFuncTypePartialSumAsymptotics_va3} that for large $x$ 
\begin{align*}
...
and when $x > e$ is sufficiently large we have 
\begin{align}
\notag
...
We also restrict to $k \leq 2\log\log x$ for all large $x$ as we required by 
