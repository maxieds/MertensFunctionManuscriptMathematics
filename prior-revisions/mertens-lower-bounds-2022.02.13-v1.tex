...
In a series of four articles published in J.~Indian~Math.~Soc.~from 1953-1954 
L.~G.~Sathe proved that 
\[
N_k(x) := \#\{n \leq x: \Omega(n) = k\} \sim \frac{x}{\log x} \cdot 
     \frac{(\log\log x)^{k-1}}{(k-1)!}, \text{ as } x \rightarrow \infty. 
\]
The original proof due to Sathe is inductive (on $k \geq 1$) and required 
technical formulas depending on both $k$ and $x$ which are difficult to study
as both $k,x \rightarrow \infty$. 
In 1954 A.~Selberg improved on Sathe's estimates \cite{SELBERG-NOTE-SATHE} 
with a shorter argument involving the sequence of 
$\{d_z(n)\}_{n \geq 1}$ where $z \in \mathbb{C}$ and 
\[
\zeta(s)^z = \sum_{n \geq 1} \frac{d_z(n)}{n^s}, \text{ for } \Re(s) > 1. 
\]
...
Asymptotic formulas for sums of the form 
\[
D_z[\varpi](x) := \sum_{n \leq x} \sum_{d|n} \varpi(d) d_z\left(\frac{n}{d}\right), 
     \text{ for } x \geq 1 \text{ and } z \in \mathbb{C}, 
\]
where $\varpi$ is strongly additive are often readily accessible via Selberg's method. 
The Selberg-Delange method is a core technique identified by Tenenbaum in 
\cite[\S II.6.1]{TENENBAUM-PROBNUMT-METHODS} \cite[\cf \S 7.4]{MV}. 
...
derived by a related application of the Selberg-Delange method. 
...
We observe that similar features in these two distributions appear even for these 
...
\subsubsection{Plots and numerical experiments}
...
\item In Figure \hlocalref{figure_MxAndNewAuxPartialSums_Comparison_Intro_v2_v1} 
      ...
      is compared and seen to be similar in shape to the magnitude of $M(x)$ on this subinterval. 
...
\item In Figure \hlocalref{figure_MxLxAndGInvx_Comparison_Intro_v2_v1} we provide the oscillatory 
      ...
      The plots in (b) can be viewed to compare the signed magnitudes of local extremum 
      on subintervals. 
...
\item In Figure \hlocalref{figure_MxAndNewAuxPartialSums_Comparison_Intro_v2_v2} we compare 
      ...
      \[
      \frac{|G(x)| (\log x) \sqrt{\log\log x}}{Q(x)} \ll (\log x) \sqrt{\log\log x}. 
      \]
      Thus, the bounding envelopes on $G(x)$ plotted in (b) are accurate up to at most a small 
      constant scaling factor at all sufficiently large $x$. 

\item In Figure \hlocalref{figure_HistgInvnDistUns_Comparisons_v1} we compare two variants of the 
...
\item In Figure \hlocalref{figure_LogarithmicRatioDensityPlot_Comparisons_v1} we define 
      ...
      produce chaotic plots (not shown) with no distinctive features.
