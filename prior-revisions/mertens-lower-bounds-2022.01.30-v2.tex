...
M(x) & = G(x) + \sum_{1 \leq k \leq x} \lambda(k) \pi\left(\Floor{x}{k}\right) |g(k)|, \\ 
...
The result in \eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1} is rewritten as 
\begin{equation}
\label{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1_v2}
M(x) = \sum_{n \leq x} \lambda(n) \left(1 + \pi\left(\Floor{x}{n}\right)\right) |g(n)|, 
     \text{ for } x \geq 1. 
\end{equation}
...
Theorem \hlocalref{prop_Mx_SBP_IntegralFormula} and 
\eqref{eqn_RmkInitialConnectionOfMxToGInvx_ProvedByInversion_v1_v2} 
express $M(x)$ as finite sums over $\lambda(n)$ whose coefficients are unsigned. 
...
the new formulas from the theorem are comparitively easier to work with than prior 
...
supplementary material on topics that can be separated from the body of the article. 
...
we prove next in Theorem \hlocalref{prop_HatAzx_ModSummatoryFuncExps_RelatedToCkn} is 
...
We will apply an extension of Tenenbaum's Selberg-Delange method proofs to extract 
