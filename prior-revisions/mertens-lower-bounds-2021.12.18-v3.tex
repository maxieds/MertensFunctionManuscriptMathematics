...
\newglossaryentry{GammaIncompleteGamma}{
    ...
    description={The incomplete gamma function is defined as $\Gamma(a, z) := \int_z^{\infty} t^{a-1} e^{-t} dt$ for 
                 ...
                 are discussed for reference in 
...
Thus, there is an essentially additive structure underneath the sequence 
$\{g^{-1}(n)\}_{n \geq 2}$. 
\end{observation} 
...
we conjecture the form of a variant of the Erd\H{o}s-Kac theorem 
...
we use the analytic methods due to H.~Davenport and H.~Heilbronn 
...
precise statements in the results given in 
...
turn the uniform asymptotics for the signed sums into core bounds we will need on the 
...
technical while mimicking as closely as possible the 
...
we conjecture that there are clearly defined 
...
\cite[Thm.~7.21; \S 7.4]{MV} suggest that the next conjecture should hold. 
On the other hand, proving that the scaled distribution of $C_{\Omega}(n)$ matches 
the analogous result for $\Omega(n)$ from the reference when the average order asymptotics 
in \eqref{eqn_ProofTag_AnsatzRVDiffFromMeanBound_v0} match 
leads to a challenging problem that is outside of the scope of this manuscript.
...
the result follows by a re-normalization of Conjecture \ref{conj_DetFormOfEKTypeThmForCOmegan_v1}. 
\end{proof} 
...
\end{align*} 
In the previous steps, we used Stirling's formula and the fact that for all natural numbers 
...
\newpage
\section{Table: Computations involving $g^{-1}(n)$ and $G^{-1}(n)$ for $1 \leq n \leq 500$} 
